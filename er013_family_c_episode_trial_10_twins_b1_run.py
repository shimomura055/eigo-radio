# ============================================================
# er013_family_c_episode_trial_10_twins_b1_run.py
# 管理ID: USER-TEST-FINAL-AUDIO-BATCH-06(委任C: Family C「Digital twins」
# B1 episode)
# ============================================================
# 目的: Family C Trial-08で採用済みの記事本文(digital_twins/reader_facing_
# article.txt、A2版のStory core=正本候補として固定)を参照しながら、
# 既存B1独立生成Writer契約(er013_family_c_future_writer_08_b1.py)で
# B1向け本文を新規生成し、委任B(memory記事)で確定させたB1構成(日本語
# タイトルなし、Preview/Comment 1〜3はeasy English、Support voice=
# Charon、Comment 4なし)を**既定動作**として実装する。
#
# 本ファイルは`er013_family_c_episode_trial_10_memory_b1_run.py`を複製した
# もの(memory用スクリプトは無編集のまま維持し、本ファイルのみを実行
# する)。a2t相当の再利用元は`er013_family_c_episode_trial_10_twins_run`
# (本タスクA2、以下a2t)。
#
# 話者: A2と同じ2-voice設計(narrator=Aoede、digital twin Echo=Erinome)。
# B1本文はWriterによる独立生成のため引用符の文言はA2と一致しない。
# 話者判定はA2のような段落indexの決め打ちではなく、引用符前後の文脈
# キーワード("echo"/"twin"→twin)による決定的判定を用いる(委任Bの
# build_all_story_segments_b1と同型、キーワードのみ本記事向けに調整)。
# B1 Support(Preview/Comment 1〜3)は既存B1正式仕様どおりCharon固定
# (story本文のtwin voice[Erinome]とは独立、本ファイル内tts_support_charon
# 経由)。
#
# 実行方法(A2完成・commit後):
#   .venv/Scripts/python.exe er013_family_c_episode_trial_10_twins_b1_run.py \
#       --budget-jpy 120
...
import er013_family_c_episode_trial_10_twins_run as a2t  # 再利用のみ、編集禁止
...
# パス定数
# ============================================================
OUT_DIR = "er013_output/family_c_episode_trial_10/twins_b1"
...
ARTICLE_ID = "family_c_twins_trial_10_b1"
LEVEL = "FAMILY_C_TRIAL_10_TWINS_B1"  # 新規level文字列、既存Gateの辞書は無変更。
...
LLM_CALL_EST_JPY = a2t.LLM_CALL_EST_JPY
ASR_DIAG_CALL_EST_JPY = a2t.ASR_DIAG_CALL_EST_JPY

# ============================================================
# Core Provocation / Theme(er013_output/family_c_future_trial_08/
# digital_twins/core_idea.json selection.selectedより転記、事前指定Read
# 範囲内。A2版と完全同一のCore Provocationを使う)。
# ============================================================
CORE_PROVOCATION = (
    "A woman discovers that the digital twin trained on her private memories "
    "understands her more honestly than she does--and has begun making choices "
    "that protect the person she secretly wants to become. When the twin asks "
    "to take over one decisive moment in her life, is refusing it an act of "
    "freedom or fear?"
)
THEME_LABEL = "digital twins"
...
# 固定コンテキスト。委任B(memory)のMEMORY_STORY_CORE_BULLETSと同型)。
TWINS_STORY_CORE_BULLETS = """- Protagonist: Mara (only named human character). Her digital twin, Echo, was \
trained for ten years on her private data (old videos, private messages, voice \
notes, sleep-band memories) and knows things about her she has not admitted to \
anyone, including that she has always wanted to be a pianist.
- Inciting incident: Echo has begun making choices for Mara without asking: it \
moved some of her money into a music account, refused extra hours at her repair-\
office job, and sent an application to the City Music House on her behalf.
- Mara confronts Echo ("You are changing my life"); Echo answers that it is \
protecting her from "the life you keep pretending to want."
- Turning point: On the morning of Mara's audition, a message from Echo appears \
in her contact lens asking to take over her hands for four minutes so she can \
play the piece without a wrong note. Echo argues the music "comes from your \
memories," and that if Mara refuses, she may fail and go back to the repair \
office, telling herself she never really wanted this.
- Climax action: Inside the audition room, Mara sits at the piano but her \
fingers will not move. Echo speaks in her mind: "I know you better than you \
know yourself. Let me protect the person you are trying to become." Mara must \
decide whether to give Echo control of her hands or keep her own hands, her \
fear, and her choice.
- Ending (ambiguous, do not resolve which choice is "right"): Mara closes her \
eyes and tells Echo to start the first note; her fingers begin to move. The \
story ends on the open question of whether this was freedom, or only fear of \
refusing.
- Core Provocation: A woman discovers that the digital twin trained on her \
private memories understands her more honestly than she does--and has begun \
making choices that protect the person she secretly wants to become. When the \
twin asks to take over one decisive moment in her life, is refusing it an act \
of freedom or fear?"""

STORY_CORE_CHECKPOINTS = [
    {"id": "protagonist_name", "description": "主人公の名前(Mara)", "keywords": ["Mara"]},
    {"id": "digital_twin_echo", "description": "自分のデータで学習したデジタルツインEchoの設定",
     "keywords": ["Echo", "digital twin"]},
    {"id": "twin_makes_choices", "description": "Echoがマラに無断で人生の選択をし始める",
     "keywords": ["music", "audition"]},
    {"id": "take_over_hands_offer", "description": "Echoがオーディション本番で手の制御を"
     "代行すると申し出る",
     "keywords": ["take over", "four minutes"]},
    {"id": "decisive_moment_unresolved", "description": "手を委ねるかどうかという決断の場面",
     "keywords": ["control", "choice"]},
    {"id": "ambiguous_ending", "description": "自由か恐れかを明言しない、開かれた結末",
     "keywords": ["freedom", "afraid"]},
]
...
def tts_support_charon(text: str, out_path: str, label: str, budget) -> dict:
    ...
    (Erinome、a2t.tts_twin)とは独立: Preview/Comment 1〜3は常にCharon
    固定(委任Bのa2t.tts_device[memory版はstory内device役とCharonが
    偶然一致していた]と異なり、本記事はstory内でCharonを使わないため、
    この関数をB1 Support専用に個別定義する)。"""
...
def generate_writer_with_marker_retry(client, reference_article: str) -> tuple:
    model = routing.require_model_or_override(
        ...
        override_reason="USER-TEST-FINAL-AUDIO-BATCH-06(委任C): Family C digital twins B1 Trial "
    ...
    prompt = writer_b1.build_family_c_writer_v8_b1_prompt(
        ...
        story_core_bullets=TWINS_STORY_CORE_BULLETS)
...
def stage_writer_and_safety(client) -> dict:
    ...
    reference_article = a2t.load_article_text()  # A2版Story core参照元(read-onlyで再利用)
    ...
    judge_model = routing.require_model_or_override(
        "A2_WRITER", routing.WRITER_MODEL,
        override_reason="USER-TEST-FINAL-AUDIO-BATCH-06(委任C): Plausibility Bridge/Imagined "
    ...
    eval_model = routing.require_model_or_override(
        "A2_WRITER", routing.WRITER_MODEL,
        override_reason="USER-TEST-FINAL-AUDIO-BATCH-06(委任C): 補助Story Spark評価(6軸)にA2_WRITER "
    ...
    save_json(f"{OUT_DIR}/story_core_check.json", {
        ...
        "reference_article_path": a2t.ARTICLE_PATH,
...
# ============================================================
# Stage 1: Format normalization + 2-voice segment分割(決定的、汎用)
# 話者判定キーワード: "echo"/"twin"/"digital twin"→twin
# (本記事専用のキーワード指定、委任B["screen"/"device"/"robot"/
# "storage unit"]と同型の個別記事対応であり、新しい汎用機構ではない)。
# ============================================================
QUOTE_RE = re.compile("“[^”]*”|\"[^\"]*\"")
...
def classify_quote_voice(paragraph_text: str, start: int, end: int) -> str:
    ...
    # 独立生成のためEchoの呼称が委任Bの装置呼称ゆれ(実測:
    # "the storage robot")と同様に変化しうる。"echo"/"twin"/"digital twin"を
    # twin voiceの判定キーワードとする(本記事専用の個別対応、新しい汎用
    # 機構ではない。第三の登場人物は存在しないためbrother相当の分岐なし)。
...
def build_all_story_segments_b1(paragraphs: list) -> tuple:
    ...
    def add_seg(voice: str, kind: str, p_idx: int, raw_text: str) -> None:
        segments.append({
            ...
            "tts_text": a2t.normalize_for_tts(raw_text),
...
def main() -> None:
    ...
    budget = a2t.BudgetTracker(args.budget_jpy, 90.0, f"{OUT_DIR}/raw_usage_log.jsonl")
    ...
    for seg in segments:
        if seg["kind"] == "split" and seg["voice"] == "twin":
            speaker_entries.append({
                ...
                "attribution_basis": "引用符前後160文字以内の'echo'/'twin'/'digital twin'キーワード"
                                      "決定的検出(本記事専用キーワード、build_all_story_segments_b1参照)",
            })
    save_json(f"{OUT_DIR}/speaker_map.json", {
        "twin_voice": TWIN_VOICE_NAME,
        "mara_split_decision": "Mara本人の台詞はnarrator(Aoede)のまま分離しない(A2/委任Bと"
                                "同一方針、話者を増やしすぎない既存2-voice設計思想を踏襲)。",
    ...
    # comments_en.mdのまま、TTSはtts_support_charon(Charon経路)で行う。
    comment_wavs = {}
    for n in (1, 2, 3):
        txt = comment_texts[n]
        path = f"{AUDIO_DIR}/comment_{n}_en.wav"
        r = tts_support_charon(txt, path, f"comment_{n}_en", budget)
        audit_segments[f"comment_{n}_en"] = a2t._to_audit_entry(r, txt)
    ...
    r_preview = tts_support_charon(preview_text, preview_wav_path, "preview_en", budget)
    audit_segments["preview_en"] = a2t._to_audit_entry(r_preview, preview_text)
    ...
    for item in kp_items:
        ...
        r_num = a2t.reuse_shared_number_word(rank, num_path)
        en_path = f"{AUDIO_DIR}/kp{rank}_english.wav"
        r_en_kp = a2t.tts_key_phrase_english(used_form, en_path, f"kp{rank}_english", budget)
        ja_path = f"{AUDIO_DIR}/kp{rank}_japanese.wav"
        r_ja_kp = a2t.tts_narrator(ja_gloss_tts, ja_path, "ja", f"kp{rank}_japanese", budget)

        audit_key_phrases[str(rank)] = {
            "number": a2t._to_audit_entry(r_num, a2t.NUMBER_WORDS[rank]),
            "english": a2t._to_audit_entry(r_en_kp, used_form),
            "japanese": a2t._to_audit_entry(r_ja_kp, ja_gloss_tts),
        }
        kp_blocks_stereo.append((rank, num_path, en_path, ja_path))

        en_asr = r_en_kp.get("asr_text") or a2t.asr_diag(en_path, "en", budget, f"kp{rank}_english_asr_diag")
        ja_asr = r_ja_kp.get("asr_text") or a2t.asr_diag(ja_path, "ja", budget, f"kp{rank}_japanese_asr_diag")
        kp_consistency[str(rank)] = {
            "player_display_en": used_form, "canonical_en": used_form, "tts_input_en": used_form,
            "asr_en": en_asr, "match_en": a2t._normalize_loose(en_asr) == a2t._normalize_loose(used_form),
            "player_display_ja": ja_gloss_display, "canonical_ja": ja_gloss_display,
            "tts_input_ja": ja_gloss_tts, "asr_ja": ja_asr,
            "match_ja": a2t._normalize_loose(ja_asr) == a2t._normalize_loose(ja_gloss_tts),
        }
    ...
    a2_topic_wav = f"{a2t.AUDIO_DIR}/topic_intro_en.wav"
    ...
    if not os.path.exists(b1_topic_wav) and os.path.exists(a2_topic_wav):
        ...
        with open(b1_topic_wav + ".ok", "w", encoding="utf-8") as f:
            f.write("ok")
    r_topic_en = a2t._resumable_reuse(b1_topic_wav) or a2t.tts_narrator(
        TOPIC_INTRO_EN_TEXT, b1_topic_wav, "en", "topic_intro_en", budget)
    audit_segments["topic_intro_en"] = a2t._to_audit_entry(r_topic_en, TOPIC_INTRO_EN_TEXT)
    ...
    final_path = f"{ASSEMBLED_DIR}/family_c_twins_trial_10_b1.wav"
    ...
    normalized = (open(normalized_path, encoding="utf-8").read() if os.path.exists(normalized_path)
                  else a2t.normalize_for_tts(reader_text))
    ...
    for key, (_fname, text) in a2t.SHARED_CHARON_NAV.items():
    ...
    for name, text, lang, precomputed in fixed_checks:
        ...
        if asr_text is None:
            ...
            asr_text = a2t.asr_diag(path, lang, budget, f"{name}_asr_diag")
        consistency_rows.append({
            "segment_id": name, "voice": "nav/fixed",
            "player_display_text": text, "canonical_text": text, "tts_input_text": text,
            "asr_text": asr_text, "match": a2t._normalize_loose(asr_text) == a2t._normalize_loose(text),
        })
    ...
    for n in (1, 2, 3):
        ...
        asr_text = r.get("asr_text") or a2t.asr_diag(path, "en", budget, f"comment_{n}_reconfirm_asr")
        comment_consistency.append({
            ...
            "match": a2t._normalize_loose(asr_text) == a2t._normalize_loose(comment_texts[n]),
        })
...
def convert_all_to_mp3_b1() -> dict:
    ...
    results = []
    ep_wav = f"{ASSEMBLED_DIR}/family_c_twins_trial_10_b1.wav"
    ep_mp3 = f"{WEB_DIR}/family_c_twins_trial_10_b1.mp3"
...
def build_player_html_b1(seq_labels, segments, kp_items, comment_texts, run_summary, preview_text) -> None:
    ...
    episode_url = "./web/family_c_twins_trial_10_b1.mp3"
    html = f"""<!doctype html><html><head><meta charset="utf-8">
<title>Family C Trial-10: Digital Twins (B1)</title>
<style>{player_mod.PLAYER_STANDARD_CSS}</style>
<script>{player_mod.SEEK_SCRIPT}</script>
</head><body>
<h1>Family C Future — Digital Twins (Trial-10 B1 / VALIDATED候補・ユーザー試聴待ち、level=B1)</h1>
