# ============================================================
# er013_family_c_episode_trial_10_twins_run.py
...
# article.txt、正本候補として固定・再生成しない)を、委任B(memory記事)で
# 確立したFamily C構成(Intro/Welcome/English Title/日本語タイトル/
# Preview/Key Phrase/Story/Comment 1-3[Comment 4なし]/Outro、既存SFX・
# pause)へ適用し、完成episodeを作る。
#
# 本ファイルは`er013_family_c_episode_trial_10_memory_run.py`を複製した
# もの(memory用スクリプトは無編集のまま維持し、本ファイルのみを実行
# する)。差し替えは設定(ARTICLE_PATH/OUT_DIR/ARTICLE_ID/日本語タイトル/
# Voice割当・話者判定キーワード/Comment位置)に限定し、Preview/Key
# Phraseは記事から新規生成する(既存Production関数を呼び出すだけ)。
# Story本文は無変更(Fact/学習内容は変えない)。
#
# 話者: 本記事は主人公マラ(Mara)と、彼女のデジタルツイン「Echo」の
# 2者が登場する(第三の人物なし)。マラ本人の台詞はnarrator(Aoede)の
# まま分離せず、Echoの台詞・表示メッセージのみtwin voiceへ分離する。
# twin voice=Erinome(選定理由: narratorのAoedeと明確に区別でき、Charon
# はB1 Support/装置voiceの既存慣例枠のため今回は使わない。Echoは機械的
# 存在というより人格的に描かれているため、既存承認Voiceの中でErinomeが
# 適する。既存4Voice候補の範囲内、新Voice探索・Voice比較Trialなし)。
#
# 実行方法:
#   .venv/Scripts/python.exe er013_family_c_episode_trial_10_twins_run.py \
#       --budget-jpy 80
...
OUT_DIR = "er013_output/family_c_episode_trial_10/twins_a2"
...
ARTICLE_ID = "family_c_twins_trial_10"
LEVEL = "FAMILY_C_TRIAL_10_TWINS_A2"  # 新規level文字列。既存level辞書(A2/B1/
...
JAPANESE_TITLE_TEXT = "デジタルツイン"  # TOPIC_TITLEの直訳(既存A2規約どおり、1件登録。
                                    # 記事本文自体には題名が存在しないため
...
TWIN_VOICE_NAME = "Erinome"  # デジタルツインEchoのvoice(既存4Voice候補、
                              # APPROVED_FOR_PRODUCTION実績あり)。選定理由は
                              # ファイル冒頭コメント参照(narratorのAoedeと
                              # 区別、Charonは今回使わない)。
...
# ============================================================
# 1. Format normalization + 2-voice segment分割(決定的、API不要)
#    段落0-indexedで33段落。Echo(digital twin)の直接発話・表示メッセージ
#    をtwin voiceへ分離する。Mara本人の台詞は委任B(Lena)同様narrator
#    (Aoede)のまま分離しない。
# ============================================================
QUOTE_RE = re.compile("“[^”]*”")

# 引用符付きでEchoが話す段落(narrator地の文と混在する段落含む、
# split_paragraph_by_quotesで引用符境界のみtwin voiceへ分離)。
TWIN_QUOTE_PARAGRAPH_INDICES = {3, 5, 11, 13, 19, 21, 25}
# 引用符ではなく太字表示(コンタクトレンズ上のメッセージ)としてEchoが
# 伝える段落(段落16、引用符なしの機械表示文。個別に話者指定、汎用機構
# 化はしない)。段落全体がtwin voiceになる。
TWIN_DISPLAY_PARAGRAPH_INDICES = {16}

# セグメント構成計画(0-indexed段落、merge=narrator地の文の連結、
# split=話者分離[引用符境界 or 全段落])。Comment挿入位置はCOMMENT_2/3_
# AFTER_PLAN_INDEXで指定する(該当planエントリ処理後に挿入)。
STORY_SEGMENT_PLAN = [
    ("merge", [0, 1, 2]),                  # 0
    ("split", 3),                          # 1  twin(mixed: "Echo said."と2引用)
    ("merge", [4]),                        # 2
    ("split", 5),                          # 3  twin(段落全体がEchoの発話)
    ("merge", [6, 7, 8, 9, 10]),           # 4
    ("split", 11),                         # 5  twin
    ("merge", [12]),                       # 6
    ("split", 13),                         # 7  twin  <- C2はこの直後
                                            #    (scene transition: 私的な会話から
                                            #    "At nine, Mara stood outside the
                                            #    audition room."への場面転換、
                                            #    累計語数約43%地点)
    ("merge", [14, 15]),                   # 8
    ("split", 16),                         # 9  twin(表示メッセージ)
    ("merge", [17, 18]),                   # 10
    ("split", 19),                         # 11 twin(mixed)
    ("merge", [20]),                       # 12
    ("split", 21),                         # 13 twin
    ("merge", [22]),                       # 14  <- C3はこの直後
                                            #    (turning point直前: "The door
                                            #    opened."の直後、演奏室に入り
                                            #    決断へ向かう場面の手前。結末
                                            #    [マラの最終判断]は明かさない。
                                            #    累計語数約73%地点)
    ("merge", [23, 24]),                   # 15
    ("split", 25),                         # 16 twin
    ("merge", [26, 27, 28, 29, 30, 31, 32]),  # 17
]
COMMENT_2_AFTER_PLAN_INDEX = 7
COMMENT_3_AFTER_PLAN_INDEX = 14
...
def build_all_story_segments(paragraphs: list) -> list:
    ...
    def add_split(idx: int, plan_index: int) -> None:
        if idx in TWIN_QUOTE_PARAGRAPH_INDICES:
            chunks = split_paragraph_by_quotes(paragraphs[idx], "twin")
        elif idx in TWIN_DISPLAY_PARAGRAPH_INDICES:
            # 引用符なしの表示メッセージ(段落全体がtwin voice)。
            chunks = [("twin", paragraphs[idx])]
        else:
...
# ============================================================
# 個別対応(2026-09-15、TTS/ASR不一致回避): 日本語Support文でEchoを英字
# "Echo"のまま書くとTTS発話が不安定になりASR不一致(TRUE_CONTENT_MISMATCH/
# ASR_VALIDATION_UNCERTAIN)が発生したため、片仮名「エコー」表記を明示指示
# する(恒久対策はOpen Item化、Prompt原則の新設ではなく本記事の個別指示)。
_JA_SUPPORT_COMMON_NOTE = (
    "表記の注意: デジタルツインの名前は必ず片仮名で「エコー」と書いて"
    "ください(英字の\"Echo\"は使わないでください)。数字は算用数字で"
    "書いてください(例: 「十年間」ではなく「10年間」)。"
)
PREVIEW_ROLE_JA = (
    "あなたは英語学習者向け音声番組のPreview(導入)を書く担当です。"
    "これから流れる物語の日本語による短い前置きを、2文程度・80〜110字"
    "程度で書いてください。物語は、自分の記憶データで学習した「デジタル"
    "ツイン」エコーと10年間暮らしてきた女性マラの物語です。エコーはマラの"
    "生活に静かに介入し始め、大事なオーディションの場面でマラの手を"
    "代わりに動かすことまで申し出ます。物語のテーマ・雰囲気・聞く価値を"
    "伝え、結末や中心的な問い(マラが最終的にどうするか)の答えを先に"
    "明かさないでください。具体的な出来事・台詞を先出ししないでください。"
    "「この先を聞きたくなる」自然な話し言葉の日本語にしてください。"
    "新しい設定・事実・登場人物を追加しないでください。" + _JA_SUPPORT_COMMON_NOTE
)
COMMENT_1_ROLE_JA = (
    "あなたは英語学習者向け音声番組で、物語が始まる直前に置く短い日本語"
    "コメントを書く担当です。これから、自分のデジタルツインエコーと暮らす"
    "女性マラの物語が始まります。結末や主人公の選択を先に明かさず、"
    "1文程度・30〜50字程度で、物語の世界に耳を傾けるよう促す短い一言を"
    "書いてください。直前に流れるPreview(短い前置き)と同じ内容を"
    "繰り返さないでください。断定的な予告にせず、自然な話し言葉に"
    "してください。新しい設定・事実・登場人物を追加しないでください。" + _JA_SUPPORT_COMMON_NOTE
)
COMMENT_2_ROLE_JA = (
    "あなたは英語学習者向け音声番組の、物語の途中に挿入する短い日本語"
    "コメントを書く担当です。これから、私的な会話の場面から一転して、"
    "主人公マラがオーディション会場の外に立つ場面へ切り替わります。"
    "結末や主人公の選択を先に明かさず、1文程度・40〜70字程度で、次に"
    "何が起きるかへ軽く注意を向ける短い日本語コメントを書いてください。"
    "断定的な予告にせず、自然な話し言葉にしてください。新しい設定・"
    "事実を追加しないでください。" + _JA_SUPPORT_COMMON_NOTE
)
COMMENT_3_ROLE_JA = (
    "あなたは英語学習者向け音声番組の、物語の途中に挿入する短い日本語"
    "コメントを書く担当です。これから、扉が開き、主人公マラがピアノの"
    "前へ進む場面が続きます。エコーに手を委ねるかどうかという大きな決断を"
    "マラが迫られていますが、結末や主人公の選択を先に明かさず、1文程度・"
    "40〜70字程度で、この場面の緊張感へ軽く注意を向ける短い日本語"
    "コメントを書いてください。断定的な予告にせず、自然な話し言葉に"
    "してください。新しい設定・事実を追加しないでください。" + _JA_SUPPORT_COMMON_NOTE
)
...
def tts_twin(text: str, out_path: str, label: str, budget: BudgetTracker) -> dict:
    ...
    bvoices.generate_voice_body_wide_margin()をそのまま呼ぶ(memory委任の
    tts_brother[Erinome]と同一実装、voice_name引数のみ共通)。"""
    reused = _resumable_reuse(out_path)
    if reused is not None:
        return reused
    budget.check_before(TTS_CALL_EST_JPY, label)
...
def main() -> None:
    ...
    # === Stage F: 物語本文segment(2-voice、全件新規TTS) ===
    ...
    for seg in story_segments:
        ...
        if seg["voice"] == "narrator":
            ...
            r = tts_twin(seg["tts_text"], out_path, seg["id"], budget)
        else:
    ...
    for seg in story_segments:
        if seg["kind"] == "split" and seg["voice"] == "twin":
            if set(seg["source_paragraph_indices"]) & TWIN_DISPLAY_PARAGRAPH_INDICES:
                attribution = ("引用符なしの表示文(段落16、コンタクトレンズ上の"
                                "メッセージ。直前の段落15'A small message appeared "
                                "in her contact lens.'がEchoからの通信であることを示す)")
            else:
                attribution = "引用符境界+直前直後の地の文('Echo said.'等)によりEchoの発話と判定"
    ...
    with open(f"{OUT_DIR}/speaker_map.json", "w", encoding="utf-8") as f:
        json.dump({
            ...
            "twin_voice_selection_reason": ("narratorのAoedeと明確に区別でき、Charonは"
                                             "B1 Support/装置voiceの既存慣例枠のため今回は"
                                             "使わない。Echoは機械的存在というより人格的に"
                                             "描かれているため、既存承認Voiceの中でErinomeが"
                                             "適する(Voice比較Trialは実施していない)。"),
            "mara_split_decision": ("Mara本人の台詞(段落1,4,10,12,18,20,30)はnarrator"
                                     "(Aoede)のまま分離しない。委任B(Lena)の設計判断を"
                                     "踏襲: 主人公は地の文と台詞が同一人物視点で連続する"
                                     "自然さを優先し、話者を増やしすぎない2-voice設計思想"
                                     "(既存4Voice候補の範囲内)。"),
            "entries": speaker_map,
        }, f, ensure_ascii=False, indent=2)
    ...
    final_path = f"{ASSEMBLED_DIR}/family_c_twins_trial_10.wav"
    ...
    boundary_defs = [
        (1, "導入部(Preview後・Story前、Full story introの直後)", None,
         story_segments[0]["id"]),
        (2, "段落13/14境界(scene transition: 私的な会話'“From the life you keep "
            "pretending to want.”'から、'At nine, Mara stood outside the audition "
            "room.'という場所・時間の転換への切り替わり。累計語数約43%地点)",
        ...
        (3, "段落22/23境界(turning point直前: 'The door opened.'の直後、"
            "'Inside, the panel waited. Mara sat at the piano.'という演奏室入室・"
            "決断場面の直前。マラの最終判断[Echoに手を委ねるか]という結末は"
            "明かさない。累計語数約73%地点)",
...
def write_family_a_reuse_map() -> None:
    rows = [
        ...
        ("Voice(Erinome=digital twin Echo)", "er012_b_family_voices_production_01",
         "委任B(Erinome=兄)と同一Voiceプールから選択、既存4Voice候補の範囲内"),
    ]
    lines = ["# Family A流用表(twins A2)\n", "| 項目 | 流用元 | 扱い |", "|---|---|---|"]
    for a, b, c in rows:
...
def convert_all_to_mp3() -> dict:
    ...
    results = []
    ep_wav = f"{ASSEMBLED_DIR}/family_c_twins_trial_10.wav"
    ep_mp3 = f"{WEB_DIR}/family_c_twins_trial_10.mp3"
...
def build_player_html(seq_labels, story_segments, kp_items, comment_texts, run_summary) -> None:
    ...
    episode_url = "./web/family_c_twins_trial_10.mp3"
    html = f"""<!doctype html><html><head><meta charset="utf-8">
<title>Family C Trial-10: Digital Twins(A2)</title>
<style>{player_mod.PLAYER_STANDARD_CSS}</style>
<script>{player_mod.SEEK_SCRIPT}</script>
</head><body>
<h1>Family C Future — Digital Twins(Trial-10 A2 / VALIDATED候補・ユーザー試聴待ち)</h1>
