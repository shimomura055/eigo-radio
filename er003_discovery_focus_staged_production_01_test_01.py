# ============================================================
# er003_discovery_focus_staged_production_01_test_01.py
# 管理ID: FAMILY-A-DISCOVERY-S2-PRODUCTION-WIRING-01
# ============================================================
# API呼び出しは一切行わない(unittest.mock.patch.objectのみ、実費¥0)。
# `er011_discovery_focus_s2_full_trial_01_test_01.py`(Trial、22テスト)の
# 検証項目を、移設先のProduction関数名(`er003_discovery_focus_staged_
# production_01`)へ移植したもの。Trialファイルはimportしない。
#
# 検証項目:
...
#   E. run_one_pattern_staged_discovery_focusの分岐(orchestration-level、
#      複合関数をpatchして分岐条件そのものを検証): 通常成功/Stage1 QA
#      blocking後regenで成功/Stage1 regen上限到達でNG_REVIEW_REQUIRED/
#      Stage2-3 exhaustion後のStage1 fallback成功/最終Ledger MAJORの
#      main_story locusでStage1 escalation成功/points locusではStage1
#      escalationせずNG_REVIEW_REQUIRED。
#   F. Production関数のidentity確認(コピー・再実装ではなく同一オブジェクト
#      を呼んでいることの機械確認)。
#   G. Dangling Reference: 本モジュールがTrial専用ファイル(er011_
#      discovery_focus_s2_full_trial_01/er011_discovery_focus_part_a_
#      standalone_trial_01_run/er011_discovery_stage3_rule_adjustment_
#      trial_09)を一切importしていないこと。
# ============================================================
...
import er003_discovery_focus_staged_production_01 as staged
...
TEST_OUT_DIR = "er003_output/discovery_focus_staged_production_01_TEST_TMP"
...
class ExtractStage1MainStoryTests(unittest.TestCase):
    def test_valid_structure(self):
        parsed = staged.extract_stage1_main_story(STAGE1_RAW_VALID)
    ...
    def test_rejects_point_headings_present(self):
        bad = STAGE1_RAW_VALID.replace("## In one line (draft)",
                                        "### A point heading\nbody\n\n## In one line (draft)")
        self.assertIsNone(staged.extract_stage1_main_story(bad))
...
class EvidenceCompressionSafetyTests(unittest.TestCase):
    def test_valid_ec_output_is_applied(self):
        with mock.patch.object(ec_editor, "run_lossless_editor",
            ...
            result = staged.apply_evidence_compression_to_points(
...
class LedgerLocalRewriteLoopTests(unittest.TestCase):
    def test_no_major_no_cycle(self):
        with mock.patch.object(vfl01, "run_deviation_check", return_value=_deviation([])):
            result = staged.run_ledger_local_rewrite_loop(
...
class Stage23RetryTests(unittest.TestCase):
    def _patched(self, overlap_side_effect, value_side_effect=None):
        ...
        return [
            mock.patch.object(staged, "run_stage2_role_planning",
                               return_value={"parsed": GOOD_PLAN, "model": "m", "response_id": "r",
                                             "prompt": "role planning prompt"}),
            mock.patch.object(staged, "run_stage3_points_writer",
    ...
    def test_success_without_retry(self):
        ...
        result = staged.run_stage2_3_with_retry(
        ...
        self.assertEqual(staged.run_stage2_role_planning.call_count, 1)
    ...
    def test_success_after_one_retry(self):
        ...
        self.assertEqual(staged.run_stage2_role_planning.call_count, 2)
        # Main Storyは全ての呼び出しでbyte一致(固定されたまま)であることを確認
        for call in staged.run_stage2_role_planning.call_args_list:
...
# ============================================================
# E. run_one_pattern_staged_discovery_focus分岐(orchestration-level)
# ============================================================
...
class RunOnePatternStagedBranchTests(unittest.TestCase):
    def _common_patches(self, fc_verdict="PASS", ledger_blocking=False, ledger_locus_unresolved=False):
        ...
        return [
            ...
            mock.patch.object(staged, "run_ledger_local_rewrite_loop",
    ...
    def test_happy_path_ok(self):
        for p in self._common_patches():
            p.start()
            self.addCleanup(p.stop)
        with mock.patch.object(staged, "run_stage1_main_story_writer",
             ...
             mock.patch.object(staged, "run_stage1_qa", return_value=_stage1_qa_ok()), \
             mock.patch.object(staged, "run_stage2_3_with_retry", return_value=_stage23_ok()):
            result = staged.run_one_pattern_staged_discovery_focus(
                object(), "a2", "topic", "ledger text", TEST_OUT_DIR)
        self.assertEqual(result["status"], "OK")
    ...
    def test_stage1_blocking_then_regen_succeeds(self):
        ...
        with mock.patch.object(staged, "run_stage1_main_story_writer",
                                return_value={"status": "STRUCTURE_PASS", "title_line": TITLE_LINE_FIXTURE,
                                              "main_story_text": MAIN_STORY_TEXT_FIXTURE, "attempts": [],
                                              "prompt": "p"}) as stage1_mock, \
             mock.patch.object(staged, "run_stage1_qa", side_effect=qa_side_effect), \
             mock.patch.object(staged, "run_stage2_3_with_retry", return_value=_stage23_ok()):
            result = staged.run_one_pattern_staged_discovery_focus(
                object(), "a2", "topic", "ledger text", TEST_OUT_DIR)
        self.assertEqual(result["status"], "OK")
    ...
    def test_stage1_blocking_exhausted_returns_ng(self):
        ...
        with mock.patch.object(staged, "run_stage1_main_story_writer",
                                return_value={"status": "STRUCTURE_PASS", "title_line": TITLE_LINE_FIXTURE,
                                              "main_story_text": MAIN_STORY_TEXT_FIXTURE, "attempts": [],
                                              "prompt": "p"}) as stage1_mock, \
             mock.patch.object(staged, "run_stage1_qa",
                                return_value=_stage1_qa_ok(blocking=True, reason="stage1_ledger_major_unresolved")):
            result = staged.run_one_pattern_staged_discovery_focus(
                object(), "a2", "topic", "ledger text", TEST_OUT_DIR)
        self.assertEqual(result["status"], "NG_REVIEW_REQUIRED")
        self.assertEqual(stage1_mock.call_count, staged.STAGE1_MAX_REGENERATIONS + 1)
    ...
    def test_stage23_exhaustion_falls_back_to_stage1_and_succeeds(self):
        ...
        with mock.patch.object(staged, "run_stage1_main_story_writer",
                                return_value={"status": "STRUCTURE_PASS", "title_line": TITLE_LINE_FIXTURE,
                                              ...
                                              "prompt": "p"}) as stage1_mock, \
             mock.patch.object(staged, "run_stage1_qa", return_value=_stage1_qa_ok()), \
             mock.patch.object(staged, "run_stage2_3_with_retry", side_effect=stage23_side_effect):
            result = staged.run_one_pattern_staged_discovery_focus(
                object(), "a2", "topic", "ledger text", TEST_OUT_DIR)
        self.assertEqual(result["status"], "OK")
        self.assertEqual(stage1_mock.call_count, 2)
    ...
    def test_final_ledger_main_story_locus_escalates_and_succeeds(self):
        ...
        patches = [
            ...
            mock.patch.object(dfp, "audit_article_directional_facts", return_value={"overall_status": "OK"}),
            mock.patch.object(staged, "run_ledger_local_rewrite_loop", side_effect=loop_side_effect),
        ]
        for p in patches:
            p.start()
            self.addCleanup(p.stop)
...
class ReusedFunctionIdentityTests(unittest.TestCase):
    def test_reused_qa_functions_are_the_same_objects(self):
        self.assertIs(staged.local_rewrite, local_rewrite)
        self.assertIs(staged.local_rewrite.rewrite_ng_item, local_rewrite.rewrite_ng_item)
        self.assertIs(staged.local_rewrite.apply_diff_qa_to_resolved_rewrite,
                       local_rewrite.apply_diff_qa_to_resolved_rewrite)
        self.assertIs(staged.local_rewrite.locate_target_sentence, local_rewrite.locate_target_sentence)
        self.assertIs(staged.local_rewrite.extract_point_context, local_rewrite.extract_point_context)
        self.assertIs(staged.local_rewrite.apply_rewrites, local_rewrite.apply_rewrites)
        self.assertIs(staged.prod_gen.run_point_overlap_qa_and_regenerate,
                       prod_gen.run_point_overlap_qa_and_regenerate)
        self.assertIs(staged.prod_gen.split_common_sections_for_point_qa, prod_gen.split_common_sections_for_point_qa)
        self.assertIs(staged.prod_gen.build_diagnostic_retry_prompt, prod_gen.build_diagnostic_retry_prompt)
        self.assertIs(staged.prod_gen.run_one_pattern, prod_gen.run_one_pattern)
        self.assertIs(staged.point_planning.run_point_value_qa, point_planning.run_point_value_qa)
        self.assertIs(staged.point_planning.build_role_planning_block, point_planning.build_role_planning_block)
        self.assertIs(staged.dfp.audit_article_directional_facts, dfp.audit_article_directional_facts)
        self.assertIs(staged.r3.run_fact_checker_with_gates, r3.run_fact_checker_with_gates)
        self.assertIs(staged.vfl01.run_deviation_check, vfl01.run_deviation_check)
        self.assertIs(staged.ec_editor.run_lossless_editor, ec_editor.run_lossless_editor)
    ...
    def test_retry_caps_match_production_values(self):
        ...
        self.assertEqual(staged.STAGE1_MAX_REGENERATIONS, 1)

    def test_focus_module_registered_and_run_one_pattern_body_untouched(self):
        self.assertIn("discovery_focus_staged", prod_gen.EDITORIAL_TYPE_MODULE_BLOCKS)
        self.assertEqual(
            prod_gen.EDITORIAL_TYPE_MODULE_BLOCKS["discovery_focus_staged"],
            prod_gen.DISCOVERY_FOCUS_MODULE_PART_A_BLOCK)


# ============================================================
# G. Dangling Reference: Trial専用ファイルをimportしていないことの確認
# ============================================================
class DanglingReferenceTests(unittest.TestCase):
    def test_module_does_not_import_trial_files(self):
        module_names = set(sys.modules.keys())
        forbidden = [
            "er011_discovery_focus_s2_full_trial_01",
            "er011_discovery_focus_part_a_standalone_trial_01_run",
            "er011_discovery_stage3_rule_adjustment_trial_09",
        ]
        # staged module自体のグローバル名前空間に、これらTrialモジュールへの
        # 参照が無いことを確認する(importしていれば `staged.<alias>` として
        # 存在するはずだが、本モジュールはこれらをimportしていない)。
        staged_globals = vars(staged)
        for forbidden_name in forbidden:
            for _key, value in staged_globals.items():
                self.assertNotEqual(getattr(value, "__name__", None), forbidden_name,
                                     f"{forbidden_name}への参照がstagedモジュール内に見つかりました")
