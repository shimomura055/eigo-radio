...
# 修正指示2回目(Fable、Opus L2レビュー指摘#4): 「役割:」行(旧行番号
# L142相当)を「どちらが正しいか」→「どの声が正しいか」へ言い換えた
# (ユーザー承認済みComment 3汎用化の範囲内、声の主体を明示するだけの
# 言い換え)。
# EDITORIAL-B-FAMILY-VOICES-3V-PRODUCTION-WIRING-PHASE1B-02-BUDGET-GUARD-
# AND-COMMENT3-01(2026-09-11ユーザー決定): タイトル行("その間に流す、
# Comment 3(役割: ...")も同じ「どの声が正しいか」側へ統一し、同一
# プロンプト内の表現混在(Opus L2レビュー`PHASE1B-01-OPUS-L2-REVIEW-01`
# 論点2)を解消した。Comment 4本文は本決定の対象外のため変更していない
# (Comment 4は別途ユーザー判断待ちのまま)。VOICES_COMMENT_3_ROLEは
# `COMMENT_ROLES`経由でB1(2V/3V)・A2と共有される単一定数のため、2V/A2の
# 将来のComment 3再生成にもこの言い換えが及ぶ(修正指示2回目の役割行
# 変更時と同じ前例・同じ意図的な非分岐、3V専用の別定義は新設していない)。
VOICES_COMMENT_3_ROLE = """あなたはPodcastのナビゲーターです。リスナーは、ある問いに対する
異なる立場からの一人称の語りをすべて聞き終わり、
これから「なぜ同じ状況を人によって違って感じるのか」という視点の深掘りを聞きます。
その間に流す、Comment 3(役割: 「どの声が正しいか」ではなく「なぜ違って感じるのか」
への視点の移動)を書いてください。
