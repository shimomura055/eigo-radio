# ============================================================
# er003_v1_sing01_assemble_v2_generate.py
# ER-003-B1-NOVEL-AUDIO-01-VOICE-01: Voice再配置後のFull Audio組み立て
# ============================================================
# Aoede=本文Listening対象、Charon=Navigator/Explanationという役割分担
# に沿ってSING01を再組み立てする。記事本文・Fact・B1 Scaffold構造は
# 一切変更しない。Audio Shell(SFX/Intro-Outro構造/segment order/
# post-processing/volume normalization/pause思想)もAUDIO-03/NOVEL-
# AUDIO-01から不要に変更しない。新規に追加した音声は、Point One/Two
# 本文直前の"First point."/"Second point."見出し(Charon)のみ
# (仕様指定の"Point One."/"Point Two."は、英語の小数点読みと同一語列の
# ためTTSが安定して発音できないことが判明し、ユーザー確認のうえ代替
# 文言を採用。詳細はVOICE-01報告書C節参照)。
#
# 実行方法:
#   .venv/Scripts/python.exe er003_v1_sing01_assemble_v2_generate.py
...
# AUDIO-03と無変更で継承するShell定数
POINT_EXPLANATION_PAUSE_SECONDS = audio03.POINT_EXPLANATION_PAUSE_SECONDS  # 0.7秒
...
# 新規: Point見出し→本文間のpause。既存Shellで唯一の「短い見出しラベル→
# 本文」という同型パターンであるPoint explanation→Previewの0.7秒を、
# 新しい値を作らずそのまま転用する(整合性確認済み、報告書D節)。
POINT_HEADING_TO_BODY_PAUSE_SECONDS = POINT_EXPLANATION_PAUSE_SECONDS
...
def load_all_sources() -> dict:
    ...
    # Navigator/Explanation役 = Charon(今回新規生成した_charon.wav)
    narration = {}
    for name in ("welcome", "preview_intro", "point_explanation", "key_phrases_intro", "full_story_intro"):
        src_name = "point_explanation_en" if name == "point_explanation" else name
        mono, sr, _, _ = common.read_wav_float(f"{NARRATION_DIR}/{src_name}_charon.wav")
    ...
    for i, name in enumerate(("num_one", "num_two", "num_three", "num_four", "num_five"), start=1):
        mono, sr, _, _ = common.read_wav_float(f"{NARRATION_DIR}/{name}_charon.wav")
        assert sr == common.SAMPLE_RATE
        narration[name] = mono
    # 新規: Point見出し(Charon)
    mono, sr, _, _ = common.read_wav_float(f"{NARRATION_DIR}/point_one_heading_charon.wav")
    narration["point_one_heading"] = mono
    mono, sr, _, _ = common.read_wav_float(f"{NARRATION_DIR}/point_two_heading_charon.wav")
    narration["point_two_heading"] = mono

    # 本文Listening対象 = Aoede(無変更、既存音声を再利用)
    b1_segments = {}
    ...
    # Point One/Two本文・In One LineはNavigator役としてCharonへ変更(新規生成)
    for name in ("point_one", "point_two", "in_one_line"):
        mono, sr, _, _ = common.read_wav_float(f"{NARRATION_DIR}/{name}_charon.wav")
        assert sr == common.SAMPLE_RATE
        b1_segments[name] = mono
...
def apply_gain_and_convert(sources: dict) -> dict:
    """gain方式はAUDIO-03/NOVEL-AUDIO-01と無変更。target_rms = Preview
    とFull Story Part1のRMS平均(いずれもAoede、無変更のため基準は
    そのまま)。Previewは無加工のアンカー。"""
...
def build_pieces_with_timeline(parts: dict) -> list:
    ...
    seq += [
        ...
        ("pause_0.8", p9a.silence_stereo(COMMENT_JA_TO_EN_PAUSE_SECONDS)),
        ("Point One heading (Charon)", parts["point_one_heading"]),
        ("pause_0.7_point_heading", p9a.silence_stereo(POINT_HEADING_TO_BODY_PAUSE_SECONDS)),
        ("Point One (Charon)", b1["point_one"]),
        ("pause_0.8_point1_to_point2", p9a.silence_stereo(POINT1_TO_POINT2_PAUSE_SECONDS)),
        ("Point Two heading (Charon)", parts["point_two_heading"]),
        ("pause_0.7_point_heading", p9a.silence_stereo(POINT_HEADING_TO_BODY_PAUSE_SECONDS)),
        ("Point Two (Charon)", b1["point_two"]),
        ("pause_1.0", p9a.silence_stereo(COMMENT_EN_TO_JA_PAUSE_SECONDS)),
...
def main():
    ...
    print("[SING01-VOICE01-ASSEMBLE] 音源読み込み開始...")
    sources = load_all_sources()
    print("[SING01-VOICE01-ASSEMBLE] gain調整開始...")
    parts = apply_gain_and_convert(sources)
    print("[SING01-VOICE01-ASSEMBLE] assemble開始...")
    result = assemble_with_timeline(parts)
    assembled = result["assembled"]

    out_path = f"{OUT_DIR}/assembled/English_Your_Way_B1_SING01_v2.wav"
    ...
    print("[SING01-VOICE01-ASSEMBLE] 完了。summary:", json.dumps(summary, ensure_ascii=False, default=str))
