...
**実施日: 2026-08-13**
**ステータス: `VALIDATED(A02単独検証) / NOT_YET_GENERALIZED`(ユーザー承認済み。
Point Balanceという設計判断はA02について検証済みだが、全ジャンルへの
一律適用・Production仕様化はまだ行っていない。詳細はK節、
[DECISION_LOG.md](DECISION_LOG.md)「ER-003-SPOKEN-FIRST-03: Point
Balance」項参照)**
...
## K. 検証済みの設計判断(Validated Design Judgment)

2026-08-13、ユーザーおよびレビュー側が本結果を承認したことを受け、
今回得られた知見を「検証済みの設計判断」として以下に整理する。
別セッションのClaude Codeがこの記録だけを読んでも、**何が確定して
何が未確定か**を誤解しないよう、確定事項と非確定事項を明確に分離する。

### K-1. 確定した設計判断(A02で検証済み)

1. **Point One / Point Twoに、本編と同程度の情報量を持たせない。**
   本編は事実の経過を伝える場所、Pointは意味づけ・別の切り口を短く
   示す場所であり、役割が異なる。SPOKEN-FIRST-02のA02では、Point One
   (144語)が本編(166語)とほぼ同等になり、この役割分担が崩れていた
   (G節)。
2. **Pointは本編の再説明ではなく、別の切り口・示唆・注意点等を短く
   提示する場所である。** 試行の内訳数値(300/80世帯等)や介入群の
   構成といった方法論的詳細は、Fact Ledgerには保持しつつ、Point本文
   からは削ってよい(D・E節)。
3. **今回のA02では、40〜50語程度(実測: Point One=47語、
   Point Two=44語)が自然に機能した。** 目標30〜60語・許容25〜70語
   という範囲設定のもとで、この長さで核心メッセージを保持したまま
   自然な文章が得られた(C節)。

### K-2. 明示的に確定していない事項(誤読防止のため明記)

4. **「各Pointを必ず40〜50語にする」という固定ルールにはしない。**
   K-1-3の数字はA02という1記事1ジャンルでの実測結果であり、
   目標として与えた30〜60語という範囲内でモデルが自然に収束した値
   にすぎない。他記事・他ジャンルに同じ数値を機械的に適用してよい
   という意味ではない。
5. **記事ジャンルや内容によって必要な長さは異なるため、「本編との
   役割・比重」を優先する。** 判断基準は絶対語数ではなく、
   「PointがK-1-1/K-1-2の役割を果たせているか」「本編に対して
   適切な比重(補足として機能する長さ)になっているか」である
   (G節の新規軸「Point proportionality」参照)。
6. **A01など、元々Pointが短く適切な記事まで機械的に圧縮しない。**
   SPOKEN-FIRST-02の実測では、A01のPoint One=37語・Point Two=60語
   であり、A02のような「第二の本編」化は起きていない。この設計判断
   は「A02で確認された問題を機械的な数値ルールとして全記事へ適用
   する」ことを意味しない。適用要否は記事ごとに個別判断する。
7. **Factを削る・まとめる際もVerified Fact Ledgerから逸脱しない
   ことを引き続き必須とする。** 圧縮のためにLedger外の情報を追加
   したり、数値のscopeを変更したりしてはならない(F節のFact
   Checker PASS・Ledger Deviation Check LEDGER_COMPLIANTは、この
   制約を満たした上での圧縮だったことの根拠)。

### K-3. まだ検証していないこと(今回のスコープ外)

- A01・ADD03への横展開(Point比重の問題が実際に発生しているか自体
  が未確認)
- 全ジャンルへの一律適用
- Production仕様(CURRENT_SPEC.md等)への組み込み
- OPEN-35の変更
- TTS/音声生成仕様の変更

これらはJ節の「次のDecision」で提案した横展開検証(選択肢1)を
経てから判断する。

### K-4. なぜPointを短くする方向になったのか(経緯の要約)

SPOKEN-FIRST-01/01-R1で確立したListening-first方式(数字密度削減)
をSPOKEN-FIRST-02でA01・A02に横展開した際、A02のPoint Oneが本編と
ほぼ同じ長さ(144語)になっていることが判明した。これは数字密度削減
とは別の問題(Pointの役割そのものが本編化していた)であり、
SPOKEN-FIRST-03はこの問題だけを狙って是正するために実施した
(A・B節)。「短くすること」自体が目的ではなく、「Pointが本来の
役割(本編の短い補足)を果たす長さに戻すこと」が目的である。
