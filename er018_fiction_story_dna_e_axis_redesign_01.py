# ============================================================
# er018_fiction_story_dna_e_axis_redesign_01.py
# 管理ID: FICTION-STORY-DNA-E-AXIS-REDESIGN-01
# ============================================================
# 目的: 前回Trial(FICTION-CORE-PROVOCATION-PROMPT-BIAS-RETRIAL-01、
# er018_fiction_core_provocation_prompt_bias_retrial_01.py)をコピーし、
# **E軸(World / distance from reality)の値リストだけ**をユーザー決定済みの
# 新6値(contemporary everyday reality / realistic but socially unusual
# situation / realistic but emotionally unusual situation / future /
# reality with one changed rule / supernatural or fantastical world)へ
# 差し替える。それ以外(A/B/C/D軸の値、2-3軸抽選ロジック、seed、Run5
# Coverage Runの仕組み、4共通原則、語数280-420、Prompt Bias除去済みPrompt文言)
# は一切変更しない。
#
# Run5 Coverage Runの母集団(旧E軸ではFuture系3値: a near future / a greatly
# changed future / technology-institution world)は、新E軸のどの値に対応
# させるか設計判断が必要だった。判断は行わず、(a) futureの1値のみに対応、
# (b) 非現実系3値(future/one changed rule/supernatural)に対応、の2案を
# 本ディレクトリのe_axis_redesign_notes.mdに記録し、**実行は(a)**(暫定判断、
# REPORTに明記)。
#
# 前回スクリプトer018_fiction_core_provocation_prompt_bias_retrial_01.pyと
# 出力er018_output/fiction_core_provocation_prompt_bias_retrial_01/、および
# さらに前のer018_fiction_core_provocation_random_dna_trial_01.pyと出力は
# 一切変更しない(read-onlyでの参照のみ、analyze stepでの類似度比較用途)。
# Status: Trial専用、未承認draft実装(`APPROVED_FOR_PRODUCTION`ではない)。
# 到達上限: VALIDATED(Production採用は本タスクの対象外)。
#
# 実行方法(root直下、4 step):
#   .venv/Scripts/python.exe er018_fiction_story_dna_e_axis_redesign_01.py \
#       --out-dir er018_output/fiction_story_dna_e_axis_redesign_01 \
#       --step dna --seed-base 20260925
#   ...--step generate --budget-jpy 15
#   ...--step analyze
#   ...--step assemble
# ============================================================
...
AXIS_DEFS = {
    ...
    "E": {
        # 新E軸(FICTION-STORY-DNA-E-AXIS-REDESIGN-01、ユーザー決定済み・逐語)。
        # 旧E軸(near future/greatly changed future/technology-institutions等)は廃止。
        "name_en": "World / distance from reality", "name_ja": "世界設定/現実との距離",
        "values": [
            ("contemporary everyday reality", "現代の日常"),
            ("realistic but socially unusual situation", "現実的だが社会的に珍しい状況"),
            ("realistic but emotionally unusual situation", "現実的だが感情的に珍しい状況"),
            ("future", "未来"),
            ("reality with one changed rule", "ルールが1つだけ違う現実"),
            ("supernatural / fantastical world", "超自然・幻想的な世界"),
        ],
    },
}
AXIS_ORDER = ["A", "B", "C", "D", "E"]
# 委任文のRun5(Coverage Run)用: 旧E軸ではFuture系3値([1,2,3]=a near future/
# a greatly changed future/technology-institution world)を母集団としていた。
# 新E軸への対応は設計判断が必要だったため、2案をe_axis_redesign_notes.mdに
# 記録し、実行は(a)futureの1値のみ(index 3)に対応させる(暫定判断)。
E_FUTURE_VALUE_INDICES = [3]  # 新E軸: index3="future"のみ(決定(a)、暫定)
...
# ------------------------------------------------------------
# Core Provocation Prompt(前回Trial er018_fiction_core_provocation_prompt_
# bias_retrial_01.pyから逐語のまま変更なし。委任文により変更禁止)
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# Story Prompt(前回Trial er018_fiction_core_provocation_prompt_bias_
# retrial_01.pyから逐語のまま変更なし。委任文により変更禁止)
# ------------------------------------------------------------
...
def process_one_run(client, out_dir: str, run_number: int, dna_entry: dict,
    ...
    theme_id_for_log = f"fiction_story_dna_e_axis_redesign_01_run{run_number}"

    provocation_model = routing.require_model_or_override(
        "A2_WRITER", routing.WRITER_MODEL,
        override_reason="FICTION-STORY-DNA-E-AXIS-REDESIGN-01: 自由生成Core "
                         "Provocation(Fiction、1-3案+選定、新E軸版)にA2_WRITER "
                         "Approved Model(Luna)を転用(新規process未定義、コスト影響なし、"
                         "DEV/Trial限定)")
    ...
    story_model = routing.require_model_or_override(
        "A2_WRITER", routing.WRITER_MODEL,
        override_reason="FICTION-STORY-DNA-E-AXIS-REDESIGN-01: 自由生成Story"
                         "(Fiction、Story DNAナッジ付き、新E軸版)にA2_WRITER Approved "
...
# 前回Trial(FICTION-CORE-PROVOCATION-PROMPT-BIAS-RETRIAL-01、Bias除去済み・
# 旧E軸)の5本。今回(新E軸)の5本との相互類似度を見るための比較専用参照
# (read-onlyで読み込むのみ、前回スクリプト・前回出力ディレクトリ自体は変更しない)。
PREVIOUS_TRIAL_OUT_DIR = "er018_output/fiction_core_provocation_prompt_bias_retrial_01"
PREVIOUS_TRIAL_RUN_COUNT = 5
...
def step_analyze(out_dir: str) -> None:
    ...
    # 前回Trial(Bias除去済み・旧E軸)の5本を比較専用ドキュメントとして追加(read-only)。
    previous_texts = {}
    previous_meta = []
    for prev_run_number in range(1, PREVIOUS_TRIAL_RUN_COUNT + 1):
        prev_path = f"{PREVIOUS_TRIAL_OUT_DIR}/runs/run{prev_run_number}/story.md"
        prev_story_md = load_text(prev_path)
        prev_reader_text = extract_reader_text(prev_story_md)
        label = f"old_run{prev_run_number}"
        previous_texts[label] = prev_reader_text
        previous_meta.append({
            "label": label, "path": prev_path, "sha256": sha256_of_file(prev_path),
            "word_count": count_words(prev_reader_text),
        })

    all_docs = {**run_texts, **reference_texts, **previous_texts}
    ...
    save_json(f"{out_dir}/similarity_matrix.json", {
        "labels": labels, "matrix": matrix,
        "note": "labels prefixed old_run* are the 5 stories from the previous "
                "trial (FICTION-CORE-PROVOCATION-PROMPT-BIAS-RETRIAL-01, prompt "
                "bias already removed, old E axis), included for cross-trial "
                "similarity only (read-only reference, not modified).",
    })
    save_json(f"{out_dir}/machine_metrics_summary.json", {
        "per_run_metrics": per_run_metrics, "reference_docs": reference_meta,
        "previous_trial_docs": previous_meta,
    })
    print(f"[step_analyze] DONE. runs={len(per_run_metrics)} "
          f"reference_docs={[m['label'] for m in reference_meta]} "
          f"previous_trial_docs={[m['label'] for m in previous_meta]}")
...
def step_assemble(out_dir: str, seed_base: int) -> None:
    ...
    all_parts = [
        "# FICTION-STORY-DNA-E-AXIS-REDESIGN-01 -- stories_all.md\n",
