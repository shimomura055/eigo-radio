# ============================================================
# er011_open112_trend_theme2_b_final_audio_rerun_01.py
...
THEME_ID = "open112_trend_theme2_b_final_audio_rerun_01"
...
LEVELS = {
    "b1b": {
        ...
        "article_id": "THEME2_B1_FINALAUDIORERUN01",
        "reused_segments": [
    ...
    "a2": {
        ...
        "article_id": "THEME2_A2_FINALAUDIORERUN01",
        "reused_segments": [
...
def preserve_stopped_audio_evidence(level_out_dir: str, label: str, result: dict, wav_path: str) -> None:
    ...
    if os.path.exists(wav_path):
        ...
        print(f"[RERUN01] {label}: status={status} のためlast attempt音声を保全: {dst}")
    else:
        print(f"[RERUN01] {label}: status={status} だが音声ファイルが存在しません"
...
def generate_kp_audio_b1(level_out_dir: str, kp_map: list) -> dict:
    ...
    for row in kp_map:
        ...
        print(f"[RERUN01][b1b] Key Phrase {rank} 英語Component生成: {used_form!r}...")
        ...
        print(f"[RERUN01][b1b] Key Phrase {rank} 日本語(TTS用テキスト)生成: {tts_text!r} "
...
def generate_kp_audio_a2(level_out_dir: str, kp_map: list) -> dict:
    ...
    for i, row in enumerate(kp_map, start=1):
        ...
        used_form = row["used_form"]
        tts_text = row["tts_text"]
        print(f"[RERUN01][a2] Key Phrase {rank} 英語Component生成: {used_form!r}...")
        with cl.segment_context(f"kp{rank}_english"):
            en_path = f"{narration_dir}/kp{rank}_en.wav"
            en_r = shared_narration.ensure_key_phrase_english_component(
                tts_gen.tts_safe_kp_en(used_form), en_path)
        preserve_stopped_audio_evidence(level_out_dir, f"kp{rank}_en", en_r, en_path)
        print(f"[RERUN01][a2] meaning_{i}(TTS用テキスト)生成: {tts_text!r} "
              f"(表示用: {row['display_gloss']!r})...")
...
def run_level(level: str) -> dict:
    ...
    print(f"[RERUN01][{level}] Key Phrase選定+Canonicalization+Redundancy QA "
    ...
    print(f"[RERUN01][{level}] Key Phrase結果: selection={sel_status} "
    ...
    print(f"[RERUN01][{level}] 本文{len(meta['reused_segments'])}segmentをTrial-13(review_lock "
          "RESOLVED確認済み)から再利用(再TTSしない)...")
    reused = reuse_non_kp_segments(level, inputs["parts"], inputs["support"])

    print(f"[RERUN01][{level}] Key Phrase音声(英語+日本語、TTS用テキスト)を新規生成...")
    ...
    if level == "b1b":
        ...
        kp_audio_status = {r: {"en": v["english"].get("status"), "ja": v["japanese_meaning"].get("status")}
                            for r, v in kp_results.items()}
    print(f"[RERUN01][{level}] Key Phrase音声status={kp_audio_status}")
    ...
    try:
        ...
        print(f"[RERUN01][{level}] Assemble GATE_BLOCKED: {e}")
...
def main() -> dict:
    ...
    results = {}
    for level in ["b1b", "a2"]:
        print(f"===== [RERUN01] level={level} 開始 =====")
        try:
            results[level] = run_level(level)
        except RuntimeError as e:
            results[level] = {"level": level, "status": "STOP", "error": str(e)}
            print(f"[RERUN01][{level}] STOP: {e}")

    with open(f"{OUT_DIR}/rerun01_summary.json", "w", encoding="utf-8") as f:
