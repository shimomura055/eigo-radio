# ============================================================
# er003_v1_b1redesign_assemble04.py
# ER-003-POINT-NOTIFICATION-01 追加調整: Point Notification直後の
# 無音ポーズ(0.4秒)を除去
# ============================================================
# ユーザーからの試聴フィードバック: Point Notification自体に予備の
# 空白(トレイリング無音)が含まれているため、その直後にさらに0.4秒の
# ポーズを足すと不必要に長く感じる。v3からの変更は、Point One/Two
# 直前のPoint Notification"直後"のポーズ(pause_0.4_notification_exit)
# のみを除去することに限定する。Notification"前"のポーズ(0.5秒)・
# heading→body間のポーズ(0.7秒)・共有Notification(1〜3)側のポーズ・
# その他のsegment/pause/Comment3文言/semantic heading/In One Line Voice
# はv3からすべて無変更で再利用する。
#
# 実行方法:
#   .venv/Scripts/python.exe er003_v1_b1redesign_assemble04.py
...
HEADING_TO_BODY_PAUSE_SECONDS = audio03.POINT_EXPLANATION_PAUSE_SECONDS
...
def apply_gain_and_convert(sources: dict) -> dict:
    ...
    # Point専用Notificationも既存Notificationと同じtarget_rms・同一処理方式で
    # gain調整する(Point One/Twoで同じ音量・処理になるよう統一)。
...
def main():
    ...
    print("[B1-POINT-NOTIF-V4] 音源読み込み開始...")
    sources = load_all_sources()
    print("[B1-POINT-NOTIF-V4] gain調整開始...")
    parts = apply_gain_and_convert(sources)
    print("[B1-POINT-NOTIF-V4] assemble開始...")
    result = assemble_with_timeline(parts)
    assembled = result["assembled"]

    out_path = f"{OUT_DIR}/assembled/English_Your_Way_B1B_IRAN01_v4.wav"
    ...
    summary = {
        "status": "OK", "article_id": "IRAN01_B1B_v4", "out_path": out_path,
        ...
        "change_from_v3": "Point Notification直後のpause_0.4_notification_exitを除去(Notification自体の予備の空白により長く感じるとのフィードバックのため)",
    }
    with open(f"{OUT_DIR}/run_summary_assemble_v4.json", "w", encoding="utf-8") as f:
        json.dump(summary, f, ensure_ascii=False, indent=2, default=str)
    print("[B1-POINT-NOTIF-V4] 完了。summary:", json.dumps(summary, ensure_ascii=False, default=str))
