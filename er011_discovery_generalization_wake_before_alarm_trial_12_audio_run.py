...
# 管理ID: FAMILY-A-DISCOVERY-GENERALIZATION-WAKE-BEFORE-ALARM-NPLUS1-TRIAL-12-AUDIO-01
# ============================================================
# 目的(ユーザー決定 2026-09-12): 既に記事生成+QA一式が完了している
# FAMILY-A-DISCOVERY-GENERALIZATION-WAKE-BEFORE-ALARM-NPLUS1-TRIAL-12
# (「なぜ目覚ましが鳴る直前に目が覚めることがあるのか?」テーマ、A2/B1B、
# Discovery Focus Module Part A単独、保険文制約なし)の記事(`er011_output/
# discovery_generalization_wake_before_alarm_trial_12/{a2,b1b}/article.md`、本タスクでは
...
# Trial-11(タオル臭テーマ)と同一設計、テーマのみ差し替え(本scriptは
# Trial-11のaudio_run.pyをコピーしテーマ固有定数のみ書き換えたもの)。
#
...
#
# 費用: 前段(記事生成、raw_usage_log.jsonl、実測¥78.98、既存・別タスク)
# とは別に、本タスク(Support+Audio)専用のusage logを新規に使う
# (`audit/raw_usage_log_audio_01.jsonl`)。本タスク単独の費用上限は¥120
# (Trial-11実績を参考)。**ただし管理ID全体[記事+Support+Audio]の合計上限は
# ¥300](ユーザー決定)であり、combined_cost_check()で記事側raw_usage_log.jsonl
# と合算して¥300超過見込みならSTOPする。同期実行のみ。バックグラウンド
# 待機・二重起動禁止。
#
# 実行方法(root直下から):
#   .venv/Scripts/python.exe er011_discovery_generalization_wake_before_alarm_trial_12_audio_run.py [stage ...]
...
ARTICLE_COST_LOG_PATH = f"{OUT_DIR}/raw_usage_log.jsonl"  # 記事生成側(run.py)のlog、参照のみ
BUDGET_JPY_CAP = 120.0
COMBINED_BUDGET_JPY_CAP = 300.0  # ユーザー決定: 管理ID全体[記事+Support+Audio]の合計上限

# ユーザー確定の日本語タイトル(2026-09-12ユーザー決定文の日本語見出しを
# そのまま使用、OPEN-137継続の既知gap: JAPANESE_TITLES辞書に本Trialの
# theme_id未登録のため、A2のjapanese_title segmentへ人手で供給する。
# 新しい主張・数字は追加していない)。
WAKE_JAPANESE_TITLE = "なぜ目覚ましが鳴る直前に目が覚めることがあるのか?"
...
def _sum_log_jpy_simple(log_path: str) -> float:
    if not os.path.exists(log_path):
        return 0.0
    total_usd = 0.0
    with open(log_path, encoding="utf-8") as f:
        for line in f:
            ...
            if not line:
                continue
            cost, _ = _call_cost_usd(json.loads(line))
            total_usd += cost
    return round(total_usd * USD_JPY, 2)


def combined_cost_check() -> dict:
    """管理ID全体(記事生成[run.py] + 本タスク[Support+Audio])の合算費用を
    ¥300上限でチェックする(ユーザー決定の総額上限)。"""
    article_cost = _sum_log_jpy_simple(ARTICLE_COST_LOG_PATH)
    audio_cost = _sum_log_jpy_simple(AUDIO_COST_LOG_PATH)
    total = round(article_cost + audio_cost, 2)
    print(f"[{THEME_ID}][combined_cost_check] 記事生成={article_cost}円 + Support/Audio={audio_cost}円 "
...
def keyphrase_stage(level: str) -> dict:
    ...
    with open(f"{out_dir}/article.md", encoding="utf-8") as f:
        article_text = f.read()
    article_id = f"DISCOVERY_GENERALIZATION_WAKE_BEFORE_ALARM_TRIAL_12_{meta['label']}"
...
def japanese_title_stage() -> dict:
    tts_gen.JAPANESE_TITLES.update({THEME_ID: WAKE_JAPANESE_TITLE})
    note = {
        "gap": "JAPANESE_TITLES辞書にtheme_id(discovery_generalization_wake_before_alarm_trial_12)未登録のため"
               "KeyErrorが発生する(OPEN-137継続の既知gap)。",
        "resolution": "ユーザー提示の日本語タイトルをTrial内で供給(新しい主張・数字は追加しない)。",
        "supplied_title": WAKE_JAPANESE_TITLE,
        "supplied_by": "user (2026-09-10 Fable委任文)",
    }
    save_json(f"{OUT_DIR}/audit/a2_japanese_title_gap_note.json", note)
    print(f"[{THEME_ID}][a2] JAPANESE_TITLES登録(gap fill・ユーザー提示タイトル): {WAKE_JAPANESE_TITLE!r}")
    return note
...
def player_stage() -> dict:
    ...
    note = """
<p class="note">
このページはFAMILY-A-DISCOVERY-GENERALIZATION-WAKE-BEFORE-ALARM-NPLUS1-TRIAL-12-AUDIO-01
...
<head>
<meta charset="utf-8">
<title>DISCOVERY-GENERALIZATION-WAKE-BEFORE-ALARM-NPLUS1-TRIAL-12-AUDIO-01</title>
<style>
{arp.PLAYER_STANDARD_CSS}
</style>
</head>
<body>
<h1>FAMILY-A-DISCOVERY-GENERALIZATION-WAKE-BEFORE-ALARM-NPLUS1-TRIAL-12-AUDIO-01 — 「なぜ目覚ましが鳴る直前に目が覚めることがあるのか?」テーマ(A2/B1B、Discovery Focus Module Part A単独)</h1>
{note}
