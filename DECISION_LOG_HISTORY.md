...
> **本ファイルの位置づけ**: 確定した意思決定の管理場所は引き続き`DECISION_LOG.md`のみである。本ファイルは`DECISION_LOG.md`が肥大化したため、古いエントリ(ER-PM-001ヘッダーチェーンの旧12件、および本体の古い決定エントリ)の**履歴全文をそのまま**切り出した先であり、別の管理場所ではない。要約・言い換え・削除は一切行っていない(原文のまま)。

---

## ER-PM-001_CHAIN

**最終更新(直前の記録): 2026-09-09(PM-CLOSEOUT-CONSOLIDATION-47、Family A再改善・Lane B継続のユーザー決定反映)**: 管理ID`PM-CLOSEOUT-CONSOLIDATION-47`(Sonnet委任、SSOT・Git担当)により、2026-09-09付ユーザー正式決定を記録する。**(1) A3-UDR-3・D2-UDR-1不承認**: A3-UDR-3(News Focus Module+Point Role hint、`FAMILY-A-COMPLETION-A3-NEWS-FOCUS-HINT-COMPARISON-TRIAL-06`)・D2-UDR-1(Discovery Focus Module、`FAMILY-A-COMPLETION-A4-DISCOVERY-LAYER3-TRIAL-07`)の**現行Production採用案をユーザーが不承認**とした。理由: News focus_hint NG率50%(Trial-06)はProduction採用水準ではない、DiscoveryのREVIEW_REQUIRED率がbaseline比約5倍(17%→67%)に増加、DiscoveryでPoint Role分類がmechanism/myth_correctionへ100%収束しPoint多様性が低下、既存仕様全体(過去の類似対策・既存Production仕様・retry/QA/Validatorとの重複・競合)を俯瞰した整理が未了であること。**Focus Module/Point Roleという考え方自体はREJECTEDではない**(既存Trial結果のStatus[VALIDATED等]は保持)。A3-UDR-3/D2-UDR-1の現行案は`APPROVED_FOR_PRODUCTION`へ進めない(今回到達可能な最大Status=VALIDATEDまたはUSER_DECISION_REQUIRED)。必要な追加Trialは承認済み、記事内容・比較Artifactのユーザー確認は可。**(2) 最優先事項**: PM/Reconciliation Gate(read-only横断整理、管理ID`FAMILY-A-POINT-QUALITY-CONTROL-RECONCILIATION-GATE-01`、実施中)を最優先とし、完了後にNews再改善(A3-UDR-3の再改善として扱う、新Trial ID可、原因分解A〜I、比較Artifact[approved/reference・baseline・現行focus_hint・改善候補]を用意、Production配線なし)・Discovery再改善(D2-UDR-1の再改善、Point多様性の確保、REVIEW_REQUIRED増加の原因分解、Ledger注記案は自動採用しない、比較Artifact[Householdオリジナル・baseline・現行Focus Module・改善案]をA2/B1Bで用意、ユーザーレビュー可能段階でSTOP可)へ進む。News/DiscoveryいずれもProductionコードは変更しない(Trial harness内)。**(3) D2-UDR-2承認**: Household Ledger FACT-03を既存Research正式経路で再検証(管理ID`HOUSEHOLD-LEDGER-FACT-03-REVERIFICATION-01`、実施中)。完成audioの遡及修正は不要、新しいFact policyは作らない。**(4) B-3V-1承認(b)**: 3V Trial-02のTensionのみ75〜90語へ短縮する再生成1回(`EDITORIAL-B-FAMILY-VOICES-3V-PERSON-VOICE-TRIAL-03`、実施中)。保持条件7点を維持できればcloseout、崩れた場合は395秒許容案を含めUSER_DECISION_REQUIREDとする。**(5) B-3V-2承認**: 良好な3V記事候補が確定したら3V Audio Trial(3 voices、3V required_structure、3V Comment文言、voice割当、TTS QA、Audio structural gate、実測尺、Fact整合、試聴artifact)を実施する。Trial成功のみでのProduction採用はしない。**(6) B-FC-1承認(b)観測継続**: cacheはProduction実装しない。量産時に記録する項目: Fact Checker A' call数・web search回数・correction attempts・**Fact A'単体費用(Trial全体費用と分離)**・episode全体費用・latency・FAIL/REVIEW/PASS件数・同一fact・sourceへの重複検索。cache導入可否は観測後に別途USER_DECISION_REQUIREDとして判断する。安全性を下げるcost optimizationは禁止。**(7) Trend Key Phrase既決事項確認**: 既に実施済み(承認済み再生成1回→不合格→既存選定経路でwork to do/「まだ取り組むべき課題」へ差し替え→QA/Gate PASS→B1B完走、commit`7b1166f`)、追加確認不要。**(8) 並列実行**: Lane A再設計とLane Bは並列可、同一共有Productionコードへの競合変更が出た場合はSTOP。**(9) 報告方法**: 各Lane 12項目(管理ID/今回やったこと/根本原因/Before-After/Trial結果/cost/regression・runtime evidence/status/Production変更有無/UDR/artifact・試聴link/remaining open items)で報告する。News/Discoveryは「何を追加したか」より「既存の仕組みをどう整理したか」を報告する。**(10) STOP条件・Closeout mandatory check**: 既存どおりに加え、A3-UDR-3/D2-UDR-1の旧Production推奨が承認済みと誤って残らないようSSOT statusを確認する運用を追加した。本タスクではOPEN_ITEMS.md(OPEN-135/OPEN-112/OPEN-120/OPEN-136/OPEN-138行)・CURRENT_SPEC.md(News/Discovery節・changelog)・`docs/pm/PM_GOVERNANCE.md`(2-2節、Fact Checker等の外部呼び出し費用分離報告の明記)・`docs/pm/MODEL_ROUTING_TRIAL_LOG.md`・`docs/pm/ACTIVE_TASK.md`・`docs/pm/RESULT_PACKET.md`へ反映した。Production/Prompt編集・新規Trial着手はいずれも実施していない。詳細は`OPEN-135`行・`OPEN-112`行・`OPEN-120`行・`OPEN-136`行・`OPEN-138`行参照。

**最終更新(直前の記録): 2026-09-09(PM-CLOSEOUT-CONSOLIDATION-36、4V Trial-02結果のSSOT反映)**: 管理ID`PM-CLOSEOUT-CONSOLIDATION-36`(Sonnet委任、SSOT・Git担当)により、`EDITORIAL-B-FAMILY-VOICES-4V-ARTICLE-TRIAL-02_REPORT.md`の結果をSSOTへ反映した。**(1) Reconciliation結論**: 一人称"I"指示は2V(Trial-07/09)promptにも元々存在せず(Trial-07ヘッダー「一人称/三人称は固定しない」、Writerの自発的選択によるもの)、ユーザーが2026-09-08に`APPROVED_FOR_PRODUCTION`(未配線)と正式決定した4項目の一つ「Voice A/Bの一人称"I"記述」原則をTrial-02 Focus Module Blockへ初めて明示適用したものであり、新原則の創作ではない(矛盾なし、STOP該当なし)。Leakage是正フィードバック方式もTrial-07由来の既存機構(Trial-01複製、コード無変更)。**(2) 結果**: 3 attempts(MAX)実施。B-4V-1(一人称欠落)は解消——4 Voiceすべてが冒頭文から一人称"I"で開始、Voiceセクション一人称使用率はVoice1/2=50%・Voice3/4=33%(Trial-01は全section0%)。B-4V-2(Analytical Leakage Check flagged)はMAX_WRITER_ATTEMPTS到達後も未解消(voice_3/4のDiscovery型逆戻り持続、Trial-01のF2 failure modeが再現、新規failure modeではない)。Fact Checker A' 3/3 PASS、Ledger Deviation MAJOR 0件(Local Rewrite不要、Trial-01より改善)、469語・385秒(目標380〜430秒内)、Point Overlap 0.0〜0.154(any_flagged=false)。Pairwise Voice Distinctness Check(一括判定方式)direction_agreement_rate=0.933・method_agreement_rate=0.933(いずれもTrial-01[0.9/0.767]より改善)。費用¥76.6(Writer¥74.49+QA¥2.08、上限¥150以内)、量産概算¥70〜80/記事(Ledger Deviation Checkerのweb_search増が主因、既存Production primitive無変更)。**Gate 1分類**: `USER_DECISION_REQUIRED`(B-4V-1達成・B-4V-2未解消のため、Leakage扱いの十分性・一人称使用率33〜50%の十分性がユーザー判断待ち)。**Gate 4**: Production・Trial-07・Trial-01いずれも`git diff`で無変更を確認、SSOT・Git操作は本タスクで初めて実施。**SSOT反映**: `OPEN_ITEMS.md`OPEN-120行(Trial-02結果・Distinctness一括方式一致率実績を蓄積記録として追記)・OPEN-132行(一人称機械保証要件への実測値[33〜50%]注記)、`docs/pm/MODEL_ROUTING_TRIAL_LOG.md`(Trial-02[Sonnet、HIGH、215k・1903秒、差し戻し0]+本タスクを追記)。**Gate 4 Dangling Reference Check**: 本エントリが参照した管理ID(`EDITORIAL-B-FAMILY-VOICES-4V-ARTICLE-TRIAL-02`、OPEN-120/132)はいずれも既存`OPEN_ITEMS.md`/本ファイル内に既出であり、未定義参照なし(結果:PASS)。`CURRENT_SPEC.md`・`er006_output/`・`er011_output/attempt_history.jsonl`はいずれも本タスクでは編集・stageしていない。**Git操作**: G1(`er012_editorial_b_voices_4v_article_trial_02.py`・`EDITORIAL-B-FAMILY-VOICES-4V-ARTICLE-TRIAL-02_REPORT.md`・`er012_output/editorial_b_voices_4v_article_trial_02/`配下成果物)とG2(SSOT2ファイル`DECISION_LOG.md`・`OPEN_ITEMS.md`+`docs/pm/MODEL_ROUTING_TRIAL_LOG.md`)を別commitでファイル名指定commitし`origin/main`へpush。詳細は`EDITORIAL-B-FAMILY-VOICES-4V-ARTICLE-TRIAL-02_REPORT.md`、`OPEN_ITEMS.md`OPEN-120/132行参照。

**最終更新(直前の記録): 2026-09-09(PM-GOVERNANCE-AUTONOMOUS-OBVIOUS-FIX-RULE-04、B-4V-1〜4決定+「自明な修正の自律実施」運用方針新設)**: 管理ID`PM-GOVERNANCE-AUTONOMOUS-OBVIOUS-FIX-RULE-04`(Sonnet委任、SSOT・Git担当、並列稼働中の4V Trial-02[`EDITORIAL-B-FAMILY-VOICES-4V-ARTICLE-TRIAL-02`]は本タスクでは触れていない)により以下を反映した。**(1) B-4V-1〜4のユーザー正式決定**: `EDITORIAL-B-FAMILY-VOICES-4V-ARTICLE-TRIAL-01`(前タスクPM-CLOSEOUT-CONSOLIDATION-34でOPEN-120行へ記録済み)が発見した新規failure mode 3件・未確定QA方式1件について、ユーザーが以下を決定した。B-4V-1(F1: 一人称"I"不使用、3 attemptとも三人称記述)=方式(i)既存対策・仕様Reconciliation Check(`PM_GOVERNANCE.md`2-1)後に一人称明示で再生成。B-4V-2(F2: Analytical Leakage CheckがVoice2/3/4でflagged状態のままMAX_WRITER_ATTEMPTS到達)=方式(i)既存Leakage feedback方式で再試行。B-4V-3(F3: Production共通`validate_point_structure()`のh3_count≠2 hard-failが4V記事を無条件で弾く)=`OPEN-132`(Phase 2 Writer配線時チェックリスト)での追跡のみとし、本Trialでの個別対応は行わない(Trial側の迂回関数`run_writer_no_search()`直接呼び出し・Production側gateはいずれも無変更のまま維持)。B-4V-4(Pairwise Voice Distinctness Check、LLMベースQA新設候補)=一括判定方式をTrialとして継続し、正式採用(モデル選定ルールの恒久化に準じたQA方式の恒久化)は一致率・精度実績の蓄積後に別途`USER_DECISION_REQUIRED`として提示する(現時点でAPPROVED_FOR_PRODUCTIONではない)。上記方針に基づき`EDITORIAL-B-FAMILY-VOICES-4V-ARTICLE-TRIAL-02`(4V Trial-02)を実施中。**(2) 運用方針「自明な修正は自律的に進める。仕様判断はユーザーに戻す」の新設**: ユーザーの意図が明確で「そのままでは不適切なので修正すべき」と自明なfailure(例: 上記B-4V-1/2)は、今後は原則として、まずFable側で既存承認仕様・既存retry/fallback・過去実装(`PM_GOVERNANCE.md`2-1のReconciliation Check)を確認し、承認済み範囲内で改善または少なくとも改善Trialを実施してからユーザーへ上げる(コスト影響評価[2-2]が必要な場合はReconciliation Checkの後・報告前に行う)。「聞いても直してとなる」事項を毎回`USER_DECISION_REQUIRED`で止めない。**ただし以下は従来どおりSTOP必須(ユーザー判断必須)**: (1)新仕様の追加、(2)既存承認仕様の変更、(3)Production挙動の意味的変更、(4)QCD影響が大きい変更、(5)複数の妥当な選択肢がありユーザー価値判断が必要な場合、(6)未承認のQA・Validator・Prompt原則のProduction採用。失敗・副作用・新たな仕様判断が発生した場合のみ、その時点で`USER_DECISION_REQUIRED`としてSTOPする。**SSOT反映**: `docs/pm/PM_GOVERNANCE.md`(11節へ新小節「自明な修正の自律実施」新設、9-1ユーザー判断選択基準へ1行追記、冒頭changelog+末尾変更履歴)、`docs/pm/PM_BRIEF.md`(1行追記)、`OPEN_ITEMS.md`(OPEN-120行末尾へB-4V-1〜4決定・4V Trial-02実施中を追記)、`docs/pm/MODEL_ROUTING_TRIAL_LOG.md`(4V Trial-02継続実績+本タスクを追記)。本タスクはProduction/Promptコードを一切変更していない(SSOT・Git反映のみ)。**Gate 4 Dangling Reference Check**: 本エントリが参照した管理ID(`EDITORIAL-B-FAMILY-VOICES-4V-ARTICLE-TRIAL-01/02`、OPEN-120/132)はいずれも既存`OPEN_ITEMS.md`/本ファイル内に既出であり、未定義参照なし(結果:PASS)。4V Trial関連の`er012_output/editorial_b_voices_4v_article_trial_02/`・`CURRENT_SPEC.md`・`er006_output/`・`er011_output/attempt_history.jsonl`はいずれも本タスクでは未変更(4V Trial-02側が並列に担当)。

**最終更新(直前の記録): 2026-09-09(PM-CLOSEOUT-CONSOLIDATION-34、OPEN-112 G1のFable最終受入=`PRODUCTION_WIRED`確定+観測run記録開始+4V本文Trial結果のOPEN-120訂正+OPEN-132 Phase 2追加)**: 管理ID`PM-CLOSEOUT-CONSOLIDATION-34`(Sonnet委任、SSOT・Git担当)により以下を反映した。**(1) OPEN-112 G1受入**: PM-CLOSEOUT-CONSOLIDATION-33で「`PRODUCTION_WIRED`候補(Fable受入待ち)」としていた`OPEN-112-DIAGNOSTIC-RETRY-POINT-BODY-REGRESSION-FIX-01`のG1修正(`build_diagnostic_section()`への前回Point One/Two実本文配線)を、Fableが承認済み挙動(ER-009-N1-DIAGNOSTIC-FULL-RETRY-CLOSEOUT-14)への回帰修正として`PRODUCTION_WIRED`と正式受入した(commit `8596f34`/SSOT `bc06d0c`は既存)。`CURRENT_SPEC.md`Diagnostic Full Retry行のG1状態表記を「候補」から確定へ更新した。Point Overlap問題自体は未解決のまま(NG率改善は別論点)であり、`OPEN_ITEMS.md`OPEN-134行のExit条件観測へ統合し継続する。**(2) 観測record初回記録**: G1のRuntime evidence(Hanshin、Production正式path`run_writer_for_theme`、mode指定なし、B1B/A2各1、いずれも`point_overlap_article_retry_attempts=2`でLoop Budget上限までretryしても解消せず`NG_REVIEW_REQUIRED`)を観測run #1(B1B)・#2(A2)として`er011_output/point_overlap_observation_log.jsonl`(README記載schemaに準拠)へ追記した。両runとも`ng_cause`は優先順位規約(Point Overlap→Value QA→Fact Checker→Ledger Deviation→その他)に従い「Point Overlap(retry後もNG)」(`lexical_flagged_final`/`value_qa_flagged_final`ともtrue)。Fact Checker/Ledger Deviationは本runtime evidence(Diagnostic Full Retry検証用の的を絞った実行)では未取得のため`fact_verdict`/`ledger_status`はnullで記録した。観測20 run中2 run消化、残り18 run。**(3) OPEN-120訂正(4V本文Trial)**: 前タスク(CONSOLIDATION-33)がOPEN-120行へ記録した「ユーザー操作により中断」を訂正する。`EDITORIAL-B-FAMILY-VOICES-4V-ARTICLE-TRIAL-01`は同一管理IDでユーザーの誤操作停止後に再開・完了した。Writer stage(attempt1〜3、Fact Checker A' 3/3 PASS、Ledger Deviation COMPLIANT)・QA stage(Point Overlap Monitoring 16値[0.042〜0.171、閾値0.40未満]、Pairwise Voice Distinctness Check[direction_agreement_rate=0.9/method_agreement_rate=0.767]、Perspective Diversity目視確認)完了、523語・推定尺411.9秒、費用合計¥37.2(上限¥300以内)。新規failure mode 3件を発見しSTOP: F1=一人称"I"不使用(3 attemptとも三人称、既存2V採用記事とは対照的、Analytical Leakage Checkに人称違反検出項目が無く検出漏れ)、F2=Analytical Leakage CheckがVoice 2/3/4でflagged状態のままMAX_WRITER_ATTEMPTS(3)到達、F3=Production共通`validate_point_structure()`の見出し数(`h3_count`)≠2 hard-failが4V記事を無条件で弾くため、Trial側が`run_writer_no_search()`直接呼び出しで迂回した(Production側のgateは無変更)。状態は`USER_DECISION_REQUIRED`(候補B-4V-1〜4提示中)。Production/Trial-07/registry/Contractは無変更。**(4) OPEN-132 Phase 2追加**: 上記F3・F1を踏まえ、Phase 2チェックリストへ(9)見出し数gateのregistry駆動化(現状h3_count=2ハードコード、Voice数可変記事対応の必須改修候補、未実装)、(10)一人称使用の機械的保証(prompt明記+専用QA項目、人手目視に依存しない仕組みの必要性実証)の2項目を追加した。**SSOT反映**: `CURRENT_SPEC.md`(Diagnostic Full Retry行のG1確定化、changelog1件)、`OPEN_ITEMS.md`(OPEN-112/134/120/132行)、`docs/pm/MODEL_ROUTING_TRIAL_LOG.md`(G1配線+CONSOLIDATION-33実績・4V Trial再開実績・本タスクを追記)、`er011_output/point_overlap_observation_log.jsonl`(観測run #1・#2追記)。**Gate 4 Dangling Reference Check**: 本エントリが参照した管理ID(`OPEN-112-DIAGNOSTIC-RETRY-POINT-BODY-REGRESSION-FIX-01`、`EDITORIAL-B-FAMILY-VOICES-4V-ARTICLE-TRIAL-01`、OPEN-112/120/132/134)はいずれも既存`OPEN_ITEMS.md`/本ファイル内に既出であり、未定義参照なし(結果:PASS)。`er006_output/`・`er011_output/attempt_history.jsonl`・`er009_n1_diagnostic_full_retry_verification_14.py`(git未追跡の既存ファイル)はいずれも編集・stageしていない。**Git操作**: G1(4V Trial関連: `er012_editorial_b_voices_4v_article_trial_01.py`・`EDITORIAL-B-FAMILY-VOICES-4V-ARTICLE-TRIAL-01_REPORT.md`・`er012_output/editorial_b_voices_4v_article_trial_01/`配下成果物)とG2(SSOT3ファイル+`docs/pm/MODEL_ROUTING_TRIAL_LOG.md`+`er011_output/point_overlap_observation_log.jsonl`)を別commitでファイル名指定commitし`origin/main`へpush。詳細は`EDITORIAL-B-FAMILY-VOICES-4V-ARTICLE-TRIAL-01_REPORT.md`、`OPEN-112-DIAGNOSTIC-RETRY-POINT-BODY-REGRESSION-FIX-01_REPORT.md`、`OPEN_ITEMS.md`OPEN-112/120/132/134行参照。

**最終更新(直前の記録): 2026-09-09(PM-CLOSEOUT-CONSOLIDATION-33、OPEN-112-DIAGNOSTIC-RETRY-POINT-BODY-REGRESSION-FIX-01のGate 3配線+CURRENT_SPEC訂正+観測Exit条件記録)**: ユーザー正式決定(2026-09-09)3件を受け、(1)G1(`build_diagnostic_section()`の前回Point本文プレースホルダー化)をA-UDR-20=(i)「改善施策ではなく承認済み挙動(ER-009-N1-DIAGNOSTIC-FULL-RETRY-CLOSEOUT-14)への回帰修正」としてSonnet(OPEN-112-DIAGNOSTIC-RETRY-POINT-BODY-REGRESSION-FIX-01)がGate 3配線した。`previous_point_one_text`/`previous_point_two_text`(既定値なし、必須引数)を追加し、唯一のProduction呼び出し元`build_diagnostic_retry_prompt()`(`er003_v1_n3_01_articles_generate.py`)が既に抽出済みのPoint One/Two本文を渡すよう修正、閾値(0.40)・`POINT_OVERLAP_ARTICLE_RETRY_MAX`(2)・retry判定ロジックは無変更。新規回帰テスト6件PASS、`run_project_regression.py`(collected=2253/passed=2250/failed=3[既知の無関係failureのみ]/errors=0)。Runtime evidence: Production正式初回path(`run_writer_for_theme`、mode指定なし、Hanshin Ledger)を実行し、attempt1でB1B/A2ともDiagnostic Full Retryが実発火(retry 2回)、捕捉した診断promptに旧プレースホルダー文字列0件・実際の前回Point One/Two本文(例: Montero fifth-inning本文)が実際に埋め込まれていることを確認した(¥24.14、上限¥60以内。実行過程でスクリプト側のバグ[`run_writer_for_theme()`の戻り値ラッピングを見落とし発火判定を誤り、不要な追加attemptへ一時進んだ]が発生したが上限超過なし、発見後修正し1回で完了することを再確認)。Sonnetは`PRODUCTION_WIRED`を宣言せず、Fable/ユーザーの最終受入待ちとした。(2)A-UDR-21=(ii)「cross_point_overlapのstill_flagged統合(OPEN-133)は今すぐ実装せず、CURRENT_SPECを実態(未実装)に合わせて訂正する」を反映し、`CURRENT_SPEC.md`「Point One対Point Twoのlexical overlap検査(追加ペア)」行の前回注記(「実装未確認・OPEN-133で追跡」)を、「retry判定[still_flagged]統合は`DEFERRED`(未実装)、将来実装は別途判断」へ訂正した(統合方針自体の`DECIDED`は維持、実装状況のみ訂正)。(3)A-UDR-22=(i)「残存NG要因(OPEN-134)は別Trialを起こさずA-UDR-7の継続観測へ統合するが、Exit条件を明文化する」を反映し、Fable案の観測Exit条件全文(対象=Production正式pathのA-Family通常News/Trend Synthesis A2/B1B run、観測量=次の20 runまたは30日、主要因判定=単一failure modeがNG runの50%以上かつ4件以上、全体NG率の再対策閾値=40%以上[8件以上]で再対策検討提示・20%以下で観測終了・中間20〜40%は10 run延長1回まで、報告=10 run中間・20 run/30日最終、baseline比較=Trial-05実測[Hanshin 67〜100%、Theme 2 50%])を`OPEN_ITEMS.md`OPEN-134行へ記録し、記録先雛形`er011_output/point_overlap_observation_log.jsonl`(空ファイル)+README(schema定義)を新設した(実際のrecord追記は今後の別タスク)。`docs/pm/PM_GOVERNANCE.md`3節(PM Closeout Mandatory Check)へ、このExit条件到達確認をGate 5/Gate 6/PM Closeoutで行う旨の項目13を追加した。あわせて(4)ユーザー操作により`EDITORIAL-B-FAMILY-VOICES-4V-ARTICLE-TRIAL-01`が中断されたため、`OPEN_ITEMS.md`OPEN-120行を「ユーザー停止・中断、再開はユーザー指示待ち」へ更新した(`er012_output/editorial_b_voices_4v_article_trial_01/`の部分成果物は残置・stage対象外、実測cumulative cost=¥35.04[b1b_run01のみ、26 API call]をReportへ転記)。**SSOT反映**: `CURRENT_SPEC.md`(changelog第24弾、Diagnostic Full Retry行・ER-011-NO18行の2箇所を更新)、`OPEN_ITEMS.md`(OPEN-112/133/134/120行)、`docs/pm/PM_GOVERNANCE.md`(3節)、`docs/pm/MODEL_ROUTING_TRIAL_LOG.md`(本委任2件[OPEN-112-DIAGNOSTIC-RETRY-POINT-BODY-REGRESSION-FIX-01・PM-CLOSEOUT-CONSOLIDATION-33]+4V Trial中断記録を追記)。**Gate 4 Dangling Reference Check**: 本エントリが参照した管理ID(`OPEN-112-DIAGNOSTIC-RETRY-POINT-BODY-REGRESSION-FIX-01`、`FAMILY-A-POINT-OVERLAP-GAP-FIX-TRIAL-05`、`EDITORIAL-B-FAMILY-VOICES-4V-ARTICLE-TRIAL-01`、OPEN-112/120/133/134)はいずれも既存`OPEN_ITEMS.md`/本ファイル内に既出、または本エントリで新規正式登録した項目であり、未定義参照なし(結果:PASS)。`er006_output/`・`er011_output/attempt_history.jsonl`・`er009_n1_diagnostic_full_retry_verification_14.py`(git未追跡の既存ファイル)はいずれも編集・stageしていない。**Git操作**: G1(修正本体: `er009_diagnostic_full_retry_modules_12.py`・`er003_v1_n3_01_articles_generate.py`・`er009_n1_diagnostic_full_retry_integration_test_13.py`・新規テスト/runtime evidence script/Report/`er011_output/open112_diagnostic_retry_point_body_regression_fix_01_runtime_evidence_01/`)commit `8596f34`。G2(SSOT+観測ログ雛形: 本エントリ含む)は後続commitで反映、`origin/main`へpush。詳細は`OPEN-112-DIAGNOSTIC-RETRY-POINT-BODY-REGRESSION-FIX-01_REPORT.md`、`OPEN_ITEMS.md`OPEN-112/120/133/134行、`er011_output/point_overlap_observation_log_README.md`参照。
**最終更新(直前の記録): 2026-09-09(PM-CLOSEOUT-CONSOLIDATION-32、FAMILY-A-POINT-OVERLAP-GAP-FIX-TRIAL-05結果のSSOT反映)**: 並列稼働中のLane A `FAMILY-A-POINT-OVERLAP-GAP-FIX-TRIAL-05`(A-UDR-18/19対応)の結果をSSOTへ反映した。A-UDR-18(G1: `build_diagnostic_section()`が前回Point本文をプレースホルダーのまま渡していた件)は**実装漏れ(承認済み範囲内)**と確定した(`git log -S`で同一commit `f46b6e1`内の検証済みscript`er009_n1_diagnostic_full_retry_production_12.py`は実本文を渡していたが、Productionへimportされたmodule版だけが後退していたことを確認)。A-UDR-19(G2)のうちcross_point_overlap値の新規診断prompt追加は**仕様追加候補としてTrialから除外**した(CURRENT_SPEC ER-011-NO18/`DECIDED`/`PRODUCTION_WIRED`が承認するのは「retry判定[still_flagged]への統合」のみ)。あわせて、その承認済みretry判定統合自体がProductionコードに未実装であること(SSOT記載と実装の不一致)を新規発見し、`OPEN_ITEMS.md`へ`OPEN-133`(`USER_DECISION_REQUIRED`)として新規登録した。また、Point Overlap NG 12本中5本(42%)がG1対象外の原因(Value QA単独FAIL・Fact Checker FAIL)であることも確認し、`OPEN-134`(`DEFERRED`、`A-UDR-7`観測項目へ統合)として新規登録した。Hanshin baseline再利用(N=3)+新規Hanshin gapfix・Theme 2 baseline/gapfix(費用超過見込みでN=2へ縮小、実測¥180.0/133 call)でNG率を比較したが改善は非単調で明確な優位は確認できなかった(詳細率はOPEN-112行参照)。新規failure mode・Trend Synthesis/通常Newsへの副作用は観測されず、Gate 4 PASS(`inspect.getsource`静的diffで1箇所以外Productionと一致)。Gate 1=`USER_DECISION_REQUIRED`(G1配線案`build_diagnostic_section()`への実Point本文引数追加の採否、およびOPEN-133の扱い)。Production/Prompt編集は未実施。`CURRENT_SPEC.md`のER-011-NO18該当箇所へ「※実装未確認・OPEN-133で追跡(2026-09-09)」の注記のみ追加(記載自体は変更なし)。`docs/pm/MODEL_ROUTING_TRIAL_LOG.md`へTrial-05実行記録(Sonnet/MEDIUM、340k token、4152秒、規律違反[バックグラウンド待機の可能性、通知なし]、費用N縮小はSTOP規定内)を追記した。詳細は`FAMILY-A-POINT-OVERLAP-GAP-FIX-TRIAL-05_REPORT.md`・`OPEN_ITEMS.md`OPEN-112/133/134行参照。
**最終更新(直前の記録): 2026-09-09(PM-CLOSEOUT-CONSOLIDATION-31、OPEN-131修正版Fable受入+4V本文Trial入力資産Ledger Trial VALIDATED記録+コスト影響評価ルールSSOT反映)**: 並列稼働中の複数Laneの結果をSSOTへ集約反映した(本タスク自体はSSOT編集・Git担当のみ、新規Trial・Production/Prompt編集は行っていない)。**(1) OPEN-131修正受入**: `OPEN-131-ATTRIBUTION-BLOCK-MULTILINE-FIX-02_REPORT.md`(Sonnet委任)が、CONSOLIDATION-30でOpusが指摘した欠陥(`build_voice_attribution_block()`が`[VOICE_n_EVIDENCE]`タグの物理1行のみを抽出しevidence本体[本文継続行・source/URL/counter_or_limitation/verification]を欠落させていた実装漏れ)を修正した。Reconciliation Check(先行実施)で`VOICE_ATTRIBUTION_RULE_TEXT`自体が「evidence内容」を約束していることから実装漏れと判定(仕様解釈の余地なし)、終端規約(空行/`[`開始行/`===`開始行の直前まで)は実Ledger実測から一意に確定した。関数シグネチャ・fail-closed挙動・ルール文言は不変。新規テスト5件PASS(既存13件無変更PASS)、project regression collected=2247/passed=2244/failed=3(既知、無関係)。Runtime evidence再取得(実Production関数、¥11.67)でblock文字数1,359→5,738(約4.2倍)に増加、B1 verdict PASS・unsupported0件は不変だが、**A2はPASS→REVIEW_REQUIRED(unsupported2件)へ変化**した——Tension段落(Voice本文ではない地の文)の客観的主張2claimがopt-inルールの免除対象外に該当し、切り詰めblockの下で見えていなかった適正な検出(TP)が回復した結果であり、false accept新規発生ではない(false accept control N=2は2/2検知を維持)。Fableは`PRODUCTION_WIRED(opt-in)`のスコープ・Statusを維持し、修正版を正式なProduction実装として受入した(旧・切り詰め版を置き換え)。A2の2件は`EDITORIAL-B-FAMILY-VOICES-A2-CROSS-AUDIT-AND-FIX-03`時点の「今回限りのHuman Review」で確認済みだった5/6件の部分集合として扱い、承認Statusは変更しない。commit `a865411`。**(2) AI Screening Ledger Trial**: `EDITORIAL-B-FAMILY-VOICES-AI-SCREENING-LEDGER-TRIAL-01_REPORT.md`(Sonnet委任、Production変更なし)が4V本文Trialの入力資産として検証済み4-Voice Verified Fact Ledgerを作成し、Gate 1=`VALIDATED`(fact22件全件CONFIRMED、Trial-07形式互換、4者代表性あり、賛否比率2対2でない、費用約¥54)。commit `6ba86f3`。**(3) コスト影響評価ルール**: `PM-GOVERNANCE-COST-IMPACT-RULE-03`で新設した`docs/pm/PM_GOVERNANCE.md`2-2節(新QA/Validator/LLM判定導入時のコスト影響評価義務化、Gate2/Gate3チェックリスト・9-1へ参照追記)・`docs/pm/PM_BRIEF.md`参照行を本タスクでGit反映した。並列稼働中の`EDITORIAL-B-FAMILY-VOICES-4V-ARTICLE-TRIAL-01`(上限¥300)は本タスクでは触れていないが、同Trialが導入予定のPairwise Voice Distinctness Check(LLM)のコスト見込み(4V有向12ペア+vs Hook4=16判定、1記事¥5〜15・処理1〜2分)を2-2節に沿ったTrial扱いの付記として`OPEN_ITEMS.md`OPEN-120行へ記録した。**SSOT反映**: `OPEN_ITEMS.md`OPEN-131行(修正受入・runtime evidence再取得・A2 2件の扱い・commit hash追記)、OPEN-132行(Fact Checker A'既定接続時はMULTILINE-FIX-02版が前提である旨を1行追記)、OPEN-120行((d)Ledger Trial VALIDATED・(e)4V本文Trial着手記録+コスト見込み付記)。`CURRENT_SPEC.md`「## B-Family(Voices)Editorial Type」節のFact Checker行を修正版(塊単位抽出・終端規約)の記述へ更新、changelog1件追加。`docs/pm/MODEL_ROUTING_TRIAL_LOG.md`「## Trial導入後の委任実績」表へ、PM-CLOSEOUT-CONSOLIDATION-30の確定token/時間(179k/778s)、OPEN-131-ATTRIBUTION-BLOCK-MULTILINE-FIX-02(Sonnet、MEDIUM、163k/1322s)、EDITORIAL-B-FAMILY-VOICES-AI-SCREENING-LEDGER-TRIAL-01(Sonnet、MEDIUM、189k/1020s)、PM-GOVERNANCE-COST-IMPACT-RULE-03(Sonnet、LOW、36k/81s)、本タスク、および並列稼働中の4V-ARTICLE-TRIAL-01(Sonnet、HIGH、Opus予算は同管理ID群で消費済みのためSonnet実装)を追記した。**Gate 4 Dangling Reference Check**: 本エントリが参照した管理ID(`OPEN-131-ATTRIBUTION-BLOCK-MULTILINE-FIX-02`、`EDITORIAL-B-FAMILY-VOICES-AI-SCREENING-LEDGER-TRIAL-01`、`PM-GOVERNANCE-COST-IMPACT-RULE-03`、`EDITORIAL-B-FAMILY-VOICES-4V-ARTICLE-TRIAL-01`、OPEN-112/120/131/132)はいずれも既存`OPEN_ITEMS.md`/本ファイル内に既出、または本エントリで新規登録した項目であり、未定義参照なし(結果:PASS)。並列稼働中の`EDITORIAL-B-FAMILY-VOICES-4V-ARTICLE-TRIAL-01`(`er012_output/editorial_b_voices_4v_article_trial_01/`)・Lane A `FAMILY-A-POINT-OVERLAP-GAP-FIX-TRIAL-05`(`er011_output/point_overlap_gap_fix_trial_05/`)・`er006_output/`・`er011_output/attempt_history.jsonl`はいずれも編集・stageしていない(4V本文Trialのregistryは読み取りのみで参照)。**Git操作**: G1(OPEN-131修正関連ファイル)commit `a865411`、G2(AI Screening Ledger Trial関連ファイル)commit `6ba86f3`、G3(`docs/pm/PM_GOVERNANCE.md`・`docs/pm/PM_BRIEF.md`・`docs/pm/MODEL_ROUTING_TRIAL_LOG.md`)commit `d0c928c`、G4(SSOT3ファイル: `CURRENT_SPEC.md`・`DECISION_LOG.md`・`OPEN_ITEMS.md`、本エントリ含む)をファイル名指定でcommitし`origin/main`へpush。詳細は`OPEN-131-ATTRIBUTION-BLOCK-MULTILINE-FIX-02_REPORT.md`・`EDITORIAL-B-FAMILY-VOICES-AI-SCREENING-LEDGER-TRIAL-01_REPORT.md`、`OPEN_ITEMS.md`OPEN-112/120/131/132行、`docs/pm/MODEL_ROUTING_TRIAL_LOG.md`参照。
**最終更新(直前の記録): 2026-09-09(PM-CLOSEOUT-CONSOLIDATION-30、3V/4V Writer Trial着手前Opusレビューのcloseout・STOP)**: Fableが3V/4V本文Writer Trialの実行計画(Sonnetへの委任案)について、着手前の第二意見としてOpus(L2 second opinion、読み取り専用診断、token 108k・379秒)へレビューを委任した結果、以下が判明したため**3V/4V本文Trialは着手前STOPとした**(Production/Prompt/Contract/registry編集・API呼び出し・Trial実行のいずれも実施していない)。(1) **委任案の前提事実誤り2件**: Writer基盤を誤って「Trial-09系」としていた(実際は`er012_editorial_b_voices_trial_07.py::run_voices_pattern_run03`)、および「Ledgerは既存Research primitiveで自動生成できる」という誤り(実際は手作業curationが必要、固定テーマ用の検証済みLedgerは未作成)。(2) **OPEN-131 Production欠陥候補の新規発見**: `registry.build_voice_attribution_block()`が`[VOICE_n_EVIDENCE]`タグの物理1行のみを抽出しevidence本体を欠落させる設計であることをOpusが指摘し、Sonnetが実Ledger(`er012_output/editorial_b_voices_trial_07/research/verified_fact_ledger.txt`、OPEN-131 evidence生成が実際に使用した同一ファイル)で読み取り専用・費用¥0の実測を行い、行ベース欠落率91.1%(123行→11行)・文字ベース欠落率87.3%(5,016文字→637文字)を確認した。OPEN-131のRuntime evidence(REVIEW_REQUIRED→PASS)はこの切り詰められたblockに対する結果であり、免除ルールのfalse accept上限は未実証。(3) **fail-open再発点2件**: `DISFLUENCY_QA_MANDATORY_SEGMENTS_BY_LEVEL`・`B_FAMILY_A2_SLOWDOWN_TARGET_SEGMENTS`がsegment名をハードコードしているため、3V/4Vの新segment名が未登録のまま音声化フェーズへ進むと、disfluency QA必須チェック・6% slowdownが静かに適用されなくなる。(4) **未承認仕様候補4件**の明示(pairwise Voice Distinctness Check新設、Comment roleのvoice数パラメータ化、required_structureの可変voice数生成方式、N=1では閾値を決定しない方針)。Opusレビュー原文は改変せず`EDITORIAL-B-FAMILY-VOICES-3V-4V-WRITER-DESIGN-OPUS-REVIEW-01_REPORT.md`へ全文転記し、Fableの検証章を付記した。`OPEN_ITEMS.md`OPEN-131行(Production修正候補+UDR注記)・OPEN-132行(Phase 2チェックリストへ4項目追加)・OPEN-120行(3V/4V本文TrialのSTOP記録)・OPEN-112行(A-UDR-17の項番併記)へ各追記した。`docs/pm/MODEL_ROUTING_TRIAL_LOG.md`へOpus委任(HIGH、差し戻し0)・本タスク(Sonnet、MEDIUM)を追記し、中間レビューTrigger「最初のOpus HIGH案件完了」への到達を記録した(採否判断ではない、記録のみ)。上記論点はいずれも`USER_DECISION_REQUIRED`のままユーザーへ提示する。**Gate 4 Dangling Reference Check**: 本エントリが参照した管理ID(`EDITORIAL-B-FAMILY-VOICES-3V-4V-WRITER-DESIGN-OPUS-REVIEW-01`、`EDITORIAL-B-FAMILY-VOICES-PHASE1-5-3V-4V-INTEGRATED-DESIGN-TRIAL-03`、OPEN-112/120/131/132)はいずれも既存`OPEN_ITEMS.md`/本ファイル内に既出、または本エントリで新規登録した項目であり、未定義参照なし(結果:PASS)。並列稼働中のLane A `FAMILY-A-POINT-OVERLAP-GAP-FIX-TRIAL-05`(`er011_output/point_overlap_gap_fix_trial_05/`)・`er006_output/`・`er011_output/attempt_history.jsonl`はいずれも編集・stageしていない。詳細は`EDITORIAL-B-FAMILY-VOICES-3V-4V-WRITER-DESIGN-OPUS-REVIEW-01_REPORT.md`、`OPEN_ITEMS.md`OPEN-112/120/131/132行、`docs/pm/MODEL_ROUTING_TRIAL_LOG.md`参照。
**最終更新(直前の記録): 2026-09-09(PM-CLOSEOUT-CONSOLIDATION-28、OPEN-129 Fable受入+OPEN-131スコープ論点+Lane A監査結果のSSOT反映)**: (1) **OPEN-129受入**: Fableが`OPEN-129-AUDIO-GATE-STRUCTURAL-COMPLETENESS-PRODUCTION-WIRING-01_REPORT.md`(共有Audio Validation Gate`verify_episode_audio_validation_gate()`へ`required_structure`引数[既定`None`=OFF]を追加、正本はB-Family registry`build_required_structure()`/A-Family`derive_a_family_required_structure()`、既存3呼び出し元は無変更、runtime evidence[既存episode12/12でfalse reject 0、4経路×5ケースで検知12/12・期待一致20/20]、新規15テストPASS、project regression collected 2242/passed 2239/既知failure3[うち1件は新規テスト追加によるテスト総数reconciliation定数の差異でありAudio Gate機能自体の回帰ではない]、mandatory化は未実装のまま)を照合し、`PRODUCTION_WIRED`(opt-in)として正式受入した(commit `2814ed5`/`34fe8dd`)。mandatory化は3V/4V Trialと次回A-Family Production run実績後に別途`USER_DECISION_REQUIRED`とする。(2) **OPEN-131スコープ論点**: `OPEN-131-MULTI-VOICE-FACT-ATTRIBUTION-PRODUCTION-WIRING-01_REPORT.md`(共有Fact Checker`build_fact_check_prompt()`へ`voice_attribution_block=""`引数追加[既定byte一致]、registryへ`fact_attribution_mode`[既定OFF]・`is_fact_attribution_mode_enabled()`[`family=="B"`コードgating]、runtime evidence[B1/A2実記事でON時REVIEW_REQUIRED→PASS、unsupported claims 5/6→0、`apply_rewrites()`後もPASS維持、¥10.24]、新規13テストPASS、A-Family無変更)を精査した結果、共有Fact Checker・registry・runner opt-in入口(`run_fact_check_b1/a2`)への配線自体は完了しているが、B-Family Production runnerの既定パイプラインはPhase 2 Writer配線保留のためFact Checkerを既定stageとして呼んでおらず、「Production正式初回pathで既定実行される」状態には至っていないと判定した。よって`PRODUCTION_WIRED`へは確定させず、`PRODUCTION_WIRED候補(スコープ注記付き、ユーザー確認待ち)`のままSSOT記載を維持する。(3) **Lane A監査結果**: `FAMILY-A-DAILY-NEWS-FOCUS-LAYER-COMPARISON-TRIAL-04_REPORT.md`(Hanshin Ledger固定でbaseline/focus×A2/B1B×N=3、NG率baseline66.7%/focus50.0%だがレベル間で効果が逆転[A2改善・B1B悪化]、費用約¥95.5、run2/3で費用実測用`cl.install()`未設置の規律違反[事後回復済み]、Gate1=`USER_DECISION_REQUIRED`)、`FAMILY-A-POINT-OVERLAP-COUNTERMEASURE-PRE-AUDIT-01_REPORT.md`(対策案(a)は既存Promptと実質重複・案(b)は既存Diagnostic Full Retry診断情報と部分重複と判定しSTOP推奨、監査過程で実装ギャップG1[前回Point本文がハードコードプレースホルダーのままWriterへ渡っていない]・G2[`cross_point_overlap`が計算されているのにretry判定・診断promptに未使用]を新規発見、新規UDR`A-UDR-18`[G1修正要否]・`A-UDR-19`[G2反映要否]を提起)を、それぞれ`OPEN_ITEMS.md`OPEN-112行へ追記した。**Gate 4 Dangling Reference Check**: 本エントリが参照した管理ID(`OPEN-129-AUDIO-GATE-STRUCTURAL-COMPLETENESS-PRODUCTION-WIRING-01`、`OPEN-131-MULTI-VOICE-FACT-ATTRIBUTION-PRODUCTION-WIRING-01`、`FAMILY-A-DAILY-NEWS-FOCUS-LAYER-COMPARISON-TRIAL-04`、`FAMILY-A-POINT-OVERLAP-COUNTERMEASURE-PRE-AUDIT-01`、OPEN-112/129/131)はいずれも既存`OPEN_ITEMS.md`/本ファイル内に既出であることを確認した(結果:PASS)。並行稼働中の他タスクはなし。`er006_output/`・`er011_output/attempt_history.jsonl`は編集・stageしていない。詳細は各`_REPORT.md`、`OPEN_ITEMS.md`OPEN-112/129/131行、`docs/pm/MODEL_ROUTING_TRIAL_LOG.md`参照。
**最終更新: 2026-09-09(PM-CLOSEOUT-CONSOLIDATION-25、B-Family A2 Fable最終受入+Lane A Trial-03・Lane B Fact Trial-02のSSOT反映)**: (1) FableがEDITORIAL-B-FAMILY-VOICES-A2-PRODUCTION-WIRING-01_REPORT.md(Gate 3 15項目充足・Gate 4 PASS、runtime evidence[level="a2"" end-to-end、承認済み13 segment+Key Phrase5件をsha256照合で再利用、Voice A/Bのみ新規TTS発火、duration=350.493秒・peak=0.95049・clippingなし、¥15.76、TTS=gemini-2.5-pro-preview-tts/ASR=gpt-4o-mini-transcribe]、`er003_v1_n3_01_assemble.py`辞書1行追加、OPEN-129はLane B runner側完全性チェック15/15一致・共有Gate側は未対策のまま、project regression 2214/2211/既知3のみ)を照合し、B-Family A2を`PRODUCTION_WIRED`として正式受入した(commit `2b2f266`/`2fbeee3`)。`OPEN_ITEMS.md`OPEN-120行へFable受入を追記。(2) FAMILY-A-POINT-ROLE-PLANNING-FOCUS-MODULE-CONNECTION-TRIAL-03(Lane A、Production/Prompt/SSOT編集なし・TTS未実行)を受入。Comparison Trial-02が発見した「Point Role PlanningへFocus Module文言が届いていない」構造的欠落への接続設計を、新規optional引数`point_role_hint_block`(既定`""`)で3案比較し、案(b)Mode別Point Role候補リストを推奨(Gate1=`VALIDATED`、既定値バイト不変・Trend Synthesis非影響・Diagnostic Full Retry引き継ぎ・Dangling Referenceなしを確認、runtime検証[Hanshin Ledger固定]でPoint Role候補語彙の実反映を確認、B1B=OK・A2は既存Point Overlap論点で`NG_REVIEW_REQUIRED`[本Trial範囲外の既知課題]、¥12.0、上限¥40以内)。Production配線は未実施(コピー版Trialのみ、Trial-only仕様への依存なし・Gate4 PASS)。`OPEN_ITEMS.md`OPEN-112行へ追記。(3) EDITORIAL-B-FAMILY-MULTI-VOICE-FACT-ATTRIBUTION-TRIAL-02(Lane B、OPEN-131、本番Fact Checker/Ledger Deviation Checker/Prompt/model routing/registry読み取りのみ・無変更)を受入。候補A'(Voice別evidenceタグ+Voice本文出典明記免除ルール)の量産再現性をN=5×14claimで検証し、Gate1=`VALIDATED`(Trial範囲内)。false_accept 0/70、false_reject 1〜2/14(試行間一致率98.6%)、事実誤り混入control5/5検知、実在人物への誇張引用5/5検知、A-Family実記事をgating無しで混在させた誤爆stress testでfalse accept 0/5、3V/4V拡張はsynthetic1件のみの設計確認(実データ未検証)、費用約¥2.6(上限¥60以内)。UDR候補5件(採用方向性・gatingのコードレベル必須化・3V4V実データ再検証・retry-simulation実データ再検証・大規模claim集合での反復検証)を提示、Production採用は引き続き`USER_DECISION_REQUIRED`。`OPEN_ITEMS.md`OPEN-131行へ追記。あわせてOPEN-129行へ、共有Audio Validation Gate本体(構造完全性)への対策Trialとして新規管理ID`OPEN-129-AUDIO-GATE-STRUCTURAL-COMPLETENESS-TRIAL-01`を起票した旨を追記した(OPEN-129自体はcloseしない)。`CURRENT_SPEC.md`「## B-Family(Voices)Editorial Type」節の状態表記を`PRODUCTION_WIRED`へ更新(changelog追記)。**Gate 4 Dangling Reference Check**: 本エントリが参照した管理ID(`EDITORIAL-B-FAMILY-VOICES-A2-PRODUCTION-WIRING-01`、`FAMILY-A-POINT-ROLE-PLANNING-FOCUS-MODULE-CONNECTION-TRIAL-03`、`EDITORIAL-B-FAMILY-MULTI-VOICE-FACT-ATTRIBUTION-TRIAL-02`、OPEN-112/120/129/131)はいずれも既存`OPEN_ITEMS.md`/本ファイル内に既出、または本エントリ内で新規正式登録(`OPEN-129-AUDIO-GATE-STRUCTURAL-COMPLETENESS-TRIAL-01`)した項目であり、未定義参照なし(結果:PASS)。並列稼働中のLane A Trial-04(`er011_output/daily_news_focus_layer_comparison_trial_04/`)・Lane B OPEN-129対策Trial(`er011_open129_*`)・`er006_output/`・`er011_output/attempt_history.jsonl`はいずれも編集・stageしていない。詳細は各`_REPORT.md`、`OPEN_ITEMS.md`OPEN-112/120/129/131行参照。
**最終更新: 2026-09-09(EDITORIAL-B-FAMILY-VOICES-A2-PRODUCTION-WIRING-01、Gate 3配線・SSOT反映)**: ユーザーが2026-09-09にB-Family A2(「フリーアドレス vs 固定席」A2、`er012_output/editorial_b_voices_a2_free_address_04/`、再試聴「問題なし」、`stay put`新版採用)を`APPROVED_FOR_PRODUCTION`と正式決定し、Gate 3(15項目)配線を実施した。Trial runner 3本(`er012_editorial_b_voices_a2_trial02_writer.py`/`..._trial02_runner.py`/`er012_b_voices_a2_cross_audit_fix_03_runner.py`/`er012_b_voices_a2_slowdown_keyphrase_regen_04_runner.py`)に分散していたA2経路(翻案Writer呼び出し・Comment/Preview日本語生成・日本語タイトル・Key Phrase A2経路・Voice A/B slowdown付きTTS・A2用Assembly・player)を、新規Lane B Production module `er012_b_family_voices_a2_production_01.py`+registry(`er012_b_family_editorial_type_registry_01.py`の`B_FAMILY_A2_CONFIG`・`get_editorial_type_a2()`)+既存Production runner(`er012_b_family_production_runner_01.py`)への`level="a2"`分岐へ正式統合した。Trialスクリプトは一切importせず、Point Overlap/Analytical Leakage Check等のmonitoring専用QAプロンプト・定数も含め全文転記(転記元とのbyte一致を単体テストで確認)。既存B1経路(`level="b1"`既定、既存関数は一切無変更)の挙動不変を、既存単体テスト14件PASS+新規19件PASSで確認。共有ファイルへの変更は`er003_v1_n3_01_assemble.py`の`DISFLUENCY_QA_MANDATORY_SEGMENTS_BY_LEVEL`辞書へ`"B_FAMILY_A2"`エントリ(標準A2と同一の対象segment集合)を1行追加したのみ(既存B1/A2エントリ・既存関数は無変更、diffで確認)。OPEN-129(Audio Gateがsegment数完全性を検証しない既知の未対策)は共有Gate側は未対策のまま据え置き、Lane B runner側の完全性チェック(`check_required_segments_completeness()`、registryの`required_segments`と実行結果を突合)で対策した(Lane B側のみの対策であることを明記)。**Runtime evidence**: ユーザー承認済み記事(`_04`)を入力に、Production正式runner(`level="a2"`)をend-to-end実行(`TTS_EXECUTION_MODE=STANDARD`)。承認済みsegment(13本文+Key Phrase5件、うちKey Phrase「stay put」は新版[v2]、sha256一致で確認)はbyte再利用(sha256照合、fail-closed)、Voice A/B本文(`point_one`/`point_two`)のみ正式Production関数(`generate_voice_body_wide_margin_with_a2_slowdown()`)で実際に新規TTSを発火し(A2 slowdown6%適用・OPEN-121/122安全機構有効・ASR verified、model=`gemini-2.5-pro-preview-tts`/ASR=`gpt-4o-mini-transcribe`)、完成episode(`er012_output/editorial_b_family_voices_a2_production_wiring_01/`、duration=350.493秒・peak=0.95049・clipping=False)・標準player.html(Gate 7 (a)〜(l)全充足)を生成した。費用¥15.76(上限¥100以内)。project-wide regression(`run_project_regression.py`、collected=2214・passed=2211・failed=3[既知の無関係failureのみ、新規failureゼロ])。`CURRENT_SPEC.md`「## B-Family(Voices)Editorial Type」節へA2仕様(構造・voice・slowdown・Comment言語・日本語タイトル・Gate対象化・11語超許容は今回限りの注記・Fact Checker運用注記[恒久化しない、機械化はOPEN-131で別途Trial中]・OPEN-129整合)を正式追記した。`OPEN_ITEMS.md`OPEN-120行を`PRODUCTION_WIRED候補(Fable受入待ち)`へ更新、OPEN-129行へ本タスクでのLane B側対策状況を追記(OPEN-129自体はcloseしない)。OPEN-131は不変(機械化Trial継続中)。**Gate 4 Dangling Reference Check**: 初回/retry/fallback/regeneration/validator/Human Reviewいずれの経路でも、Trial script(`er012_editorial_b_voices_trial_*.py`等)・未承認仕様(3V/4V、Fact Checker A'、OPEN-129対策の共有Gate実装)への参照が無いことを確認した(結果:PASS、詳細はReport参照)。詳細は`EDITORIAL-B-FAMILY-VOICES-A2-PRODUCTION-WIRING-01_REPORT.md`参照(Gate 3チェックリスト15項目・Gate 4表を含む、`PRODUCTION_WIRED`確定はFable最終受入待ち)。
**最終更新: 2026-09-08(PM-CLOSEOUT-CONSOLIDATION-24、Lane B A2再生成[_04、Voice A/B slowdown+Key Phrase旧/新比較]のSSOT反映)**: `EDITORIAL-B-FAMILY-VOICES-A2-SLOWDOWN-AND-KEYPHRASE-REGEN-04_REPORT.md`が、ユーザー決定4項目(CONSOLIDATION-22記載のLane B側決定)を反映し`er012_output/editorial_b_voices_a2_free_address_04/`へ再Assemblyした結果をSSOTへ反映した。Voice A/B(point_one/point_two)へ標準A2の既存6% slowdown機構(`A2_SLOWDOWN_TARGET_SEGMENTS`・`apply_a2_slowdown_postprocess`、無変更)を適用するため、Phase 1 `PRODUCTION_WIRED`モジュール`er012_b_family_voices_production_01.py::generate_voice_body_wide_margin()`へ完全後方互換の追加引数`style_prefix_override`(既定None)を追加した(既存Production呼び出し[Phase 1 runner含む]は本引数を渡さないため無影響、Phase 1単体テスト14件PASS、project regression 2192/2195・既知3失敗のみ)。実測slowdown 5.96〜5.97%、point_one/point_twoとも1 attemptでPASS。Key Phrase「stay put」は既存承認済み再生成経路`review_lock.approve_regenerate()`で新版を1回生成し、旧版(`kp4_en_old_v1.wav`)・新版(`kp4_en_new_v2.wav`)とも既存QA(ASR照合・disfluency)PASSを確認した上で新版を採用(旧版は削除せず保全、Master Audio Store manifest/cacheへは書き込んでいない)。Fact Checker複合Voice帰属のREVIEW_REQUIREDは、既存`record_human_approval()`を流用せず専用の承認recordを新規作成し「今回限り・恒久運用にしない」旨を明記して通過させた(恒久対応は`EDITORIAL-B-FAMILY-MULTI-VOICE-FACT-ATTRIBUTION-TRIAL-01`[OPEN-131]側で別途検討)。11語超18/38語は新上限を設けず既存article.mdをそのまま採用した。Assembly: 349.962秒・peak 0.95049(headroom safety valve未適用)・clippingなし、narration 41ファイル中38ファイルが`_03`とsha256一致(byte再利用)、差分3ファイルが今回の意図的再生成対象。費用¥7.23。Gate 1=`VALIDATED`(ユーザー再試聴前、`APPROVED_FOR_PRODUCTION`は未宣言)。player: `er012_output/editorial_b_voices_a2_free_address_04/player.html`(Key Phrase旧/新比較行を追加)。`OPEN_ITEMS.md`OPEN-120行へ追記した。**Gate 4 Dangling Reference Check**: 追記が参照した管理ID(`EDITORIAL-B-FAMILY-VOICES-A2-SLOWDOWN-AND-KEYPHRASE-REGEN-04`、`EDITORIAL-B-FAMILY-VOICES-A2-CROSS-AUDIT-AND-FIX-03`、`EDITORIAL-B-FAMILY-MULTI-VOICE-FACT-ATTRIBUTION-TRIAL-01`、OPEN-120/131、`B-A2-9`)はいずれも既存`OPEN_ITEMS.md`/本ファイル内に既出であることを確認した(結果:PASS)。Production側の変更は`generate_voice_body_wide_margin()`への後方互換引数追加のみで、既存の安全機構(ASR Cascade・disfluency gate・repetition QA・connected speech equivalence layer・Human Review Lock・retry上限)はいずれも無改変。Fact Checker専用承認recordは今回限りの例外運用として明記されており、恒久的な仕様変更ではない。詳細は`EDITORIAL-B-FAMILY-VOICES-A2-SLOWDOWN-AND-KEYPHRASE-REGEN-04_REPORT.md`、`OPEN_ITEMS.md`OPEN-120行参照。2026-09-08(PM-CLOSEOUT-CONSOLIDATION-23、Lane A-1比較Trial・Lane B Fact帰属TrialのSSOT反映)**: Lane A-1(`FAMILY-A-DAILY-NEWS-FOCUS-LAYER-COMPARISON-TRIAL-02`)とLane B(`EDITORIAL-B-FAMILY-MULTI-VOICE-FACT-ATTRIBUTION-TRIAL-01`、OPEN-131起票分)のTrial結果をSSOTへ反映した(両Laneとも本タスクではSSOT記録のみ、追加Trial・追加API呼び出し・コード変更は行っていない)。**Lane A-1**: Hanshin Ledger固定でbaseline/focus×A2/B1B計4本(text-only、¥24.1)を比較した結果、baseline A2/B1B・focus A2はPoint Overlap Loop Budget上限到達で`NG_REVIEW_REQUIRED`、focus B1BのみOK(retry 1/2)。`run_one_pattern()`内のPoint Role PlanningがFocus Module文言を受け取っていないという構造的発見があり、Focus Moduleの因果効果とrun-to-run変動をN=1では切り分けられない。Gate 4 PASS、Gate 1=`USER_DECISION_REQUIRED`(設計自体はVALIDATEDのまま)。**Lane B**: 複合Voice構成でFact CheckerがREVIEW_REQUIREDを返す原因を、Voice別evidenceタグがLedgerに存在するのに5-section本文へ未配線という構造的ミスマッチと特定した。Candidate A'(Voice別evidenceタグ+出典明記免除ルール)がfalse_accept 0/false_reject 1/6で最良だが、6claim・1〜2回試行の小規模Trialで非決定性も観測されたため再現性は未検証。Gate 1=設計方向VALIDATED(再現性未検証)。両Laneとも採用判断は`USER_DECISION_REQUIRED`のままユーザーへ委ねる。`OPEN_ITEMS.md`OPEN-112行(Lane A-1、UDR候補6件+A-UDR-7観測4件追加)・OPEN-131行(Lane B、UDR候補4件)へ各追記した。**Gate 4 Dangling Reference Check**: 追記が参照した管理ID(`FAMILY-A-DAILY-NEWS-FOCUS-LAYER-COMPARISON-TRIAL-02`、`EDITORIAL-B-FAMILY-MULTI-VOICE-FACT-ATTRIBUTION-TRIAL-01`、OPEN-112/131)はいずれも既存`OPEN_ITEMS.md`/本ファイル内に既出であることを確認した(結果:PASS)。Production/Promptコードは無変更、API呼び出し0件。並列稼働中のLane B再生成タスク(`er012_output/editorial_b_voices_a2_free_address_04/`等)・`CURRENT_SPEC.md`・`er006_output/`は一切編集・stageしていない。詳細は`FAMILY-A-DAILY-NEWS-FOCUS-LAYER-COMPARISON-TRIAL-02_REPORT.md`・`EDITORIAL-B-FAMILY-MULTI-VOICE-FACT-ATTRIBUTION-TRIAL-01_REPORT.md`、`OPEN_ITEMS.md`OPEN-112/131行参照。2026-09-08(PM-CLOSEOUT-CONSOLIDATION-22、Lane A2件+Lane B並行結果へのユーザー決定[A-UDR-8/9/10、Lane B側決定]反映)**: 並行稼働Lane 3件(Lane A-1 Trial`FAMILY-A-DAILY-NEWS-FOCUS-LAYER-COMPARISON-TRIAL-02`起票、Lane B 2件`er012_*`)自体のSSOT反映ではなく、直前のユーザー決定をSSOTへ確定反映した(本タスクが唯一のSSOT編集・Git担当)。**A-UDR-8**: News固有層設計案(Focus Module+Major/Daily Gate)を承認、Hanshin Ledger固定でFocus Moduleなし/ありのA2/B1比較Trial(`FAMILY-A-DAILY-NEWS-FOCUS-LAYER-COMPARISON-TRIAL-02`)へ進む(最大`VALIDATED`、配線はユーザー判断前に行わない、内容自体は本タスクでは規定しない)。**A-UDR-9**: Household(冷蔵庫クリスパー)は単一起点イベントを持たないため通常News referenceから除外し、別Editorial Type(Discovery/Why系)候補として別途整理する(通常Newsの定義をHouseholdに合わせて広げない)。`CURRENT_SPEC.md`「## 通常News(Major/Daily News)Reference仕様」節「Reference記事の正式指定」から、Householdを同系列referenceの併記対象から除外する編集を行った(Hanshin・Healthのみ残す)。**A-UDR-10**: Health=単一起点研究発表がある場合はMajor/Dailyとして扱う(`DECIDED`ではなく運用方針・設計Trial段階と明記)。Major/Daily Gateの非対称性は現状許容、myth-correction候補は保留。将来自動Mode判定を行う場合は対称性ではなく排他的・再現可能な判定ロジックが必要という点を新規Open Item`OPEN-130`(`DEFERRED`)として登録した。**Lane B側決定**(記録のみ、実施は別タスク`er012_*`側): Voice A/Bへ既存6% slowdown適用、Key Phrase「stay put」は承認済み再生成経路で1回再生成、11語超18/38語は許容(新上限なし)、Fact Checker複合Voice帰属は今回のみHuman Review経路で通す(恒久運用にしない)、複合Voice Fact Checker帰属の機械化Trialを新規管理ID`EDITORIAL-B-FAMILY-MULTI-VOICE-FACT-ATTRIBUTION-TRIAL-01`として起票(最大`VALIDATED`、新規Open Item`OPEN-131`として登録)、`B_FAMILY_A2`のAudio Validation Gate辞書(`DISFLUENCY_QA_MANDATORY_SEGMENTS_BY_LEVEL`)登録はB-Family A2 Production wiring実施時(OPEN-129整合確認と合わせて対応)、後続順序=B-Family A2 wiring→OPEN-129対策Trial→複合Voice Fact帰属Trial(OPEN-131)→3V/4V本文Trial(OPEN-129対策とFact帰属Trialが独立であれば並列可)。`OPEN_ITEMS.md`OPEN-112行・OPEN-120行へ各追記、新規`OPEN-130`(Mode自動判定の排他性・再現可能性、`DEFERRED`)・`OPEN-131`(複合Voice Fact帰属機械化Trial起票、`USER_DECISION_REQUIRED`)を登録した。**Gate 4 Dangling Reference Check**: 追記が参照した管理ID(`FAMILY-A-DAILY-NEWS-FOCUS-LAYER-DESIGN-TRIAL-01`、`FAMILY-A-DAILY-NEWS-FOCUS-LAYER-COMPARISON-TRIAL-02`、`EDITORIAL-B-FAMILY-VOICES-A2-CROSS-AUDIT-AND-FIX-03`、`EDITORIAL-B-FAMILY-MULTI-VOICE-FACT-ATTRIBUTION-TRIAL-01`、OPEN-112/120/129/130/131)はいずれも既存`OPEN_ITEMS.md`/本ファイル内に既出または本エントリ内で新規正式登録した項目であり、未定義参照なし(結果:PASS)。Production/Promptコードは無変更、API呼び出し0件。本タスク以外(Lane A-1 Trial`er011_*`、Lane B`er012_*`、Report群)は一切編集・stageしていない。詳細は`CURRENT_SPEC.md`「通常News(Major/Daily News)Reference仕様」節changelog(第16弾)、`OPEN_ITEMS.md`OPEN-112/120/130/131行参照。2026-09-08(PM-CLOSEOUT-CONSOLIDATION-21、Lane B A2横断監査結果のSSOT反映)**: `EDITORIAL-B-FAMILY-VOICES-A2-CROSS-AUDIT-AND-FIX-03_REPORT.md`が、TRIAL-02成果物と既存A2標準を横断監査した結果(B-2[日本語タイトル欠落]は配線漏れと判定し修正・再生成、B-1[Voice A/Bのslowdown未適用]・B-4[Key Phrase「stay put」の可聴性]は既存正式決定・Production経路と整合するためSTOP、B-3[Charon/Aoede役割]は既承認B-Family規約と一致、B-5[Audio Validation Gate`DISFLUENCY_QA_MANDATORY_SEGMENTS_BY_LEVEL`に`B_FAMILY_A2`未登録]を新規発見)をSSOTへ反映した。`er012_output/editorial_b_voices_a2_free_address_03/`(japanese_title 1segment新規TTS、他は`_02`のバイト同一wav再利用、340.80秒、¥0.65)、Gate1=`VALIDATED`。`OPEN_ITEMS.md`OPEN-120行・OPEN-129行へ追記。詳細は本ファイル「PM-CLOSEOUT-CONSOLIDATION-21」節・`EDITORIAL-B-FAMILY-VOICES-A2-CROSS-AUDIT-AND-FIX-03_REPORT.md`参照。2026-09-08(PM-CLOSEOUT-CONSOLIDATION-20、Lane A-2`FAMILY-A-DAILY-NEWS-FOCUS-LAYER-DESIGN-TRIAL-01`結果のSSOT反映)**: News固有層(Layer3 News Focus Module Major/Daily variant・Mode判定基準)設計Trialの結果をSSOTへ反映した(Gate 1=`VALIDATED`[設計]、Gate 4 PASS、費用¥0、API呼び出し0件、コード・Prompt・Production変更なし)。Point Role候補7件(DESIGN-08 4件+本Trial新規3件[mechanism/beyond-the-headline factor/certainty limitation])、Trend Synthesis Focus Moduleとの差分表、Major/Daily Gate 6項目(消去法的性質でTrend Gate6条件と非対称であることを自己注記)を提示した。実記事(Hanshin/Health/Household)を精読した結果、**Household(冷蔵庫クリスパー)記事は単一起点イベントが存在せずMajor/Daily・Trend Synthesisいずれの対象でもない可能性が高いという新規発見**があり、`FAMILY-A-DAILY-NEWS-SPEC-DRAFT-01`・`FAMILY-A-LEGACY-NEWS-ASSET-SURVEY-01`が前提としていた「Hanshin/Health/Household一括Newsreference」の枠組み修正要否を含む`USER_DECISION_REQUIRED`候補6件(設計案[§1]採否/Household発見[§2.2]への対応/Health記事の境界[単一研究集約 vs Trend外部集約]の扱い/Gate非対称性の扱い/myth-correction候補[§3候補7]の採否/検証Trial実施可否[Hanshin Ledger再利用、A2+B1B概算¥30〜80])を提起した。採用判断はユーザーへ委ね、`CURRENT_SPEC.md`側の仕様変更は行っていない。`OPEN_ITEMS.md`OPEN-112行へ追記した。詳細は`FAMILY-A-DAILY-NEWS-FOCUS-LAYER-DESIGN-TRIAL-01_REPORT.md`参照。2026-09-08(FAMILY-A-DAILY-NEWS-REFERENCE-FORMALIZATION-01、Lane A-1、ユーザー決定A-UDR-5/6/7反映): ユーザー決定(A-UDR-5)「Hanshin系現役Production資産を通常News(Major/Daily News)の正式referenceとしてCURRENT_SPECへ反映する。ただし既にDECIDED/PRODUCTION_WIRED済みの内容だけを正式化し、B1-A等obsolete仕様は混ぜず、ADD03(イラン)・A02(英SNS)は題材・構造referenceに限定し、Trend Synthesis専用仕様(Focus Module・Engagement原則・Trend Gate等)は混ぜない」を受け、`CURRENT_SPEC.md`へ新設「## 通常News(Major/Daily News)Reference仕様」節(「## News Editorial Mode(Trend Synthesis)」節の直後)を追加した。内容は既存各節(CEFR-A2構造・音声仕様/B1/Cross-level仕様/Key Phrase/Audio Assembly)への参照+「通常Newsに適用される」ことの明記のみで、既存DECIDED仕様の重複転記はしていない。Hanshin(`ER-003-A2-B1-N3-01`、Health/Household含む)を構造・Writer・Fact Safety・音声のreference実装として正式指定し(`DECIDED`)、ADD03/A02は題材・音声構造referenceに限定(本文生成コードパス・完成音声は非再利用、B1-A方式はobsolete)。News固有層(Layer3 News Focus Module・Mode判定基準)は内容を書かず「未設計・設計Trial起票済み(`FAMILY-A-DAILY-NEWS-FOCUS-LAYER-DESIGN-TRIAL-01`、Lane A-2、並行稼働中の別管理ID)」とだけ明記した。obsolete除外リスト(B1-A・P-series専用script・Natural English Source方式・旧Preview分量・旧trim margin・ER-010設計文書)は既存の置換記録(ER-003-B1-B2-SCOPE-FIX-01、DECISION_LOG.md 5711行、ER-011-PREVIEW-ROLE-AND-NUMERIC-PRECISION-PRINCIPLE-PRODUCTION-WIRING-01、ER-011-NO18-A2-TIGHT-SPEECH-AND-TRIM030-PRODUCTION-WIRING-23、OPEN-112行[2026-09-04]参照)へ紐付けて明記した。**Gate 4 Dangling Reference Check**: 追記テキストが参照した管理ID(ER-003-A2-STRUCT-02〜04、ER-003-A2-SPEC-FREEZE-01、ER-003-A2-B1-N3-01、ER-003-B1-B2-SCOPE-FIX-01、ER-003-SPOKEN-FIRST-03、ER-009-N1-LEDGER-DEVIATION-RECALIBRATION-02、ER-010-NO9-FACTCHECK-POLICY-AND-POINT-COMPRESSION-DIAGNOSTIC-12、ER-011-PREVIEW-ROLE-AND-NUMERIC-PRECISION-PRINCIPLE-PRODUCTION-WIRING-01、ER-003-CROSSLEVEL-AUDIO-02、ER-003-B1-NOVEL-AUDIO-01系、ER-003-POINT-NOTIFICATION-01、ER-011-NO18-A2-TIGHT-SPEECH-AND-TRIM030-PRODUCTION-WIRING-23)は全件、既存`CURRENT_SPEC.md`/`DECISION_LOG.md`内に既出であることをgrepで確認した(結果:PASS)。Trial-only・未承認仕様(Trend Focus Module、Discovery Layer3、Reference Digest等)は参照していない。新規仕様追加が必要な事態は発生せず、STOPなし。あわせてユーザー決定(A-UDR-7)「Trend Synthesis A2のPoint Overlap run間分散(0.31〜0.66)は観測継続、Trial不要、Production閾値・Prompt・retry機構は変更しない」を`OPEN_ITEMS.md`OPEN-112行へ追記した(観測項目4点: Overlap値/retry頻度/NG_REVIEW_REQUIRED発生/Cost・latency)。Discovery deferredは据え置き継続(変更なし)。コード・Prompt・API呼び出しはいずれも実施していない。詳細は`FAMILY-A-DAILY-NEWS-REFERENCE-FORMALIZATION-01_REPORT.md`、`FAMILY-A-DAILY-NEWS-SPEC-DRAFT-01_REPORT.md`参照。2026-09-08(PM-CLOSEOUT-CONSOLIDATION-16、Lane A[`FAMILY-A-BRANCH-FACT-CHECK-02`、Discovery/News/Trend Synthesis枝別事実確認、読み取り専用]とLane B[`EDITORIAL-B-FAMILY-VOICES-PHASE1-5-FOUR-VOICES-STRUCTURE-DESIGN-TRIAL-02`、4 Voices構造設計Trial]を相互独立に並行実施しSSOTへ反映した。**Lane A**(読み取り専用、コード・Prompt・Production無変更): Family A(Discovery/News/Trend Synthesis)の3枝を個別に事実確認し、Discoveryは共通Production配管がPRODUCTION_WIRED済み(ただしDiscovery固有ではなく全テーマ共通)、Discovery固有Layer3 Focus ModuleはVALIDATED止まりでProduction採否はDEFERRED、通常News(Major/Daily News)はOPEN-112-NEWS-MODE-DESIGN-08の設計文書のみでTrial実施0件、Trend Synthesisはmode自体のProduction採用・配線がDEFERRED(Theme 2完成音声のみAPPROVED_FOR_PRODUCTION、mode本体とは別論点)であることを確認した。**Lane B**(記事本文・音声生成なし、費用¥0): 2026-09-08ユーザー決定(4 Voices拡張YES・選定軸=案A)を受け、4 Voices対応の構造設計案3つ(S1: 4見出し直列/S2: 2ペア構造/S3: 軸提示先出し)を比較しS1を推奨(Gate 1=`VALIDATED`)、未承認仕様8件(構造採用・Comment 2/3文言変更・4声voice選定・physical_structure新規値・QA 6ペア・REQUIRED_SEGMENTS拡張・目標尺380〜430秒・Tension 2軸交差)を特定した。両Laneとも採用判断は`USER_DECISION_REQUIRED`のままSTOP。`OPEN_ITEMS.md`OPEN-112行(Lane A)・OPEN-120行(Lane B)へ各1点追記、`CURRENT_SPEC.md`は仕様変更なしのためchangelog参照のみ追記。詳細は`FAMILY-A-BRANCH-FACT-CHECK-02_REPORT.md`・`EDITORIAL-B-FAMILY-VOICES-PHASE1-5-FOUR-VOICES-STRUCTURE-DESIGN-TRIAL-02_REPORT.md`参照)。2026-09-08(PM-CLOSEOUT-CONSOLIDATION-15、Lane A[`FAMILY-A-DESIGN-FIX-
...
B1 rank2(変換対象外の「～」)・A2 rank2(無関係な既存TTS問題)の2件の
Gate/Lock停止でB1・A2ともEpisode Assembly未完成、全体`USER_DECISION_
REQUIRED`。詳細は本ファイル該当エントリ・`OPEN-117-KEYPHRASE-DISPLAY-TTS-
SEPARATION-TRIAL-02_REPORT.md`参照)。2026-09-06(ER-011-TTS-EXECUTION-MODE-SWITCH-PRODUCTION-WIRING-01、ユーザーが`APPROVED_FOR_PRODUCTION`と承認した「TTS実行モード切替を1箇所追加する最小変更」を、`er006_batch_tts_wiring_01.py::make_batch_tts_call_fn()`内の環境変数`TTS_EXECUTION_MODE`[`BATCH`/`STANDARD`、既定`BATCH`、不正値はValueError即停止]による1箇所の分岐として`PRODUCTION_WIRED`まで配線した[Production call site6箇所・voice・style instruction・retry cascade・Validator・Master Audio Storeは無変更]。選択モードは`tts_execution_mode`フィールドとして`er006_batch_tts_wiring_01.py::_record()`[Batch]/`er005_cost_logger.py::_patch_gemini()`[Standard、TTSのみ]の既存ログへ追加記録[既存キーは無変更]、Master Audio Storeのcache identityにはモードを含めない[同一model・canonical textなら音質同等のため既存資産再利用を優先]。実API確認[2026-09-06、Key Phrase英語"opt out"、`er011_output/tts_execution_mode_switch_wiring_01/`]: `STANDARD`は`client.models.generate_content`を2attempt[3.185秒・2.727秒]でstatus=OK・ASR検証PASS・`tts_execution_mode=STANDARD`記録を確認、既定[未設定]は`client.batches.create`を4attempt[80〜156秒/件、既存実測レンジ91〜167秒と整合]で`tts_execution_mode=BATCH`記録を確認[この回のASR内容検証は既知のTTS/ASR run-to-run非決定性によりSTOPPEDだったが配線自体とは無関係]。新規単体テスト11件PASS、既存Batch wiring回帰14件・TTS retry回帰22件PASS、プロジェクト全体回帰2084件中2081 PASS[残り3件は本タスク前から存在する既知の無関係failureとgit stashで確認済み]。詳細は本ファイル該当エントリ・`ER-011-TTS-EXECUTION-MODE-SWITCH-PRODUCTION-WIRING-01_REPORT.md`参照)。2026-09-06(PM-GOVERNANCE-DEV-TTS-STANDARD-SYNC-01、TTS方式の運用基準を「正式リリース前=Standard同期/正式リリース後の実量産=Batch API」へ統一し、docs/pm/PM_GOVERNANCE.md 7節・CURRENT_SPEC.mdへ反映した。詳細は本ファイル該当エントリ参照)。2026-09-06(PM-GOVERNANCE-LOCAL-FILE-LINK-RULE-07、試聴・閲覧依頼はfile:// URL形式で提示するルールをPM_GOVERNANCE 9-2へ追加)。2026-09-06(OPEN-117-KEYPHRASE-DISPLAY-TTS-SEPARATION-TRIAL-01、Key Phrase日本語訳の「表示用の辞書的表記」(「～を示す」)と「TTS読み上げ用テキスト」(「なになにを示す」等)を分離する方式のPhase 1隔離Trial。主対象4件×2run+対照A/B+代替2件、計12入力すべて`status=OK`(RECHECK-01の「～」直接投入[3/3 STOPPED]とは対照的)。`VALIDATED`(Phase 1、Production採用ではない)。聴取者による自然さの主観評価は新規`USER_DECISION_REQUIRED`。Production code/Prompt/`keywords_canonicalized.json`スキーマ/CURRENT_SPECは無変更。詳細は本ファイル該当エントリ・`OPEN-117-KEYPHRASE-DISPLAY-TTS-SEPARATION-TRIAL-01_REPORT.md`参照)。2026-09-06(OPEN-117-KEYPHRASE-TILDE-GATE-RECHECK-01、Key Phrase日本語gloss「～」placeholderゲートの過剰判定疑いをTrial-only TTS確認(Production Batch API)+過去データ調査で再評価。ゲートでSTOPPEDになる3件はTTS/ASRで実際に不合格になる内容であり緩和条件は`REJECTED`、ゲート・Prompt規約Bとも`VALIDATED`(現状維持)。過去artifact再点検要否のみ`USER_DECISION_REQUIRED`。Production code/Prompt/CURRENT_SPECは無変更。詳細は本ファイル該当エントリ・`OPEN-117-KEYPHRASE-TILDE-GATE-RECHECK-01_REPORT.md`参照)。2026-09-06(ER-011-ASSEMBLY-HEADROOM-SAFETY-VALVE-PRODUCTION-WIRING-01、OPEN-115[A2/B1 Assemblyヘッドルーム不在]をユーザーが`APPROVED_FOR_PRODUCTION`と決定し、`stage_assemble_a2`/`stage_assemble_b1`共通の最終段ヘッドルーム安全弁[閾値0.98、超過時のみ一律スカラーgain、記録必須、適用後も1.0超なら`RuntimeError`]を`PRODUCTION_WIRED`まで配線した。`CURRENT_SPEC.md`283行目付近の「Preview/Bodyは無調整」誤記も訂正した。詳細は本ファイル該当エントリ参照)。2026-09-06(OPEN-112-TREND-THEME2-RECORD-CONSOLIDATION-18、Theme2[若者の旅行]Trend Synthesis Trial系列[Trial-11〜17・Opus診断-14/-15]の記録をSSOTへ統合し、Trial-13レポートの時系列整理・誤記2件訂正、A2 peak原因確定[計測-16]、Key Phrase Validator修正Trial[-17]を記録、新規OPEN-115[A2/B1 Assemblyヘッドルーム不在]・OPEN-116[ASR Validator数字保護ゲート誤判定+Key Phrase gloss規約不足]を登録した。Production採用判断・配線は一切行っていない。詳細は本ファイル該当エントリ参照)。2026-09-06(PM-GOVERNANCE-ADAPTIVE-REPORTING-06、ユーザー向け報告の5構造[現在の状態/未解決問題/次にやること/ユーザー判断/補足・技術詳細]を、固定必須構造から必要な項目だけを選ぶ候補セクション制[adaptive reporting]へ`docs/pm/PM_GOVERNANCE.md`「9節」上で改訂した。詳細は本ファイル該当エントリ参照)。2026-09-06(PM-GOVERNANCE-REPORT-FORMAT-AND-AUTONOMOUS-GIT-05、ユーザー向け報告フォーマットを番号対応する5構造[現在の状態/未解決問題/次にやること/ユーザー判断/補足・技術詳細]へ改訂し、通常の安全なcommit/pushをClaude側が自律実行する運用を`docs/pm/PM_GOVERNANCE.md`「9節・10節」へ反映した。詳細は本ファイル該当エントリ参照)。2026-09-06(PM-GOVERNANCE-AGENT-WAIT-AND-DUPLICATE-LAUNCH-RULE-04、待機中Agentの再開リスクと同一管理IDへの多重起動禁止を`docs/pm/PM_GOVERNANCE.md`「8節」へ追記した。詳細は本ファイル該当エントリ参照)。2026-09-05(PM-GOVERNANCE-USER-FACING-EXPLANATION-STYLE-03、
ユーザー向け説明スタイル原則を`docs/pm/PM_GOVERNANCE.md`「9節」へ新設した。
詳細は本ファイル該当エントリ参照)。2026-09-05(PM-GOVERNANCE-PARALLEL-AGENT-POLICY-02、Agent並列起動の原則を`docs/pm/PM_GOVERNANCE.md`「8節」へ新設し、独立タスク[ファイル・出力先が重ならない]であればFableの判断で複数sonnet-workerを並列起動可へ条件付き緩和した[ユーザー決定(b)]。詳細は本ファイル該当エントリ参照)。2026-09-05(PM-GOVERNANCE-TTS-MODE-CONFIRMATION-01、TTS方式[Batch API/Standard同期]の明示・確認原則を`docs/pm/PM_GOVERNANCE.md`「7節」へ新設。詳細は本ファイル該当エントリ参照)。2026-09-05(PM-HANDOFF-CHATGPT-001-CLOSEOUT-SSOT-01、ChatGPT旧PM引き継ぎHandoff[PM-HANDOFF-CHATGPT-001]をユーザーが`HANDOFF_VALIDATED`として受け入れ正式closeoutし、PM運用原則4点[PM Gate 1〜7・PM Closeout Mandatory Check・1記事ずつ完結(ユーザー明示defer・non-blocking・OPEN_ITEMS追跡・APPROVED_FOR_PRODUCTION取消しでないこと・後続作業が未配線項目を完了扱いしないことの5条件を満たす場合の例外含む)・安全≠成功]を新設`docs/pm/PM_GOVERNANCE.md`(正本)へ明文化した(CLAUDE.md・docs/pm/PM_BRIEF.mdは短い参照のみ、CURRENT_SPEC.mdは無変更)。defer5件[Trend overclaim severity/Point Overlap閾値/Reference Digest/Point長さ/OPEN-83自動照合実装]・重要3点[Discovery採否/Engagement採否/News Ledger自動Research化]を記録し、OPEN_ITEMS.md OPEN-112行のdangling link(実在しない`OPEN-112-DISCOVERY-4LAYER-ADOPTION-READINESS-07_REPORT.md`参照)を実在するDECISION_LOG.mdエントリへ修正した。2026-09-05のユーザー訂正により本タスクはgit add/commit/pushを実施せず文書修正のみでSTOP。先行するHandoff証跡commit`aecd776`(docs/pm/ACTIVE_TASK.md・RESULT_PACKET.mdを`.gitignore`除外に反し`git add -f`済み、未push)の取り扱いは`USER_DECISION_REQUIRED`。詳細は本ファイル該当エントリ参照)。**2026-09-05追記(PM-HANDOFF-CLOSEOUT-GIT-02)**: ユーザー判断により`aecd776`は`git reset --mixed HEAD~1`で取り消し(未pushのためorigin影響なし)。`docs/pm/ACTIVE_TASK.md`・`docs/pm/RESULT_PACKET.md`はGit履歴へ残さず`.gitignore`対象のまま。Handoff証跡は`docs/pm/PM-HANDOFF-CHATGPT-001_REPORT.md`のみを本GIT-02のcloseout commitで正式反映。2026-09-05(OPEN-112-TREND-ENGAGEMENT-REFERENCE-AB-TRIAL-10、Trend Synthesis記事の「面白さ・意外性の弱さ」に対しEntertainment根底指示(施策1)とFact source禁止のReference Digest(施策2)をB1のA/B比較で検証(Production Prompt/code変更なし、新Validator追加なし)。施策1だけで両パターンとも時系列列挙から反転・対比構成へ明確に改善し、Point Overlap/Value QAも両パターンとも余裕をもってPASS。施策2は安全・低コスト(約¥0.43/記事)だが今回1回ずつの比較では明確な品質改善は確認できず。Trend overclaimはBで0件(Trial-09の同種条件では4件のMINOR)。副次的にLedger入力ミス3件目(6月14日合意と6月17日MOU署名日の混同)が新たに判明し既存Fact Checkerが検知。判定は`USER_DECISION_REQUIRED`。詳細は本ファイル該当エントリ・`OPEN-112-TREND-ENGAGEMENT-REFERENCE-AB-TRIAL-10_REPORT.md`参照)。2026-09-05(OPEN-112-TREND-SYNTHESIS-MINIMAL-PROMPT-TRIAL-09、News系Trend Synthesisモード用の最小限Focus Module Promptを実在テーマ(米国・イラン、ホルムズ海峡の長期膠着、2026年9月に実際にWebSearchで新規収集)でA2×2・B1B×1の実記事Trialとして検証(Production Prompt/code変更なし、新Validator追加なし)。Trend Gate 6条件全PASSで`TREND_READY`。懸念していたPoint One/Twoの単純Signal列挙・trend overclaim(複数Signal統合の因果・確信度誇張)はいずれも実際に発生したが、既存Point Value QA・既存Ledger Deviation Checker(専用カテゴリなしの既存タグ)がそれぞれ正しく検知し可視化した(EXISTING_QA_SUFFICIENT寄り、新規Validator不要と判断)。Point Overlap QAの固定lexical閾値(0.40)がNews/Trend特有の共有語彙で誤ってflagされ続ける事例、B1BでFact Checkerが本Trial作成Ledger自体の精度誤り(日付照合ミス・未確認個数混入)をFAILとして検知した事例が判明し、USER_DECISION_REQUIRED候補3点を報告のみ行った(登録・実装・配線はいずれも今回未実施)。判定は`USER_DECISION_REQUIRED`。詳細は本ファイル該当エントリ・`OPEN-112-TREND-SYNTHESIS-MINIMAL-PROMPT-TRIAL-09_REPORT.md`参照)。2026-09-05(OPEN-112-NEWS-MODE-DESIGN-08、News系(A Family)をMajor/Daily NewsとTrend Synthesisの2モードに整理し、Discovery/Whyと同じ4層構造で成立するかをread-only設計した(コード変更・Prompt正式文言確定・Trial実行・記事生成のいずれも未実施)。既存4slot・既存Point Role Planningの上で両モードとも成立する設計に到達しSTOP条件はいずれも発生せず`NEWS_MODE_DESIGN_READY`と判定。既存Fact Safetyのスコープ外にあるtrend overclaim/source strength Gapが未解決として残ることをリスク明示。詳細は本ファイル該当エントリ・`OPEN-112-NEWS-MODE-DESIGN-08_REPORT.md`参照)。2026-09-05(OPEN-113-POINT-CONTEXT-PRODUCTION-WIRING-AND-NO18-B1-REGEN-04、OPEN-113 Trial-03でVALIDATEDとなったPoint-context-only方式(Local Rewrite発火時、対象文が属するPoint全体を参考contextとして渡すだけでRule文言・System Prompt・Retry回数・API call数は無変更)をユーザーが正式採用し、`er010_ledger_local_rewrite_09.py`(`extract_point_context()`追加)・`er003_v1_n3_01_articles_generate.py::run_one_pattern()`へ`PRODUCTION_WIRED`。実Production関数を直接使うRegression(Trial-01〜03と同一4項目)で全件が意味重複なくattempt1解決、Point context実渡しを確認(`point_context_found=true`)。No.18 B1を実Production経路(4層Prompt不使用)で再生成した結果はLedger COMPLIANTだがLocal Rewrite自体は不発火(既知の重複問題は4層Prompt Trial出力限定だったことを示唆)、A2は再生成せず副作用なしをコード構造・fixture両面で確認。既存回帰33件PASS(新規5件追加)、プロジェクト全体回帰2075件中2072 PASS(残り3件は本タスク前から存在する既知の無関係failureとgit stashで確認済み)。Ledger Deviation Checkerの判定揺れ(OPEN-114候補)を4セッション目として再観測、未登録のまま候補報告のみ。詳細は本ファイル該当エントリ参照)。2026-09-04(ER-011-TTS-STANDARD2-MINIMAL1-PRODUCTION-WIRING-FINAL-26、wiring-25の「callerがmax_attempts=6・10等を渡す既存呼び出し元の総予算は縮小しない」という方針をユーザーが撤回し、対象3関数(`generate_charon_japanese`/`generate_a2_japanese_with_fallback`/`generate_english_segment_with_fallback`)内部で`min(max_attempts, PRODUCTION_MAX_TTS_ATTEMPTS)`により無条件でTOTAL3回へクランプするよう変更(`PRODUCTION_WIRED`)。実TTS provider(gemini-3.1-flash-tts-preview)・実ASR Verificationを使い、controlled input(標準経路style_prefixのみ一時差し替え、fallback側は無変更)でStandard1 NG→Standard2 NG→Minimal Fallback実発火(3回目)→PASSという経路を実際に実証(mock不使用、`fallback_used=true`・`total_attempt_count=3`)。Azure Secondary・Key Phrase専用4回構成への非干渉を確認、regression追加・全PASS。詳細は本ファイル該当エントリ参照)。2026-09-04(ER-011-TTS-STANDARD2-MINIMAL1-PRODUCTION-WIRING-25、TTS標準経路+minimal instruction fallbackの2段構成を持つ3関数(`generate_charon_japanese`/`generate_a2_japanese_with_fallback`/`generate_english_segment_with_fallback`)について、標準経路が早期returnせず消費し切ると`fallback_budget`が構造的に常に0になりfallbackが実質的に発火しない不具合(日本語側で実Production incident 2件を確認、英語Key Phrase側OPEN-103と同根)を、ユーザー正式決定「標準2回+Minimal instruction fallback1回=合計3回」で修正・Production配線した(`PRODUCTION_WIRED`)。`er011_human_review_lock_01.py`へ新定数`PRODUCTION_STANDARD_TTS_ATTEMPTS`(2)/`PRODUCTION_MINIMAL_FALLBACK_TTS_ATTEMPTS`(1)を追加し、標準経路の消費量を`standard_attempts`(既定2)回に固定するだけの最小修正(fallback予算計算式自体は無変更)。`max_attempts`を既定3より大きく明示指定する既存呼び出し元(6回・10回)の総予算は縮小せず、fallbackが常に0固定だった不具合修正の効果だけを及ぼした。Key Phrase英語Component専用の独立4回構成(`generate_key_phrase_component_verified`)はこの計算式を使っておらず対象外、regression testで非干渉を保証。Runtime evidence: mock境界+実Production関数直接駆動でCase A(標準1PASS)〜D(標準2+fallback1とも不合格でSTOPPED)を確認、特にCase Cで旧設計では構造的に絶対発火しなかったfallbackが3回目のattemptとして実際に発火・採用されることを確認。regression`er007_ja_tts_retry_path_fix_test_01.py`を全面改訂(4→20テスト、旧仕様前提の非現実的mock[attempts_log=[]]も実際の消費量へ修正)、プロジェクト全体2067件中2064 PASS(残り3件は本タスク開始前の診断[ER-011-PREEXISTING-4-FAILURES-RISK-DIAGNOSTIC-24]で特定済みの無関係な既存2根本原因のみ、本タスク対象のJA retry test失敗は解消)。OPEN-103はfallback_budget=0の構造問題解消を追記しつつ、核心の「default」誤発音[PROVIDER_VARIANCE]自体は未解決のため`DEFERRED / NON-BLOCKING`のまま維持。詳細は本ファイル該当エントリ参照)。2026-09-04(ER-011-NO18-A2-TIGHT-SPEECH-AND-TRIM030-PRODUCTION-WIRING-23、ユーザー正式採用済みの2点(A2 `tight_speech_only()` removal・Key Phrase trim safety margin 0.30秒)を量産Production仕様として恒久的に配線(いずれも`PRODUCTION_WIRED`)。今回はNo.18記事音声の再生成は行わず(ユーザーが21Rの完成音声を承認済みのため)、コード変更2箇所のみで完結: (1)`er003_v1_n3_01_assemble.py::load_a2_sources()`からtight_speech_only()再crop呼び出しを削除(初回/retry/reassembly/regenerationが共有する唯一の関数のため1箇所で正式後続経路すべてに反映)。(2)Key Phrase Master Audio Storeのcache identity(`er006_audio_cost_pilot_02_shared_narration.py`)へtrim policy version("v2_margin030")を追加し、0.20秒時代[2026-08-17〜09-02]に生成済みの既存Key Phrase資産99件が新規リクエストへ誤ってcache hitしうる状態を解消(No.18 kp5「be powered off」の既承認0.30秒資産は新versionキーへsha256一致で移行、無駄な再TTSなし)。Runtime evidence: 新規Key Phraseテキストでの実Production呼び出しでtrailing margin=0.30秒を確認、No.18 A2をコード変更後のstage_assemble_a2()(monkeypatch不使用)で再AssemblyしユーザーAssemble承認済み21R候補音声とsha256完全一致(byte-for-byte同一)を確認。単体テスト12件PASS、全体regression 2051件中2047 PASS(残り4件はer003_test_bad[意図的fixture]・er003_test_p2j_investigate 2件[OPEN-77、既知のmeta-test集計バグ]・er007_ja_tts_retry_path_fix_test_01[本タスク変更前から存在、git stashで無関係を確認]、いずれも本タスクと無関係な既存failureと確認)。詳細は本ファイル該当エントリ参照)。2026-09-04(ER-011-NO18-EVIDENCE-COMPRESSION-A-PRODUCTION-WIRING-AND-FINAL-CANDIDATE-AUDIO-21R、ユーザーがTrial-18/19/20を経て正式採用したPattern A(Representative Metric + Supporting Trend)+ Listener-Friendly Numeric Precisionを、既存Evidence Compression Editor(方式C)のPromptへProduction正式配線した(`PRODUCTION_WIRED`)。No.18 A2/B1を実Production経路で再生成し両レベルで実発火・Fact安全性(Ledger COMPLIANT、causal drift/Fact統合0件)を確認、既存の再利用可能な音声パーツはbyte-for-byte再利用しつつ変更segmentのみ再TTSし、A2(Trial-15のtight_speech_only条件を使用、`FINAL CANDIDATE READY FOR USER LISTENING`)・B1(通常経路)とも最終候補音声を完成させユーザー提出した。Pattern B/Cは`DEFERRED CANDIDATE / NOT REJECTED`のまま、tight_speech_only removal自体はユーザー試聴後まで`PRODUCTION_WIRED`へ昇格させない。詳細は本ファイル該当エントリ参照)。2026-09-04(ER-011-NO18-A2-EVIDENCE-COMPRESSION-ABC-PRECISION-EXTENSION-TRIAL-20、Trial-19で再現性を確認したPattern A/B/Cに、共通の追加ルール「Listener-Friendly Numeric Precision」(不要な小数精度を安全に落とす判断ルール)を重ね、各Pattern3回・計9 runで比較。Pattern Aは3/3 runで一貫して小数[99.71/108.95]を`about 100/about 109`へ丸め、丸め後もF-005/F-006両Factを区別したまま保持(Fact混同なし)、Precisionルールの狙い通りに最も一貫して機能した。Pattern Bは3 run中1 runのみ小数を保持・2 runは丸めるという非一貫な挙動を示し、Precisionルールの適用強度自体の再現性がA/Cより低いことが新たに判明(核となる挙動[両Fact保持・安全性]自体は3/3 runで安定)。Pattern Cは元々差分再表現[約9/約10点]の時点でPrecision的判断を内包しており、ルール追加前後で出力に変化なし(実質no-op)。9 run中、causal drift・Fact削除・数値計算誤りは0件。Ledger Deviation Checkの指摘は全て記事末尾の既存Hook文等、数値表現と無関係な箇所に集中しており、OPEN-98と同種の非決定性の再確認にとどまる(新規Open Item追加なし)。自動判定heuristicが1件[`pattern_c_precision_02`]を「Fact統合疑いあり」と誤検知したが、手動確認の結果、2つのFactの数値[9/10]は文中で明確に区別されたまま保持されており、実際のFact混同ではなかった(Fact Checkerも両差分を「概ね正確」と確認)。No.9への静的汎用性評価は、Pattern Cの構造的適用困難という結論を維持しつつ、Pattern Aは丸めの積極化によりsurvey型での代表指標選定の要注意度がやや上がり、Pattern Bは精度適用の一貫性の低さから従来よりさらに慎重な検証が必要と評価を更新。Production Prompt・CURRENT_SPEC・No.18記事・Audioはいずれも無変更、`USER_DECISION_REQUIRED`のまま。詳細は本ファイル該当エントリ参照)。2026-09-03(ER-011-NO18-A2-EVIDENCE-COMPRESSION-ABC-REPRODUCIBILITY-TRIAL-19、前TrialのBaseline/Pattern A/B/Cを各5回・計20 run実行し再現性を確認。Pattern A/B/Cの核となる変換(代表指標保持/結論優先/差分再表現)はいずれも5/5 runで安定・Pattern Cの数値変換は5/5正確。最重要の発見は2点: (1)全20 runで一字一句同一・未変更のHook文がLedger Deviation Checkにより「検出なし/MINOR/MAJOR」とrun間で異なる重大度判定を受け、Pattern Cの3/5 run `LEDGER_DEVIATION`の主因はF-005/F-006本体ではなくこの既存文への判定ゆらぎだったこと(安全チェック機構自体のrun間非決定性)。(2)追加ルールなしのBaseline単独でも異なる2つのFact(attention/processing speed)を単一の概数ペアへ統合する事例が1件発生し、Ledger Deviation Checkがこれを検出しなかったこと(Pattern Cの「Fact別検証済み丸め」の必要性を裏付け)。correlation→causation drift・記事構造を壊す大規模書き換えは20 run中0件。No.9への静的汎用性評価は前回の結論(Pattern C適用不可、A/Bは要注意で適用可能)を維持しつつBaseline統合リスクの注意点を追記。Production Prompt・CURRENT_SPEC・No.18記事・Audioはいずれも無変更、`USER_DECISION_REQUIRED`のまま。詳細は本ファイル該当エントリ参照)。2026-09-03(ER-011-NO18-A2-EVIDENCE-COMPRESSION-EXTENSION-ABC-TRIAL-18、No.18 A2の同一Writer出力を共通入力として、既存Evidence Compression Editor[方式C]のPromptを変更せず、末尾に追加ルールだけを差し込んだ3パターン[A: Representative Metric+Supporting Trend/B: Conclusion-First Numeric Necessity Test/C: Listener-Friendly Numeric Re-expression]をBaseline対照込みでTrial。4バージョンともLedger Deviation Check=LEDGER_COMPLIANT、Fact Checker=REVIEW_REQUIRED[4バージョンで指摘内容が同一の既存論点のみ、新規指摘0件]、correlation→causation drift 0件を確認。Baseline(追加ルールなし)を今回改めて実行したところ、既存Production記事とは異なりF-005/F-006の数値4つ全てを省略する結果になり、既存方式Cがこの境界ケースでrun-to-runの揺れを持つことを新たに確認。Pattern CはFact Checkerが独自に元論文から同じ差分値[9.24/10.09]を算出し再表現の正確性を裏付け。No.9への静的汎用性評価では、Pattern A/Bは概念上適用可能[Bはより慎重な運用が必要]、Pattern Cは同一指標2条件比較専用でNo.9のような3設問survey構造には数学的に適用できないことを確認。Production Prompt・CURRENT_SPEC・No.18記事・Audioはいずれも無変更、`USER_DECISION_REQUIRED`のまま。詳細は本ファイル該当エントリ参照)。2026-09-03(ER-011-EVIDENCE-COMPRESSION-HISTORY-AND-FAILURE-AUDIT-17、Evidence Compression/Numeric Compressionの導入経緯・過去Trial・不具合を一次資料[DECISION_LOG.md本体・OPEN_ITEMS.md・CURRENT_SPEC.md・git log]から履歴監査(実装なし)。方式B[Compression-aware Writer]がcausal drift[相関→因果の強まり]実例によりREJECTEDとなった経緯、方式C[Lossless Editor]採用の理由、No.9 OPEN-100[Case 2: SPEC_TOO_WEAK、DEFERRED]の完全な経緯、No.18との異同を再構成した。副次的に「Editorial Type」がProductionコードに未実装であることが判明し、新規OPEN-112として登録(`DEFERRED / NON-BLOCKING FOR CURRENT EVIDENCE COMPRESSION TASK`)。新しいCompression仕様・Prompt・Trial・記事修正・Production変更はいずれも実施していない、`DIAGNOSTIC COMPLETE`。詳細は本ファイル該当エントリ参照)。2026-09-03(ER-011-NO18-CONNECTED-SPEECH-READING-RESOLVER-PRODUCTION-WIRING-08、ユーザー正式決定によりOPEN-107のEnding-Clarity fallbackをProductionから撤回(`WITHDRAWN`)し、Trial-07でVALIDATED判定済みのB1 Connected Speech Validator・A2 Reading ResolverをProduction配線した(`PRODUCTION_WIRED`)。既存regression(English 57件・Japanese 35件他)全PASS、新規regression 15件全PASS(セッション前から存在する無関係な既存失敗2件をgit stashで切り分け確認)。No.18のB1 `comment_2`/`comment_3`・A2 `comment_1`が、review_lockの累積予算超過によるlockをapprove_regenerate()で解除した上での実Production呼び出しにより、実際のTTS/ASR出力に対して新Validatorで解決することを確認(OPEN-110/OPEN-111解消)。No.18 A2/B1が今回初めて両レベルとも完成(B1 351.324秒・A2 347.338秒、いずれもclipping無し)。詳細は本ファイル該当エントリ参照)。2026-09-03(ER-011-CONNECTED-SPEECH-AND-A2-READING-TRIAL-07、Track A: ASR差分を即座に「TTS誤発音」とせず、語境界のconnected speech[音の弱化・融合・再分節]として説明可能かを判定する候補ロジックを3パターン[歯擦音連続/破裂音連続/再分節]限定でTrialし、実データ3件[studies suggest/opened to/survey suggest]を正しく分類、3件の陰性対照ケースでfalse acceptなしを確認して`VALIDATED`と判定(OPEN-107仕様変更なし)。Track B: A2 OPEN-111について、前回[Trial-06、全文LLM自由生成、REJECTED]とは異なる新方式(既存pykakasi機械変換の差分位置だけを検出し、その漢字の読み候補をpykakasi辞書から取得してLLMに1つだけ選ばせる、自由生成させない)をTrialした結果、`comment_1`は1回のLLM呼び出しで解消し、他正常segmentへの副作用は0件(差分がない箇所にLLMが触れないため構造的に副作用なし)、コストはTrial-06比で大幅に低い(概算¥0.01〜0.04/記事)ことを確認して`VALIDATED`と判定(単発呼び出しの非決定性は明記)。両TrackともProduction配線は一切行っていない。詳細は本ファイル該当エントリ参照)。2026-09-02(ER-011-NO18-B1-LISTENING-AND-A2-OPEN111-READING-TRIAL-06、B1`comment_2`/`comment_3`/`opened`(OPEN-107参考比較)について、canonical全文・TTS入力・各attemptのASR結果・実音声・対象語前後の単語を人間が試聴確認できるArtifactを作成(文面変更・Validator変更・retry追加・新fallback追加はいずれも実施せず)。A2 OPEN-111について、Writerとは別の「Reading担当」役割による文脈込み全文ひらがなcanonical reading方式をA2限定でTrialし、「後/あと」問題自体は解決できることを確認したが、Reading担当自身が無関係な新しい読み誤りを2件(本番PASS中の正常segmentを含む)発生させたため`REJECTED`と判定(コストは1記事あたり概算¥0.8〜1.6で問題なし、品質面の理由)。Production配線は一切行っていない。詳細は本ファイル該当エントリ参照)。2026-09-02(ER-011-NO18-OPEN109-110-FINAL-CLOSEOUT-04、OPEN-107のEnding-Clarity fallbackについて、検出ロジック(y→ies不規則複数形・複数語replace diffの語単位分解)と配線範囲(B1 Comment segment、`voice01.generate_charon_english`経路)の2つのgapを是正し(regression 20件PASS)、B1`comment_2`を実際にProduction経路で再生成した結果、通常3attempt全NG→fallback実発火(初のProduction実発火確認)も、fallback自体は`study`[単数形]発話を解消できず`comment_2`は未解消のまま。`comment_3`("survey"→"surveys")はraw pipeline診断(5条件×3attempt)を実施し、「複合主語構文で動詞に直接隣接する名詞の数が不安定になる」という仮説を得たが方針決定はせず。OPEN-109は、精密化済みLedgerからA2記事をProduction正式Writer pathで再生成した結果fact_verdict=`PASS`・ledger_status=`LEDGER_COMPLIANT`となり(B1Bより精度指摘が少ない)、Support/Key Phraseも再生成しPASS、A2記事テキストは完成・確定。続くA2 Audio Stage実行で`comment_1`(日本語、「後」/「あと」の孤立漢字異読み、CURRENT_SPEC.md記載済みの既知の残存限界の新実例)が新規発見されAudio Validation Gateが正しくblock、新規OPEN-111として追加。再発防止としてCross-Level Consistency Check(A2/B1片側修正でのcloseを禁止するSSOT運用ルール)をCURRENT_SPEC.mdへ新設。A2/B1とも episode assembly未完成のまま。詳細は本ファイル該当エントリ参照)。2026-09-02(ER-011-NO18-OPEN107-PRODUCTION-WIRING-AND-FINAL-AUDIO-03、ユーザーがEnding-Clarity fallback[OPEN-107]を正式承認、B1 News本文5segmentへProduction配線(`er011_ending_clarity_fallback_01.py`、regression 13件PASS、実Production call site統合、CURRENT_SPEC.mdへ`PRODUCTION_WIRED`記載)。ユーザー指定のB1B Point One本文2文の個別差し替えを反映し、既存公式QA工程を再実行(いずれもnon-blocking)、Support/Key Phraseも再生成しB1Bテキストを完成・確定(`RESOLVED`)。No.18 A2/B1 Audio Stage(同期TTS)を実行した結果、B1は13segment中11件・A2は14segment中13件がOKとなったが、B1`comment_2`/`comment_3`(新しい単数/複数形TTS誤発音、OPEN-110新規)・A2`full_story_part1`(STOPPED)がAudio Validation Gateにより正しくblock。加えてA2記事本文がLedger精密化前の記述(Pew丸め表現・出典未確認の"hard-to-resist habit")を含んだままであることを新たに発見し(OPEN-109新規)、A2 Audioの最終化を保留。詳細は本ファイル該当エントリ参照)。2026-09-02(ER-011-NO18-OPEN108-LEDGER-REFINE-AND-OPEN107-ENDING-FALLBACK-TRIAL-02、Track A[OPEN-108]: No.18 B1BのFact Checker FAILの原因を調査した結果、Research Layer抽出段階で混入した不正確なLedger記述(F-011「hard-to-resist habit」がPew報告書本文に実在しないことを独立2回のWebFetchで確認、F-009の「約40%」丸めも実際は42%/25%/24%/56%の内訳と判明)だったと特定。Fact Checker自体は変更せず、Verified Fact Ledgerをsource-groundedに精密化した上で既存Production正式Writer pathからB1Bを1回だけ再生成した結果、fact_verdict=REVIEW_REQUIRED(non-blocking)・ledger MINOR 1件のみとなりFAIL解消、Support/Key Phraseも再生成しPASS、OPEN-108は`RESOLVED_PENDING_USER_TEXT_REVIEW`。Track B[OPEN-107]: 通常segmentは変更せず通常retryでもNGが続いたsegmentだけにEnding-Clarity instructionを追加するfallback候補を隔離Trial(Production関数呼び出し時のみ一時monkeypatch、恒久変更なし)。結果はcondition依存で一様でなく(raw pipelineの完全文+文脈条件はNormal 2/3→Ending-Clarity 1/3で悪化、実Production相当条件[Batch API+retry cascade]はNormal 0/3→Ending-Clarity 2/2で大幅改善)、`USER_DECISION_REQUIRED`のまま維持。両TrackともNo.18の個別ハードコード修正は一切行っていない。詳細は本ファイル該当エントリ参照)。2026-09-02(ER-011-NO18-PRODUCTION-SPEC-IMPROVEMENT-01、No.18で発見された3件の問題(Key Phraseの文脈依存人称表現・5件相互の意味重複・Pointが重複していなくても新しい価値を持たない場合がある)を、個別のNo.18修正ではなく汎用Production仕様として改善し、単体テスト26件追加(全125件PASS)、正規Production経路からNo.18を再生成(A2完成、B1はFact Checker FAILでNG_REVIEW_REQUIRED、既存policy通り)。OPEN-107はProductionと分離したDiagnostic Trialで原因範囲(完全な一文のprosodyに起因、断片は12/12正解・完全な一文は6件中2件で誤発音)を診断したが新Production仕様は未採用のままUSER_DECISION_REQUIRED。詳細は本ファイル該当エントリ参照)。2026-09-02(ER-011-NO18-DISCOVERY-WHY-FULL-PRODUCTION-RUN-01、No.18「Why Is It So Hard to Ignore a Notification?」をDiscovery/Why型としてNo.9までのProduction Wired仕様のみで正式Production実行。A2は完成[378.634秒、USER_FINAL_AUDIO_REVIEW_REQUIRED]、B1は新規TTS失敗モード[OPEN-107、`USER_DECISION_REQUIRED`]によりepisode assembly未完成。詳細は本ファイル該当エントリ参照)。2026-09-02(ER-010-NO9-FINAL-APPROVAL-CLOSEOUT-AND-FULL-STATUS-AUDIT-28、ユーザーがNo.9 A2[354.162秒、function-word reduction版]・B1[335.754秒、既存承認維持]をいずれも最終承認。No.9関連management ID全24件をRepository横断監査し、Trial仕様・採用/却下/deferred仕様・修正bug・Open Item・one-off例外を完全棚卸しした結果、未処理USER_DECISION_REQUIRED・Dangling Reference・未配線APPROVED仕様は0件を確認(CURRENT_SPEC.mdの「Formatting禁止」行の記録漏れ1件のみ発見・補完、コード自体は既にPRODUCTION_WIRED済み)。OPEN-103は恒久課題[Gemini TTSの`default`誤発音]自体を`DEFERRED / NON-BLOCKING`のまま維持しつつA2側Blockingの解消を追記(恒久課題自体はCLOSEしない)。No.10以降のSSOT補助資料`ER-010_NO9_FINAL_CLOSEOUT.md`を新規作成。No.9 A2/B1を`FINAL USER APPROVED / CLOSED`、No.9開発全体を`CLOSED WITH DEFERRED OPEN ITEMS`[OPEN-100・OPEN-103恒久課題は意図的deferred維持]として正式close)。2026-09-02(ER-010-NO9-FUNCTION-WORD-REDUCTION-PRODUCTION-WIRING-AND-A2-FINAL-27-R1、Trial 26で検証したfunction-word/article reduction原則をユーザーが正式採用し、`KEY_PHRASE_MINIMAL_INSTRUCTION_PREFIX`(Primary)・`KEY_PHRASE_ENGLISH_LOCK_INSTRUCTION`(Fallback、Minimalを起点に構築されるため自動適用)へProduction配線。Dangling Reference Check・既存27テスト回帰・新規9テストいずれも全PASS。実Production path(Trial専用scriptではない)から「a catch」+article fixture3件[a chance/an idea/the answer]+非function-word系既存Key Phrase3件[guilt tipping/push back/starting point]の計7件を生成し全件Attempt 1 PASS、article/function-wordを含まないKey Phraseへの悪影響は確認されず。「a catch」はduration 1.011秒・「a」区間0.08秒[旧0.28秒の約29%]まで短縮、`kp4_en.wav`を実際に差し替えNo.9 A2を再Assembly[354.162秒、clipping無し、Key Phrase 4は1回のみ想定位置]。`default`[kp2]・他Key Phrase[kp1/3/5]は無変更をsha256で確認、OPEN-103は無変更のまま。CURRENT_SPEC.mdへ正式追加・OPEN-106は`RESOLVED / CLOSED`。完成音声+全文script+「a catch」新旧比較のUser Listening Artifact提示、`USER_FINAL_AUDIO_REVIEW_REQUIRED`)。2026-09-01(ER-010-NO9-A2-KEYPHRASE-ARTICLE-REDUCTION-DIAGNOSTIC-AND-TRIAL-26、No.9 A2 Key Phrase「a catch」の冠詞"a"が不自然に強く・長く発音されるとのユーザー試聴フィードバックを受け診断。現行Production Key Phrase Minimal instructionにfunction word[冠詞等]を弱く読む原則が存在しないことを既存仕様監査で確認[実装漏れではなく新規safeguard候補]。既存文言を一切変更せず末尾に一般原則を1文追加しただけの隔離Trialを実施し[「a catch」固有のhardcodeなし]、Attempt 1でmachine PASS。実測envelope解析で「a」区間が0.28秒→0.07秒[約1/4]・peak RMSがcatch本体を下回る関係へ好転・フレーズ全体が約34%短縮したことを客観確認。Current Production版とTrial版を並べたUser Listening Artifactを提示、Production一般仕様への正式採用は行わずOPEN_ITEMSで`USER_DECISION_REQUIRED`。`default`[OPEN-103]・Attempt 4 one-off固定assetは無変更)。2026-09-01(ER-010-NO9-A2-ATTEMPT4-ONEOFF-FINAL-AUDIO-25、ユーザー正式決定によりTrial 21 ENGLISH_LOCK attempt=2[通し4回目=「Attempt 4」]をNo.9 A2 `kp2_en`限定のone-off固定assetとして採用。machine validator上の`TTS_FAILURE`判定は書き換えず`one_off_fixed_asset_override`として追記、Production Episode Assembly Gateの既存承認メカニズム[`human_approved_segments.json`]で正式にunblock。ローカルdisfluency QA実施後、No.9 A2 episodeを実際にAssembly[355.002秒、clipping無し]し完成、完成音声+全文scriptのUser Listening Artifactを提示、ユーザーの最終試聴承認待ち。OPEN-103は恒久課題としては`DEFERRED / NON-BLOCKING`のまま、A2完成のBlockingは解消)。2026-09-01(ER-010-NO9-A2-DEFAULT-FIXED-ASSET-FINALIZATION-23-R1、`default`個別対応として新規TTS Trialなしで既存固定音声資産を探索。既存`kp2_en.wav`は実ASR再確認により「別のKey Phrase[Guilt tipping]との取り違え」と確定。No.9 A2/B1BのFull Story本文に地の文として"default"が登場する箇所を新規発見し、既にRESOLVED(B1Bはユーザー承認済み)のsegmentからfaster-whisper[ローカル無料]のword-level timestampで単語単位を切り出した候補3件を作成。いずれもローカルASRは"default"のみを検出する一方、Production ASRは短い誤認識語を文頭に付け足す既知のASR挙動が見られ、断定できないため人間の試聴判断に委ねる形でArtifact提示。Trial 21 Attempt 3/4は使用せず、新規TTSも行っていない。A2 Assemblyは未実施、OPEN-103は`USER_DECISION_REQUIRED`)。2026-09-01(ER-010-NO9-KEYPHRASE-MINIMAL-ENGLISHLOCK-PRODUCTION-WIRING-22、ユーザー正式決定「Minimal instruction最大2回→NGならEnglish language lock付きMinimal最大2回(合計最大4回)」をKey Phrase英語Component生成のProduction正式初回経路へ全面配線[`generate_key_phrase_component_verified()`書き換え、旧`ENGLISH_STYLE_PREFIX`標準経路起点の構成から離脱、他segmentのTTS retry上限3回は無変更]。Runtime evidence取得: 実TTS Case A["push back"、Minimal Attempt 2でPASS、fallback未使用]・境界モックCase B/C[Minimal 2回NG→English Lock即PASSへ実際に遷移、cumulative_tts_attempts=3で二重会計なし]。既存18テスト全PASS(新規5件追加)。`default`は個別例外として一般仕様から分離、OPEN-103は`DEFERRED / NON-BLOCKING`、Trial 21のEnglish Lock Attempt 1/2をNo.9限定fixed asset候補としてユーザーへ提示[未採用]。既存4件[kp1/3/4/5]は旧経路で既にRESOLVED済みのため無変更のまま)。2026-09-01(ER-010-NO9-KEYPHRASE-ENGLISH-LOCK-FALLBACK-TRIAL-21、Key Phrase「default」1語を対象に、量産候補retry仕様[Minimal instruction最大2attempt→NGならEnglish language lock付きMinimal最大2attempt、合計最大4attempt]を隔離Trial。4attemptすべて不合格(Minimal 2回=日本語カタカナ「デフォルト」、English Lock 1回目="defaut"[英字だが誤スペル]、English Lock 2回目=日本語カタカナへ逆戻り)。duration anomalyは4回とも無し(改善は維持)だが、content accuracy失敗(非英語発話)はEnglish lockを追加しても解消せず、Trial判定は`REJECTED`。Production instruction・review lock・retry budgetはいずれも無変更、OPEN-103は`USER_DECISION_REQUIRED`のまま維持)。2026-09-01(ER-010-NO9-KEYPHRASE-MINIMAL-INSTRUCTION-MINI-TRIAL-20-R2、No.9 A2正式Key Phrase5件[guilt tipping/default/push back/a catch/starting point]でMinimal instruction Mini-Trial[各語1回→NGのみ最大2回retry、最大3attempt]を実施。5件中4件[guilt tipping/push back/a catch/starting point]はAttempt 1でPASS[duration anomaly無し、Validator NORMALIZED_MATCH/EXACT_MATCH]。`default`は3attemptとも誤発音(FAIL)で再現性を確認: 前回Trialの英語内誤認識["Dieselt"]とは異なり、今回は3回とも発話言語自体が英語からずれた[デフォルト/デフォルト/默认]。duration anomaly自体は0件で解消したが、content accuracy失敗が形を変えて残存。Production instruction・review lock・retry budgetはいずれも無変更、OPEN-103は`USER_DECISION_REQUIRED`のまま維持)。2026-09-01(ER-010-NO9-KEYPHRASE-MINIMAL-INSTRUCTION-TRIAL-AND-RETRY-ACCOUNTING-FIX-19、Key Phrase英語音声Minimal instruction Trial[OPEN-103、opt outはVALIDATED相当・defaultは単発試行でTTS_FAILURE]・review lockネスト二重会計バグ修正[OPEN-105、CLOSED])。2026-09-01(ER-010-NO9-B1-APPROVAL-AND-OPEN103-TTS-DIAGNOSTIC-18、No.9 B1音声のUser Approval記録・完成音声提示ルールの再確認・OPEN-103 TTS request payload監査)。2026-09-01(ER-010-NO9-A2-KEYPHRASE-AUDIO-ISSUES-103-104-17、OPEN-103/104個別診断・OPEN-104実装バグ修正)。2026-08-31(ER-010-NO9-STORYTELLING-NOJARGON-PRODUCTION-WIRING-06、Storytelling First/No JargonをProduction初回Writerへ正式実装・Meaning First REJECTED確定・No.9新規再生成候補をProduction経路から取得。2026-08-31、ER-010-N1-SPEC-LIFECYCLE-PRODUCTION-GATE-04、仕様Lifecycle・Dangling Reference Checkの正式導入をDecisionとして記録。2026-08-29、ER-008-N8-FINAL-CLOSEOUT-24、地名/施設名CompressionのNo.8正式反映・Writer Point Balance prompt強化・cost計算モジュールの単価バグ修正・Stephen Reicher発音PASS確定)**
...
## ER-011-TTS-EXECUTION-MODE-SWITCH-PRODUCTION-WIRING-01: TTS実行モード切替(環境変数)をdrop-in factory1箇所へProduction配線

**日付**: 2026-09-06
**区分**: Implementation Hardening(サービス・生成仕様は無変更。TTS実行方式[Batch/Standard]をコードから明示的に選択可能にする配線)

**内容**: ユーザーが`APPROVED_FOR_PRODUCTION`と正式承認した「TTS実行モード切替を1箇所追加する最小変更」を実装した。Production TTS生成6call siteが共通に呼ぶ唯一のdrop-in factory`er006_batch_tts_wiring_01.py::make_batch_tts_call_fn(model, voice, ...)`内に、環境変数`TTS_EXECUTION_MODE`(値`BATCH`/`STANDARD`、大文字小文字非依存、既定`BATCH`)を読む分岐を1箇所追加した(`resolve_tts_execution_mode()`)。`STANDARD`時は`er003_b1_p7a_audio.make_tts_call_fn_for_model(model, voice)`(その モジュールが無い実行環境向けの防御として`er002_gemini_client.make_tts_call_fn`へフォールバック)を返す(`_make_standard_tts_call_fn()`)。`BATCH`/`STANDARD`以外の値は、静かにBATCHへフォールバックせずValueErrorで即停止する(fail-closed)。返す関数の呼び出し形状(`tts_call_fn(prompt: str) -> bytes`)・voice・style instruction・Structured Separation・retry cascade・Validator・Master Audio Storeは無変更、call site6箇所(`er003_v1_crosslevel_audio_02_common.py`[repro01配線済み関数を再利用]/`er003_v1_repro01_main_generate.py`/`er003_v1_sing01_news_tail_fix.py`/`er003_v1_sing01_point_headings_aoede.py`/`er003_v1_sing01_voice01_generate.py`/`er003_v1_n3_01_tts_generate.py`)も無変更。

選択されたモードはruntime evidenceとして記録する: Batch経路は既存の`er006_batch_tts_wiring_01.py::_record()`が書く`raw_usage_log.jsonl`エントリへ`tts_execution_mode: "BATCH"`を追加(既存キーは無変更)。Standard経路は`er005_cost_logger.py::_patch_gemini()`(全Standard `generate_content`呼び出しをprocess単位でmonkeypatchする既存の共通ロガー)側で、`response_modalities=["AUDIO"]`のTTS呼び出しにのみ`tts_execution_mode: "STANDARD"`を追加する(text呼び出し[QA/Writer等]には付与しない)。Master Audio Storeのcache identity(`er006_master_audio_store_01.py`)にはモードを含めない判断とした。理由: 同一canonical text・voice・trim policyであれば、同一TTS model(Batch/Standardは同一modelを異なる実行方式で呼ぶだけ)による音声品質は同等であり、モードをキーに含めると既存の再利用可能な資産(既にBatch/Standardいずれかで生成済みのキャッシュ)が不要に再TTSされてしまうため、既存資産の再利用を優先した。

**採用理由**: PM-GOVERNANCE-DEV-TTS-STANDARD-SYNC-01(2026-09-06)で「正式リリース前は原則Standard同期・正式リリース後の実量産はBatch API」という運用基準が決定済みだったが、それまではmonkeypatch等の一時的な手段でしか切替できず、コードから明示的・恒久的に選択できる手段が無かった。ユーザーがこの最小配線を`APPROVED_FOR_PRODUCTION`と承認した。

**根拠レポート**: `ER-011-TTS-EXECUTION-MODE-SWITCH-PRODUCTION-WIRING-01_REPORT.md`

**Runtime evidence**: 新規単体テスト11件PASS(`er011_tts_execution_mode_switch_wiring_01_test.py`、既定/大文字小文字/不正値ValueError/Batch経路委譲/Standard経路委譲[p7a優先・gclientフォールバック]/呼び出し形状同一/cost logger記録[Batch/Standard双方]を検証)。既存回帰: `er006_batch_tts_wiring_01_test.py`14件PASS、`er007_ja_tts_retry_path_fix_test_01.py`22件PASS。プロジェクト全体回帰(`run_project_regression.py`)2084件中2081 PASS(残り3件`er003_test_bad`/`er003_test_p2j_investigate`×2は本タスク前から存在する既知の無関係failureとgit stashで確認済み)。

実API確認(2026-09-06、Key Phrase英語"opt out"、既存の承認済み本番文言を再利用、`er003_v1_repro01_main_generate.py::generate_key_phrase_component_verified()`経由の実Production call site、専用出力先`er011_output/tts_execution_mode_switch_wiring_01/`、既存完成音声には非接触):
- `TTS_EXECUTION_MODE=STANDARD`: `client.models.generate_content`を実際に2attempt呼び出し(elapsed 3.185秒・2.727秒)、`status=OK`(ASR検証PASS)、`raw_usage_log.jsonl`へ`tts_execution_mode=STANDARD`を確認。
- 未設定(既定): `client.batches.create`を実際に4attempt呼び出し(elapsed 80.5/73.3/155.8/117.0秒、既存実測レンジ91〜167秒と整合)、`raw_usage_log.jsonl`へ`tts_execution_mode=BATCH`を確認。この回はPrimary/Fallback合計4attemptともASR内容検証で`STOPPED`(過去の同一Key Phraseは1回のみのSmoke確認でPASSしていたため、既知のTTS/ASR run-to-run非決定性であり、配線自体の不具合ではないと判断。Batch job自体は4回とも`client.batches.create`経由で正常にSUCCEEDEDしている)。

**状態**: `PRODUCTION_WIRED`。

**commit**: 本タスクのcommit(hashはcommit後にGitHub上で確認可能)。

**影響するCURRENT_SPEC項目**: 「Gemini TTS実装方式(Batch API)」行・「TTS」行(いずれも「実行モード切替」注記を追加、既存文は無変更)。

---

## PM-GOVERNANCE-DEV-TTS-STANDARD-SYNC-01: TTS方式の運用基準を「正式リリース前=Standard同期/正式リリース後の実量産=Batch API」へ統一

**日付**: 2026-09-06
**区分**: PM運用ルール+TTS運用SSOT(サービス・生成仕様そのものではなく、TTS実行方式の選択基準)

**内容**: TTS方式(Batch API / Standard同期)の決定基準を、従来の「Production
相当かどうか」ではなく「正式リリース前か、正式リリース後の実量産か」で
統一した。正式リリース前(DEV/Trial/診断/Production相当テスト/量産相当
テスト/Production正式経路を使ったruntime確認/A2・B1等の完成候補生成/
Production wiring後の実データ確認/回帰確認/少数・複数記事の量産模擬を
含む)は原則すべてStandard同期を既定とする。正式リリース後の実ユーザー
向けサービスとしての実際の量産生成はBatch APIを使用する(正式方式)。
例外(Batch固有挙動の検証/Standard・Batch差異確認自体がテスト目的/
ユーザー明示指定/Fableが必要と判断した場合)はBatchを使ってよいが理由を
記録する。`docs/pm/PM_GOVERNANCE.md`「7節」を7-1(決定基準)・7-2(例外)・
7-3(既存の明示・確認原則)へ改訂し、`CURRENT_SPEC.md`の「Gemini TTS実装
方式(Batch API)」行・「TTS」行へ、Batch APIは正式リリース後の実量産の
正式方式でありStandard同期使用はProduction正式経路であることを否定しない
旨の注記を追加した(既存文は削除せず維持)。

**採用理由**: Batch APIは1件あたり実測91〜167秒の完了待ちが発生し
(retry発生時はさらに積み重なる)、Trial・runtime確認の所要時間を大幅に
延ばす一方、コスト差(Batch/Standardの実費差)は小額であり、正式リリース
前の検証速度を優先する判断をユーザーが行った。「Production相当だから
Batch」という誤解を防止する目的も含む。

**根拠レポート**: なし(PM運用ルール+TTS運用SSOTの文書改訂のみ、Trial
実行なし)。
...
## OPEN-117-KEYPHRASE-DISPLAY-TTS-SEPARATION-TRIAL-01: Key Phrase日本語訳の表示用/TTS用分離方式のPhase 1隔離Trial(Production変更なし)

**種別**: 短い隔離Trial(ユーザー承認済み、2026-09-06。Phase 1のみ、Production code/Prompt変更なし)

**背景**: 直下のOPEN-117-KEYPHRASE-TILDE-GATE-RECHECK-01で、「～」を含む訳語はTTS(Production Batch API)で安定して読めず3/3不合格(対照「何かを示す」は1/1合格)と確定していた。規約B(「～」不使用)は維持したまま、「表示用の辞書的表記」(「～を示す」)と「TTS読み上げ用テキスト」(「なになにを示す」等の口語的言い換え)を分離する方式が有効かをTrial確認した。

**実装(Trial、Production Batch API、RECHECK-01と同一手法で`generate_charon_japanese`を直接呼ぶ)**: 主対象4件(示す/指す/につながる/意味する)のTTS用表現(「なになに」形)×各2run(再現性確認)、対照A「何かを示す」、対照B複合形「なになにを示す、なになにを指し示す」、代替2件(「なにかを示す」「あるものを示す」)、計12入力。結果: **12件すべて`status=OK`**(9/12はattempt1で`PHONETIC_MATCH`/`NORMALIZED_MATCH`、残り3件もattempt2で回復、fallback未使用、異常長0件)。RECHECK-01の「～」直接投入(3件とも3回上限まで使い切ってSTOPPED)とは明確に異なる結果。

**追加発見(read-only、既存Production記録)**: `er006_output/pool_pilot_01/pool_benches_luna/b1b/audit/tts_generation_results.json`(ER-006-KP5-CANONICAL-BUG-01の「associate with」二重slot placeholder実データ)で、TTSが「〜」「…」を実在の日本語単語「何々」として自発的に音声化する既存の傾向を確認し、今回「なになに」が安定した理由と整合した。

**意味対応・一般化案・影響範囲(read-only、実装なし)**: 「～」と「なになに」は役割は対応するがニュアンスは完全一致ではない。聴取者にとっての自然さは客観指標では判定できず新規`USER_DECISION_REQUIRED`として残した。一般化案(表示→TTS自動変換の規則化 vs 選定Prompt側での二重生成)、影響範囲(選定Prompt/Canonicalizationスキーマ/Master Audio Store cache identity[`canonical_text_hash`の対象がTTS用テキストになる]/review_lock/Assembly/表示側)をread-only列挙した(実装せず)。ゲート(`detect_gloss_placeholder_notation`)は今回のTTS用テキストが「～」を含まないため対象外のまま、ゲート自体は無変更。

**Closeout分類**: `VALIDATED`(Phase 1の範囲でProduction標準Batch APIによる安定動作を確認、Production採用ではない)。聴取者による自然さの主観評価は`USER_DECISION_REQUIRED`。Phase 2(Production正式経路での配線・Theme 2 A2/B1再生成)は本タスクでは実施していない。

cost 約$0.01572(約¥2.52)。Production code・Prompt・`keywords_canonicalized.json`スキーマ・`CURRENT_SPEC.md`・status格上げはいずれも無変更。詳細は`OPEN-117-KEYPHRASE-DISPLAY-TTS-SEPARATION-TRIAL-01_REPORT.md`。

---

## OPEN-117-KEYPHRASE-DISPLAY-TTS-SEPARATION-TRIAL-02: Key Phrase表示用/TTS用分離方式のPhase 2(Theme 2 A2/B1、Production正式経路+Trialアダプタ、Production変更なし)

**種別**: Trial(ユーザー承認済み、2026-09-06。Production code/Prompt/スキーマ変更なし)

**背景**: Phase 1(上記、`VALIDATED`)で低レベルTTS関数を直接呼ぶ隔離Trialにより分離方式の安定動作を確認した。Phase 2は、Theme 2(若者の旅行、B条件)のA2/B1で、Key Phrase選定(Trialコピー選定Prompt、gloss側「～」「〜」のみ許可)からcanonicalization・Master Audio Store経由TTS・ASR Cascade検証・Assemblyまでを、**量産時と同じProduction正式経路**(いずれも無変更)+Trialアダプタで確認した。
...
**Closeout分類**: `USER_DECISION_REQUIRED`(分離方式の核自体はProduction正式経路上で`VALIDATED`だが、本Runでは両レベルとも完成音声が得られず`USER_FINAL_AUDIO_REVIEW_REQUIRED`に未到達。変換規則の適用範囲拡張要否[数値placeholder型「～％」等への対応]、A2 rank2の`REGENERATE_APPROVED`承認要否、Production採用時のスキーマ変更[表示用/TTS用2フィールド化]着手要否がユーザー判断事項)。

cost 約$0.0571(約¥9.14)。Theme 2累計(Trial-11〜17・OPEN-117 Phase 1/2)約¥134、上限¥1,000に余裕あり。Production code・Prompt・`keywords_canonicalized.json`スキーマ・`CURRENT_SPEC.md`・status格上げはいずれも無変更。詳細は`OPEN-117-KEYPHRASE-DISPLAY-TTS-SEPARATION-TRIAL-02_REPORT.md`。

---

## OPEN-117-KEYPHRASE-TILDE-GATE-RECHECK-01: Key Phrase日本語gloss「～」placeholderゲートの再評価(Trial-only TTS確認、Production変更なし)

**種別**: Implementation Hardening(read-only調査+隔離Trial。Production code/Prompt変更なし、記録のみ)

**背景**: OPEN-116でTrial-13 kp4_ja(canonical gloss「～を示す、～を指し示す」)がplaceholder検出ゲート(`er003_audio_tts_asr_safety.detect_gloss_placeholder_notation`)でTTS呼び出し前にSTOPPEDになった件について、ゲートが過剰判定ではないかをユーザー指示で再評価した。

**確認1(Trial、Production Batch API、`er003_v1_sing01_voice01_generate.generate_charon_japanese`をゲートより手前の低レベル関数として直接呼び、仮にゲートが無かった場合の実挙動を確認)**: (a)「～を示す」(b)Trial-13実gloss「～を示す、～を指し示す」(c)対照「何かを示す」(d)波ダッシュ版「〜を示す」。結果: (a)(b)(d)はいずれも標準2回+fallback1回(計3回)とも`TRUE_CONTENT_MISMATCH`でSTOPPED。ASRは記号自体を音として書き起こすのではなく、文頭の裸の助詞「を」を含む不完全な断片を不安定に発話し回によって異なる空似語(「お、死滅。」「推しミス」「頭を閉めす。」等)・脱落・filler(「お」「それを」)を生成した。(b)は2回の標準attemptで「示す、指し示す」(「を」を欠落)と一貫した書き起こしになったが、canonicalとの完全一致には至らずSTOPPED。異常長ハルシネーションは今回発生せず(全attempt 1.4〜2.96秒、閾値内)。(c)は1回目で`NORMALIZED_MATCH`。cost 約$0.0101(約¥1.62)。

**過去データ調査(read-only)**: 全記事の`keywords_canonicalized.json`から「～/〜/…」を含むgloss13件を発見(Opus診断-15の報告と一致)。うち実際にゲートでSTOPPEDになるのは3件のみ(「中～高強度の運動」=範囲notation、「～を…だと宣言する」=未解消の二重slot、Trial-13 kp4_ja)、残り10件は先頭1箇所のみのため`tts_safe_ja()`のlstripでゲート到達前に解消済み(ブロック形3件、という既存の報告と整合)。

**追加発見(read-only、実Production記録)**: ゲート通過済み10件のうち実際に本番TTS生成記録が残っている6件を確認した結果、3件はSTOPPED(うち1件`〜のデータを利用する`はER-005-AUDIO-WASTE-REDUCTION-01[異常長検知]導入前の記録で121秒・84秒の指示文パラフレーズ型ハルシネーションを含む、別の1件`に新しい機会を開く`は12attempt全て無音)、1件`に楽勝する`は現行の縮小予算(標準2+fallback1=計3回)では通らない5回目でようやく合格。約50%規模の実失敗率が確認された。

**結論**: (1)現行ゲートでSTOPPEDになる3件はTTS/ASRで実際に不合格になる内容であり、緩和条件(例: 文頭または読点直後の「～」+直後が助詞、かつ他のplaceholder記号なし)を導入しても使える音声にはならず、Production TTS/ASR retry予算を浪費するだけ(緩和条件案は`REJECTED`)。(2)本質的な問題はゲートの範囲ではなく、辞書的slot省略gloss(裸の助詞から始まる断片)自体がTTSにとって不安定なこと。(3)2026-09-06にProduction配線済みのPrompt規約B(`b1_p2_keywords_l_prompt_template.txt`、「～」等を一切使わず「示す、指し示す」のように助詞ごと省略した言い切り形へ書き換える指示)はこの根本原因(裸の助詞始まり)を直接解消する設計になっており、範囲(全形の「～」禁止、規約Aと合わせ)も妥当。ゲート・規約Bとも**現状維持を推奨**。過去10件のartifactは規約B配線(2026-09-06)以前の記録であり、規約Bにより新規記事では再発しにくいと判断するが、それらartifactに対応する過去生成済み音声資産の再点検要否は`USER_DECISION_REQUIRED`(本タスクでは判断・実装いずれも行っていない)。

**Closeout分類**: `VALIDATED`(ゲート・規約Bとも現状維持が妥当、緩和条件は`REJECTED`)。過去artifact再点検の要否のみ`USER_DECISION_REQUIRED`。

Production code・Prompt(規約B含む)・CURRENT_SPEC・status格上げはいずれも無変更。詳細は`OPEN-117-KEYPHRASE-TILDE-GATE-RECHECK-01_REPORT.md`。

---

## OPEN-112-TREND-THEME2-SERIES-11-17: Theme 2(若者の旅行)Trend Synthesis Trial系列(Trial-11〜17・Opus診断-14/-15)の記録

**種別**: 記録統合(既に実行済みのTrial結果をSSOTへ反映するもの。本エントリ自体はProduction採用判断・新規実装を含まない)

1. **Trial-11(2026-09-05、OPEN-112-ENGAGEMENT-REFERENCE-CROSS-TOPIC-AB-TRIAL-11)**: script `er011_open112_engagement_reference_cross_topic_ab_trial_11.py`、出力 `er011_output/open112_engagement_reference_cross_topic_ab_trial_11/`。2テーマ(週4日勤務/ゆっくり旅行)ともTrend Gate 6条件PASSで`TREND_READY`(単純増加ではなく、意識と実態のGap構造)。Engagement根底指示(施策1)は3テーマ中3テーマ(Trial-10含む)で時系列列挙からGap/Tension構造への改善が再現し、Trial範囲内でVALIDATED相当の再現性。Reference Digest(施策2)は安全(Fact漏洩0件)・低コスト(約¥0.4/記事)だがEntertainment性への追加効果は`REFERENCE_EFFECT_UNCLEAR`のまま。新Trade-off発見: Main StoryがGap構造を取ると、Point TwoがFull Storyの言い換えになりやすく既存QA(Point Value QA/Point Overlap)でNG_REVIEW_REQUIRED(Human Review行き)になりやすい(Theme2 Aで実際に発生、既存安全装置が正しく機能した事例として記録)。本文修正0件、Reference Fact leakage 0件。Trial全体cost 約$0.6751(約¥107)。**最終Status: USER_DECISION_REQUIRED**。詳細は`OPEN-112-ENGAGEMENT-REFERENCE-CROSS-TOPIC-AB-TRIAL-11_REPORT.md`。

2. **Trial-12(2026-09-05、OPEN-112-TREND-THEME2-B-LEDGER-FIX-AND-A2-B1-TEXT-TRIAL-12)**: script `er011_open112_trend_theme2_b_a2_b1_text_trial_12.py`、出力 `er011_output/open112_trend_theme2_b_a2_b1_text_trial_12/`。F-203(観光庁「海外旅行に関する意識調査」)の対象母集団誤り(旧版「海外旅行経験者限定」)を一次資料(観光庁公式プレスリリースPDF)で「Z世代[19〜25歳]全体400人」に修正し、独立re-verificationでCONFIRMED。1回目実行はOpenAI API残高不足(insufficient_quota)で`BLOCKED`停止、ユーザー入金後の2回目実行でB1/A2テキストを生成し既存QAチェーン(Fact Checker/Ledger Deviation/Point Value QA/Point Overlap)を全通過。Point Overlap Article Retry発火はB1=2回(解消)・A2=1回(解消)。本文修正0件、Reference Fact leakage 0件。累計cost 約$0.4283(約¥68)。**最終Status: TEXT_READY_FOR_USER_REVIEW**。ユーザー決定: A2の1文2数字が2箇所・Point長さ超過(tolerance内)・見出しの若者帰属表現は、Trial音声として許容する。詳細は`OPEN-112-TREND-THEME2-B-LEDGER-FIX-AND-A2-B1-TEXT-TRIAL-12_REPORT.md`。

3. **Trial-13(2026-09-05〜06、OPEN-112-TREND-THEME2-B-A2-B1-FULL-AUDIO-TRIAL-13)**: script `er011_open112_trend_theme2_b_full_audio_trial_13.py`、出力 `er011_output/open112_trend_theme2_b_full_audio_trial_13/`。Sonnet実行2回目・3回目(ユーザーが上限超過を明示承認した例外)の記録。B1はkp3_ja_charon(標準経路2回+fallback1回、計3回不合格。ASR文字起こしは標準2回とも「中央値は約二泊。」、fallback1回は「中央地は約二泊。」で、いずれも表記ゆれ・同音混同を`TRUE_CONTENT_MISMATCH`と誤判定)・kp4_en(Minimal instruction 2回+English language lock 2回、計4回不合格。ASR文字起こしは毎回"Point two."でcanonical"point to"との混同)がHuman Review Lock STOPPED、kp4_ja(canonical gloss「～を示す、～を指し示す」の未発話placeholder記号「～」検出によりTTS呼び出し前に独立してSTOPPED、kp4_en STOPPEDの結果を待って未着手になったものではない)を合わせ**計3件**不合格となり、Audio Validation Gateが`EPISODE_BLOCKED_BY_AUDIO_VALIDATION`でassemblyを正常にブロックした(override等なし)。A2は全24 segment OKだが、Assembly最終段`write_wav_float()`のクリッピング防止assert(peak=1.0350188975890593)で書き出し時にクラッシュ(Trial scriptの`except RuntimeError`では捕捉されない未処理例外)、完成音声なし。3回目でも完全に同一のpeak値・同一Gate結果を再現(決定論的)。TTSはBatch API(Production標準)のみ使用。D4遵守(override・fallback追加・手動unblockなし)。運用上の問題: バックグラウンド待機Agentの自動再開により同一管理IDへ2回目・3回目(ユーザー例外承認)が関与し、「RESOLVED状態は再TTSをブロックしない」という既存Production設計(意図的仕様)により、意図しない重複TTS(8 segment、さらに3回目の再実行時にも6 segment、合計約¥8.03)が発生。Theme2累計cost 約$0.73946(約¥117.57、¥1,000上限比約11.76%)。**最終Status: USER_DECISION_REQUIRED**(B1 Key Phrase 3件の扱い、A2クリッピング原因調査・対応要否、意図しない重複再TTSへの対応)。**2026-09-06訂正(本エントリ、OPEN-112-TREND-THEME2-RECORD-CONSOLIDATION-18)**: 元Reportの§4/§10(kp3_ja_charonのASR文字起こし記載「中央値は約回数泊。」、およびkp4_jaを「(未実行)/kp4_enがSTOPPEDのため生成自体に到達せず」とした記載)は、`b1b/audit/tts_generation_results.json`(実データ)およびOpus診断-15の確認結果と不整合な誤記だったため、Report本体を修正済み(正しくは上記の通り、kp4_jaはkp4_enとは独立のplaceholder検出によるSTOPPED)。詳細は`OPEN-112-TREND-THEME2-B-A2-B1-FULL-AUDIO-TRIAL-13_REPORT.md`(訂正反映済み)。

4. **Opus診断-14(OPEN-112-TREND-THEME2-B-A2-CLIPPING-ASSERT-DIAGNOSTIC-14、ファイルレポートなし)**: A2クリッピングの原因仮説としてPreview経路の無調整(仮説A)を最有力視したが、後続の計測-16の実測により仮説Aは棄却された(要旨のみ本エントリへ記録)。

5. **計測-16(2026-09-06、OPEN-112-TREND-THEME2-B-A2-PEAK-MEASUREMENT-16)**: 出力 `er011_output/open112_trend_theme2_b_a2_peak_measurement_16/`(script `er011_open112_a2_peak_measurement_16.py`)。読み取り専用計測(Production変更0・有償API呼び出し0)。**原因確定(仮説B確定)**: `a2_point_one`segmentがgain適用時にmax_peak=0.95上限へ張り付き、その後`mono_24k_to_stereo_target()`の`resample_poly`(24kHz→48kHz)がゲイン制御を経由しないため内容依存で0.05%〜8.9%のオーバーシュートが発生し、0.95→1.03502へ増加してassertを発火させた。Previewの生peakは常時0.70前後で無関係(仮説A不成立)。過去のA2 assemble完了14本中3本(pool_n4_supermarket/pool_n6_delivery/pool_n5_cafes)が既に0.95上限付近に達しており、「以前から構造的に再発しうる状態だった」ことが判明。`CURRENT_SPEC.md`283行目「Preview/Bodyは無調整のアンカー」という記述と、実装(`apply_a2_gain`がBodyにもgain適用している)の乖離を確認した(仕様記述訂正候補として記録、実装・SPEC変更はしていない)。対策候補2案(Preview経路peak上限追加=実測不適合、最終段の全体ヘッドルーム安全弁+gainログ先行書き出し=実測と整合)を評価のみ行い、実装は一切していない。**Status: 原因確定(対策は未実装、USER_DECISION_REQUIRED)**。詳細は`OPEN-112-TREND-THEME2-B-A2-PEAK-MEASUREMENT-16_REPORT.md`。

6. **Opus診断-15(OPEN-112-TREND-THEME2-B-B1-KEYPHRASE-STOP-DIAGNOSTIC-15、ファイルレポートなし)**: Trial-13 B1のKey Phrase停止3件の原因を診断。真因はTTS側の発話品質ではなく、(JA)`er007_ja_asr_validator_01.py`の数字保護ゲートが助数詞「つ」限定の漢数字正規化しかカバーせず「泊」等を数字消失相当と誤判定、(EN)`er006_preprod_hardening_01_validation._convert_cardinal_words()`がASR側"two"のみ数字変換しcanonical側"to"がstopword除外され既存homophone機構に到達不能、(JA gloss)選定Promptの規約不足により「～」等のplaceholder記号がgloss本文に残存する、という既存Validator/選定Prompt側の仕組み上の問題と特定。量産時の発生頻度は確度低〜中で5〜6記事に1本程度停止しうると見積もり。ユーザー決定: 完全同音(CMU辞書ARPAbet完全一致)はPASSとして扱う方針(Human Review逃がしではなく正式なPASS条件として)。**Status: 原因特定完了、対策候補はTrial-17で検証(USER_DECISION_REQUIRED)**。

7. **Trial-17(2026-09-06、OPEN-112-TREND-THEME2-B-KEYPHRASE-VALIDATOR-FIX-TRIAL-17)**: script `er011_open112_kp_validator_fix_trial_17_ja.py`/`_en.py`/`_en_runner.py`/`_track_c.py`、出力 `er011_output/open112_kp_validator_fix_trial_17/`。恒久対策候補の隔離Trial(Production配線なし)。Track A(JA数字ゲート一般化、閉じた助数詞リスト方式): 対象バグ修正は健全(既存fixture39件中37件無変化+1件無害変化)だが、「日」を含むフルリスト採用時に慣用句「三日坊主」でregression1件(正しくPASSすべきものがFAILへ悪化)が発生、「日」「人」除外の縮小版で解消することも確認。**Status: USER_DECISION_REQUIRED**(リスト範囲の最終選択)。Track B(EN同音判定、Approach2推奨): 既存fixture65件+homophone fixture10件でregressionゼロ、Approach1は複合基数を含む長文でindex参照が構造的に不健全であることを実証。**Status: VALIDATED**(Approach2推奨、Approach1不採用推奨)。Track C(選定Prompt規約、Trial専用コピーPromptのみ・Production Prompt無変更): 規約A(gloss数字→漢数字)・B(「～」等placeholder禁止)は1回の実行(n=1)で遵守を確認、規約C(短い機能語終端KP検出)は検出のみに留まり防止には至らなかった(確率的遵守である旨明記)。**Status: USER_DECISION_REQUIRED**(規約A/B採用可否、規約Cの扱い)。cost 約$0.1185(約¥19)。詳細は`OPEN-112-TREND-THEME2-B-KEYPHRASE-VALIDATOR-FIX-TRIAL-17_REPORT.md`。

8. **Trial-10閲覧(OPEN-112-TREND-TRIAL10-ARTICLE-REVIEW-01)**: Trial-10記事の抽出閲覧のみ、新規Fact Safety論点なし。

**共通確認事項**: 上記いずれもProduction Prompt/routing/QA閾値・Topic Master・SSOT(`CURRENT_SPEC.md`)への変更を伴わない。`APPROVED_FOR_PRODUCTION`・`PRODUCTION_WIRED`到達は0件。新規Open Item OPEN-115(A2/B1 Assemblyヘッドルーム不在)・OPEN-116(ASR Validator数字保護ゲート表記ゆれ誤判定+Key Phrase gloss規約不足)を本エントリと合わせて`OPEN_ITEMS.md`へ新規登録した。詳細は各Reportおよび`OPEN_ITEMS.md` OPEN-112行の2026-09-06追記参照。

---

## PM-GOVERNANCE-LOCAL-FILE-LINK-RULE-07: 試聴・閲覧依頼はfile:// URL形式で提示するルールを9-2へ追加

**日付**: 2026-09-06
**区分**: PM運用ルール(サービス・生成仕様ではない)

**内容**: `docs/pm/PM_GOVERNANCE.md`「9-2. PMとしての説明原則」へ、
ユーザーへ試聴・閲覧を依頼するローカル成果物(音声player・html・音声
ファイル等)は必ず`file:///C:/Users/tensh/eigo-radio/<相対パス>`形式の
URL(Ctrl+クリックで開ける)で提示する箇条書きを追加した。パスのみの記載や
`SendUserFile`等での送付は行わず、sonnet-workerのReport/RESULT_PACKETにも
同形式で記載させる。

**採用理由**: ユーザーのクライアントでは、パス記載のみや`SendUserFile`での
送付では試聴用ファイルが開けないため(2026-09-06ユーザー指示)。

**根拠レポート**: なし(PM運用ルールの文書編集のみ、Trial実行なし)。

**状態**: 文書化完了(PM運用ルールの明文化のみ、Production
コード・Prompt・`CURRENT_SPEC.md`・`OPEN_ITEMS.md`は無変更)。

**commit**: `1094c49`(2026-09-06)。

**影響するCURRENT_SPEC項目**: なし。

---

## PM-GOVERNANCE-ADAPTIVE-REPORTING-06: ユーザー向け報告を固定5構造から候補セクション制(adaptive reporting)へ改訂

**日付**: 2026-09-06
**区分**: PM運用ルール(サービス・生成仕様ではない)

**内容**: `docs/pm/PM_GOVERNANCE.md`「9-1. 報告の基本構造(5構造)」を
「9-1. 候補セクションと選択基準(adaptive reporting)」へ改訂し、5構造
(現在の状態/未解決問題/次にやること/ユーザー判断/補足・技術詳細)を
「毎回全部埋める固定必須構造」から「その回答の目的に応じて必要な項目だけを
選ぶ候補セクション」へ変更した。各セクションの選択基準(現在地・実行状況を
伝える必要がある場合のみ/ユーザーが認識・管理すべき未解決事項がある場合の
み/今後の作業を知らせる意味がある場合のみ/本当に必要な場合のみ/判断に
必要な場合のみ)を明記し、番号対応(未解決問題→次にやること→ユーザー判断)・
推奨案付き原則・`USER_DECISION_REQUIRED`を省略しないことは維持した。
「レス」「報告」で別フォーマットに分けず、1つの共通ルールでFableが必要量を
調整する。9-2の説明原則は維持しつつ、9-1と重複する項目(「まず現在地を
簡潔に示す」「推奨できる場合は推奨案を示す」等)を9-1側へ寄せて整理し、
`USER_DECISION_REQUIRED`を隠さない旨を明文化した項目を追加した(内容は
削除せず統合)。`.claude/agents/sandwich-pm.md`「## 報告」節の9節参照も
候補セクション制(必要な項目だけ出す・番号対応・推奨付き)へ最小限更新した。

**採用理由**: 単純な指示へのレスでも固定5構造の全項目を出すと冗長で
分かりづらいというユーザー判断(2026-09-06)。情報を隠す変更ではなく、
不要なセクション表示を省略して読みやすくするもの。

**比較した選択肢**: 5構造自体を廃止する案、「レス」と「報告」で別
フォーマットを新設する案は採らず、既存5構造を維持したまま「固定必須」を
「候補セクション+選択基準」へ変更し、単一ルールのままFableが分量を調整する
案を採用した。

**却下理由**: 別フォーマット新設案は、`USER_DECISION_REQUIRED`の扱いが
フォーマットによって曖昧になるリスクがあるため採らなかった。
...
**commit**: `6204293`(2026-09-06)。

**影響するCURRENT_SPEC項目**: なし。

---

## PM-GOVERNANCE-REPORT-FORMAT-AND-AUTONOMOUS-GIT-05: ユーザー向け報告フォーマットの改訂(5構造・番号対応)とcommit/push自律実行運用をPM_GOVERNANCEへ反映

**日付**: 2026-09-06
**区分**: PM運用ルール(サービス・生成仕様ではない)

**内容**: `docs/pm/PM_GOVERNANCE.md`「9. ユーザー向け説明スタイル」を
「9. ユーザー向け報告フォーマットとPMとしての説明原則」へ改訂し、旧
「推奨フォーマット」(結論/重要ポイント/問題/ユーザー判断/技術詳細)を、
番号対応する5構造(現在の状態/未解決問題/次にやること/ユーザー判断/
補足・技術詳細)へ置き換えた。未解決問題→次にやること→ユーザー判断は
同じ番号で対応させ、ユーザー判断には原則としてFable/PMの推奨案と推奨理由を
必ず添える(ユーザー本人の主観評価そのものが必要な場合は推奨不要)。
従来の説明原則15項目のうち5構造と矛盾しないもの(1段落1論点・raw report
転記禁止・UNKNOWN/UNVERIFIED/推定の区別・安全≠成功・簡潔要求時の圧縮・
追加詳細要求時のみ展開・「要するに」・良い例優先等)は「PMとしての説明
原則」として維持した。あわせて「10. commit / push運用」を新設し、通常の
安全なcommit/pushは原則としてClaude側(Fable→sonnet-worker)が自律実行し
毎回ユーザーへ確認しない運用へ変更した。ただし、commit/pushによって
ユーザー判断結果が変わりうる・Production採用判断を含む・rollback等の
破壊的操作・merge conflict等で複数の意味ある選択肢がある・意図しない
ファイル混入の可能性がある等、Claude側だけで安全に確定できない場合のみ
ユーザー判断を求める。1節の責任分担を、ユーザー=「破壊的操作・
Production採用判断を含むcommit/push等10節が定める例外時の判断」、
Fable/sonnet-worker=「10節が定める通常の安全なcommit/pushは自律実行」へ
整合させた。既存8節の「ファイル名指定の`git add`のみ、`-A`禁止」・
`CLAUDE.md`「Git運用ルール」の履歴書き換え等(amend・rebase・force push)は
必ずユーザーに確認する原則は無変更のまま維持した。

**採用理由**: Trial-11〜13期間の報告が長く判断しにくかったこと、commit
確認の往復がボトルネックだったことを受けたユーザー方針確定(2026-09-06)。

**比較した選択肢**: 旧「結論先出し+重要点3〜5点」形式を維持する案は
採らず、未解決問題と対応(次にやること・ユーザー判断)を番号で明示的に
紐づける5構造へ全面置き換える案を採用した。commit/push運用は、従来の
「毎回ユーザー確認」を維持する案、全面自律化する案の中間として、
破壊的操作・Production採用判断等の例外条件のみユーザー判断を残す案を
採用した。

**状態**: 文書化完了(PM運用ルールの明文化のみ、Production
コード・Prompt・`CURRENT_SPEC.md`・`OPEN_ITEMS.md`は無変更)。

**commit**: `7459d25`(2026-09-06)。

**影響するCURRENT_SPEC項目**: なし。

---

## PM-GOVERNANCE-AGENT-WAIT-AND-DUPLICATE-LAUNCH-RULE-04: 待機中Agentの再開リスクと同一管理IDへの多重起動禁止をPM_GOVERNANCE 8節へ追記

**日付**: 2026-09-06
**区分**: PM運用ルール(サービス・生成仕様ではない)

**内容**: `docs/pm/PM_GOVERNANCE.md`「8. Agent並列起動の原則」へ、
「待機中Agentの再開リスクと同一管理IDへの多重起動禁止」を追記した。
バックグラウンド待機(sleep・ポーリング・「通知を待つ」)を仕掛けたAgentは
セッションが終了したように見えても後から自動再開しうるため、Fableはこれを
「終了済み」とみなさないこと、同一管理ID・同一出力先へ追加のAgentを起動
しないこと(既存Agentの終了[担当プロセスの終了と最終報告の受領]を確認する
前に代替Agentを投入しないこと)、Agentの待機は前面同期(foregroundの
`sleep`等、結果を待ってから次へ進む形)を原則としバックグラウンド待機で
自セッションを終了しないこと(Fableは委任文でこれを明示する)を明記した。
あわせて、これは「ファイル・出力先が重ならない独立タスクの並列」を
制限するものではなく、禁止されるのは同一タスク(同一管理ID・同一出力先)への
多重関与であることを明確化した。既存の8節の箇条書き・独立タスク並列可の
方針(ユーザー決定(b))は削除・改変していない。

**採用理由**: 2026-09-05〜06、Trial-13(OPEN-112-TREND-THEME2-B-A2-B1-
FULL-AUDIO-TRIAL-13)でsonnet-workerがバックグラウンド待機(sleep/ポーリング)を
仕掛けてセッションを終えたため、Fableが「終了済み」と誤認し、同一管理ID・
同一出力先へ3回目・4回目のAgentを追加起動した。その後、2回目・3回目の
Agentが自動再開して作業を続け、同一管理IDに複数Agentが関与し、一時ファイル
(`ACTIVE_TASK.md`/`RESULT_PACKET.md`)・Reportの上書き競合と、8 segmentの
重複TTS(約¥4)が発生したことを受けたユーザー指示(2026-09-06)。

**比較した選択肢**: 並列起動を全面禁止に戻す案は採らず、独立タスク
(ファイル・出力先が重ならない)であればFable判断で並列起動可という
既存方針(PM-GOVERNANCE-PARALLEL-AGENT-POLICY-02、ユーザー決定(b))は維持し、
同一タスクへの多重関与の禁止だけを明確化・強化する案を採用した。

**状態**: 文書化完了(PM運用ルールの明文化のみ、Production
コード・Prompt・`CURRENT_SPEC.md`・`OPEN_ITEMS.md`は無変更)。

**commit**: `26833ab`(2026-09-06)。

**影響するCURRENT_SPEC項目**: なし。

---

## PM-GOVERNANCE-USER-FACING-EXPLANATION-STYLE-03: ユーザー向け説明スタイル原則をPM_GOVERNANCEへ明文化

**日付**: 2026-09-05
**区分**: PM運用ルール(サービス・生成仕様ではない)

**内容**: Fableがユーザーへ報告・説明する際、Sonnetの技術レポートをそのまま
転記せず、証拠を失わずに「ユーザーが判断しやすい言葉」へ翻訳することを
`docs/pm/PM_GOVERNANCE.md`「9. ユーザー向け説明スタイル(USER-FACING
EXPLANATION STYLE)」として新設した。結論先出し・重要点3〜5点への絞り込み・
専門用語や管理IDの必要時のみの使用・1段落1論点・問題説明時の
「問題→影響→対応」順・`USER_DECISION_REQUIRED`時の判断事項明示・選択肢
提示時の違いと推奨案・技術詳細の末尾分離・「安全≠成功」の説明・
UNKNOWN/UNVERIFIED/推定の区別・簡潔要求時の圧縮・追加詳細要求時の展開・
「要するに」の付記・悪い例より良い例の優先、という原則15項目と推奨
フォーマットを明文化した。あわせて、Sonnet/OpusからFableへの報告
(`RESULT_PACKET.md`・ER/OPEN Report)は従来どおり証跡・原文を省略せず
詳細に記録すること(証跡はSSOT/Report側に残し、ユーザー向け説明で圧縮する)
を明記し、`.claude/agents/sandwich-pm.md`の「報告」節へ本原則への参照を
1行追加した。

**採用理由**: 2026-09-05、Trial-11〜13の一連の報告でSonnetの技術レポートに
近い長文報告が続き、ユーザーの判断負荷が高かったことを受けたユーザー指示。

**状態**: 文書化完了(PM運用ルールの明文化のみ、Production
コード・Prompt・`CURRENT_SPEC.md`・`OPEN_ITEMS.md`は無変更)。

**commit**: `c888f51`(2026-09-05)。

**影響するCURRENT_SPEC項目**: なし。

---

## PM-GOVERNANCE-PARALLEL-AGENT-POLICY-02: Agent並列起動の原則をPM_GOVERNANCEへ明文化(ユーザー決定(b))

**日付**: 2026-09-05
**区分**: PM運用ルール(サービス・生成仕様ではない)

**内容**: sonnet-workerの並列起動ルールを、従来の「ユーザーの指示なしに複数
Agentを並列起動しない」から、「対象ファイル・出力先が重ならない独立タスク
であれば、Fableの判断で並列起動可(ただし一時ファイル`docs/pm/ACTIVE_TASK.md`・
`docs/pm/RESULT_PACKET.md`の衝突回避を必須)」へ緩和し、
`docs/pm/PM_GOVERNANCE.md`「8. Agent並列起動の原則」として新設した。
`docs/pm/PM_BRIEF.md`「禁止事項」・`.claude/agents/sandwich-pm.md`の
対応記述もPM_GOVERNANCE 8節を参照する条件付き許可の文言へ更新した。
Agent Teams不使用・Opus診断1回上限・Sonnet委任上限(管理IDごと合計2回)は
変更していない。

**採用理由**: 2026-09-05、Trial-13(OPEN-112-TREND-THEME2-B-A2-B1-
FULL-AUDIO-TRIAL-13)が稼働中に、PM_GOVERNANCE明文化・Git反映を並行実施する
実運用が生じ、衝突回避の要件(一時ファイル・出力先・Git stage分離)が明確に
なったことを受け、ユーザーが選択肢(b)を決定した。

**比較した選択肢**: (a) 現状維持(ユーザー指示なしに複数Agent並列起動しない)
/ (b) 独立タスク[ファイル・出力先が重ならない]であればFable判断で並列起動可
[採用] / (c) 並列起動を完全禁止する明文化。

**状態**: 文書化完了(PM運用ルールの明文化のみ、Production
コード・Prompt・`CURRENT_SPEC.md`・`OPEN_ITEMS.md`は無変更)。

**commit**: `ec4a307`(2026-09-05)。

**影響するCURRENT_SPEC項目**: なし。

## PM-GOVERNANCE-TTS-MODE-CONFIRMATION-01: TTS方式(Batch/Standard)の明示・確認原則をPM_GOVERNANCEへ明文化

**日付**: 2026-09-05
**区分**: PM運用ルール(サービス・生成仕様ではない)

**内容**: TTS生成を含むタスク定義(`docs/pm/ACTIVE_TASK.md`・Sonnet委任文)に
使用するTTS方式(Batch API / Standard同期)を必ず明記すること、明記が
無い場合はFableが着手前にユーザーへ確認すること、方式の切り替え・混在は
ユーザー明示承認時のみでProduction忠実性・所要時間・コスト差を必ず記録
すること、着手前に方式に応じた所要時間・コスト見込みを提示することを
`docs/pm/PM_GOVERNANCE.md`「7. TTS方式(Batch API / Standard同期)の明示・
確認原則」として新設した。

**採用理由・根拠**: `CURRENT_SPEC.md`ではGemini TTSの実装方式として
Batch API(`client.batches.create()`)がProduction標準として既に採用・
配線済み(ER-006-TTS-BATCH-WIRING-SOT-CLEANUP-01、2026-08-22)。Batchは
料金約50%減だが1件あたり実測91〜167秒(retry分積み上がる)と遅く、
Standard同期は数秒だが割引なし。2026-09-05のTrial-13
(OPEN-112-TREND-THEME2-B-A2-B1-FULL-AUDIO-TRIAL-13、Theme 2 B A2/B1
音声化)で、タスク定義にTTS方式の明記がなく所要時間の見込みが
ユーザーへ事前共有されなかったことを受け、ユーザーが本原則の正式化を
指示した。

**状態**: 文書化完了(PM運用ルールの明文化のみ、Production
コード・Prompt・`CURRENT_SPEC.md`・`OPEN_ITEMS.md`は無変更)。

**commit**: `570097f`(PM-GOVERNANCE-TTS-MODE-CONFIRMATION-01-GIT、2026-09-05)。

**影響するCURRENT_SPEC項目**: なし(`CURRENT_SPEC.md`「Gemini TTS実装方式
(Batch API)」は正本のまま参照のみ、内容変更なし)。

## PM-HANDOFF-CHATGPT-001-CLOSEOUT-SSOT-01: ChatGPT旧PM引き継ぎHandoffの正式closeout・PM運用原則4点のSSOT明文化

**日付**: 2026-09-05
**区分**: PM運用ルール(サービス・生成仕様ではない)

**内容**:
1. ChatGPT旧PM→Fable(sandwich-pm)へのPM移管Handoffについて、
   PM-HANDOFF-CHATGPT-001の照合結果(CONFLICT 0件、資料が「未確認」としていた
   2点の解消、CHATGPT_ONLY_CANDIDATE 7件、APPROVED未配線1件[OPEN-83関連]の
   新規発見)をユーザーが`HANDOFF_VALIDATED`として受け入れた(User Decision A)。
   照合記録は`docs/pm/PM-HANDOFF-CHATGPT-001_REPORT.md`(非SSOT、Handoff証跡、
   本タスク時点では未commit)。
2. PM運用原則4点(PM Gate 1〜7・PM Closeout Mandatory Check・1記事ずつ完結・
   安全≠成功)を、新設`docs/pm/PM_GOVERNANCE.md`を正本として正式明文化した
   (User Decision B)。`CLAUDE.md`・`docs/pm/PM_BRIEF.md`には短い参照のみ追加し、
   全文は複製していない。`CURRENT_SPEC.md`へはGate全文を追加していない。
   `docs/pm/PM_GOVERNANCE.md`には上記4点に加え、「責任分担
   (Fable/Sonnet/Opus/ユーザー)」「deferの成立条件」も明文化している。
   「1記事ずつ完結」原則には、次の5条件を**すべて**満たす場合に次工程へ
   進める例外を明記した(2026-09-05ユーザー訂正): (1)ユーザーが当該論点を
   明示的にdeferを承認したこと、(2)non-blockingであることを確認したこと、
   (3)`OPEN_ITEMS.md`で追跡継続していること、(4)`APPROVED_FOR_PRODUCTION`の
   取消しではないこと、(5)後続作業が未配線項目を誤って完了扱いしないこと。
3. 明示defer 5件(User Decision C): Trend overclaim severity方針変更 /
   News・Trend向けPoint Overlap閾値(0.40)見直し / Reference Digest追加検証・
   採否 / Point長さ目安(25〜70語)見直し / OPEN-83発音自動照合の未配線実装。
   **OPEN-83については、ER-008-N8-HUMAN-APPROVAL-AND-PROPER-NOUN-
   PRONUNCIATION-SPEC-16で`APPROVED_FOR_PRODUCTION`となった判定基準変更が
   引き続き`PRODUCTION_WIRED`未達のまま維持され、今回のdeferは承認取消ではなく
   APPROVED未配線項目として追跡継続することを明記した。**
4. 今後個別にユーザー判断する重要3点(defer扱いにしない): Discovery 4-layer
   Focus ModuleのProduction採用可否 / Engagement根底指示の正式Production採用
   可否 / News Ledger作成を既存自動Research経路へ寄せるか。いずれも今回は
   採否判断・実装ともに実施していない。
5. `OPEN_ITEMS.md`のdangling link(OPEN-112行が参照していた実在しないファイル
   `OPEN-112-DISCOVERY-4LAYER-ADOPTION-READINESS-07_REPORT.md`)を、実在する
   DECISION_LOG.mdエントリ(OPEN-112-DISCOVERY-4LAYER-FINAL-ADOPTION-READINESS-
   AND-OPEN114-REGISTER-07、commit 141084d)への参照へ修正した。
6. 本タスクでProductコード・Prompt・runtime・Trial script・Production script
   は無変更。`APPROVED_FOR_PRODUCTION`/`PRODUCTION_WIRED`の新規付与・変更なし。
7. **2026-09-05ユーザー訂正により、本タスクはgit add/commit/pushを実施せず
   文書修正のみでSTOPした**(`docs/pm/ACTIVE_TASK.md`・`docs/pm/RESULT_PACKET.md`は
   一時ファイルとして引き続きgit追跡・commit対象外)。Handoff証跡3ファイル
   (`docs/pm/ACTIVE_TASK.md`・`docs/pm/RESULT_PACKET.md`・
   `docs/pm/PM-HANDOFF-CHATGPT-001_REPORT.md`)は、ユーザー訂正前に
   commit `aecd776`(未push)として作成済みであり、`docs/pm/ACTIVE_TASK.md`・
   `docs/pm/RESULT_PACKET.md`は`.gitignore`の除外設定に反し`git add -f`で
   強制追加されていた。この扱いはユーザー訂正(git add -f禁止)と矛盾するため、
   本commitの取り扱い(維持/取り消し)についてユーザー・Fableの判断を要する
   (`USER_DECISION_REQUIRED`)。
   **2026-09-05追記(PM-HANDOFF-CLOSEOUT-GIT-02)**: ユーザー判断により
   `aecd776`は`git reset --mixed HEAD~1`で取り消し(未pushのためorigin
   影響なし)。`docs/pm/ACTIVE_TASK.md`・`docs/pm/RESULT_PACKET.md`はGit
   履歴へ残さず`.gitignore`対象のまま。Handoff証跡は
   `docs/pm/PM-HANDOFF-CHATGPT-001_REPORT.md`のみを本GIT-02のcloseout
   commitで正式反映。

**Handoff Status**: `HANDOFF_CLOSED`(SSOT明文化・defer記録・dangling link修正の
範囲において。ただし上記7のcommit `aecd776`取り扱いは`USER_DECISION_REQUIRED`)。

**根拠レポート**: `docs/pm/PM-HANDOFF-CHATGPT-001_REPORT.md`、
`docs/pm/PM_GOVERNANCE.md`

**commit**: 本タスクではgit操作を実施していない(ユーザー訂正によりSTOP)。
先行するcommit `aecd776`(PM-HANDOFF-CHATGPT-001証跡3ファイル)は
本タスク開始直後・ユーザー訂正受領前に作成済み(未push)。

**影響するCURRENT_SPEC項目**: なし(本タスクはPM運用ルールのみ、
CURRENT_SPEC.mdは無変更)。

---

## OPEN-112-TREND-ENGAGEMENT-REFERENCE-AB-TRIAL-10: Entertainment根底指示+Reference DigestのA/B比較Trial(イラン/ホルムズ海峡)

**日付**: 2026-09-05
**区分**: サービス・生成仕様のTrial検証(既存Production Writer/QA関数は無変更のまま呼び出し、実際に記事を生成して測定)

**内容**: Trial-09で確認された「Trend Synthesisは構造としては成立するが、Main Storyが時系列の
出来事列挙に寄り、記事としての興味・意外性・エンターテインメント性が弱い」という問題に対し、
(1) Writer根底指示へ「面白さ・意外性・エンターテインメント性」原則を明示追加する施策と、
(2) Fact Ledgerとは別に、実際に収集した高品質記事4本から構成・切り口のみを抽出したReference Digest
(Fact source使用禁止)をWriterへ渡す施策を、同一テーマ・同一Ledger・B1B×1ずつのA/B比較で検証した。
A=施策1のみ、B=施策1+施策2。詳細は
[OPEN-112-TREND-ENGAGEMENT-REFERENCE-AB-TRIAL-10_REPORT.md](OPEN-112-TREND-ENGAGEMENT-REFERENCE-AB-TRIAL-10_REPORT.md)、
生データは`er011_output/open112_trend_engagement_reference_ab_trial_10/`。

**設計結論の要点**:
- 施策1(Entertainment/Storytelling原則)だけで、両パターンとも「A happened. Then B happened」型の
  時系列列挙から、反転・対比を軸にした構成へ明確に改善した。Point Overlap QA(固定閾値0.40)は
  両パターンとも0.08〜0.17と大きく余裕を持ってPASSし、Point Value QAも両パターンとも初回生成で
  全項目PASS(Trial-09で問題になった、News/Trend記事特有の共有語彙によるPoint Overlap閾値接近は
  今回再現しなかった)。
- 施策2(Reference Digest)は、安全性(Reference由来の具体的Fact混入0件)とコスト
  (既存pricing snapshotで算出、約¥0.43/記事、テーマ単位で使い回せばさらに低下)の両面で
  問題は見られなかったが、今回1回ずつの比較ではA(施策1のみ)との明確な品質差は確認できなかった。
- Trend overclaimはBで0件(`LEDGER_COMPLIANT`、deviations 0件。Trial-09の同種条件では4件のMINOR)。
  AはFact Checkerが前段でFAILしたためLedger Deviation Checkerに未到達、overclaim測定不能。
- AのFact Checker FAILは、Writerの創作ではなく、Trial-09由来のLedgerに元から存在した
  3件目の精度誤り(6月14日の停戦覚書「合意」と、6月17日の正式MOU署名日の混同)が原因で、
  本Trial自身が実施した事前Ledger Verificationでも発見できず、既存Fact Checkerの独立Web検索で
  初めて発覚した(Trial-09で発見した2件[日付照合ミス・未確認個数]の修正後もなお、3件目の
  人手Ledger精度誤りが現実化したことになる)。

**採用理由**: タスク方針「できるだけ軽い方法で改善可能かを検証する」に従い、新Validator・新Fact
Checker・Overlap閾値変更を先に行わず、既存Production Writer/QAチェーン(`gen.run_one_pattern`、
無変更)をそのまま経由させて実際の記事を生成し、施策1単体の効果と施策2の追加効果を分離して
実測することを優先した。Reference Digest生成は新規routing processを追加せず、既存の承認済み
`B1_WRITER` contractを再利用した。

**比較した選択肢**: (1) 施策2(Reference Digest)についてAとの品質差が明確でなかったことを理由に
今回この場で不採用と判断する案は、1回ずつの比較のみでの結論であり判断材料として不十分なため
見送り、USER_DECISION_REQUIRED候補として報告するに留めた。(2) 3件目のLedger精度誤りを本Trial内で
独断で修正し再実行する案は、Loop Budgetの趣旨(生成回数を無闇に増やさない)およびタスクの
「今回禁止」に照らし、原因を特定・報告するに留め、再実行は行わなかった。

**却下理由**: 上記の通り、いずれも「今回は変更・追加実行せず、まず実測結果を報告する」という方針、
およびタスク自体の禁止事項(Production Prompt/code変更・新Validator追加・Overlap閾値変更)により、
実装・追加実行は行っていない。

**根拠レポート**: [OPEN-112-TREND-ENGAGEMENT-REFERENCE-AB-TRIAL-10_REPORT.md](OPEN-112-TREND-ENGAGEMENT-REFERENCE-AB-TRIAL-10_REPORT.md)

**commit**: 本エントリと同時にcommit・push。

**影響するCURRENT_SPEC項目**: なし(Trial専用スクリプト・Trial専用出力ディレクトリのみ、Production
Prompt/コード/Topic Masterへの変更は無い)。

---

## OPEN-112-TREND-SYNTHESIS-MINIMAL-PROMPT-TRIAL-09: Trend Synthesis最小限Focus Module Promptの実記事Trial(イラン/ホルムズ海峡)

**日付**: 2026-09-05
**区分**: サービス・生成仕様のTrial検証(既存Production Writer/QA関数は無変更のまま呼び出し、実際に記事を生成して測定)

**内容**: [OPEN-112-NEWS-MODE-DESIGN-08_REPORT.md](OPEN-112-NEWS-MODE-DESIGN-08_REPORT.md)で設計した
Trend Synthesis Focus Module(最小限Prompt)を実装し、実在テーマ(米国・イラン、ホルムズ海峡の
長期膠着、2026年9月に実際にWebSearch/WebFetchで新規収集)でA2×2・B1B×1の記事Trialを実施した。
詳細は[OPEN-112-TREND-SYNTHESIS-MINIMAL-PROMPT-TRIAL-09_REPORT.md](OPEN-112-TREND-SYNTHESIS-MINIMAL-PROMPT-TRIAL-09_REPORT.md)、
生データは`er011_output/open112_trend_synthesis_minimal_prompt_trial_09/`。

**設計結論の要点**:
- Trend Gate(6条件、機械的閾値ではなく定性Gate)は全てPASSし`TREND_READY`。Mode判定2問テストも
  `TREND_SYNTHESIS`を再確認し、生成後の記事内容もこの判定を裏付けた。
- 既存Topic Master(7月分2件)との重複チェックを実施し、状態変化・新しい強いSignal・十分な期間経過の
  いずれも満たすため重複ではないと判定。
- 懸念1(Point One=Signal A・Point Two=Signal Bの単純列挙): A2初回生成で実際に発生したが、
  既存Point Value QAが`qa_not_full_story_paraphrase`/`qa_adds_new_value`で正しく検知し、
  Diagnostic Full Retryで解消した。
- 懸念2(trend overclaim、複数Signal統合の因果・確信度誇張): A2の追加生成でPoint Two内に
  MINOR deviation 4件として実際に発生したが、専用カテゴリの無い既存Ledger Deviation Checkerの
  `changed_causality`/`changed_certainty`/`unsupported_new_claim`タグが全件検知した。
- 新発見1(Point Overlap lexical閾値0.40の問題): 意味的にPASSする内容でも、News/Trend記事が
  共有せざるを得ない固有名詞・事実語彙(mines/gas/capacity/navy/talks等)により機械的にNGとなる
  事例が発生。Point-only regenerationは既存仕様(ER-008-N8-FINAL-QA-HARDENING-21)により無効化
  されているため、記事全体のDiagnostic Full Retry上限到達に直結した。再現性確認の追加生成では
  1回のretryで解消し、構造的な行き詰まりではなく閾値付近の不安定さと判定。
- 新発見2(Trend Ledger作成プロセスの精度リスク): 本Trialでは既存の自動Researcherパイプライン
  (`er003_v1_en_direct_vfl_01_generate.py`)を使わず人手でWebSearch結果をLedger化したところ、
  日付の曜日照合ミス・未確認の具体的個数の混入という精度誤りが2件現実化した。いずれもFact Checkerの
  独立Web検索が正しく検知し、記事がFAILのまま黙って通過することはなかった。

**採用理由**: タスク方針「なるべく軽く」に従い、新Validator・新しい複雑なQAを先に作らず、既存Production
Writer/QAチェーン(`gen.run_one_pattern`、無変更)をそのまま経由させて実際の記事を生成し、影響を
実測することを優先した。

**比較した選択肢**: (1) Trend overclaim対策として専用の新Validator/新Ledger deviationカテゴリを
先に実装する案は、今回のTrial結果(既存カテゴリで実際に検知できた)を踏まえ不採用。(2) Point Overlap
lexical閾値を今回のTrial内で独断で変更する案は、タスクの「今回禁止」(新Validator追加・既存QA変更を
含意しうる変更)に抵触しうるため見送り、USER_DECISION_REQUIRED候補として報告するに留めた。

**却下理由**: 上記の通り、いずれも「今回は変更せず、まず実測する」という方針、およびタスク自体の
禁止事項(Production Prompt/code変更・新Validator追加)により、実装は行っていない。

**根拠レポート**: [OPEN-112-TREND-SYNTHESIS-MINIMAL-PROMPT-TRIAL-09_REPORT.md](OPEN-112-TREND-SYNTHESIS-MINIMAL-PROMPT-TRIAL-09_REPORT.md)
...
## OPEN-112-NEWS-MODE-DESIGN-08: News系(A Family) Major/Daily News・Trend Synthesis 4層構造設計

**日付**: 2026-09-05
**区分**: サービス・生成仕様の設計整理(read-only設計、コード実装・Prompt正式文言確定は今回行わない)

**内容**:
News系(A Family)をMajor/Daily NewsとTrend Synthesisの2モードに整理し、
Discovery/Whyと同じ4層構造(Layer1 Common Writing Contract/Layer2 A Family
Common Skeleton/Layer3 Focus Module/Layer4 Article-specific Inputs)で
成立するかを設計した。コード変更・Production実装・Prompt正式文言確定・
Trial実行・記事生成はいずれも行っていない。詳細は
[OPEN-112-NEWS-MODE-DESIGN-08_REPORT.md](OPEN-112-NEWS-MODE-DESIGN-08_REPORT.md)
参照。

**設計結論の要点**:
- Layer 1/2はDiscovery/Whyと完全共有、変更なし。
- Layer 3は「News Focus Module」という単一Moduleではなく、Major/Daily
  variantとTrend Synthesis variantの2つの相互排他Moduleとして設計
  (Discovery Moduleと同じAnchor 1箇所への単一挿入方式を踏襲)。
- Major/Daily・Trend SynthesisいずれもMain Story/Point One/Point Two/In
  One Lineの既存4slotで成立し、Point One/Twoの役割は固定せず既存Point
  Role Planning機構で記事ごとに動的選択する設計とした。
- Mode判定基準として「単一起点質問」「集約質問」の2問を設計し、
  「重要度が高い」「ソースが複数」という誤った判定基準を明示的に排除。
  阪神・イラン情勢等の具体例で境界を確認済み(境界例は上記Reportに掲載)。
- Trend Synthesis固有ルールとして、Point Twoにcounter-signal/limitation
  を既定候補として優先検討する規則をLayer 3側に設計(Layer 1の一般原則
  を肥大化させない設計判断)。
- 既存Fact Safety(Ledger Deviation Checker)のスコープ外にある
  「trend overclaim」「source strength混同」への対応が未解決のGapとして
  残ることを明示。これはOPEN-112-07で既に指摘済みの既知Gapと同系統だが、
  Trend Synthesisではより強く顕在化しやすいと評価。
- STOP条件(4slotに載らない/Common Skeletonとの重大矛盾/1 Moduleへの
  統合困難/Mode判定が曖昧すぎる/Personalizationとの矛盾/Fact Safetyが
  危険すぎる)はいずれも発生せず、`NEWS_MODE_DESIGN_READY`と判定した。

**採用理由**: 既存のDiscovery/Why Layer 3実装パターン(単一Anchor挿入・
既存Point Role Planningの流用)がNews系にもそのまま適用できることを
確認し、無理な新規Writer構築を避けられる設計に到達したため。

**比較した選択肢**: (a)Major/DailyとTrend Synthesisを1つのModuleに
統合する案 → 責務が複雑化しSTOP条件に抵触するため不採用。(b)Point One/
Twoの役割をMode別に固定する案 → Ledger内容と噛み合わない記事で機械的に
低評価されるリスクがあるため不採用、既存Point Role Planningの動的選択を
採用。

**却下理由**: 上記(a)(b)参照。

**根拠レポート**: [OPEN-112-NEWS-MODE-DESIGN-08_REPORT.md](OPEN-112-NEWS-MODE-DESIGN-08_REPORT.md)

**commit**: (本エントリと同時にcommit予定)

**影響するCURRENT_SPEC項目**: なし(今回はCURRENT_SPEC.mdを変更していない。
4層仕様・News Focus Moduleは正式採用されていないため)。
...
**日付**: 2026-09-05
**区分**: サービス・生成仕様(Local Rewriteが生成する本文そのものの安全性・非重複性に関わる)

**内容**:
OPEN-113 Trial-03(`er011_open113_point_context_only_trial_03.py`)で
VALIDATEDとなった「Local Rewrite発火時、対象文が属するPoint全体を
Rewriteモデルへ参考contextとして渡すだけ(Rule文言・System Prompt・
Retry回数・Ledger再チェック方法は一切変更しない)」という方式を、
ユーザーが正式に`APPROVED_FOR_PRODUCTION`と決定し、Production正式
経路(`er010_ledger_local_rewrite_09.py`・`er003_v1_n3_01_articles_
generate.py::run_one_pattern()`)へ配線した。

**実装**:
- `er010_ledger_local_rewrite_09.py`へ`extract_point_context()`を追加。
  見出し行(`#`)で記事をsectionへ区切る汎用実装で、Main Story/Point One/
  Point Two/In One Lineのいずれも同一ロジックで扱う(Point One/Twoに
  限定した特別扱いはしない、既存Local Rewrite適用範囲全体と整合)。
- `REWRITE_ATTEMPT{1,2,3}_TEMPLATE`へ、Trial-03で検証済みの
  `POINT_CONTEXT_BLOCK`(ラベル付き・指示文なしの参考情報ブロック)を
  追加。既存のREWRITE_SYSTEM_PROMPT・各attemptの指示文言は一字一句
  変更していない。
- `rewrite_ng_item()`の引数へ`point_context`を追加(呼び出し元が
  `extract_point_context()`で取得し、取得失敗時のみ旧来の前後1文へ
  fallback)。DELETE機能・新Validator・新Rule文はいずれも追加していない。

**Regression(実Production関数を直接使用、Trial専用実装ではない)**:
`er011_open113_production_wiring_regression_04.py`で、Trial-01〜03と
同一の3fixture/4項目(No.18 B1既知重複ケース、No.9 tip_screens A2
2件、No.18 notifications A2 1件)を、実際の`local_rewrite.rewrite_ng_
item`/`extract_point_context`/`apply_rewrites`を直接呼び出して再検証。
4/4項目がattempt1で意味重複なく解決し、`point_context_found=true`
(実際にPoint全文が渡されたこと)を記録した。全文再判定で2件に一時的な
MAJORが出たが、記事・Rewrite文を一切変えない追加の全文再判定1回で
再現せず(`LEDGER_COMPLIANT`に復帰)、Rewrite対象文とは無関係な既存
Ledger Deviation Checkerの判定揺れ(下記Open Item候補参照)による
ものと確認した(disambiguation目的の追加API call、Rewriteの追加なし)。

**No.18 B1の実Production再生成**:
`er006_pool_pilot_01_writer.py::run_writer_for_theme`が内部で呼ぶ
`gen.run_one_pattern`と同一の呼び出し経路(B1_B_DIRECT_INSTRUCTION・
build_common_block・build_prompt、4層Prompt[`er011_open112_a_family_
4layer_prompt_trial_05.py`]は不使用)で、現行承認済みの
`pool_n18_notifications_specfix_v2`のTopic/Verified Fact Ledgerを
そのまま再利用してNo.18 B1のみを再生成した(出力先は新規theme_id、
既存承認済みartifactは上書きしていない)。結果はLedger COMPLIANT
(MINOR2件のみ)・Local Rewrite自体は不発火(新規MAJORが発生しな
かったため)。既知の重複問題はそもそも4層Prompt Trialの出力でのみ
観測されたものであり、現行Production系譜(通常のB1_B_DIRECT_
INSTRUCTION経路)には元々存在しなかったことを示唆する結果である。
これは「runtimeでPoint-context機能が実発火したことの直接証拠」には
ならないため、上記の実Production関数によるcontrolled runtime
fixture(Regression実行)を、Local Rewrite実発火・Point context実渡し・
重複解消・Ledger compliantの正式なruntime evidenceとして扱う。

**A2への影響**:
A2は再生成していない。コード変更は、Local Rewriteループの
`major_items`が空でない場合のみ実行される分岐内に限定されており、
Ledger Deviationが検出されない(=大多数の)通常生成では新規コードは
一切実行されないことをコード構造上確認した。加えて、A2水準のfixture
(No.9 tip_screens A2 2件・No.18 notifications A2 1件)もRegressionで
PASSしている。既存承認済みA2 artifactはいずれも無変更のまま。

**コスト・API call**:
Local Rewrite呼び出し回数の計算式(1attemptあたりgenerate 1回+check
1回)は無変更、追加API callはなし。Point contextブロックによる
prompt token増加は、実測(4fixture)で1回あたり平均約75語(約100
token)であり、Verified Fact Ledger本文に比べ軽微。

**テスト**:
`er010_n9_production_integration_09_test_01.py`に新規テスト5件
(`ExtractPointContextTests`4件・`test_point_context_included_in_
user_prompt_without_changing_system_prompt`)を追加し、既存28件と
合わせ33件全PASS。プロジェクト全体回帰(`run_project_regression.py`)
は2075件中2072 PASS。残り3件(`er003_test_bad.FixtureTests.test_
case_0`の意図的fixture、`er003_test_p2j_investigate.py`の
...
count_matches_er003_at_p2i_era`というOPEN-77既知のmeta-test集計
バグ)は、本タスクの変更をgit stashで一時的に外した状態でも同一の
アサーション不一致(例: `1037 != 1032`)で失敗することを確認し、
本タスクと無関係な既存failureであると確認した。

**関連Open Item候補(未登録、今回も実装・登録せず)**:
Trial-01〜03に続き4セッション目として、Ledger Deviation Checkerの
全文判定に、Rewrite対象文とは無関係な既存section(今回はNo.18 B1の
「In one line」結論文や、No.18 A2の見出し「The habit begins before
the first message」)がrun-to-runで検出なし/MINOR/MAJORと揺れる
現象を再度観測した。ユーザーが正式に指示している通り、この論点
(仮ID: OPEN-114)は今回も修正・登録のいずれも行わず、候補として
報告のみ行う。

**状態**: `APPROVED_FOR_PRODUCTION`→`PRODUCTION_WIRED`
**採用理由**: Trial-03が現行Prompt文言・API call数を一切変更せず
4/4件VALIDATEDとなったこと、実Production関数による直接Regressionで
同じ結果が再現したこと、A2への副作用がコード構造上・fixture両面で
確認されなかったこと。
**比較した選択肢**: OPEN-113 Trial-01(Prompt契約変更+DELETE追加、
過剰DELETEでUSER_DECISION_REQUIRED)、Trial-02(階層契約、REJECTED)。
**根拠レポート**: `OPEN-113-POINT-CONTEXT-ONLY-TRIAL-AND-POSTCHECK-
INTEGRATION-AUDIT-03_REPORT.md`(Trial-03)、生データ:
`er011_output/open113_production_wiring_regression_04/`、
`er006_output/pool_pilot_01/pool_n18_notifications_open113_04_
b1_regen/`。
**commit**: 本エントリ追加と同一commit(コミットメッセージ参照)。
**影響するCURRENT_SPEC項目**: 「Ledger Deviation MAJOR時の局所
Rewrite(Local Rewrite)」行(2026-09-05追記)。

---

## ER-011-TTS-STANDARD2-MINIMAL1-PRODUCTION-WIRING-FINAL-26: caller指定max_attempts(6・10等)の例外を撤廃し、対象Production経路を例外なくTOTAL3回へ統一・実TTS/実ASR runtime evidence取得

**日付**: 2026-09-04
**区分**: Implementation Hardening(caller指定値による上限突破の防止)+ Runtime evidence強化(実API呼び出しでの実証)

**内容**:
直前のER-011-TTS-STANDARD2-MINIMAL1-PRODUCTION-WIRING-25は、標準経路+
minimal instruction fallbackの2段構成を持つ3関数(`generate_charon_
japanese`/`generate_a2_japanese_with_fallback`/`generate_english_
segment_with_fallback`)の内訳を「標準2回+fallback1回=合計3回」へ
固定したが、2点の残件があった。

1. 一部callerが`max_attempts=6`(`er006_audio_cost_pilot_02_shared_
   narration.py::ensure_fixed_japanese_segment`、`generate_charon_
   japanese_with_reading_safety`/`generate_a2_japanese_with_reading_
   safety`の既定値)・`max_attempts=10`(`er003_v1_iran01_b1_kp_
   homophone_fix.py`の一回限りscript)を明示的に渡しており、
   wiring-25の「該当しない別用途の総予算は縮小しない」という設計判断
   により、実際にはこれらの呼び出しで「標準2+fallback4」「標準2+
   fallback8」となる余地が残っていた(ユーザーが再確認した正式仕様
   「Standard2+MinimalFallback1=TOTAL3」と不一致)。
2. Fallback実発火のruntime evidenceがTTS API/ASR境界のmock+実
   Production関数直接駆動(integration test)に留まり、実TTS provider・
   実ASR Verificationを使った証拠が未取得だった。

ユーザーが今回、上記1の「該当しない別用途は縮小しない」という
wiring-25の方針を明示的に撤回し、「対象Production経路では例外なく
この3回上限に統一する」ことを正式に再確認・承認した。

**実装(残件1の解消)**: 対象3関数それぞれの本体冒頭で、callerが渡した
`max_attempts`を`min(max_attempts, review_lock.PRODUCTION_MAX_TTS_
ATTEMPTS)`により無条件でクランプするよう変更した。

1. `er003_v1_sing01_voice01_generate.py::generate_charon_japanese()`
2. `er003_v1_n3_01_tts_generate.py::generate_a2_japanese_with_fallback()`
3. `er003_v1_crosslevel_audio_02_common.py::
   generate_english_segment_with_fallback()`

これにより、callerが何を渡しても(`max_attempts=6`・`10`等を含む)、
`fallback_budget = max_attempts - len(standard_attempts_log)`は
必ず`3-2=1`以下になり、標準2回+fallback1回=TOTAL3回を超えて発火する
余地は構造的に存在しない。修正はこの3関数の内部のみで完結しており、
呼び出し元(`ensure_fixed_japanese_segment`・`generate_charon_
japanese_with_reading_safety`・`generate_a2_japanese_with_reading_
safety`・`er003_v1_iran01_b1_kp_homophone_fix.py`等)のコード自体は
変更していない(呼び出し元がmax_attempts=6/10を渡しても、対象関数側で
自動的にTOTAL3へ収束するため、呼び出し元ごとに個別修正する必要が
ない)。

**対象範囲の確認**: `max_attempts`を3超で明示指定する全call siteを
grepで棚卸しした結果、上記3関数(またはそのwrapper経由)以外の
call site(`er003_b1_p9a_audio.py::generate_narration_snippet_
verified`・`er003_audio_tts_asr_safety.py::generate_tts_with_
fallback`・`er003_v1_a2_audio_01/02_generate.py`・`er003_v1_b1_
scaffold_audio_01/03_generate.py`等)はいずれも標準+fallbackの
2段構成を持たない単一ループの別関数(`generate_narration_snippet_
verified_strict`の直接呼び出し等)であり、今回のTOTAL3クランプの
対象外(§2で明示的に変更禁止とされたAzure Secondary・Key Phrase専用・
Fact Ledger・Diagnostic Full Retry等と同様、別仕様のまま無変更)。

**実装(残件2の解消・実TTS/実ASR runtime evidence)**:
`er011_final26_runtime_evidence_01.py`を新規作成し、実Production関数
`generate_charon_japanese()`を実TTS provider(`gemini-3.1-flash-tts-
preview`、voice=Charon)・実ASR Verification(Cascade、Azure Secondary
不使用)で直接呼び出した。「controlled input」(自然発生を待たない、
安全にテスト可能な検証用入力)として、標準経路のstyle_prefix
(`p9a.JAPANESE_STYLE_PREFIX`)のみをこの1回の実行中だけ「本文を読まず
別の固定フレーズを話す」よう指示するadversarial instructionへ
一時差し替えた(fallback側`MINIMAL_INSTRUCTION_PREFIX_JA`は無変更、
TTS呼び出し経路・ASR Verification経路・fallback関数はいずれも本番の
まま)。

実行結果(`er011_output/final26_runtime_evidence_01/runtime_evidence.
json`、run_id=`final26-20260904T031439Z`):
- Attempt 1(標準・実TTS/実ASR): ASR書き起こし「こんにちは、今日は
  いい天気ですね。」、`audio_classification=TRUE_CONTENT_MISMATCH`、
  `verified=false`。
- Attempt 2(標準・実TTS/実ASR): 同上、ASR書き起こし同一内容、
  `TRUE_CONTENT_MISMATCH`、`verified=false`。
- Attempt 3(Minimal instruction fallback・実TTS/実ASR、無変更の
  `generate_charon_japanese_minimal_instruction()`が実際に発火):
  ASR書き起こし「これはテストです。」(入力textと完全一致)、
  `audio_classification=EXACT_MATCH`、`verified=true`。
- `total_attempt_count=3`、`fallback_used=true`、`final_status=OK`、
  `asr_verified=true`、4回目の呼び出しは発生していない
  (attempts_logの長さで確認)、`clipping_detected=false`。

3回とも実際にTTS API呼び出し・ASR API呼び出しが行われており(音声
ファイルも実際に保存・確認済み)、mockは一切使用していない。これにより
「Standard1 NG→Standard2 NG→Minimal Fallbackが実際に3回目として
発火」という経路を、実TTS/実ASRで直接実証した。

**Azure Secondaryへの影響**: 無変更(今回のevidence取得でもAzure
Secondary routingは呼び出していないことを`azure_secondary_used:
false`で確認)。

**Key Phrase TOTAL4への影響**: `generate_key_phrase_component_
verified()`はwiring-25と同様、今回の変更対象外(独立した4回構成、
今回の`min(max_attempts, PRODUCTION_MAX_TTS_ATTEMPTS)`クランプを
一切参照しない)。regression testで非干渉を再確認。

**Regression**: `er007_ja_tts_retry_path_fix_test_01.py`のうち、旧
wiring-25の「callerが渡した大きいmax_attempts分だけfallbackが拡張
される」ことを検証していた3テストを、今回のTOTAL3クランプ挙動を
検証する内容へ更新し(`test_larger_explicit_max_attempts_still_
converges_to_total_three`・`test_max_attempts_ten_also_converges_
to_total_three`・日本語A2/英語側の同種テスト追加、計22テスト)、
全PASSを確認。プロジェクト全体regression結果は本エントリ末尾および
CURRENT_SPEC.mdを参照。

**CURRENT_SPEC.md**: 「TTS生成の同一segment総試行回数上限」行へ
「FINAL-26改訂」を追記し、wiring-25の「該当しない別用途の総予算は
縮小しない」という記述を撤回・修正した。

**OPEN_ITEMS.md**: OPEN-103(英語Key Phrase「default」のPROVIDER_
VARIANCE、恒久課題)へ、今回の撤回・クランプ変更がKey Phrase専用
経路には引き続き無関係であることを追記した(status・`DEFERRED /
NON-BLOCKING`は変更なし)。

**Git**: 本エントリ末尾に追記(commit SHA等は最終報告参照)。

---

## ER-011-TTS-STANDARD2-MINIMAL1-PRODUCTION-WIRING-25: TTS標準経路2回+Minimal instruction fallback1回(合計3回)の内訳をProduction正式仕様として配線

**日付**: 2026-09-04
**区分**: Implementation Hardening(標準+fallback2段構成の予算配分バグ修正)、内訳自体はユーザー正式決定によるサービス仕様確定

**内容**:
直前の診断タスク(ER-011-PREEXISTING-4-FAILURES-RISK-DIAGNOSTIC-24)で、
既存のTTS retry設計(`fallback_budget = max(0, max_attempts -
len(standard.attempts_log))`、ER-008-ASR-VARIANT-HARDENING-AND-RETRY-15
Part B導入)が、標準経路が早期returnせず最後まで回った場合
`len(attempts_log) == max_attempts`に必ず一致するため、
minimal instruction fallbackの予算が構造的に常に0になり実質的に一度も
発火しない不具合を持つことが判明していた(日本語側で実Production
incident 2件[No.18 A2 `comment_1`、pool_n9_tip_screens A2]を確認、
英語Key Phrase側の同種問題[OPEN-103]と同根)。

ユーザーが正式に、標準+fallback2段構成を持つ関数について以下を
Production量産仕様として採用した。

```
STANDARD_ATTEMPTS = 2
MINIMAL_FALLBACK_ATTEMPTS = 1
TOTAL_MAX_ATTEMPTS = 3
```

Attempt 1・2 = 標準Prompt(初回+retry1回)、2回ともNGの場合のみAttempt 3
= Minimal instruction fallback。Attempt 3でもValidator/VerificationがNG
ならSTOPPED/Human Reviewへ進む(既存のfail-closed処理を維持、誤った
音声は採用しない)。

**実装**: `er011_human_review_lock_01.py`へ新定数`PRODUCTION_STANDARD_
TTS_ATTEMPTS`(2)・`PRODUCTION_MINIMAL_FALLBACK_TTS_ATTEMPTS`(1)を追加し
(既存`PRODUCTION_MAX_TTS_ATTEMPTS`=3とのassertで整合を保証)、標準+
fallback2段構成を持つ3関数に`standard_attempts`パラメータ(既定
`PRODUCTION_STANDARD_TTS_ATTEMPTS`)を追加した。

1. `er003_v1_sing01_voice01_generate.py::generate_charon_japanese()`
   — 標準経路のループ上限を`max_attempts`から`standard_attempts`へ変更。
2. `er003_v1_n3_01_tts_generate.py::generate_a2_japanese_with_fallback()`
   — 標準経路呼び出しの`max_attempts`引数を`standard_attempts`へ変更。
3. `er003_v1_crosslevel_audio_02_common.py::
   generate_english_segment_with_fallback()`(Key Phrase以外の英語
   ナレーションsegment用) — 同様に`standard_attempts`へ変更。

fallback側の予算計算式自体(`max(0, max_attempts - len(...))`)は変更
していない。標準経路の消費量を`standard_attempts`回に固定するだけで、
既定値(`max_attempts`=3・`standard_attempts`=2)では自動的に
`3-2=1`回分がfallbackへ残るようになる、という最小の修正。

**該当しない別用途を勝手に変更しない**: 既存の呼び出し元の一部は
`max_attempts`を明示的に既定(3)より大きい値で呼んでいる
(`er006_audio_cost_pilot_02_shared_narration.py::
ensure_fixed_japanese_segment`が`max_attempts=6`、`er003_v1_
b1redesign_audio_generate.py`/`er003_v1_n3_01_tts_generate.py`の
`generate_charon_japanese_with_reading_safety`/`generate_a2_japanese_
with_reading_safety`が既定`max_attempts=6`、`er003_v1_iran01_b1_kp_
homophone_fix.py`の一回限りscriptが`max_attempts=10`)。これらの
call siteの総予算(6・10)を今回3へ縮小することはしていない
(`standard_attempts`は常に既定2のまま、差分[例: 6-2=4回]がそのまま
fallbackへ回る形にすることで、「fallbackが常に0固定だった」不具合修正
の効果だけを及ぼし、これらのcall siteが元々持っていた総試行回数の
余裕は変更しない、という保守的な解釈を採用した)。

**Key Phrase英語Component専用仕様への非干渉**: `generate_key_phrase_
component_verified()`(ER-010-NO9-KEYPHRASE-MINIMAL-ENGLISHLOCK-
PRODUCTION-WIRING-22で確立済み、Primary Minimal最大2+Fallback English
Lock最大2=合計4)は、標準経路を持たずPrimary/Fallbackがそれぞれ独立した
固定予算で`generate_narration_snippet_verified_strict`を呼ぶ設計であり、
今回の`fallback_budget`計算式自体を使っていないため、今回の変更の対象
外(無変更)。新規regression testでこの非干渉を保証した。

**Azure Secondaryへの影響**: 固有名詞発音判定用のSecondary ASR
routing(`er006_secondary_asr_01.py`/`er007_ja_secondary_validator_
01.py`等)は、今回変更した3関数のいずれからも変更していない(呼び出し
経路・条件は無変更)。

**Runtime evidence(Case A〜D)**: 実TTS/ASR呼び出しの境界(TTS API・
ASR transcribe)をmockし、標準+fallback2段構成を持つ実際の
Production関数(`generate_charon_japanese`/`generate_a2_japanese_
with_fallback`/`generate_english_segment_with_fallback`、いずれも
ロジック自体は無変更のまま実行)を直接駆動する形で、以下4パターンを
新規regression testで確認した(コードの制御フロー自体の正しさを
検証する目的のため、追加の実TTS課金は行っていない)。
- Case A(標準1回目PASS): 1回で終了、fallback未呼び出し。
- Case B(標準1 NG・標準2 PASS): 2回で終了、fallback未呼び出し。
- Case C(標準1 NG・標準2 NG・Fallback PASS): 標準2回消費後、
  実際にminimal instruction fallbackが3回目のattemptとして発火し
  採用される(`fallback_used=True`、`standard_attempts_log`長さ2・
  `fallback_attempts_log`長さ1を確認)。旧設計ではこのCase Cの
  3回目が構造的に絶対発火しなかった。
- Case D(標準1 NG・標準2 NG・Fallback NG): 標準2回+fallback1回
  (合計3回)とも不合格でSTOPPED(fail-closed、誤音声採用なし)。

**Regression**: `er007_ja_tts_retry_path_fix_test_01.py`を全面改訂
(旧4テスト→新20テスト)。旧`test_stop_retrying_false_still_retries_
normally`(標準2+fallback2=4回を期待する、旧設計前提の古い回帰テスト
で、直前の診断タスクで「既存4 failureの1つ」として特定済みだった)は、
新仕様(標準2+fallback1=3回)を検証する新テスト群へ置き換えた。
`A2JapaneseFallbackStopRetryingTests`の既存2テストが使っていた
`attempts_log=[]`という非現実的なmock(標準経路の実際の消費量を反映
しておらず、fallback予算が常にmax_attempts分残るという誤った前提で
書かれていたため、旧設計の不具合を覆い隠していた)を、実際の
`standard_attempts`(2)件分のattempts_logへ修正した。プロジェクト
全体regression: `collected=2067 passed=2064 failed=3`。failed=3は
本タスク開始前の診断(ER-011-PREEXISTING-4-FAILURES-RISK-DIAGNOSTIC-24)
で特定済みの、本タスクと無関係な既存2根本原因(OPEN-77のprefix
命名規則問題1件、frozen定数陳腐化2件、いずれもテスト集計ツール内部の
監査専用問題でProduction非接続)のみで、本タスクが対象とした
JA retry testの失敗は解消された(旧2051件中4 failed→新2067件中
3 failed、新規追加16テストは全PASS)。

**CURRENT_SPEC.md**: 「TTS生成の同一segment総試行回数上限」の行を、
標準2+fallback1=3の内訳・旧設計の不具合・該当しない別用途を縮小しない
方針を明記する形で更新(`PRODUCTION_WIRED`)。

**OPEN_ITEMS.md(OPEN-103)**: 本Open Itemが2026-09-01に診断した
fallback_budget=0の構造的不具合は、Key Phrase英語Component以外の
2段構成関数について今回解消したことを追記した。ただし本Open Itemの
核心である「default」1語の非決定的な誤発音自体(PROVIDER_VARIANCE)は
今回の対象外であり未解決のまま(Key Phrase Componentは2026-09-01の
WIRING-22で独立した4回構成へ全面置換済みのため、そもそも今回の
fallback_budget計算式を使っていない)。OPEN-103のstatus(`DEFERRED /
NON-BLOCKING`)は変更していない。新規Open Itemの追加登録は行っていない
(不要と判断)。

**Git**: コード3ファイル・テスト1ファイル・CURRENT_SPEC.md・
OPEN_ITEMS.md・本エントリをcommit・origin/mainへpush。

---

## ER-011-NO18-A2-TIGHT-SPEECH-AND-TRIM030-PRODUCTION-WIRING-23: A2 tight_speech_only() removal・Key Phrase trim 0.30秒を量産Production仕様として正式配線

**日付**: 2026-09-04
**区分**: サービス・生成仕様(正式採用・Production配線)

**内容**:
ユーザーが正式採用済みの以下2点を、今後の量産全体で確実に効くProduction
仕様として恒久的に配線した。No.18 A2/B1の完成音声自体はユーザーが既に
承認済みのため、今回は記事・音声の再生成は行わず、コード変更のみで
完結させた。

1. **A2 `tight_speech_only()` removal** — `er003_v1_n3_01_assemble.py::
   load_a2_sources()`が、Key Phrase英語音声(`kp{rank}_en.wav`)へ適用
   していた`tight_speech_only()`(語頭・語末の再crop)呼び出しを削除。
   generation-timeに確保した`KEY_PHRASE_TRIM_SAFETY_MARGIN_SECONDS`
   (0.30秒)のmarginがそのままAssemblyへ渡るようになる。B1側
   (`load_b1_sources()`)はER-008-N7-BASELINE-RESET-AND-MIDDLE-DEFER-01
   で既にこの再cropを行っておらず、この変更でA2もB1と同じ挙動に揃う。
   `load_a2_sources()`は初回Assembly・retry/reassembly・regenerationの
   すべてが経由する唯一の関数(`stage_assemble_a2()`からの呼び出しが
   1箇所であることを単体テストで固定)であるため、この1箇所の変更で
   正式後続経路すべてに整合して反映される。`tight_speech_only()`の
   関数定義自体(`er003_b1_p7c_audio.py`)は他script(repro01/02・
   crosslevel・iran01等のlegacy/parallel生成経路)からの参照が残るため
   削除しない。
2. **Key Phrase trim safety margin 0.30秒のcache identity保証** —
   `er006_audio_cost_pilot_02_shared_narration.py::
   ensure_key_phrase_english_component()`が使うMasterAudioKeyの
   `style_instruction_version`へ、新設した`KEY_PHRASE_TRIM_POLICY_
   VERSION`("v2_margin030")を含めた。`MasterAudioKey.EQUALITY_FIELDS`
   (`er006_master_audio_store_01.py`)は生成時のsafety marginそのものを
   識別子に含まない設計であるため、versioningを入れない場合、
   Wiring-12(2026-09-03)でKEY_PHRASE_TRIM_SAFETY_MARGIN_SECONDSを
   0.20→0.30秒へ変更した後も、0.20秒時代([2026-08-17〜09-02]生成)の
   既存Key Phrase Master資産が同一textの新規リクエストへ誤ってcache
   hitしうる状態だった。実際にMaster Audio Store manifestを確認した
   ところ、該当style_instruction_id("key_phrase_english_component")の
   既存99エントリすべてが2026-08-22〜09-02生成(=0.20秒時代)で、
   `audio_processing_version`/`style_instruction_version`ともに旧値
   "v1"のままだったことを確認した(実害が既に発生していたわけではない
   が、次にこれらと同一textのKey Phraseが別記事で必要になった時点で
   静かに旧margin資産が再利用される状態だった)。version文字列を
   trim policyへ紐づけることで、旧margin時代の資産とは異なる
   master_audio_idとなり、自然にcache missとなって現行margin(0.30秒)
   で再生成される。No.18 Key Phrase 5「be powered off」については、
   Wiring-12で既に0.30秒版へ手動昇格・ユーザー承認済みの資産がある
   ため、無駄な再TTSを避けるべく、この資産を旧keyから新keyへ
   sha256一致を確認のうえ移行した(新規TTSではなく既存資産の引き継ぎ)。

**Primary/Fallback整合**: `generate_key_phrase_component_verified()`の
Primary(Minimal instruction)・Fallback(English language lock)呼び出しは
いずれも既にWiring-12時点で`safety_margin_seconds=KEY_PHRASE_TRIM_
SAFETY_MARGIN_SECONDS`を明示的に渡す実装になっており、本タスクでの
コード変更は不要だった(単体テストでこの事実を新たに固定した)。

**Runtime evidence**:
- 過去99件と重複しない新規Key Phraseテキスト("put the phone away")を
  実際に`ensure_key_phrase_english_component()`経由(Master Audio Store
  →`generate_key_phrase_component_verified()`)で生成し、
  `cache_miss_reason="no_existing_master"`・`reused=False`(新version
  キーでの初回のため正しくcache miss)・`trailing_margin_retained_
  seconds=0.30`・Primary(Minimal instruction)1回でASR verified=True・
  disfluency_checked=Trueを実データで確認した(actual model_id=
  gemini-2.5-pro-preview-tts、Aoede、Gemini Batch API経由)。
- No.18 A2を、コード変更後の`stage_assemble_a2()`(monkeypatch不使用、
  既存の承認済みnarration資産のみ使用、新規TTS 0件)で実際に再Assembly
  し、出力wavのsha256がユーザー承認済み21R候補音声(`English_Your_Way_
  A2_POOL_N18_NOTIFICATIONS_SPECFIX_V2_EC_A_PRECISION_21R.wav`)と
  **完全に一致(byte-for-byte同一)**することを確認した(351.824秒・
  peak 0.93346・clipping無し、いずれも21Rの数値と一致)。これは
  「Trial-15のmonkeypatchで作った候補音声」と「今回コード変更後の
  正式Production Assembly」が寸分違わず同じ結果を生むことの直接証拠
  であり、A2完成候補音声の内容自体は一切変わらない。

**Regression**: 新規単体テスト12件(`er011_no18_tight_speech_and_
trim030_production_wiring_23_test_01.py`、A2 no-re-crop固定・B1無変更
確認・Primary/Fallback margin一致固定・final-sound safeguard文言存在
確認・MasterAudioKey version分離のロジックテスト[sandbox化した
Master Audio Store]の5カテゴリ)全PASS。既存Master Audio Store/
Key Phrase/Assemble Gate関連の既存回帰63件全PASS。プロジェクト全体
regression(`run_project_regression.py`、collected=2051)は2047 PASS・
4 failed。4件はいずれも本タスク以前から存在する既知の無関係な
failureと確認した: `er003_test_bad.FixtureTests.test_case_0`(回帰
harness自体の自己診断用の意図的な失敗fixture)、
`er003_test_p2j_investigate`の2件(OPEN-77、prefix別集計ロジックの
既知meta-testバグ)、`er007_ja_tts_retry_path_fix_test_01::
test_stop_retrying_false_still_retries_normally`(日本語TTS retry経路、
本タスクの変更対象と無関係。`git stash`で本タスクの変更を一時退避した
状態でも同一の失敗が再現することを確認し、本タスクによる新規regression
ではないと判定した)。

**状態**:
- A2 `tight_speech_only()` removal: `PRODUCTION_WIRED`(§19相当の
  全条件クリア: ユーザー正式採用済み、A2初回Production Assembly実装
  済み、retry/reassembly整合[唯一の呼び出し元]、Trial-onlyでない、
  runtime evidence[margin保持・byte-for-byte一致]あり、tests PASS、
  CURRENT_SPEC/DECISION_LOG/OPEN_ITEMS更新、Git反映、承認済み音声
  挙動と完全一致)。
- Key Phrase trim margin 0.30秒: `PRODUCTION_WIRED`(generation path・
  Primary・Fallback・cache/reuse整合・旧0.20秒誤reuse防止・A2 Assembly
  再cropなし・runtime evidence・regression PASS・SSOT更新・Git反映の
  全条件クリア)。
- No.18 A2/B1完成音声: ユーザー承認済みのまま変更なし(再生成していない)。
- Pattern B/C: `DEFERRED CANDIDATE / NOT REJECTED`のまま(本タスクの
  スコープ外、変更なし)。
- OPEN-100/OPEN-112: 変更なし(本タスクはEvidence Compressionではなく
  Audio Assembly/Master Audio Store側の配線のため無関係)。

**今回実施しなかったこと**:
- No.18記事・音声の再生成(ユーザー承認済みのため不要、既存資産のみ使用)。
- Master Audio Store既存99件のうち、No.18 kp5以外の98件の遡及的な
  0.30秒版への再生成(次に該当textが必要になった時点で自然に新version
  キーへ再生成される設計、遡及一括再生成はQCD上不要と判断)。
- Master Audio Store cache identityの大規模再設計(trim policy version
  文字列の追加という必要最小限の変更のみ)。
- OPEN-112(Editorial Type Production Wiring Reconciliation)への着手。

**今回のリポジトリ変更**:
- `er003_v1_n3_01_assemble.py`(`load_a2_sources()`からtight_speech_
  only()呼び出しを削除)
- `er006_audio_cost_pilot_02_shared_narration.py`(`KEY_PHRASE_TRIM_
  POLICY_VERSION`定数新設、`ensure_key_phrase_english_component()`の
  MasterAudioKeyへ適用)
...
- `er011_no18_tight_speech_and_trim030_production_wiring_23.py`
  (新規、移行・runtime evidence取得・A2再Assembly比較スクリプト)
- `CURRENT_SPEC.md`/`DECISION_LOG.md`/`OPEN_ITEMS.md`(本エントリ含む
  更新)
- `er006_output/master_audio_store_01/manifest.json`(kp5エントリの
  新versionキーへの移行データを追加、既存エントリは削除・変更なし)

**根拠レポート**: 本エントリ。実行ログは`er011_output/23_wiring_run.log`・
`er011_output/no18_tight_speech_and_trim030_production_wiring_23/
wiring23_final_result.json`。

**影響するCURRENT_SPEC項目**: 「Audio Implementation Detail」節の
「English Key Phrase trim safety margin」行(更新)、「A2 Key Phrase
英語音声のAssembly時re-crop」行(新設)、「Key Phrase Master Audio
Storeのcache identityとtrim policy」行(新設)。

---

## ER-011-NO18-EVIDENCE-COMPRESSION-A-PRODUCTION-WIRING-AND-FINAL-CANDIDATE-AUDIO-21R: Pattern A + Precision正式Production配線、No.18 A2/B1最終候補音声完成

**日付**: 2026-09-04
**区分**: サービス・生成仕様(正式採用・Production配線)

**内容**:
Trial-18/19/20を経てユーザーが正式採用した以下2ルールを、既存Evidence
Compression Editor(方式C、Lossless Editor)のProduction Promptへ配線した。

1. **Pattern A: Representative Metric + Supporting Trend** — 同一研究・
   調査内の複数指標が同じ方向を示す場合、最も代表性の高い指標1つの比較
   数値だけを保持してよい。その他の指標はFactとして残しつつ具体数値を
   省き、trend表現("showed the same pattern"等)へ言い換えてよい。
2. **Listener-Friendly Numeric Precision**(Pattern Aの後に適用) — 残す
   価値があると判断した数値について、意味(比較方向・story上の大きさ・
   閾値・解釈・値の区別)が変わらない場合のみ、より単純な概数を優先して
   よい。丸めは異なるFact・指標の統合を許可しない。

文言はTrial-18/19/20で検証済みのものと完全に同一(No.18固有語のhardcode
なし)。既存の絶対禁止事項(Fact追加/削除禁止・causation強化禁止等)は
無変更。単体テスト9件追加(`er011_no18_evidence_compression_a_precision_
21r_test_01.py`)、既存のEvidence Compression関連regression(dates_22・
locations_23等)・Point Overlap/Diagnostic Full Retry関連regression計57件
全PASS。

Production callsite調査により、`_generate_and_compress_article()`が
初回生成(`run_one_pattern()`)とDiagnostic Full Retryの両方から共通で
呼ばれる**単一のcall site**であることを確認し(regression testで固定)、
別々の配線が必要な複数経路は存在しないことを確認した。

**No.18 A2/B1の実Production再生成**:
Writerは既存の承認済み出力(OPEN-108/109でLedger精密化済み)をそのまま
再利用し(新規Writer LLM呼び出しなし、Point/Preview/Comment等の無関係な
書き換えを防止)、Evidence Compression以降の全工程(Point Overlap/Value
QA・Fact Checker・Ledger Deviation Check・Local Rewrite・Directional
Fact Precheck)は実Production関数(`gen.run_one_pattern()`)をコード変更
なしでそのまま実行した。結果、両レベルで実際にPattern A+Precisionが発火
(例: `99.71/108.95` → `about 100/about 109`、processing speedはtrend
表現化)し、Ledger Deviation Check=`LEDGER_COMPLIANT`(0件、両レベル)、
Point Overlap article retry=0回(両レベル)を確認した。

A2初回試行では、Editorが(非決定的に)研究者氏名("Upshaw, Stevens,
Ganis, and Zabelina"等)を本文に残す変種を生成し、ASRがこの固有名詞を
誤認識("Ganas, and Zablona"等)したことでHuman Review Lockへ到達した
(`full_story_part1`=`ASR_VALIDATION_UNCERTAIN`、`full_story_part2`=
`HUMAN_REVIEW_REQUIRED`)。ASR転写を精査した結果、誤認識は氏名部分に
限定されており、Pattern A/Precisionの対象数値部分("about 100"/
"about 109"/"showed the same pattern"等)は一貫して正確に転写されて
いたことを確認した。強制承認(Human Approval)で押し通す代わりに、同じ
実Production経路をもう一度実行し(Editorの非決定性を利用)、研究者氏名を
一般化した変種を取得して解消した(2回目: Fact Checker=`PASS`、Ledger=
`LEDGER_COMPLIANT`、Directional Fact Precheck=`PASS`、全14 segmentが
Audio Validation Gateで`OK`)。B1は初回で全segment`OK`。

**音声パーツ再利用**: Support(Preview/Comment)・Key Phraseは今回
Scaffoldを再実行せず(再実行するとLLMが無関係な箇所まで新しい日本語文言
を生成してしまうため)、specfix_v2の成果物をbyte-for-byteそのまま再利用
した。`parts.json`を新articleから機械的に再構築し、旧`parts.json`との
フィールド単位diffで実際にテキストが変わったsegmentだけを実Production
TTS関数で再生成した(A2: `full_story_part1`/`full_story_part2`/
`point_one`の3件、B1: 同3件+`point_two`の4件。残り11segmentは再利用)。
Key Phrase 5("be powered off")はsha256が完全一致し、Trial-15で承認
済みの0.30秒trim marginの資産(旧0.20秒版ではない)であることを確認した。

**A2最終候補Assembly**: ユーザーが既に試聴・承認済みのTrial-15条件
(`tight_speech_only()`を一時的に恒等関数化する、Production正式コード
`er003_v1_n3_01_assemble.py`は無変更)を、Trial-15と同じcontextmanager
手法で適用した。**この適用は今回の候補生成限定であり、tight_speech_only
removal自体をProduction正式採用したことを意味しない**。B1はこの制約と
無関係な通常経路。

**完成結果**: A2 351.824秒(peak 0.933、clipping無し)・B1 357.964秒
(peak 0.793、clipping無し)。フルスクリプト・Key Phrase一覧・Evidence
Compression差分ハイライト・segment再利用/再TTS内訳・cost内訳付きの
ユーザー試聴用Artifactを作成し提示した。

**今回実施しなかったこと**: Pattern B/Cの正式採用・tight_speech_only
removalのProduction正式反映・CURRENT_SPECへのtight_speech_only記載・
OPEN-100のclose・OPEN-112の着手は、いずれも意図的に行っていない。

**状態**:
- Pattern A + Listener-Friendly Numeric Precision: `PRODUCTION_WIRED`
- Pattern B / C: `DEFERRED CANDIDATE / NOT REJECTED`(変更なし)
- tight_speech_only removal: `VALIDATED / USER_DECISION_REQUIRED FOR
  PRODUCTION ADOPTION`(A2最終候補はこの条件で作成済み、正式採用は
  ユーザー試聴後)
- No.18 A2: `FINAL CANDIDATE READY FOR USER LISTENING`
- No.18 B1: 完成(通常経路、tight_speech_only制約なし)

**今回のリポジトリ変更**:
- `er003_v1_n3_01_evidence_compression_editor.py`(Pattern A+Precision
  追加ルールをProduction Promptへ配線)
- `er011_no18_evidence_compression_a_precision_21r_test_01.py`(新規、
  regression 9件)
...
- `er011_no18_evidence_compression_a_precision_21r_audio_stage.py`
  (新規、音声segment差分再利用+Assembly driver)
- `er011_no18_evidence_compression_a_precision_21r_cost_compute.py`
  (新規、cost集計)
- `er006_output/pool_pilot_01/pool_n18_notifications_specfix_v2_ec_a_
  precision_21r/`(新規、No.18最終候補の記事・音声・監査ログ一式。既存の
  `pool_n18_notifications_specfix_v2`は無変更のまま保持)

**根拠レポート**: ユーザー提示Artifact(No.18 Final Candidate、フルスクリ
プト・Evidence Compression差分・cost付き)

**影響するCURRENT_SPEC項目**: 「QA / Human Review」節へ新規行「Evidence
Compression Editor(方式C)追加ルール: Pattern A + Listener-Friendly
Numeric Precision」を追加(`PRODUCTION_WIRED`)。tight_speech_only関連の
記載は追加していない(未採用のため)。

---

## ER-011-NO18-A2-EVIDENCE-COMPRESSION-ABC-PRECISION-EXTENSION-TRIAL-20: Pattern A/B/Cへの共通Precisionルール追加Trial(各3回×3パターン=9 run、Production未採用)

**日付**: 2026-09-04
**区分**: サービス・生成仕様(Trial、未採用)

**背景**: Trial-19でPattern A/B/Cの核となる変換(代表指標保持・結論優先・差分再表現)がいずれも5/5 runで安定することを確認した。次の観点として、各Patternが「不要な小数精度を安全に落とせるか」「丸めによってFact統合・意味変化を起こさないか」という、数値の精度そのものに対する判断力を追加で確認する必要が生じたため、3 Pattern共通の追加ルール「Listener-Friendly Numeric Precision」をTrial-19のPattern定義に重ねてTrialした。

**内容**:
1. No.18 A2の同一pre-editor Writer出力・同一Verified Fact Ledgerを共通入力とし、Pattern A/B/Cを各3回、同一model・同一reasoning設定で実行(計9 run)。Pattern A/B/Cの既存ルール文(Trial-18/19と同一)・既存Evidence Compression Editor Promptは無変更。今回追加したのは3 Pattern共通の英語Precisionルール(「値を保持する場合は聴取に必要な最小限の精度のみ残す」「丸めはFact統合の許可ではない」という趣旨、機械的な小数削除は明示的に禁止)のみ。Baselineは再実行せず、Trial-19の結果を参考として引用した。
2. **Pattern A**: 3/3 runで、代表指標(attention score)の小数(99.71/108.95)を`about 100`/`about 109`へ一貫して丸め、かつ丸め後もF-005・F-006両方のFactを区別したまま保持した(処理速度は既存ルール通りtrend文化され、数値自体を持たないため丸め値との数値衝突が起きない構造)。Trial-19(Precisionルールなし)では5/5 runとも小数を保持していたため、最も明確にPrecisionルールの狙い通りに動いたPatternだった。
3. **Pattern B**: 3 run中1 run(`pattern_b_precision_01`)のみ小数(99.71/108.95)を保持し、残り2 runは`about 100`/`about 109`へ丸めるという非一貫な挙動を示した。両Fact保持・Ledger安全性という核となる挙動自体は3/3 runで安定していたが、Precisionルールの適用強度自体の再現性はA/Cより低いという新しい知見が得られた。
4. **Pattern C**: 3/3 runとも`about 9 points lower`/`about 10 points lower`という、Trial-19時点と同一の差分再表現を出力した。Pattern C自体の設計が既にPrecision的な判断(絶対値ではなく整数の差分幅への再表現)を内包しているため、Precisionルール追加による出力上の変化は確認できなかった(実質no-op)。
5. 数値精度そのものの誤り(numeric calculation error)は9 run中0件。Pattern Cの全runでFact Ledgerの実測差分(9.24/10.09)から見て誤差1.0以内の正確な整数化を確認した。
6. **重要な発見**: `pattern_c_precision_02`は「The attention score was about 9 points lower, and processing speed was about 10 points lower, ...」と1文に両Factをまとめたが、2つの数値(9/10)は文中で明確に区別されたまま保持されており、Fact Checkerも両差分を独立に「概ね正確」と確認した。自動判定スクリプトの粗いheuristic(文単位の重複チェック)がこれを「Fact統合疑いあり」と誤検知したが、手動確認の結果、Trial-19の`baseline_03`(2つの異なるFactを同一の概数ペアへ実際に混同した事例)とは異なり、実際のFact混同は起きていなかった。
7. Ledger Deviation Checkの指摘(9 run中3件: `pattern_c_precision_01`のMAJOR 1件、`pattern_c_precision_03`のMINOR 2件)は全て、記事末尾の既存Hook文("...a phone that never rings.")や記事前半の既存表現など、数値表現とは無関係な箇所に集中していた。Trial-19で確認したOPEN-98(Ledger Deviation Checkerのrun-to-run非決定性、2026-08-31にユーザー判断で追跡不要と決定済み)と同種の現象の再確認であり、新規Open Itemは追加していない。correlation→causation drift(相関→因果の強まり)は9 run中0件。
8. Fact Checkerの指摘(unsupported claims)は全run共通で、pre-editor記事の時点で既に存在していた「silent phone」という表現や研究者解釈の断定的提示に関するものであり、今回のPrecisionルールや数値の丸めが原因で新たに生じたものではないことをFact Checkerレポート本文で確認した。
9. 記事全体の文単位diffでは、9 run全てで削除0・追加0、置換のみ(6〜8文/34文)で、記事構造を壊す大規模な書き換えは確認されなかった。
10. No.9 OPEN-100への静的汎用性評価(No.9再生成なし)を更新した。Pattern Cの構造的適用困難という結論は維持しつつ、Pattern Aは丸めが積極化した分、survey型記事(3設問)へ転用する場合の代表指標選定という編集判断への要注意度がやや上がると評価。Pattern Bは今回判明した精度適用の一貫性の低さから、survey型記事への転用にはTrial-19時点よりさらに慎重な検証が必要と評価を更新した。

**今回実施しなかったこと**: Production Prompt変更・CURRENT_SPEC変更・No.18 article.md上書き・Writer再生成・TTS/Audio・OPEN-100 close・OPEN-112着手・Pattern Production wiring・Precision rule Production wiring・4つ目のPattern追加。Baselineの再実行も行っていない(Trial-19の結果を参考引用のみ)。

**状態**: `USER_DECISION_REQUIRED`(Pattern A/B/Cとも個別にREJECTEDではない。Claude非拘束的な参考順位: 1位Pattern C[Precisionルールとの相性が良く実質no-op、副作用なし]、2位Pattern A[Precisionルールの狙い通り一貫して安全に丸めた]、3位Pattern B[核となる挙動は安定だがPrecisionルールの適用強度がrun間で割れ再現性が低い])。

**今回のリポジトリ変更**: 新規Trialスクリプト(`er011_no18_a2_evidence_compression_abc_precision_extension_trial_20.py`)・新規出力ディレクトリ(`er011_output/no18_a2_evidence_compression_abc_precision_extension_trial_20/`、9 run分のarticle・audit・trial_summary.json)・レポート生成スクリプト(`er011_output/build_trial20_report.py`)・本DECISION_LOG.md/OPEN_ITEMS.md更新のみ。

**根拠レポート**: ハイライト付き全文比較Artifact(9 run分、Precision Decision Table・Trial-19比較表込み)をユーザーへ提示済み。

**影響するCURRENT_SPEC項目**: なし(Trialのみ、正式仕様は無変更)。

---

## ER-011-NO18-A2-EVIDENCE-COMPRESSION-ABC-REPRODUCIBILITY-TRIAL-19: Evidence Compression拡張ルールA/B/C 再現性Trial(各5回×4パターン=20 run、Production未採用)

**日付**: 2026-09-03
**区分**: サービス・生成仕様(Trial、未採用)

**背景**: 前タスク(ER-011-NO18-A2-EVIDENCE-COMPRESSION-EXTENSION-ABC-TRIAL-18)で
比較したBaseline/Pattern A/B/Cは、それぞれ1回ずつの実行結果に基づいていた。
Trial-18自体がBaselineの1回再実行で既存Production記事と異なる結果(数字4つ
全省略)を得たことから、LLM出力のばらつきを踏まえた再現性を確認する必要が
生じ、本Trialを実施した。

**内容**:
1. No.18 A2の同一pre-editor Writer出力・同一Verified Fact Ledgerを共通入力とし、
   Baseline/Pattern A/Pattern B/Pattern Cを各5回、同一model・同一reasoning設定
   で実行(計20 run)。Editor Prompt自体はTrial-18から一切変更していない。
2. Pattern A/B/Cの核となる変換(A: 代表指標保持+trend文、B: 結論優先+attention
   数値保持、C: 差分約9/約10への再表現)は、いずれも5/5 runで安定して機能し、
   Pattern Cの差分値は5/5 run全てで検証済みの正確な値(9.24→9、10.09→10)を
   出力した。
3. **重要な発見①**: 全20 runで一字一句同一・未変更のHook文
   ("...a phone that never rings.")が、Ledger Deviation Checkにより run間で
   「検出なし」「MINOR」「MAJOR」と異なる重大度に判定された。Pattern Cで
   3/5 runが`LEDGER_DEVIATION`となった主因はこの既存文への判定ゆらぎであり、
   F-005/F-006の数値再表現そのものへの指摘は20 run中0件だった。安全チェック
   機構(Ledger Deviation Check)自体に、Editor Patternとは独立したrun間ゆらぎ
   があることが新たに判明した。
4. **重要な発見②**: 追加ルールなしのBaselineの1 run(`baseline_03`)が、
   attention(99.71/108.95)とprocessing speed(98.48/108.57)という異なる2つの
   Factを"about 100 / about 109"という単一の概数ペアへ統合して表現し、Ledger
   Deviation Checkはこれを検出しなかった。Pattern Cが要求する「Fact別・検証
   済みの丸め」は、この種の暗黙的な数値統合を防ぐ具体的効果を持つことが
   示唆される。
5. Fact Checkerは20 run中19件が`REVIEW_REQUIRED`(既存の指摘内容のみ)、
   `baseline_04`のみ`FAIL`(記事冒頭「無音のスマートフォン」という既存の
   表現が実際の電源オフ条件と異なるという、pre-editor入力に既に存在する
   論点をFact Checker自身が今回のみ強く評価したもの)。correlation→causation
   drift(相関→因果の強まり)は20 run中0件。
6. 記事全体の文単位diffでは、全20 runとも削除0・追加0、置換のみ(5〜12文/34文)
   で、記事構造を壊す大規模な書き換えは確認されなかった。
7. No.9 OPEN-100への静的汎用性評価(No.9再生成なし)は、Pattern Cが3設問survey
   構造には数学的に適用できないことを改めて確認した上で、発見④(Baselineの
   Fact統合リスク)がsurvey型記事でより深刻化しうる点を新たな注意点として追記。

**今回実施しなかったこと**: Production Prompt変更・CURRENT_SPEC変更・No.18
article.md上書き・Writer再生成・TTS/Audio・OPEN-100 close・OPEN-112着手・
4つ目のPattern追加。

**状態**: `USER_DECISION_REQUIRED`(Pattern A/B/Cとも個別にREJECTEDではない。
Claude非拘束的な参考順位: 1位Pattern C、2位Pattern A、3位Pattern B。ただし
安全チェック機構自体のrun間ゆらぎという新知見は、Pattern選定とは独立に
別途検討が必要)。

**今回のリポジトリ変更**: 新規Trialスクリプト
(`er011_no18_a2_evidence_compression_abc_reproducibility_trial_19.py`)・
新規出力ディレクトリ(`er011_output/no18_a2_evidence_compression_abc_reproducibility_trial_19/`、
20 run分のarticle・audit・trial_summary.json)・レポート生成スクリプト
(`er011_output/build_trial19_report.py`)・本DECISION_LOG.md/OPEN_ITEMS.md更新のみ。

**根拠レポート**: ハイライト付き全文比較Artifact(20 run分)をユーザーへ提示済み。

**影響するCURRENT_SPEC項目**: なし(Trialのみ、正式仕様は無変更)。

---

## ER-011-NO18-A2-EVIDENCE-COMPRESSION-EXTENSION-ABC-TRIAL-18: Evidence Compression拡張ルールA/B/C比較Trial(Baseline対照込み、Production未採用)

**日付**: 2026-09-03
**区分**: サービス・生成仕様(Trial、未採用)

**背景**: 前タスク(ER-011-EVIDENCE-COMPRESSION-HISTORY-AND-FAILURE-AUDIT-17)の
履歴監査で確定した通り、現行Evidence Compression Editor(方式C、Lossless
Editor)は「近似・重複する同一指標の数字」しか圧縮対象にしておらず、No.9
OPEN-100・No.18双方で「同一研究の中で異なる指標が同じ方向を示す」場合に
数字が圧縮されず並列するSPEC_TOO_WEAKが既知の限界として残っていた。本
タスクはこの限界に対する3種類の追加ルール案(Pattern A/B/C)を、既存の
安全思想(Fact追加禁止・削除禁止・correlation→causation禁止・certainty
強化禁止等)を一切変更せずにTrialし、比較のみを行う。

**内容**: No.18 A2のWriter出力(`pre_editor_article.md`、Fact-safe、Writer
再生成なし)を共通入力とし、既存Evidence Compression Editor Prompt(無変更)
の末尾へ、共通目的文(英語)+各Pattern追加ルール(日本語)だけを差し込んだ
4本(Baseline対照含む)を生成(`er011_no18_a2_evidence_compression_extension_
abc_trial_18.py`)。

- Pattern A(Representative Metric + Supporting Trend): F-005(attention
  score)の数値99.71/108.95を保持し、F-006(processing speed)は
  "Processing speed was also lower..." のtrend文へ言い換え。F-006の
  Fact自体は文として残存。
- Pattern B(Conclusion-First / Numeric Necessity Test): F-005/F-006とも
  「不可欠でない」と判定され、数値0で"Both attention and
  processing-speed scores were lower..."に統合(Factとしては両方残存)。
- Pattern C(Listener-Friendly Numeric Re-expression): 絶対値を差へ再表現
  ("about 9 points lower"[108.95-99.71=9.24の丸め]/"about 10 points
  lower"[108.57-98.48=10.09の丸め])。Fact Checkerが独自にVFLの数値から
  同じ差分を計算し、記事の再表現値が正確であることを検証。
- Baseline(既存Editor Prompt無変更のまま同条件で再実行): 追加ルールを
  一切与えていないにもかかわらず、この回はF-005/F-006の数値を4つとも
  すべて省略し、"Both the attention score and processing speed were
  lower..."とだけ書いた。既存Production記事(2026-08-26生成)は同じ
  BaselineがF-005/F-006の数値4つとも保持しており、既存方式Cの出力が
  この境界ケースでrun-to-runで揺れることを新たに確認した(方式Cの
  「安定している」という評価は、方式Bとの相対比較としては引き続き妥当
  だが、絶対的に決定論的ではない)。

**主な発見**:
1. 4バージョンいずれも、Ledger Deviation Check(hook-aware、Production
   同一手順)はLEDGER_COMPLIANT。検出されたMINOR(Baseline 2件/Pattern A
   1件/Pattern B 1件/Pattern C 0件)はすべてHook・In One Line等の
   既存の解釈的表現(研究者解釈の断定化・実験条件の一般化)であり、
   F-005/F-006の数値圧縮そのものを指摘したものは0件だった。
2. Fact Checker(独立Web検索、Production同一モデル)は4バージョンとも
   REVIEW_REQUIRED(既存policy上non-blocking)。指摘内容は4バージョンで
   完全に同一の既存論点(第一研究の解析対象69人/54人への言及漏れ、
   "silent phone"という一般化、N2振幅を「余分な努力」と解釈する記述、
   Pew調査の言い回し)のみで、数値圧縮によって新規に生じた指摘は
   0件だった。
3. correlation→causation drift(過去No.7方式Bで実際に発生した最重要NG)
   のキーワード走査(because/due to/causes/leads to等)は4バージョンとも
   0件、新規の因果的接続表現は確認されなかった。
4. Pattern Cの数値再表現(9.24→"about 9"、10.09→"about 10")は、Fact
   Checkerが元論文を独立に検索し、同じ差分値を算出して裏付けた。
5. No.9への静的な汎用性評価(No.9記事は再生成せず、テキストのみ比較):
   No.9 Point Two(78%/44%/36%)はNo.18のF-005/F-006(同一指標・2条件の
   比較)と異なり、3つの別々のsurvey質問への回答である。Pattern Aは
   「代表1指標+他はtrend文」として概念上適用可能。Pattern Bも
   「結論優先+必要性判定」として概念上適用可能だが、3つの異なる質問を
   1つの結論文へ統合しすぎるとFact同士の区別が失われるリスクがあり、
   No.18より慎重な運用が必要。Pattern Cの「差分への再表現」は、
   No.18のような同一指標2条件比較には自然に適用できるが、No.9のような
   「3つの異なる質問への回答」には数学的な差分計算の対象がなく、
   構造的に適用できない(No.18専用ではないが、survey複数設問系には
   そのままでは汎用しない)。

**今回実施しなかったこと**: Production Prompt変更、CURRENT_SPEC正式仕様
追加、No.18 `article.md`上書き、TTS/Audio生成・変更、OPEN-100のclose、
OPEN-112への着手、No.9記事の再生成。

**状態**: `USER_DECISION_REQUIRED`(§0 Gateの通りREJECTED/VALIDATEDへの
確定は行わず、Trial結果として保存のみ)。3パターンいずれもFact safety上の
重大な問題は確認されなかったが、採用可否・3パターンのうちどれを次の
正式仕様候補とするかはユーザー判断待ち。

**今回のリポジトリ変更**: `er011_no18_a2_evidence_compression_extension_
abc_trial_18.py`(新規Trialスクリプト)、`er011_output/no18_a2_evidence_
compression_extension_abc_trial_18/`(4バージョンの記事全文・Editor raw
response・Ledger Deviation結果・Fact Checker結果・trial_summary.json)を
追加。`er003_v1_n3_01_evidence_compression_editor.py`等のProduction
コードは無変更。

**根拠レポート**: 本Decision Log本エントリ、`er011_output/no18_a2_
evidence_compression_extension_abc_trial_18/trial_summary.json`

**影響するCURRENT_SPEC項目**: なし(方式Cの正式仕様[CURRENT_SPEC.md
「Evidence Compression(方式C、Lossless Editor)」行]は無変更のまま)

---

## ER-011-EVIDENCE-COMPRESSION-HISTORY-AND-FAILURE-AUDIT-17: Evidence Compression/Numeric Compressionの導入経緯・過去Trial・不具合の一次資料監査(履歴監査のみ、実装なし)

- **日付**: 2026-09-03
- **区分**: 履歴監査(Implementation Hardeningでもサービス仕様変更でもない。新規Compression仕様設計・Prompt変更・Trial・記事修正・Production変更はいずれも実施していない)
- **背景**: 直前の診断(ER-011-NO18-EVIDENCE-COMPRESSION-RUNTIME-DIAGNOSTIC-16)で、No.18 A2の4数値列挙(attention score/processing speed)がEvidence Compression Editorにより意図的に対象外(Case 2: SPEC_TOO_WEAK、No.9 OPEN-100と同型)と判定された。次の仕様再検討に進む前に、「なぜ現在の方式C(Lossless Editor)に落ち着いたのか」を一次資料(DECISION_LOG.md本体・OPEN_ITEMS.md・CURRENT_SPEC.md・git log)から正確に再構成する目的で、履歴・原因監査タスクを単独で実施した
- **内容**: git log(`-i --grep`)およびDECISION_LOG.md全文検索により、Evidence Compression関連の全エントリ(ER-008-N7-MIDDLE-SPEC-STORY-BALANCE-KEYPHRASE-AUDIT-01[NOT_WIREDの発見]→ER-008-TTS-FALLBACK-AND-EVIDENCE-COMPRESSION-03[script-only初回実証]→ER-008-FALLBACK-TRIGGER-MITIGATION-AND-EVIDENCE-COMPRESSION-AB-04[方式B/C比較・方式C推奨]→ER-008-AUDIO-VALIDATION-GATE-AND-EVIDENCE-MAJOR-AUDIT-05[方式C MAJOR精査]→ER-008-EVIDENCE-COMPRESSION-PROD-AND-N7-AUDIO-06[ユーザー正式採用・Production配線]→ER-008-N8-FINAL-CONTENT-COMPRESSION-RETRY-22[日付・数値対象化、ER-22]→ER-008-N8-FINAL-PRODUCTION-HARDENING-23[地名・施設名対象化、ER-23]→ER-008-N8-FINAL-CLOSEOUT-24[No.8正式反映、ER-24]→ER-010-NO9-FACTCHECK-POLICY-AND-POINT-COMPRESSION-DIAGNOSTIC-12[No.9 OPEN-100発見]→ER-010-NO9-OPEN100-DEFER-AND-PRODUCTION-AUDIO-13[OPEN-100正式DEFERRED])を時系列で再構成した。CURRENT_SPEC.md 262行目(Evidence Compression正式spec全文)・OPEN_ITEMS.md OPEN-100行(No.9のCase 2 SPEC_TOO_WEAK正式記録)を一次資料として直接確認した
- **主な発見**:
  1. **導入の原点**: No.7で"Evidence Compression"という言葉が使われていたのはその場限りの手動編集方針に過ぎず、当時のProduction Writer Promptには一切実装が無かった(`NOT_WIRED`、ER-008-N7-MIDDLE-SPEC-STORY-BALANCE-KEYPHRASE-AUDIT-01で発覚)。
  2. **方式B(Compression-aware Writer、`EVIDENCE_COMPRESSION_BLOCK`)がREJECTEDになった直接の実データ**: ER-008-TTS-FALLBACK-AND-EVIDENCE-COMPRESSION-03で、No.7 B1候補にFact safety不変条件を追加する前の初期方式Bを試した結果、Ledger Deviationがbaseline1件→candidate6件(新規MAJOR3件)へ悪化し、"...because some offices are questioning the human cost of desk sharing"のような、元のcorrelational evidenceより踏み込んだ因果的表現が新規混入した(correlation→causation drift)。この実例が、方式Bを既定offのまま維持し方式Cを推奨とする直接の根拠になった(ER-008-FALLBACK-TRIGGER-MITIGATION-AND-EVIDENCE-COMPRESSION-AB-04)。
  3. **方式C(Lossless Editor)が採用された理由**: 強化したFact safety不変条件(相関→因果変換禁止・scope拡張禁止・hedging削除禁止等)のもとで方式B/Cを再比較した結果、両方式とも因果強化表現の新規混入は0件だったが、方式Cの方が固有名詞削減の安定性が高く(全section・両levelで0件)、Ledger DeviationもBaseline水準に近かった。方式Bは実行ごとのばらつきが大きく、この回はB1のLedger Deviationが1件→6件へ悪化した。ユーザーが方式Cを正式採用しProduction配線(ER-008-EVIDENCE-COMPRESSION-PROD-AND-N7-AUDIO-06、`PRODUCTION_WIRED`)。
  4. **方式Cが意図的に対象外とする範囲**: Writer側(`COMMON_BLOCK_TEMPLATE`Storytelling First)・Editor側(`evidence_compression_editor.py`)とも、対象は「同じtrendを示す近似・重複した数字」に限定される。Verified Fact Ledgerが最初から別Factとして分離登録している「異なる意味の複数指標」(No.9の[F-011]/[F-012]/[F-013]、No.18の[F-005]/[F-006])は、定義上圧縮対象に含まれない。
  5. **No.9 OPEN-100の完全な経緯**: No.9 Point Two(78%/44%/66%→59%/36%の5値)で発覚。Root Cause分類はCase D(現行Numeric Compressionの対象範囲が狭すぎる)+Case F(Storytelling FirstがPoint Two特有の「survey readout調」を防げていない)、最終分類はCase 2: SPEC_TOO_WEAK。より強い方式B相当の指示は既に上記2.のcausal drift実例により不採用と決まっているため、単純な有効化は推奨しないと明記された。ユーザーは「過去にもTrial済みで短期解決できる問題ではない」「方式Bは過去にFact Safety上のリスクが実証済み」「今後Editorial Type導入でsurvey/numeric-heavy記事比率自体が下がる見込み」を理由に、CLOSEでもREJECTEDでもなく`DEFERRED / NON-BLOCKING`として保留した(2026-09-01)。
  6. **No.18との異同**: No.18(2026-09-03発見)は、No.9と同じくEditorが実際にruntime発火し、同じ「意味の異なる複数指標(注意力の点数/処理速度の点数)が並ぶ」パターンで同じくCase 2: SPEC_TOO_WEAK相当と判定される。差異は、No.18ではEditorが呼ばれたことを`evidence_compression_editor_raw.json`の実API呼び出し記録で直接確認できた点(No.9診断時はEditor側の生成物diffで間接確認)のみで、構造的には同一の既知の限界の2件目の実例である。
  7. **"Editorial Type"は未実装**: No.9のOPEN-100が挙げた「Editorial Type導入によりsurvey/numeric-heavy比率が下がる」という将来見込みの前提そのものが、grep監査(0件マッチ)によりProductionコードに一切実装されていないことが判明した。関連する未コミット設計文書2件は存在するが、コード上の分岐は無い。放置すると「Editorial Type別の挙動差」を前提にした将来判断が実装のない仮定の上に乗るリスクがあるため、新規`OPEN-112`(Editorial Type Production Wiring Reconciliation)として登録した(`DEFERRED / NON-BLOCKING FOR CURRENT EVIDENCE COMPRESSION TASK`)。
- **今回実施しなかったこと**: 新しいCompression Prompt設計、Productionコード変更、No.18/No.9本文の書き換え、Article/Audio再生成、OPEN-100の変更(`DEFERRED / NON-BLOCKING`のまま無変更)、Editorial Typeの実装着手。タスク仕様の禁止事項(§14)を全て遵守した
- **状態**: `DIAGNOSTIC COMPLETE`。次のCompression仕様設計に必要な論点(異なる指標でも同方向なら一部省略を許すか等)は洗い出したが、いずれも今回は結論を出していない。`USER_DECISION_REQUIRED`は無し(新たなBlockingは発見していない)
- **今回のリポジトリ変更**: [OPEN_ITEMS.md](OPEN_ITEMS.md)へ`OPEN-112`(Editorial Type Production Wiring Reconciliation)を新規追加、[DECISION_LOG.md](DECISION_LOG.md)本エントリを追加。CURRENT_SPEC.mdは無変更(Evidence Compressionの正式仕様自体に変更なし)
- **根拠レポート**: ER-011-EVIDENCE-COMPRESSION-HISTORY-AND-FAILURE-AUDIT-17完了報告(チャット内、17項目形式)
- **影響するCURRENT_SPEC項目**: なし(調査のみ、既存の「Evidence Compression(方式C、Lossless Editor)」節は無変更)
...
## ER-011-ASSEMBLY-HEADROOM-SAFETY-VALVE-PRODUCTION-WIRING-01(2026-09-06、OPEN-115[A2/B1 Assembly最終段ヘッドルーム不在]をユーザーが`APPROVED_FOR_PRODUCTION`と決定・`PRODUCTION_WIRED`まで配線)

**背景**: OPEN-112-TREND-THEME2-B-A2-PEAK-MEASUREMENT-16で、A2/B1 Assemblyの各segmentは24kHz mono・gain適用直後の時点で`compute_gain_for_target_rms(max_peak=0.95)`によりpeak<=0.95に制限されるが、直後の`mono_24k_to_stereo_target()`(`resample_poly`による24kHz→48kHzアップサンプリング)がゲイン制御の外にあり、内容依存で最大+8.9%のオーバーシュートを起こすことが実測確定した(Trial-13 A2の`Point One`で0.95000→1.03502、`write_wav_float()`のクリッピング防止assertで書き出し拒否・完成音声0本)。過去のA2 assemble完了14本のうち3本が既に0.95上限付近(0.95000〜0.95534)に達しており、構造的に再発しうる状態であることも確認済み。ユーザーが2026-09-06に本対策案を`APPROVED_FOR_PRODUCTION`と決定した。

**実装内容**(`er003_v1_n3_01_assemble.py`):

1. **最終段ヘッドルーム安全弁**(`apply_headroom_safety_valve()`、新設): `assemble_with_timeline()`の出力(完成ミックス)のpeakが`HEADROOM_PEAK_THRESHOLD`(新設定数、既定0.98)を超えたときだけ、エピソード全体へ一律スカラーgain(`閾値/peak`)を掛けて閾値以内へ収める(相対バランスは変えない)。閾値以下なら一切変更せず、渡された配列をそのまま返す(byte同一を保証)。`assemble_with_timeline()`がnp.concatenateのみ[重ね合わせ無し]のため、完成ミックスのpeakは必ずseq中いずれかのpieceのpeakと一致するという構造(計測-16で実測確認済み)を利用し、最大peakのpieceをそのまま「原因piece」として特定・記録する。適用有無・適用前後peak・スカラー値・原因piece名は、`stage_assemble_a2`/`stage_assemble_b1`が`gain_report.json`・`timeline.json`とあわせて新設`headroom_report.json`および`run_summary_assemble.json`(`headroom_safety_valve`キー)へ必ず記録する(静かに救済しない、安全≠成功原則)。安全弁適用後もなおpeakが1.0を超える異常時は、`write_wav_float()`のassertへ到達させず、原因pieceを含むドメイン例外`RuntimeError`(`ASSEMBLY_HEADROOM_SAFETY_VALVE_INSUFFICIENT`)で停止する(`write_wav_float()`側のassert自体は最後の砦として維持、削除していない)。
2. **診断性改善**: `gain_report.json`・`timeline.json`・`headroom_report.json`はいずれも`write_wav_float()`より前に書き出す(書き出し失敗時にも診断証跡が残るようにするため、順序を変更)。
3. **配線範囲**: `stage_assemble_a2()`/`stage_assemble_b1()`それぞれに`apply_headroom_safety_valve()`の呼び出しを1箇所ずつ追加した。この2関数は`er006_pool_pilot_01_audio.py`(量産Production正式経路)を含む初回・retry・reassembly・regenerationのすべてが共有する唯一の関数であるため、この1箇所の変更で正式経路すべてに整合して反映される。`max_peak`(0.95)自体・`mono_24k_to_stereo_target()`等の共通関数・LUFS mastering導入・Preview経路への個別上限追加はいずれも変更していない(非採用、影響範囲最小化)。
4. **CURRENT_SPEC.md記述の訂正**: 283行目付近「音量調整」の「Preview/Bodyは無調整のアンカー」という記述が、実装(Bodyも他segmentと同様にgain適用、Previewのみ無調整)と一致していなかったため訂正した。あわせて本安全弁の仕様(閾値・記録・例外)を新規行として追記した。

**Regression**: 新規単体テスト6件(`er011_assembly_headroom_safety_valve_production_wiring_01_test_01.py`)全PASS — (a)peak<=閾値でbyte同一(`assertIs`で同一オブジェクト確認)、(b)peak>閾値で一律gain適用・原因piece記録・相対バランス維持、(c)安全弁適用後も1.0超なら`RuntimeError`(閾値を意図的に変更した分岐テスト)、(d)`write_wav_float()`失敗時にも`gain_report.json`/`timeline.json`/`headroom_report.json`が先に存在すること(`stage_assemble_a2`を合成sourcesでmonkeypatchし実際に確認)、加え`stage_assemble_a2`/`stage_assemble_b1`それぞれに呼び出しが1箇所だけであることを固定する2件。既存のAssembly関連回帰(`er011_no18_tight_speech_and_trim030_production_wiring_23_test_01.py`・`er008_n8_qa_hardening_21_gate_test_01.py`・`er008_a2_slowdown_invariant_19_test_01.py`・`er008_audio_validation_gate_05_test.py`、計36件)は全PASS。プロジェクト全体regression(`run_project_regression.py`、collected=2081)は2078 PASS・3 failed。3件は本タスク以前から存在する既知の無関係failureと`git stash push -- er003_v1_n3_01_assemble.py`による切り分けで確認済み(`er003_test_p2j_investigate`のOPEN-77既知meta-test集計バグ、母体である`er003_test_bad.FixtureTests.test_case_0`は回帰harness自体の意図的fixtureでこの3件のouter集計には含まれない自己診断)。

**Runtime evidence**(新規TTS/ASR課金なし、既存wavのみ使用):

1. **Trial-13 A2**(`er011_output/open112_trend_theme2_b_full_audio_trial_13/a2/`)を、修正後の`stage_assemble_a2()`で`--assemble-only`実行(narration再生成なし)。安全弁が実際に発火し、`headroom_report.json`/`run_summary_assemble.json`へ`{"threshold": 0.98, "peak_before": 1.0350189, "cause_piece": "Point One", "cause_piece_peak": 1.0350189, "applied": true, "scalar": 0.94684262, "peak_after": 0.98}`が記録された。完成音声(duration 372.146秒、peak 0.98、clipping無し)が`assembled/English_Your_Way_A2_OPEN112_TREND_THEME2_B_FULL_AUDIO_TRIAL_13.wav`として初めて生成された(OPEN-115発見時点では書き出し不能だった)。原因pieceは計測-16の実測(`Point One`)と一致。同時に実行したB1側は本タスクと無関係なKey Phrase 3件(kp3_japanese/kp4_english/kp4_japanese、Human Review Lock STOPPED、OPEN-116管轄)によりGate Blockedのままで、本タスクのスコープ外として変更していない。
2. **No.18 A2の閾値以下regression**: `run_summary_assemble.json`記載の`pool_n18_notifications_specfix_v2/a2`(peak 0.85444)は、その後のER-011-NO18-A2-TIGHT-SPEECH-AND-TRIM030-PRODUCTION-WIRING-23(2026-09-04、`load_a2_sources()`のKey Phrase英語音声`tight_speech_only()`再crop除去)以降、同資産が再Assemblyされていない古い版であることが判明した(再現するとKey Phrase blockが約0.8秒×5伸び、duration 347.978→351.978秒とbyte不一致になる。これは本タスクのheadroom変更とは無関係な、既に`DECISION_LOG.md`ER-011-NO18-...-23エントリに記録済みの正当なコード進化であり、本タスクの新規regressionではない)。Wiring-23自身が「コード変更後の`stage_assemble_a2()`で再Assemblyしユーザー承認済み候補音声とbyte-for-byte一致を確認済み」と記録している最新資産`pool_n18_notifications_specfix_v2_ec_a_precision_21r/a2`(peak 0.93346、duration 351.824秒)を正しい回帰対象として使用し、narration/key_phrases/audit(tts_generation_results.json)を`er011_output/assembly_headroom_wiring_01/n18_a2_regress/`へコピーした上で本修正後の`stage_assemble_a2()`を実行した。結果はduration 351.824秒・peak 0.93346・`headroom_safety_valve.applied=false`(閾値0.98未満のため無変更)、出力wavのsha256は`97034b358e082271b87d67b42e1ecbfb3f4d4180058e1b1bd2d4508efc71e7f9`でユーザー承認済み資産と完全一致(安全弁が閾値以下では何もしないことを実データで確認)。

**状態**: `PRODUCTION_WIRED`(§Gate3相当の全条件クリア: ユーザー正式採用済み、A2/B1初回Production Assembly実装済み・単一箇所配線[retry/reassembly/regenerationと共有]、Trial-onlyでない、runtime evidence[Trial-13発火・No.18閾値以下byte一致]あり、新規テスト6件+既存回帰36件+プロジェクト全体2078/2081 PASS[残り3件は既知無関係]、`CURRENT_SPEC.md`/`OPEN_ITEMS.md`更新、Git反映、approved specとProduction挙動一致)。OPEN-115は`RESOLVED / PRODUCTION_WIRED`へ更新。

**今回実施しなかったこと**: `max_peak`(0.95)自体の変更、`mono_24k_to_stereo_target()`等共通関数の改修、LUFS mastering導入、Preview経路への個別上限追加、TTS/ASRの再生成、No.18本番資産(`pool_n18_notifications_specfix_v2`・`_ec_a_precision_21r`とも)への書き込み(regressionは別出力先`er011_output/assembly_headroom_wiring_01/n18_a2_regress/`のみ)、OPEN-116(ASR Validator数字保護ゲート・Key Phrase gloss規約)への着手(並行タスクの管轄)。

**根拠レポート**: `ER-011-ASSEMBLY-HEADROOM-SAFETY-VALVE-PRODUCTION-WIRING-01_REPORT.md`、`OPEN-112-TREND-THEME2-B-A2-PEAK-MEASUREMENT-16_REPORT.md`、`OPEN-112-TREND-THEME2-B-A2-B1-FULL-AUDIO-TRIAL-13_REPORT.md`
...
## KEYPHRASE-EN-ASR-FALSE-REJECTION-CASCADE-PROD-WIRING-01(2026-09-06、ユーザーが2026-09-06に対策(c)[Primary ASRへの逐語書き起こしprompt + 非ラテン文字時のSecondary再判定Cascade]を`APPROVED_FOR_PRODUCTION`と正式決定し、英語Key Phrase Component経路限定で`PRODUCTION_WIRED`まで配線。OPEN-119を`RESOLVED / PRODUCTION_WIRED`へ更新)

**背景**: OPEN-119(`KEYPHRASE-EN-TTS-ROOTCAUSE-DIAGNOSTIC-01`)で確定した「TTSは英語で正しく発話しているのに、Primary ASR(OpenAI`gpt-4o-mini-transcribe`、`language=en`)が「新常態」等の非英語文字列へ意味変換しfalse rejectする」問題に対し、`KEYPHRASE-EN-ASR-FALSE-REJECTION-CASCADE-TRIAL-01`(ユーザー承認済み隔離Trial)が対策候補(a: 非ラテン文字主体判定によるSecondary Cascade拡張、b: OpenAI ASRへのprompt付与、c: a+b併用)を検証し、false rejection率(陽性7件)がbaseline 57.1%→(a)14.3%→(b)0.0%→(c)0.0%、陰性対照3件でfalse accept 0件という結果を得ていた。ユーザーが対策(c)の採用を、**適用範囲を英語Key Phrase経路に限定**する条件で正式承認した。

**適用範囲の限定方法**: 新規機能は全て「既定None/False、明示的に渡した呼び出し元だけが有効化される」引数追加として実装し、既存の全呼び出し元(本文segment・Point見出し・Preview・Comment等)は無変更のまま維持した。
1. `er006_asr_provider_routing_01.py::_transcribe_openai_mini()`/`transcribe()`へ`prompt`引数(既定`None`)を追加。渡した場合のみOpenAI APIの`audio.transcriptions.create()`へ`prompt`kwargとして転送する。
2. `er003_v1_repro01_main_generate.py::generate_narration_snippet_verified_strict()`へ`asr_prompt`(既定`None`)・`enable_non_latin_cascade`(既定`False`)引数を追加。`language=="en"`の場合のみ`asr_prompt`を`routing.transcribe()`へ転送する(`language=="ja"`の場合は`asr_prompt`の値に関わらず常に`prompt=None`で呼ぶ、テストで確認済み)。
3. `generate_key_phrase_component_verified()`のPrimary(Minimal instruction)・Fallback(English language lock)両呼び出しのみが、新規定数`KEY_PHRASE_EN_ASR_NO_TRANSLATE_PROMPT`(Trialで検証した文言"The audio is spoken in English. Transcribe it verbatim in English, exactly as spoken, using English spelling. Do not translate it, and do not write it in Japanese, Chinese, or any other language or script."を一字一句複製、canonical phrase自体は含めない)と`enable_non_latin_cascade=True`を明示的に渡す。この関数は`ensure_key_phrase_english_component()`(Master Audio Store経由)を含む全ての英語Key Phrase Component生成の単一Production入口であるため、この1箇所への配線でPrimary初回・Minimal→English lock fallback・REGENERATE_APPROVED後の再生成を含む全経路をカバーする。

**Cascade実装(条件a)**: `er006_secondary_asr_01.py`へ`non_latin_dominance_info()`/`is_non_latin_dominant_mismatch()`を新設した(Trialの判定式をそのまま移植: ラテン文字[基本ラテン+主要発音区別符号]とCJK/かな/ハングル文字をそれぞれ数え、`CJK文字数/(ラテン文字数+CJK文字数) >= NON_LATIN_DOMINANT_THRESHOLD`[=0.5、両方0ならFalse]を「非ラテン文字主体」と判定)。`evaluate_attempt_with_cascade_detail()`へ`enable_non_latin_cascade`引数(既定`False`)を追加し、`not verified and cascade_enabled and enable_non_latin_cascade and is_non_latin_dominant_mismatch(asr_text)`の場合のみ、既存Secondary ASR関数(`get_full_text_via_azure_stt_with_phrase_list`、Azure en-US、既存実装のまま再利用)を1回呼び、その結果を既存`val.classify_asr_match()`で再判定する。合格すれば`verified=True`へ更新、不合格なら`cls`は元のPrimary#1判定のまま以降の既存`cascade_eligible`(`entity_like`/`homophone_candidate`)判定へそのまま処理を継続する(=reject維持、既存Cascade条件・数字ゲート・否定ゲート・homophone判定・Connected Speech Validatorの判定順序は一切変更していない)。`evaluate_attempt_with_cascade()`(3-tuple互換の公開ラッパー)へは`detail_out`引数(既定`None`)を追加し、dictを渡すとCascade詳細全体([steps]/[cascade_invoked]/[non_latin_cascade_invoked]等)をin-placeで取得できるようにした(既存の戻り値tuple形状・全既存呼び出し元の挙動は無変更)。

**attempt保存への記録**: `generate_narration_snippet_verified_strict()`内の`save_tts_attempt_audio()`呼び出しのmetadataへ`asr_prompt_applied`/`non_latin_cascade_enabled`/`non_latin_cascade_invoked`/`cascade_invoked`/`cascade_steps`(Secondary raw出力を含む)を追加した。英語Key Phrase Component経路以外は`asr_prompt`/`enable_non_latin_cascade`が既定のまま渡されないため、これらのフィールドは常にFalse/Noneのまま記録され、適用範囲限定の裏付けとして監査可能になっている。

**Master Audio Store cache identityへの影響**: 無し。`ensure_key_phrase_english_component()`が使う`MasterAudioKey`(language/speaker_voice/tts_model_id/canonical_text/level/style_instruction_id/style_instruction_version)はASR設定を含まない設計であり、ASR側の変更(prompt付与・Cascade拡張)は生成される音声の内容自体に影響しないため、cache identityの定義を変更する必要はないと判断した(理由をここに明記)。

**テスト・回帰**: 新規テスト16件PASS。(1)`er006_secondary_asr_01_test.py`へ9件追加: 非ラテン文字閾値境界(ratio=0.5でTrue/0.4でFalse)、`enable_non_latin_cascade`既定Falseなら非ラテン文字+不一致でもSecondary不発動、Secondary一致でPASSへ再判定、Secondary不一致でreject維持、ラテン文字のみの誤表記(cashless型「Káslis」)では非ラテン文字判定の対象外のため不発動(Trial §4の既知の限界の再現)、`KEYPHRASE-EN-ASR-FALSE-REJECTION-CASCADE-TRIAL-01`陰性対照3件(「新常態」→Secondary`Xinjio Tai.`/「デフォルト」→Secondary`Defaulto.`/「new formal」→非ラテン文字でないため不発動)の実raw出力をfixture化しfalse accept 0件を確認、`detail_out`動作確認2件。(2)`er006_asr_provider_routing_01_test.py`へ2件追加: prompt既定Noneなら`create()`kwargsに`prompt`が一切含まれない、明示指定時のみ転送される(既存の`test_japanese_routes_to_azure`は本タスクと無関係な既存の失敗[ja routingが実際にはOPEN-119より前のER-007でopenai_asrへ変更済みなのにテスト記述が更新されていない、本タスク開始前から同一の失敗を確認済み]のため、新規2件は個別に直接実行してPASSを確認した)。(3)新規`er011_keyphrase_en_asr_false_rejection_cascade_prod_wiring_01_test_01.py`5件: 適用範囲確認(既定呼び出しは`prompt=None`/`enable_non_latin_cascade=False`のまま・明示呼び出しのみ転送・日本語分岐は`asr_prompt`を渡しても常に無視)、`generate_key_phrase_component_verified()`のPrimary/Fallback両方が実際に新規引数を下流へ渡していることの配線確認。既存回帰: `er006_preprod_hardening_01_validation_test.py`57件・`er008_asr_variant_hardening_15_homophone_en_test.py`6件・`er006_secondary_asr_01_test.py`既存20件・`er011_human_review_lock_01_test_01.py`18件・`er011_tts_attempt_audio_retention_wiring_01_test.py`9件(新規kwargs追加に伴い既存テストのmock関数signatureへ`enable_non_latin_cascade`/`detail_out`を追加、これは私の変更が既存の固定シグネチャmockを壊した回帰でありテスト側を修正して解消した)、いずれもPASS。`er011_no18_connected_speech_reading_resolver_wiring_08_test.py`15件(1件[実LLM呼び出しを伴うReading Resolver item 9]は非決定性により1回目FAIL・2回目PASSを確認、本タスクの変更対象外のモジュールのため無関係と判断)。`run_project_regression.py`は変更前(git stashでのbaseline)collected=2104/passed=2101/failed=3、変更後collected=2109(新規5件追加)/passed=2106/failed=3で、失敗3件はいずれも本タスク以前から存在する既知の無関係failure(`er003_test_bad.FixtureTests.test_case_0`[意図的なself-check fixture]・`er003_test_p2j_investigate`のOPEN-77既知meta-test集計3件)であり、baseline/candidateで完全に同一であることを確認した(新規failureゼロ)。

**Runtime evidence(Standard同期、実API・小額課金)**: `er011_kp_en_asr_false_rejection_prod_wiring_01_runtime_evidence.py`(`TTS_EXECUTION_MODE=STANDARD`)。陽性2件: 実Production経路(`generate_key_phrase_component_verified`)で「new normal」「cashless」を実際にTTS生成し、いずれもPrimary ASR(prompt付き)がattempt1で正しい英語書き起こし("New Normal"/"Cashless")となりPASS(Cascade不要、`status=OK`)。陰性1件: Trialで生成済みの既存音声(`kp_en_asr_false_rejection_cascade_trial_01/audio/neg_ja_shinjoutai.wav`、日本語TTSで「新常態」を発話)を再利用し、実際のProduction Cascade関数(`evaluate_attempt_with_cascade`、`enable_non_latin_cascade=True`)へ直接投入した結果、prompt付きPrimaryでも`心状態`(非ラテン文字優勢)のままのためSecondary Cascadeが実発動し、Secondary(Azure)も`Xinjio Tai.`で不一致のままreject維持(`verified=False`、`final_classification=TTS_FAILURE`)を確認した。実測cost: Gemini TTS2回・OpenAI ASR3回・Azure Secondary ASR1回、いずれも短い音声のため合計数円程度(STOP閾値を大きく下回る)。出力先`er011_output/kp_en_asr_false_rejection_prod_wiring_01/`(`evidence_result.json`・`raw_usage_log.jsonl`・`player.html`)。

**根拠レポート**: `KEYPHRASE-EN-ASR-FALSE-REJECTION-CASCADE-PROD-WIRING-01_REPORT.md`、`KEYPHRASE-EN-TTS-ROOTCAUSE-DIAGNOSTIC-01_REPORT.md`、`KEYPHRASE-EN-ASR-FALSE-REJECTION-CASCADE-TRIAL-01_REPORT.md`
...
## PM-CLOSEOUT-CONSOLIDATION-42: 3V Person-Voice Trial-01結果のSSOT反映

Sonnet(sonnet-worker)が2026-09-09、Fable(PM)からの委任に基づき、
`EDITORIAL-B-FAMILY-VOICES-3V-PERSON-VOICE-TRIAL-01`(3人物版perspective_map
[応募する一人の人/採用担当・HM/経営者=責任を持つ人物]、Fairness/Legalは
Tensionの外部制約へ、Ledger不変・再割当のみ)の結果をSSOTへ反映した。

**Trial-01結果の要点**: attempt1=OK/PASS、attempt2=OK/REVIEW_REQUIRED、
attempt3=NG_REVIEW_REQUIRED(既存安全装置Local Rewrite 3サイクル後も
`human_review_required`1件が残存)。**仮説は支持された**(経営者Voiceの
Analytical Leakage flagが4V Trial-02[3 attempts全てFAIL]と対照的に、本Trial
のattempt1・attempt2ではいずれも0件)。一方で新しい相互作用効果を発見した:
人物化により未裏付けの個人claimが増え、それがLedger Deviation Checkerの
MAJOR判定→Local Rewriteのhedging表現を誘発し、そのhedgingがnarrator調の
Leakageを別経路から再導入する。Tension外部制約統合基準
(`leak_tension_constraint_integration`)は3/3で未達成、語数530語・推定尺
約412秒(3V目標325〜355秒を超過)、2対1構図は非検出、Pairwise Voice
Distinctness Check実績はdirection/method agreement=0.933/0.933(4V Trial-02と
同水準)、費用¥93.63(量産概算¥90〜100/記事)。Fact Checker A'のweb search
実測(6回/11回/10回)によりLedger Deviation Checker自体のweb search呼び出し
0回を本Trialでも確認し、4V Trial-02 Reportの誤記帰属を訂正した。Gate1=
`USER_DECISION_REQUIRED`、Gate4 PASS、STOPなし。

**新規発見(記録のみ)**: 物理命名`point_one/two/three`(Production 2V命名の
1段拡張)とQAスキーマ`voice_1..3`の命名乖離を確認した(4Vの`voice_1..4`
命名とは異なる設計選択)。

**Fable判定**: 「自明な修正は自律」の範囲内で、承認済み設計の適用
(Tension構造の明示・目標尺からの語数配分・体験claimのLedger根拠付け)による
Trial-02をすでに起動済み(並列稼働中、結果は別途追記予定)。

**SSOT反映**: `OPEN_ITEMS.md`OPEN-120行(Trial-01結果、仮説支持、未達3点、
相互作用効果、Trial-02実施中、Distinctness実績2件目)・OPEN-136行(3V
Trial-01でもLDC検索0・Fact Checker A' 6/11/10を確認、4V-02 Report誤記の
訂正済み)・OPEN-132行(命名乖離の統一をPhase 2項目に追加)へ追記した。
`docs/pm/MODEL_ROUTING_TRIAL_LOG.md`へTrial-01を確定値(Sonnet、HIGH、
319k token・3132s、差し戻し0)へ更新し、Trial-02(Sonnet、HIGH継続)・
本タスク(Sonnet、LOW)を新規行として追記した。

**根拠**: `EDITORIAL-B-FAMILY-VOICES-3V-PERSON-VOICE-TRIAL-01_REPORT.md`。
Git操作: `er012_editorial_b_voices_3v_person_voice_trial_01.py`・同Report・
`er012_output/editorial_b_voices_3v_person_voice_trial_01/`配下一式
(json/md/txt/jsonlのみ、wavなし)をG1、`OPEN_ITEMS.md`・`DECISION_LOG.md`・
`docs/pm/MODEL_ROUTING_TRIAL_LOG.md`をG2としてファイル名指定でcommitし
`origin/main`へpush。並列稼働中のTrial-02成果物、Lane A(`er011_*`)3件、
`CURRENT_SPEC.md`、`er006_output/`、`er011_output/attempt_history.jsonl`、
