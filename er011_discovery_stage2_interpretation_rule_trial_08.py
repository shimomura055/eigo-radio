...
# FAMILY-A-DISCOVERY-STAGE2-INTERPRETATION-RULE-TRIAL-08 (Lane A, D2-UDR-1再改善)
# ============================================================
# 目的(ユーザー決定 2026-09-09、D-1=(a)): Discovery/Why Layer3 Focus
# Module(Trial-05 DISCOVERY_FOCUS_MODULE_BLOCK、断定回避規則:161-165
# 相当)が「解釈を押しすぎる」問題について、Part A(¥0机上分析、別途)で
# 特定した「既存規則の具体性不足(scope-generalizationを明示的に禁止
# していない)」という仮説に基づく、Trend Synthesis Focus Module
# (:385-393、証拠の範囲を超えた一般化禁止の既存文言)と同型の1文追加のみ
# の最小調整版Focus Moduleを、現行版(Household Ledger v4使用に更新)と
# 比較するArticle-only N=3 Trial。
#
# **Trial(Production実装ではない)**。Production/Prompt/共有module/
# registry/SSOT編集・Ledger改変・Fact Checker緩和・Git操作は一切行わない。
# TTSは実行しない(text-onlyまで)。Role文字列分類は行わない(D-2=(a)、
# Point本文の目視比較に切り替え)。
#
# ------------------------------------------------------------
# 再利用(import・無変更):
...
#     Ledger v4を含む最新verified_fact_ledger.txtをそのまま参照)/
...
#     DISCOVERY_FOCUS_MODULE_BLOCK本文(段落)をそのまま再利用し、見出し
#     行のみ改稿(Trial-07と同型)。本体は一字一句不変(current_focus
#     条件)。adjusted_focus条件は、この不変本体の断定回避段落
#     (:161-165相当)へ1文だけ追加する(Trend Synthesis Focus Module
#     :389-390と同型の文言を再利用、新原則の追加ではない)。
#   - er005_cost_logger(cl): install / logging_context。無変更。
#
# 費用上限: ¥100(超過見込みならN=2へ縮小、それでも超過ならSTOP。
# Trial-07の実測費用[discovery_focus条件 平均¥12.68/本]を踏まえ、
# run_idxごとにバッチ実行し、都度cumulative費用を確認しながら進める)。
...
THEME_ID = "discovery_stage2_interpretation_rule_trial_08"
OUT_DIR = f"er011_output/{THEME_ID}"

N_RUNS_PLANNED = 3  # ユーザー指定。費用状況に応じてrun_idx=3をスキップする場合がある(下記run_budget_guard参照)。
BUDGET_JPY = 100.0

HOUSEHOLD_THEME = next(t for t in prod_gen.THEMES if t["theme_id"] == "household")
HOUSEHOLD_TOPIC_JA = HOUSEHOLD_THEME["topic"]
HOUSEHOLD_LEDGER_PATH = HOUSEHOLD_THEME["ledger_path"]  # v4(2026-09-09、FACT-03修正済み)を直接参照
...
# ------------------------------------------------------------
# current_focus: Trial-05本体をそのまま再利用、見出しのみTrial-08向けに
...
CURRENT_HEADER_LINE = (
    ...
    "RULE-TRIAL-08でHousehold Ledger v4(FACT-03修正版)によるN=3再検証中。Production未採用。"
...
# ------------------------------------------------------------
# adjusted_focus: Part A(¥0分析)案1(scope-clause追加)。断定回避段落
# (現行:161-165相当)の末尾へ、Trend Synthesis Focus Module:389-390
# 「証拠が一部の当事者・一部の期間・一部の地域にしか及ばない場合は、
# その範囲を実際より広く一般化しないでください」と同型の1文を追加する
...
ADJUSTED_HEADER_LINE = (
    "【Discovery/Why Focus(記事タイプ固有の焦点、最小調整版[Part A案1、未承認候補]。"
    "既存の断定回避段落の末尾へscope-generalization禁止の1文のみ追加[Trend Synthesis Focus "
    "Module:389-390と同型の文言を再利用]、他は現行版[Trial-05/Trial-07]と一字一句同一。"
    "FAMILY-A-DISCOVERY-STAGE2-INTERPRETATION-RULE-TRIAL-08でHousehold Ledger v4による"
    "N=3検証中。Production未採用。editorial_mode=\"discovery_why\"は未登録の想定名)】"
)
...
# ------------------------------------------------------------
# claim→evidence id 機械分類用キーワード(Trial-07から再利用、無変更)。
# ------------------------------------------------------------
...
def build_run_metadata() -> dict:
    return {
        "management_id": "FAMILY-A-DISCOVERY-STAGE2-INTERPRETATION-RULE-TRIAL-08",
        "mode_supply_path": "手動Mode判定+手動Ledger供給(Trial-07と同じ機構)。",
        ...
        "two_axis_determination": {
            ...
            "result": "DISCOVERY_WHY(Household、Trial-07と同一)。",
        },
        "ledger_version": "v4(2026-09-09、HOUSEHOLD-LEDGER-FACT-03-REVERIFICATION-01[OPEN-138]で"
                          "FACT-03[イチゴ・柑橘類の高湿度家庭用設定]をusable: noへ修正済み)。",
        "role_string_classification": "実施しない(D-2=(a)、Role文字列のヒューリスティック分類は廃止し、"
                                       "Point本文の目視比較[comparison.html]へ切り替え)。",
    }


# ============================================================
# 費用実測ヘルパー(Trial-07と同一の参照元・同一ロジック)。
# ============================================================
USD_JPY = 160.0
...
def write_cost_summary() -> dict:
    ...
    result = {
        ...
        "methodology": "全て実測usage(actual)。単価はer005_output/cost_baseline_01/"
                       "pricing_snapshot.json(OFFICIAL_SOURCE、Trial-07と同一参照元・同一ロジック)。"
...
if __name__ == "__main__":
    ...
    if which == "setup":
        ...
        print("usage: python er011_discovery_stage2_interpretation_rule_trial_08.py "
