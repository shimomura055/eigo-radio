# ============================================================
# er011_daily_news_focus_layer_comparison_trial_02_cost_compute.py
# FAMILY-A-DAILY-NEWS-FOCUS-LAYER-COMPARISON-TRIAL-02 (Lane A-1) 補助
# ============================================================
# raw_usage_log.jsonl(cl.install()による全API call実測)を、公式pricing
# snapshot(er005_output/cost_baseline_01/pricing_snapshot.json、既存
# er011_no18_cost_compute_01.pyと同一参照元・同一算出ロジック)で集計する。
# theme タグ(daily_news_focus_layer_comparison_trial_02_baseline /
# _focus)でbaseline/focus条件別に費用を分離する。
...
result = {
    ...
    "methodology": "全て実測usage(actual)。単価はer005_output/cost_baseline_01/pricing_snapshot.json"
                   "(OFFICIAL_SOURCE、er011_no18_cost_compute_01.pyと同一参照元・同一ロジック)。"
                   "text-onlyのため provider=openai (Writer/FactCheck/LedgerDeviation/PointOverlapQA/"
                   "PointRolePlanning等) のみを想定。TTS/ASRのproviderが記録されていた場合は"
                   "ValueErrorで停止する(想定外API callの見落とし検出)。",
