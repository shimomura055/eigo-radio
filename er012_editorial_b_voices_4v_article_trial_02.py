# ============================================================
# er012_editorial_b_voices_4v_article_trial_02.py
# 管理ID: EDITORIAL-B-FAMILY-VOICES-4V-ARTICLE-TRIAL-02
# ============================================================
# Lane: Lane B(Lane A・SSOT統合タスクとは完全に独立)。**Trial専用**
# (Production/Trial-07/registry/Contract編集禁止、SSOT・Git操作禁止)。
# 並列稼働中: Governance追記+SSOT統合タスク(SSOT・Git担当、本ファイルとは
# 無関係)。
#
# 本ファイルは`er012_editorial_b_voices_4v_article_trial_01.py`(前タスク、
# 保持・無変更)の経路をそのまま複製した新規ファイルであり、Trial-01ファイル
# 自体は一切変更していない。ユーザー決定(2026-09-09、Trial-01 Reportの
# STOP 2件を承認済み範囲内の改善として実施)を受け、Trial-01からの変更点は
# 以下の2点のみ:
# (a) B_FAMILY_VOICES_4V_FOCUS_MODULE_BLOCKへ、Voiceセクション本文を一人称
#     "I"で書く指示を明示的に追加した(新原則の創作ではなく、ユーザーが既に
#     2026-09-08に承認済みの原則[DECISION_LOG PM-CLOSEOUT-CONSOLIDATION-05
#     「(4) Voice A/Bの一人称"I"記述」をAPPROVED_FOR_PRODUCTIONと決定]を、
#     この記事のVoiceセクションへそのまま適用したもの。Hook/Tension/
#     Closingの人称指示は変更していない。詳細はReport §1 Reconciliation
#     参照)。
# (b) 既存のAnalytical Leakage Check是正メモ経路(`run_analytical_leakage_
#     check_4v`+`build_leakage_corrective_note_4v`、`run_pipeline_4v`内の
#     MAX_WRITER_ATTEMPTS=3ループ)をそのまま再利用し、是正フィードバックで
#     再試行する(Trial-01と同一、新しいVoice別語り口指示は追加していない)。
# それ以外(Voice Card・Ledger・Fact Checker A'・Ledger Deviation+Local
# Rewrite・Distinctness Check・Overlap Monitoring/Controls・Comment 1〜4・
# required_structure 4V review等)はTrial-01と同一のロジックをそのまま
# 再利用する。Trial-01の再開専用関数(`run_writer_stage_resume_from_
# interrupted_attempt3`、前回セッションの中断固有の対応)は本Trial-02では
# 不要のため含めていない(中断が発生した場合は改めてSTOPし報告する)。
#
...
#
# 本ファイルの土台自体は`er012_editorial_b_voices_trial_07.py`(2 Voices、
# 5区切り)のWriter経路(`run_voices_pattern_run03`等)であり(Opusレビュー
# 指摘0-A: Trial-07自体は`er012_editorial_b_family_production_phase1_
# test_01.py`がProduction定数とのbyte一致をテストしているため編集禁止)、
# Trial-07本体は一切変更していない。Production(`er012_b_family_voices_
# production_01.py`・`er012_b_family_editorial_type_registry_01.py`・
...
# CONFIRMEDのため本Trialでは不採用、VALIDATED。Trial-01と同一Ledgerを
# そのまま再利用、新規Ledger作成なし)。本ファイルはこの既存Ledger・
# `perspective_map.md`を読み取り専用で参照するのみで、新規Research呼び
# 出しは行わない。
#
...
# 操作、3V作業、閾値変更、バックグラウンド待機、完了報告後の自動復帰。
#
# STOP条件: Reconciliationで一人称指示の出所が承認記録と矛盾/新しい語り口
# 指示や新原則の追加が必要/MAX_WRITER_ATTEMPTS(3)到達後もLeakage解消せず
# (その場合は結果を提示してSTOP、prompt原則追加の提案は未承認候補として
# 記載のみ)/費用上限(¥150)/Production・Trial-07・registryへの書込みが
# 必要/4V記事が賛否2対2化/新規failure mode。
#
...
THEME_ID = "editorial_b_voices_4v_article_trial_02"
OUT_DIR = "er012_output/editorial_b_voices_4v_article_trial_02"
...
def run_fact_check_a_prime_4v(article_text: str, ledger_text: str, out_dir: str) -> dict:
    ...
    print(f"[4V-ARTICLE-TRIAL-02] Fact Checker A'呼び出し開始(fact_attribution_mode_enabled={enabled})...")
    ...
    print(f"[4V-ARTICLE-TRIAL-02] Fact Checker A'完了。final_status={fc_record.get('final_status')} "
...
B_FAMILY_VOICES_4V_FOCUS_MODULE_BLOCK = """【B Family Voices/Perspective Focus Module(4 Voices版、EDITORIAL-B-FAMILY-VOICES-4V-ARTICLE-TRIAL-02。
Production未採用、この記事タイプ専用の骨格再定義。TRIAL-01[2 Voices版[Trial-07]を土台に4 Voicesへ拡張した骨格]を
そのまま踏襲し、Voiceセクションの人称指示のみ追加した）】
...
【人称(重要、この記事タイプで新規に明示。ユーザーが既に承認済みの原則を適用するもので
あり、新しい語り口ルールを追加するものではありません)】
4人のVoiceセクション(Voice 1〜4)の本文はすべて、その人物自身が"I"で語る一人称で書いて
...
def build_candidate_prompt(candidate_template: str, master_full_text: str, topic: str,
    ...
    # Reconciliation finding(Trial-01で発見済み、本Trial-02でも同じ対応を
    # 継続): gen.COMMON_BLOCK_TEMPLATEは
    # OPEN-112-TREND-SYNTHESIS-MODE-PRODUCTION-WIRING-01でTrial-07作成後に
    # `{editorial_type_module_block}` placeholderが追加されており、Trial-07の
    # build_candidate_prompt()と同じ`.format()`呼び出し方法だとKeyErrorになる
    # (Trial-07自体は無変更のまま、Production側の後方互換設計[gen.build_
    # common_block()の既定値editorial_type_module_block=""]に合わせてここでは
    # 明示的に""を渡す)。
...
def run_phase_a(audit_dir: str) -> dict:
    ...
    print(f"[4V-ARTICLE-TRIAL-02][Phase A] clean_single_insert_confirmed={clean_single_insert}")
...
def run_overlap_monitoring_4v(sections: dict, out_dir: str) -> dict:
    ...
    summary = {
        "qa_status": "OK",
        "note": ("EDITORIAL-B-FAMILY-VOICES-4V-ARTICLE-TRIAL-02: monitoring専用(合否判定には"
    ...
    print(f"[4V-ARTICLE-TRIAL-02] Overlap monitoring(16値)完了。any_flagged={summary['any_flagged']}")
    return summary
...
def run_analytical_leakage_check_4v(client, sections: dict, model: str, reasoning_effort: str,
    ...
    print(f"[4V-ARTICLE-TRIAL-02][Leakage Check] attempt{attempt}: any_flagged={result['any_flagged']} "
...
def run_ledger_deviation_and_local_rewrite(client, theme_id: str, label: str, article_text: str,
    ...
    print(f"[4V-ARTICLE-TRIAL-02][{theme_id}] {label}: ledger逸脱チェック開始(Hook-aware)...")
    deviation_result = vfl01.run_deviation_check(
        client, verified_ledger_text, article_text, model=ledger_model, hook_aware=True)
    print(f"[4V-ARTICLE-TRIAL-02][{theme_id}] {label}: deviation overall_status="
    ...
    while major_items and cycle < local_rewrite.MAX_REWRITE_CYCLES:
        ...
        print(f"[4V-ARTICLE-TRIAL-02][{theme_id}] {label}: Local Rewrite cycle {cycle}/"
        ...
        for idx, deviation in enumerate(major_items, start=1):
            ...
            print(f"[4V-ARTICLE-TRIAL-02][{theme_id}] {label}: cycle {cycle} NG item {idx}: "
        ...
        print(f"[4V-ARTICLE-TRIAL-02][{theme_id}] {label}: cycle {cycle} Local Rewrite後、Ledger全体を再判定...")
        ...
        print(f"[4V-ARTICLE-TRIAL-02][{theme_id}] {label}: cycle {cycle} 再判定 overall_status="
...
# ============================================================
# Reconciliation finding(Trial-01で発見済み[B-4V-3、OPEN-132で追跡中の
# 迂回]、本Trial-02でも同じ迂回を継続、未解決のまま変更していない):
...
# 素通りしていたが、4Vは###がちょうど4つになるため必ず弾かれる、Trial-01で
# 実測確認済み: attempt1でstatus=STRUCTURE_INVALID_POINT_COUNT_OR_BODY・
...
def _generate_and_compress_article_4v(client, theme_id: str, label: str, prompt: str, out_dir: str,
    ...
    if raw_result is None:
        print(f"[4V-ARTICLE-TRIAL-02][{theme_id}] {label}: writer技術的失敗(通信障害等)。")
    ...
    if apply_evidence_compression:
        with open(f"{out_dir}/audit/pre_editor_article.md", "w", encoding="utf-8") as f:
            f.write(article_text)
        print(f"[4V-ARTICLE-TRIAL-02][{theme_id}] {label}: Evidence Compression(Lossless Editor)呼び出し開始...")
        ...
        print(f"[4V-ARTICLE-TRIAL-02][{theme_id}] {label}: Evidence Compression完了。"
...
def run_voices_pattern_4v(client, theme_id: str, label: str, prompt: str, verified_ledger_text: str,
    ...
    print(f"[4V-ARTICLE-TRIAL-02][{theme_id}] {label}: Overlap monitoring(16値)開始...")
    ...
    print(f"[4V-ARTICLE-TRIAL-02][{theme_id}] {label}: metrics={metrics} seven_section_report={seven_section_report}")
    ...
    if verdict == "FAIL":
        print(f"[4V-ARTICLE-TRIAL-02][{theme_id}] {label}: Fact CheckerがFAILと判定。NG_REVIEW_REQUIREDとして報告します。")
        return {
    ...
    if ledger_result["remaining_major_count"] or ledger_result["any_human_review_required"]:
        print(f"[4V-ARTICLE-TRIAL-02][{theme_id}] {label}: Local Rewrite cycleを尽くしてもLedger MAJOR残存/"
    ...
    if apply_directional_fact_precheck:
        print(f"[4V-ARTICLE-TRIAL-02][{theme_id}] {label}: 比較方向Fact事前チェック開始(rule-based、¥0)...")
        ...
        print(f"[4V-ARTICLE-TRIAL-02][{theme_id}] {label}: 比較方向Fact事前チェック完了。overall_status={directional_precheck_status}")
...
def run_pipeline_4v(client, theme_id: str, label: str, base_prompt: str, verified_ledger_text: str,
    ...
    for attempt in range(1, MAX_WRITER_ATTEMPTS + 1):
        ...
        print(f"[4V-ARTICLE-TRIAL-02] Writer attempt {attempt}/{MAX_WRITER_ATTEMPTS} 開始(out_dir={attempt_dir})...")
        t0 = time.time()
        ...
        if result.get("status") != "OK" or not result.get("article_text"):
            ...
            print(f"[4V-ARTICLE-TRIAL-02] attempt {attempt}: status={result.get('status')}のためLeakage Checkをスキップします。")
            break
        ...
        if sections is None:
            ...
            print(f"[4V-ARTICLE-TRIAL-02] attempt {attempt}: 7区切り構造が検出できずLeakage Checkをスキップしました。")
            break
        ...
        if not leakage["any_flagged"]:
            print(f"[4V-ARTICLE-TRIAL-02] attempt {attempt}: Analytical Leakage Check flagged項目なし。確定。")
            break
        if attempt == MAX_WRITER_ATTEMPTS:
            print(f"[4V-ARTICLE-TRIAL-02] attempt {attempt}: 最大attempt数に到達。flagged項目が残った状態の"
...
def run_writer_stage() -> dict:
    ...
    if not phase_a["phase_a_pass"]:
        print("[4V-ARTICLE-TRIAL-02] Phase Aで意図しない差分を検出したため、Writerへ進まずSTOPします。")
    ...
    print(f"[4V-ARTICLE-TRIAL-02] Writer + Analytical Leakage Checkパイプライン開始(最大{MAX_WRITER_ATTEMPTS} attempts)...")
    ...
    print(f"[4V-ARTICLE-TRIAL-02] 完了。total_attempts={pipeline_result['total_attempts']} "
...
def run_comments_1_and_4(client, sections: dict, out_dir: str) -> dict:
    ...
    with open(f"{out_dir}/comments_1_to_4.json", "w", encoding="utf-8") as f:
        json.dump(result, f, ensure_ascii=False, indent=2, default=str)
    print(f"[4V-ARTICLE-TRIAL-02][Comments] comment_1={c1['text'][:60]!r} comment_4={c4['text'][:60]!r}")
    return result
...
def run_distinctness_check_full(client, sections: dict, out_dir: str) -> dict:
    ...
    for a, b in itertools.permutations(voice_keys, 2):
        ...
        print(f"[4V-ARTICLE-TRIAL-02][Distinctness] directed {key}: "
    ...
    summary = {
        "note": ("EDITORIAL-B-FAMILY-VOICES-4V-ARTICLE-TRIAL-02: Pairwise Voice Distinctness Check"
    ...
    with open(f"{out_dir}/pairwise_voice_distinctness_check.json", "w", encoding="utf-8") as f:
        json.dump(summary, f, ensure_ascii=False, indent=2, default=str)
    print(f"[4V-ARTICLE-TRIAL-02][Distinctness] 完了。direction_agreement_rate={direction_agreement_rate} "
          f"method_agreement_rate={method_agreement_rate} directed_elapsed={directed_elapsed}s batch_elapsed={batch_elapsed}s")
    return summary


# ============================================================
# 既存2V記事参照(Reconciliation finding、Trial-01で発見済み、本Trial-02
# でも同じ参照先を継続):
...
def run_overlap_controls(sections: dict, out_dir: str) -> dict:
    ...
    with open(f"{out_dir}/overlap_qa_controls.json", "w", encoding="utf-8") as f:
        json.dump(summary, f, ensure_ascii=False, indent=2, default=str)
    print(f"[4V-ARTICLE-TRIAL-02][Overlap Controls] positive={positive_result['overlap_ratio']} "
...
def build_word_count_and_duration_estimate(sections: dict, out_dir: str) -> dict:
    ...
    with open(f"{out_dir}/word_count_and_duration_estimate.json", "w", encoding="utf-8") as f:
        json.dump(result, f, ensure_ascii=False, indent=2)
    print(f"[4V-ARTICLE-TRIAL-02][Duration Estimate] variable_word_count_4v={variable_word_count_4v} estimate={estimate}")
    return result
...
def review_required_structure_4v_trial() -> dict:
    ...
    print(f"[4V-ARTICLE-TRIAL-02][OPEN-129 review] {review}")
    return review


# ============================================================
# 一人称使用率(機械計測、¥0のrule-based monitoring。Gate・Leakage Checkの
# 判定には使わない。B_FAMILY_VOICES_4V_FOCUS_MODULE_BLOCKへ追加した一人称
# 指示[Voiceセクションのみ"I"、Hook/Tension/Closingは対象外]が実際に
# 満たされているかを、section別に文単位で機械測定する)。
# ============================================================
...
def build_first_person_usage_report(sections: dict, out_dir: str) -> dict:
    per_section = {}
    for label in SEVEN_SECTION_LABELS:
    ...
    result = {
        "note": ("機械計測(rule-based、¥0、正規表現`\\bI\\b`で文中に一人称'I'[I'm/I'veの"
                 ...
                 "QA monitoring専用。Voice 1〜4は一人称指示の対象、Hook/Tension/Closingは"
                 "対象外(三人称のまま、低い比率が期待値)。"),
    ...
    print(f"[4V-ARTICLE-TRIAL-02][First Person Usage] {ratios}")
    return result
...
def run_qa_stage() -> dict:
    ...
    if os.path.exists(distinctness_path):
        print("[4V-ARTICLE-TRIAL-02] pairwise_voice_distinctness_check.json既存のため再利用(再生成しない)。")
    ...
    print("[4V-ARTICLE-TRIAL-02] QA stage完了。")
    return qa_summary
