# ============================================================
# er011_point_overlap_gap_fix_trial_05_cost_compute.py
# FAMILY-A-POINT-OVERLAP-GAP-FIX-TRIAL-05 (Lane A) 補助
# ============================================================
# raw_usage_log.jsonl(cl.install()による全API call実測)を、公式pricing
# snapshot(er005_output/cost_baseline_01/pricing_snapshot.json、既存
# Trial-04 cost_computeと同一参照元・同一算出ロジック)で集計する。
...
def main():
    ...
    by_target_theme = defaultdict(float)
    ...
    for theme, cost in by_theme.items():
        ...
        if len(parts) == 2:
            ...
            # 形式: {target_theme}_{condition}_{level_dir}
            segs = cond_level.rsplit("_", 2)
            if len(segs) == 3:
                target_theme, condition, level = segs
                by_target_theme[target_theme] += cost
                by_condition[f"{target_theme}_{condition}"] += cost
                by_level[level] += cost

    result = {
        "usd_jpy_rate": USD_JPY,
        "methodology": "全て実測usage(actual)。単価はer005_output/cost_baseline_01/"
                       "pricing_snapshot.json(OFFICIAL_SOURCE、Trial-04 cost_computeと同一参照元・"
                       "同一ロジック)。text-onlyのためprovider=openai(Writer/FactCheck/"
                       "LedgerDeviation/PointOverlapQA/PointRolePlanning等)のみを想定。TTS/ASRの"
                       "providerが記録されていた場合はValueErrorで停止する(想定外API callの見落とし検出)。",
        "by_theme_jpy": {k: round(v * USD_JPY, 1) for k, v in by_theme.items()},
        "by_target_theme_jpy": {k: round(v * USD_JPY, 1) for k, v in by_target_theme.items()},
