# ============================================================
# er003_v1_repro01_preview_audio_generate.py
# ER-003-REPRO-01-PREVIEW-AUDIO: A02 Preview音声生成(初回1回のみ)
# ============================================================
# A01最終Preview(preview_japanese_only_short.wav)と完全に同一の生成
# 経路(er003_b1_p9a_audio.generate_narration_snippet、単一TTS call、
# ASR検証付きretryは使わない)を、承認済みA02台本へそのまま適用する。
# 新しいinstruction/style/後処理は一切追加しない。
#
# 実行方法:
#   .venv/Scripts/python.exe er003_v1_repro01_preview_audio_generate.py
...
MANAGEMENT_ID = "ER-003-REPRO-01-PREVIEW-AUDIO"
ARTICLE_ID = "A02"
...
def run() -> dict:
    ...
    generation_record = {
        ...
        "dynamics3_note": ("A01の最終Preview(preview_japanese_only_short.wav)の生成関数"
                           "(generate_narration_snippet)自体がDynamics3を呼び出していないことを"
                           "コード確認済み。同一関数をそのまま使用したため、A02でもDynamics3は"
                           "適用していない(推測で新規追加していない)。"),
        "path": out_path,
