# ============================================================
# er001b10_hanshin_common_spec.py
# ER-001B-10: 阪神への共通最終仕様適用テスト
# ============================================================
# 目的: ER-001B-9(老老介護)と対になる音声を阪神記事でも作る。
# Emotional+Connected+Level2 / Point One・Point Two / In One Line必須
# / セクション間0.8秒を適用した全文音声を1回だけ新規TTS生成し、
# その同一音源から無処理版(C0)とDynamics 3適用版(C1)を作る。
# C0とC1をそれぞれ別にTTS生成しない(生成ゆらぎを比較へ混入させない)。
#
# 背景: これまでの実験では、阪神は「Level2+Dynamics3」(er001b8、
# Point番号なし)と「Point番号あり」(er001b7b、Today's Tiger Points
# セクションのみの断片・Dynamics3なし)が別々にしか存在せず、
# 老老介護(er001b9)のような「共通最終仕様を適用した全文」が
# 存在しなかった。このスクリプトはその欠落を埋め、阪神・老老介護の
# 2本を同じ基準で比較できる状態にする(ER-002-S0調査での指摘に基づく)。
#
# 台本の正本(er001b5_hanshin_script.json)は変更しない。
...
# 計算式をそのまま再利用する(er001b9_caregiving_common_spec.pyと
# 同一実装、対象記事と話者のみ差し替え)。ER-001B-6/7A/7B/8/9の
# スクリプト・マニフェスト・既存音声には一切手を加えない独立スクリプト。
# 本番パイプライン(generate_test.py等)には影響しない。
#
# 使い方:
#   python er001b10_hanshin_common_spec.py
...
script = load_script(HANSHIN_SCRIPT_PATH)
script_sha256 = sha256_file(HANSHIN_SCRIPT_PATH)

body_section = next(s for s in script["sections"] if s["type"] == "body")
points_section = next(s for s in script["sections"] if s.get("heading") == "Today’s Tiger Points")
...
# チャンク2: Today's Tiger Points + Point One + 小見出し1本文 + Point Two + 小見出し2本文
CHUNK2_LINES = (
...
CHUNKS = [("body", CHUNK1_TEXT), ("Today’s Tiger Points", CHUNK2_TEXT), ("In One Line", CHUNK3_TEXT)]
...
print(f"検証: 正本JSON({HANSHIN_SCRIPT_PATH})は未変更。SHA-256: {script_sha256[:16]}...", flush=True)

# ============================================================
# ブロック2: 演技指示(ER-001B-9と一字一句同一。基本指示+Level 2+忠実性ルール)
# ============================================================
...
# ============================================================
# ブロック3: 道具(音量正規化・PCM<->WAV。ER-001B-6/9と同一)
# ============================================================
...
# ============================================================
# ブロック4: クライアント初期化・モデル設定(ER-001B-6/9と同一)
# ============================================================
load_dotenv()
...
SAMPLE_RATE = 24000
MAX_RETRY = 2
MAX_CONTENT_ATTEMPTS = 5  # ER-001B-9と同じ方針: 最大5回、5回とも不合格なら停止して報告
...
def record_call(label):
    entry = {
        ...
        "script": "er001b10_hanshin_common_spec.py",
        "pattern": label,
    }
...
2. today_points_count: how many times is "{points_section['heading']}" spoken (expected: exactly 1)?
...
12. numbers_changed: true if any number is spoken incorrectly or changed.

Return ONLY valid JSON, no other text, in exactly this shape:
{{"title_count": 1, "today_points_count": 1, "point_one_count": 1, "subheading1_count": 1, "point_two_count": 1, "subheading2_count": 1, "in_one_line_count": 1, "dropped_content": false, "duplicated_content": false, "non_english_or_extraneous_speech": false, "unauthorized_wording_changes": false, "numbers_changed": false, "notes": "brief explanation in English"}}"""
    ...
    for attempt in range(5):
        try:
            ...
            checks = {
                ...
                "today_points_count": result.get("today_points_count") == 1,
...
2. Report how many times each of these exact phrases is spoken as its own distinct spoken element: "{script['title']}", "{points_section['heading']}", "Point One", "{sub1['heading']}", "Point Two", "{sub2['heading']}", "In One Line".
3. State whether the transcript matches the approved source text exactly (any additions, omissions, or word substitutions?).

Return ONLY valid JSON, no other text, in exactly this shape:
{{"transcript": "...", "title_count": 1, "today_points_count": 1, "point_one_count": 1, "subheading1_count": 1, "point_two_count": 1, "subheading2_count": 1, "in_one_line_count": 1, "matches_source_exactly": true, "differences_note": "brief explanation in English"}}"""
...
print("ER-001B-10: 阪神への共通最終仕様適用テスト")
...
for attempt in range(1, MAX_CONTENT_ATTEMPTS + 1):
    print(f"[試行{attempt}/{MAX_CONTENT_ATTEMPTS}] 阪神 × {VOICE} × Level2+Point番号 を生成中...", flush=True)
    audio = b""
    chunk_records = []
    for j, (chunk_label, chunk_text) in enumerate(CHUNKS, 1):
        call_label = f"hanshin_{VOICE}_common_a{attempt}_c{j}_{chunk_label}"
    ...
    if passed:
        ...
        rejected_name = f"er001b10_hanshin_level2_numbered_raw_REJECTED_attempt{attempt}.wav"
...
if c0_audio is None:
    ...
    with open("er001b10_manifest.json", "w", encoding="utf-8") as f:
        json.dump(manifest_partial, f, ensure_ascii=False, indent=2)
    raise SystemExit(
        f"エラー: {MAX_CONTENT_ATTEMPTS}回すべて技術検品に不合格でした。"
        f"er001b10_manifest.jsonに経緯を記録して停止します。"
    )

# ============================================================
# ブロック8: C0保存
# ============================================================
C0_FILE = "er001b10_hanshin_level2_numbered_raw.wav"
C1_FILE = "er001b10_hanshin_level2_numbered_dynamics3.wav"
...
# ============================================================
# ブロック9: 客観指標・Dynamics 3(ER-001B-8/9と同一実装・同一パラメータ)
# ============================================================
...
LUFS_METHOD_NOTE = (
    ...
    "2段階ゲーティングで積分ラウドネスを近似計算したもの(ER-001B-7A/8/9と同一実装)。"
...
print("Dynamics 3(ER-001B-8/9と同一パラメータ)をC0へ適用中...", flush=True)
...
manifest = {
    "experiment_id": "ER-001B-10",
    "status": "OK",
    "purpose": "阪神記事への共通最終仕様(Emotional+Connected+Level2/Point番号/In One Line必須/0.8秒無音/Dynamics3)の適用テスト。ER-001B-9(老老介護)と対になる音声を作り、ER-002-S0調査で見つかった非対称を解消する。",
...
print()
print("-" * 50)
print("er001b10_manifest.json を保存しました。")
