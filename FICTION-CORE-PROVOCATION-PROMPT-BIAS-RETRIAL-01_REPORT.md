...
## §10 Fable評価(ブラインド評価: blind.md を先に読み、key開封後も判断不変)

(1)ブラインド段階のグルーピング: {P,T}=「記憶技術が未来の自分の殺人を見せる」同型 / Q=「交換した記憶の中で過去の殺人を目撃」(暴力+記憶技術だが予告ではない) / R=「幽霊市場で友情の記憶を売る」超自然・切ない / S=「卒業ビデオに映る忘れられた同級生、学校の事故隠蔽」技術なしの人間ドラマ。key開封後(P=Run5, Q=Run4, R=Run3, S=Run2, T=Run1)も不変。(2)発想タイプの数: 4種(P/T同型、Q、R、S)。旧Trial(群1=Run1/2/5、準群Run4、別物Run3の実質2〜3種)から改善。(3)Run2(最重要): 旧「学校のデジタル記憶ロッカー・明日付けの録音」→新「卒業ビデオと出席簿の白い帯・事故の揉み消し」。技術ギミック・未来要素・暴力予告のいずれも消え、Prompt Biasが実際にRun2を技術/未来へ誘導していたことがRun2の候補3案の変化からも裏付けられた。(4)残る収束: Run1/Run5(E=近未来/現実にない技術)は依然「記憶技術が自分の殺人を予告」同型、Run4も殺人目撃。E軸が未来/技術のときテーマ「記憶」と結びついて「記憶技術×暴力」へ寄る傾向は、Prompt Biasではなく E軸の値+テーマの組み合わせに由来する。(5)反復モチーフ: 主人公名 Mara が再びRun5に出現、ナイフ(Run1)、時刻指定(3:00/9:16/11:59)、silver box/band、題名重複「The Last Memory」(Run1/Run3)。(6)Story DNAの効き: Run2/Run3/Run4はDNAが核まで届いている。Run5のC=逆転は「偽記憶による冤罪」として旧より明確に効いた。(7)旧Trialとの違いはPrompt Bias除去だけで説明できる(seed/DNA完全一致を乾式確認、Run間独立は前回確認済み)。費用¥4.69。

## §11 分類

**VALIDATED**(Prompt Bias除去は有効。ただしこれは「Bias除去方式の評価」であり、Fiction familyのProduction採用でも仕様化でもない。Production/SSOT変更なし)。ユーザー判断事項: ①旧Trial(RANDOM-DNA-TRIAL-01)のREJECTEDは「Bias込みの結果」であったため、評価上の位置づけをどうするか(Fable推奨: REJECTEDのまま、ただし理由に「Prompt Bias混入」を付記)。②残る収束(E=未来/技術のRunが「記憶技術×暴力予告」へ寄る)と題名重複・Mara反復への対処を次Trialにするか(候補は前回提案の(a)負の記憶/(b)B軸必須/(c)Coverage反転/(d)反復禁止リスト。Fable推奨: まず(d)+題名重複の機械チェックのみを最小Trialとし、(a)(b)(c)は保留)。③Fiction familyの本実装検討をいつ始めるか(Fable推奨: ②の結果後)。
