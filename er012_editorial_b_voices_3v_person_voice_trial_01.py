# ============================================================
# er012_editorial_b_voices_3v_person_voice_trial_01.py
# 管理ID: EDITORIAL-B-FAMILY-VOICES-3V-PERSON-VOICE-TRIAL-01
# ============================================================
# Lane: Lane B(並列稼働中: Lane A Gap Audit[読取]、Ledger Deviation
# Checkerコスト調査[読取、`er012_output/editorial_b_voices_4v_article_
# trial_02/`のログを読むだけ]、SSOT統合。いずれも本ファイルとは無関係)。
# **設計修正Trial**(Production/Trial-07/registry/Contract編集禁止、
# SSOT・Git禁止)。
#
# ユーザー決定(2026-09-09、正式): 4V Trial-01/02(Business/Legal Voiceの
# Analytical Leakageが3 attemptで解消せず)を受け、今回のテーマでは
# 4 Voicesを3 Voicesへ変更する。Production採用ではなく新しい設計Trial。
# 原因仮説: Business/EfficiencyとFairness/Legalが具体的人物ではなく抽象的
# 分析軸だったため、WriterがResearch/Evidenceを人物の経験・責任として語らず、
# 数値・ROI・制度・規制・法律の解説へ戻った。Promptを強める前にVoice設計を
# 修正する。この仮説が支持されるかを検証する。
#
# 3 Voices: (1) Voice 1=仕事に応募する一人の人(Applicant、Algieba)/
# (2) Voice 2=採用担当者/Hiring Manager(Erinome)/ (3) Voice 3=経営者
# (採用コスト・速度・会社運営・結果に実際に責任を持つ人物、Business/
# Efficiencyという抽象軸ではない、Schedar)。Fairness/Legal/HR Governance
# (旧4V Voice 4)はVoiceから外し、Tension/Closing側の統合・制約材料とする
# (詳細: `er012_output/editorial_b_voices_3v_person_voice_trial_01/
# research/perspective_map_3v.md`)。テキストのみのため音声は生成しない。
#
# 本ファイルの土台: `er012_editorial_b_voices_4v_article_trial_02.py`
# (4V、7区切り。保持・無変更)の経路をそのまま複製し、6区切り(Hook/
# Voice 1〜3/Tension/Closing)へ縮小したもの。Trial-02ファイル自体は
# 一切変更していない。命名: 見出し構造(split関数の内部キー・required_
# structure segment名)は`point_one/point_two/point_three`
# (Production 2V正式命名`point_one`/`point_two`[registry.py
# B_FAMILY_B1_REQUIRED_SEGMENTS]をpoint_threeへ1つだけ拡張したもので、
# 4Vが選んだ`voice_1..4`命名とは異なる設計選択。この命名自体が本Trialの
# 評価対象[未承認候補、Gate 1 §7参照]）。QAスキーマ(Analytical Leakage
# Check・Pairwise Distinctness Check)は4V踏襲でvoice_1/voice_2/voice_3
# キーを使う(物理構造キーとQAスキーマキーをあえて分離し、両命名方式を
# 同時に記録する)。
#
# Ledger: `er012_output/ai_screening_ledger_trial_01/research/
# verified_fact_ledger.txt`(4V Trial-01/02と同一、再利用。新規Research
# 呼び出しなし、Ledger本文は一切改変しない)。`perspective_map.md`
# (4V版、既存)は読み取り専用で参照するのみ。3人物版
# `perspective_map_3v.md`(本Trial新規作成、Step 0成果物)を
# `er012_output/editorial_b_voices_3v_person_voice_trial_01/research/`
# に保存済み。
#
# Fact Checker A'(opt-in、OPEN-131): 4V Trial-02と同じ呼び出しチェーン
...
# 改変、SSOT・Git操作、閾値変更、バックグラウンド待機、完了報告後の自動
# 復帰。
#
# STOP条件: 新規failure mode/費用上限(¥150)/見出し数6が成立しない状態が
# retry上限まで継続/Production・Trial-07・registry書込みが必要/新しい
# 語り口指示が必要と判断される(記載のみ)/3者が賛否構図化。
#
...
THEME_ID = "editorial_b_voices_3v_person_voice_trial_01"
OUT_DIR = "er012_output/editorial_b_voices_3v_person_voice_trial_01"
...
PERSPECTIVE_MAP_3V_PATH = f"{OUT_DIR}/research/perspective_map_3v.md"  # 本Trial新規作成(Step 0)
...
def run_fact_check_a_prime_3v(article_text: str, ledger_text: str, out_dir: str) -> dict:
    ...
    print(f"[3V-PERSON-VOICE-TRIAL-01] Fact Checker A'呼び出し開始(fact_attribution_mode_enabled={enabled})...")
    ...
    print(f"[3V-PERSON-VOICE-TRIAL-01] Fact Checker A'完了。final_status={fc_record.get('final_status')} "
...
B_FAMILY_VOICES_3V_FOCUS_MODULE_BLOCK = """【B Family Voices/Perspective Focus Module(3 Voices・Person-Voice版、EDITORIAL-B-FAMILY-VOICES-3V-PERSON-VOICE-TRIAL-01。
Production未採用、この記事タイプ専用の骨格再定義。4V[Trial-02]を土台に3 Voicesへ縮小し、3人目のVoice(経営者)を
「具体的人物」として再設計したもの。人称指示・Evidence脇役原則等は4V版から変更していない）】
...
【Tensionの役割(重要、3Voices版で新規設計。design.md B-7「共通前提→分岐点→非対称性」の
3段構造を土台に、外部制約[fairness/bias/accountability/law/compliance]の統合を追加した
もの)】
...
ください。以下の3段構造として書いてください(これは執筆時の思考の補助であり、本文に
「第1段」「第2段」というラベルをそのまま書く必要はありません):
...
3. 非対称性+外部制約: Applicantはプロセスに対する発言権を持たない(判断される側)、
   Recruiter・Hiring Managerは日々ツールを運用する現場だが導入を決める側ではない、
   Business Ownerは導入するかどうかを決め、結果の責任を負う側、というプロセス上の力関係の
   非対称を明示してください。そのうえで、**3人のうち誰か1人、あるいは3人を単純に足し合わせた
   だけでは、この状況の答えにはならないことを示してください。**3人の外側には、応募者が不当に
   差別されていないかを事後に検証する具体的な力(バイアス監査の義務付け、AIを高リスクに
   分類する規制、過去に差別的な採用AIが中止・提訴された実例、地域によって規制の整備状況が
   異なるという現実)が既に存在しており、これが3人それぞれの選択肢を現実に制約しています。
   以下のevidence(Ledgerの[VOICE_4_EVIDENCE]タグ由来、この記事では独立したVoiceではなく
   Tensionの中でこの制約を描くための素材として使う)から、Tensionの自然な流れの中に**1〜2件
   だけ**、人を主語にした自然な話し言葉で織り込んでください(列挙・解説にしないでください):
...
非対称性として描き、そのうえでこの3人の外側に存在する制約を統合してください。**3人を単純に
「応募者1人 vs 採用担当+経営者2人」のような2対1の陣営へ分けないでください。** 単に
...
未検証monitoring値325〜355秒[尺]の語数換算に基づく、後述の語数・尺見積り参照)】
記事全体の総語数は、**約330語をsoft targetとしてください**(hard capではありません)。
目安配分(soft guidance): Hook 50〜60語程度 / 各Voice 55〜70語程度(3人合計約165〜210語)/
Tension 60〜75語程度 / Closing 40〜50語程度。この配分は目安であり、自然な文章の流れ・
...
def run_phase_a(audit_dir: str) -> dict:
    ...
    print(f"[3V-PERSON-VOICE-TRIAL-01][Phase A] clean_single_insert_confirmed={clean_single_insert}")
...
def run_overlap_monitoring_3v(sections: dict, out_dir: str) -> dict:
    ...
    summary = {
        "qa_status": "OK",
        "note": ("EDITORIAL-B-FAMILY-VOICES-3V-PERSON-VOICE-TRIAL-01: monitoring専用(合否判定には"
    ...
    print(f"[3V-PERSON-VOICE-TRIAL-01] Overlap monitoring(9値)完了。any_flagged={summary['any_flagged']}")
    return summary
...
def run_analytical_leakage_check_3v(client, sections: dict, model: str, reasoning_effort: str,
    ...
    print(f"[3V-PERSON-VOICE-TRIAL-01][Leakage Check] attempt{attempt}: any_flagged={result['any_flagged']} "
...
def build_leakage_corrective_note_3v(leakage_result: dict) -> str:
    ...
    lines.append(
        ...
        "- Compactness: 記事全体の総語数は約330語がsoft targetです(hard capではありません)。"
        ...
        "外部制約(fairness/bias/accountability/law)は1〜2件だけ、自然な話し言葉で織り込んでください。\n"
...
def run_ledger_deviation_and_local_rewrite(client, theme_id: str, label: str, article_text: str,
    ...
    print(f"[3V-PERSON-VOICE-TRIAL-01][{theme_id}] {label}: ledger逸脱チェック開始(Hook-aware)...")
    deviation_result = vfl01.run_deviation_check(
        client, verified_ledger_text, article_text, model=ledger_model, hook_aware=True)
    print(f"[3V-PERSON-VOICE-TRIAL-01][{theme_id}] {label}: deviation overall_status="
    ...
    while major_items and cycle < local_rewrite.MAX_REWRITE_CYCLES:
        ...
        print(f"[3V-PERSON-VOICE-TRIAL-01][{theme_id}] {label}: Local Rewrite cycle {cycle}/"
        ...
        for idx, deviation in enumerate(major_items, start=1):
            ...
            print(f"[3V-PERSON-VOICE-TRIAL-01][{theme_id}] {label}: cycle {cycle} NG item {idx}: "
        ...
        print(f"[3V-PERSON-VOICE-TRIAL-01][{theme_id}] {label}: cycle {cycle} Local Rewrite後、Ledger全体を再判定...")
        ...
        print(f"[3V-PERSON-VOICE-TRIAL-01][{theme_id}] {label}: cycle {cycle} 再判定 overall_status="
...
def _generate_and_compress_article_3v(client, theme_id: str, label: str, prompt: str, out_dir: str,
    ...
    if raw_result is None:
        print(f"[3V-PERSON-VOICE-TRIAL-01][{theme_id}] {label}: writer技術的失敗(通信障害等)。")
    ...
    if apply_evidence_compression:
        with open(f"{out_dir}/audit/pre_editor_article.md", "w", encoding="utf-8") as f:
            f.write(article_text)
        print(f"[3V-PERSON-VOICE-TRIAL-01][{theme_id}] {label}: Evidence Compression(Lossless Editor)呼び出し開始...")
        ...
        print(f"[3V-PERSON-VOICE-TRIAL-01][{theme_id}] {label}: Evidence Compression完了。"
...
def run_voices_pattern_3v(client, theme_id: str, label: str, prompt: str, verified_ledger_text: str,
    ...
    print(f"[3V-PERSON-VOICE-TRIAL-01][{theme_id}] {label}: Overlap monitoring(9値)開始...")
    ...
    print(f"[3V-PERSON-VOICE-TRIAL-01][{theme_id}] {label}: metrics={metrics} six_section_report={six_report}")
    ...
    if verdict == "FAIL":
        print(f"[3V-PERSON-VOICE-TRIAL-01][{theme_id}] {label}: Fact CheckerがFAILと判定。NG_REVIEW_REQUIREDとして報告します。")
        return {
    ...
    if ledger_result["remaining_major_count"] or ledger_result["any_human_review_required"]:
        print(f"[3V-PERSON-VOICE-TRIAL-01][{theme_id}] {label}: Local Rewrite cycleを尽くしてもLedger MAJOR残存/"
    ...
    if apply_directional_fact_precheck:
        print(f"[3V-PERSON-VOICE-TRIAL-01][{theme_id}] {label}: 比較方向Fact事前チェック開始(rule-based、¥0)...")
        ...
        print(f"[3V-PERSON-VOICE-TRIAL-01][{theme_id}] {label}: 比較方向Fact事前チェック完了。overall_status={directional_precheck_status}")
...
def run_pipeline_3v(client, theme_id: str, label: str, base_prompt: str, verified_ledger_text: str,
    ...
    for attempt in range(1, MAX_WRITER_ATTEMPTS + 1):
        ...
        print(f"[3V-PERSON-VOICE-TRIAL-01] Writer attempt {attempt}/{MAX_WRITER_ATTEMPTS} 開始(out_dir={attempt_dir})...")
        t0 = time.time()
        ...
        if result.get("status") != "OK" or not result.get("article_text"):
            ...
            print(f"[3V-PERSON-VOICE-TRIAL-01] attempt {attempt}: status={result.get('status')}のためLeakage Checkをスキップします。")
            break
        ...
        if sections is None:
            ...
            print(f"[3V-PERSON-VOICE-TRIAL-01] attempt {attempt}: 6区切り構造が検出できずLeakage Checkをスキップしました。")
            break
        ...
        if not leakage["any_flagged"]:
            print(f"[3V-PERSON-VOICE-TRIAL-01] attempt {attempt}: Analytical Leakage Check flagged項目なし。確定。")
            break
        if attempt == MAX_WRITER_ATTEMPTS:
            print(f"[3V-PERSON-VOICE-TRIAL-01] attempt {attempt}: 最大attempt数に到達。flagged項目が残った状態の"
...
def run_writer_stage() -> dict:
    ...
    if not phase_a["phase_a_pass"]:
        print("[3V-PERSON-VOICE-TRIAL-01] Phase Aで意図しない差分を検出したため、Writerへ進まずSTOPします。")
    ...
    print(f"[3V-PERSON-VOICE-TRIAL-01] Writer + Analytical Leakage Checkパイプライン開始(最大{MAX_WRITER_ATTEMPTS} attempts)...")
    ...
    print(f"[3V-PERSON-VOICE-TRIAL-01] 完了。total_attempts={pipeline_result['total_attempts']} "
...
def run_comments_1_and_4(client, sections: dict, out_dir: str) -> dict:
    ...
    with open(f"{out_dir}/comments_1_to_4.json", "w", encoding="utf-8") as f:
        json.dump(result, f, ensure_ascii=False, indent=2, default=str)
    print(f"[3V-PERSON-VOICE-TRIAL-01][Comments] comment_1={c1['text'][:60]!r} comment_4={c4['text'][:60]!r}")
    return result
...
def run_distinctness_check_full(client, sections: dict, out_dir: str) -> dict:
    ...
    for a, b in itertools.permutations(point_keys, 2):
        ...
        print(f"[3V-PERSON-VOICE-TRIAL-01][Distinctness] directed {key}: "
    ...
    summary = {
        "note": ("EDITORIAL-B-FAMILY-VOICES-3V-PERSON-VOICE-TRIAL-01: Pairwise Voice Distinctness Check"
    ...
    with open(f"{out_dir}/pairwise_voice_distinctness_check.json", "w", encoding="utf-8") as f:
        json.dump(summary, f, ensure_ascii=False, indent=2, default=str)
    print(f"[3V-PERSON-VOICE-TRIAL-01][Distinctness] 完了。direction_agreement_rate={direction_agreement_rate} "
...
def run_overlap_controls(sections: dict, out_dir: str) -> dict:
    ...
    with open(f"{out_dir}/overlap_qa_controls.json", "w", encoding="utf-8") as f:
        json.dump(summary, f, ensure_ascii=False, indent=2, default=str)
    print(f"[3V-PERSON-VOICE-TRIAL-01][Overlap Controls] positive={positive_result['overlap_ratio']} "
...
def build_word_count_and_duration_estimate(sections: dict, out_dir: str) -> dict:
    ...
    with open(f"{out_dir}/word_count_and_duration_estimate.json", "w", encoding="utf-8") as f:
        json.dump(result, f, ensure_ascii=False, indent=2)
    print(f"[3V-PERSON-VOICE-TRIAL-01][Duration Estimate] variable_word_count_3v={variable_word_count_3v} estimate={estimate}")
    return result
...
def review_required_structure_3v_trial() -> dict:
    ...
    print(f"[3V-PERSON-VOICE-TRIAL-01][OPEN-129 review] {review}")
    return review
...
def build_first_person_usage_report(sections: dict, out_dir: str) -> dict:
    ...
    print(f"[3V-PERSON-VOICE-TRIAL-01][First Person Usage] {ratios}")
    return result
...
def run_qa_stage() -> dict:
    ...
    if os.path.exists(comments_path):
        print("[3V-PERSON-VOICE-TRIAL-01] comments_1_to_4.json既存のため再利用(再生成しない)。")
    ...
    if os.path.exists(distinctness_path):
        print("[3V-PERSON-VOICE-TRIAL-01] pairwise_voice_distinctness_check.json既存のため再利用(再生成しない)。")
    ...
    print("[3V-PERSON-VOICE-TRIAL-01] QA stage完了。")
    return qa_summary
