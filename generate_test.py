...
8. MAYA OPENER VARIETY: look at MAYA's PREVIOUS turn (the one immediately before her current turn, even if a LEO turn sits in between - "previous MAYA turn" always means the last time MAYA spoke, not literally the line right above). Her current turn's opening word(s) (the first 1-2 words) must NOT match that previous MAYA turn's opening word(s). This bans back-to-back repeats of the same opener (e.g. two "So..." starts with only a LEO turn between them) - it does NOT ban an opener from returning later once something else has been used in between. This rule applies to EVERY MAYA turn, including the mid-split MAYA turn introduced by the LEO SPLIT RULE below - it counts as a MAYA turn for this purpose too. Vary openers naturally across "So...", "Wait, so...", "Does that mean...?", "Oh, so...", "Hmm, so...", "Then...", "But..." and similar - whatever fits the moment, as long as it differs from her immediately preceding turn's opener.

9. SPOKEN WORDS ONLY - NO STAGE DIRECTIONS: every turn's text must contain ONLY the words that speaker actually says out loud. NEVER include third-person narration or stage directions of any form (e.g. "Maya laughs and points at the coffee", "He pauses.", "*sighs*") - this text is fed directly to a text-to-speech engine, so anything written there gets read aloud as if the character were speaking it. If you want to convey a gesture, expression, or emotion, express that ENTIRELY through the wording and rhythm of the spoken line itself (word choice, punctuation, fragments) - never by describing the action in the third person.

# How to convert each beat
- Each beat becomes: MAYA's turn realizes her maya_move (in the form of her maya_type), then LEO's leo_point is delivered as TWO separate LEO turns with ONE substantive MAYA turn between them (see LEO SPLIT RULE below) - so a beat with a leo_point normally yields 4 turns (MAYA move -> LEO part 1 -> MAYA mid-turn -> LEO part 2), not 2-3. Stay near the beat's word budget across the two LEO turns combined.
- LEO SPLIT RULE (applies to every beat that has a leo_point, no exceptions): split leo_point at a natural break in its own content into two LEO turns, each roughly half to two-thirds of the original leo_point's length - do not cut content, just divide where it already breaks naturally. Between the two LEO turns, MAYA gets ONE short turn that actually moves the conversation forward - NOT a content-free filler like "Oh..." or "Huh." Instead, it must be either (a) a brief summary/confirmation of LEO's first part IN MAYA'S OWN WORDS (e.g. "So basically...?", "Wait, so that means...?"), or (b) a short follow-up question that naturally grows out of what LEO just said. Either way, keep it to about one short sentence, and it should function as the setup that makes LEO's second part feel like a direct answer/continuation. This mid-split MAYA turn IS subject to the normal maya_type rhythm rule (no more than 2 "question" turns of this kind in a row, same as any other question), and it does NOT get its own beat.
