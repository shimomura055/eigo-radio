# ============================================================
# er014_output/user_test_news_2ep_01/ai_control/run_pipeline.py
# 管理ID: USER-TEST-NEWS-2EP-COMPLETION-01-RESUME-02 (Theme 2: AI Control)
#
...
# space_weapons/run_pipeline.pyの複製(topicのみ差替え、Prompt本文は
# build_researcher_prompt/build_verification_promptへ渡すtopic引数の
# 範囲のみで変更)。
#
...
#
# 出力先: er014_output/user_test_news_2ep_01/ai_control/
...
#
# 実行方法(root直下から):
#   .venv/Scripts/python.exe er014_output/user_test_news_2ep_01/ai_control/run_pipeline.py [stage ...]
...
THEME_ID = "user_test_news_2ep_01_ai_control"
BASE_DIR = "er014_output/user_test_news_2ep_01/ai_control"
RESEARCH_DIR = f"{BASE_DIR}/research"
TOPIC_ID = "ai_control_evidence_today"
BUDGET_JPY_CAP = 700.0

# ユーザー指示原文の区別要件を、既存build_researcher_promptのtopic引数
# として渡す(Promptテンプレート自体は無変更)。Fable補足: 直近AI safety/
# capabilityニュース起点、実証済み能力/確認済み事実/企業・研究者の将来
# リスク発言/Singularity等の仮説/未確認推測を分離、「AIが人間の制御を
# 超える」と事実認定しない、恐怖を煽らない。
TOPIC_EN = (
    "Current news topic: recent developments in AI safety and AI "
    "capability that relate to the question of whether AI could "
    "eventually become difficult for humans to control, and what "
    "evidence exists today. In researching this, clearly distinguish "
    "between: (1) AI capabilities and incidents that have actually "
    "been demonstrated, tested, or confirmed by credible reporting, "
    "peer-reviewed research, or official statements from AI labs, "
    "researchers, or governments (e.g., specific benchmark results, "
    "documented model behaviors, safety evaluations, or incidents "
    "that were independently verified); (2) forward-looking warnings "
    "or risk statements made by AI company leaders, researchers, or "
    "safety organizations about future AI systems, clearly labeled as "
    "their stated opinion or prediction rather than as an established "
    "fact; (3) speculative or theoretical concepts such as "
    "superintelligence, an intelligence explosion, or 'the "
    "singularity', clearly separated from both (1) and (2) as "
    "hypotheses that are not currently confirmed or observed; and "
    "(4) what current AI systems and research can and cannot actually "
    "do today with respect to human oversight, controllability, or "
    "shutdown/correction mechanisms. Do not state or imply that AI "
    "has already surpassed human control, or that loss of human "
    "control is imminent or inevitable, unless directly and "
    "explicitly supported by a confirmed source as a current fact "
    "(not a prediction). Do not sensationalize and do not use "
    "fear-inducing language beyond what sources directly support. "
    "Focus the coverage on what is actually known today and how it "
    "differs from speculation, using only facts confirmed by web "
    "search (no invented details, no facts supplied from the "
...
def keyphrase_stage(level: str) -> dict:
    ...
    with open(f"{out_dir}/article.md", encoding="utf-8") as f:
        article_text = f.read()
    article_id = f"USER_TEST_NEWS_2EP_AI_CONTROL_{label}"
