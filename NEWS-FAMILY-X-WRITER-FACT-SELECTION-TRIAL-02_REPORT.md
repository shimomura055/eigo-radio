...
(1)客観差: Controlのみ「なぜ(AIと気づかれて切られる)」の段が欠落。B1/B2/B3は因果4段すべて明示、Storyline外混入はB1の2件(約半数・opt-out)のみ、driftは4条件とも無し。語数はControl 404/B1 414/B2 **503(超過)**/B3 390。(2)B2はFactを5件に絞ったのに語数超過。「なぜ」を第5・6段落で言い換えて2回説明しており、Fact数と語数は単純比例しない1データ点。(3)B2/B3とも「September 22, 2026」の日付を使用(Fact 010ロールバックに付随)。Storyline上は不要な時点情報でTest4に該当し得る。(4)B3はBriefに含めなかったスコープ注記(「常に人間ではない」)をWriterが自発生成(1回の観測)。(5)Fact選定はSonnetが4テストを全18 Factに適用して行い、LLMには選ばせていない。費用¥2.28。Storytellingの優劣はFableも断定しない(ユーザー試読)。

## §13 分類(Fable最終判断)

**VALIDATED**(Trial範囲、1テーマ1回。Production採用ではなく、記事品質の最終判断はユーザー試読による)。ユーザー判断事項: ①試読のうえ、Fact選定基準としてB1/B2(因果必須)/B3(最小核)のどれを次段階の基準にするか(Fableは採否推奨をしない。客観差: B3のみ語数・因果・混入の全指標で目標内)。②Brief作成時のStoryline選定を AI自動/人間1行指定(Fable推奨: 人間1行指定)。③日付等の時点情報をBriefから外すか(Fable推奨: Storyline上不要なら外す)。
