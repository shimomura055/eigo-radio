...
| Personalized News A2(新版、`B-FAMILY-VOICES-POSITION-AND-EVIDENCE-SPEC-01`再生成) | A2(B-Family新規topic、2V) | 完成(Analytical Leakage Check attempt1で0件PASS[`leak_position_blur`含む全7項目]、TTS/ASR14segment全OK、Human Review Lock発生なし) | Audio Validation Gate PASS(duration=331.192秒、peak=0.95492、clipping=False) | 未試聴(基盤`PRODUCTION_WIRED`のまま、記事内容の正式ユーザーOK判定は再試聴待ち。新題"The Same Feed, Two Different Mornings"/「同じフィード、二つの違う朝」) | `https://rawcdn.githack.com/shimomura055/eigo-radio/a54d0201f667d3fa56d6422ddeaa9fc998b0d49a/user_test/unified.html?src=er012_output/b_family_a2_new_topic_production_01/personalized_news_2v_a2/player.html&level=A2&en=The%20Same%20Feed%2C%20Two%20Different%20Mornings&ja=%E5%90%8C%E3%81%98%E3%83%95%E3%82%A3%E3%83%BC%E3%83%89%E3%80%81%E4%BA%8C%E3%81%A4%E3%81%AE%E9%81%95%E3%81%86%E6%9C%9D`(E2E evidence: `er012_output/b_family_a2_new_topic_production_01/personalized_news_2v_a2/web/e2e_playback_evidence.json`) |
| Personalized News B1(既知flag残存、`B-FAMILY-VOICES-POSITION-AND-EVIDENCE-SPEC-01`再生成試行) | B1(B-Family、2V) | **未変更**(既存音声`voices/audio/b1_2v_v2/b1b/`のまま。再生成試行はattempt2でFact Checker A' FAILによりSTOPしたため音声化未実施) | Audio Validation Gate PASS(既存音声、無変更) | 未試聴/既存Leakage残存flagのまま(offline再評価でvoice_b 6項目+tension 2項目FAIL、`leak_position_blur`はPASS。再生成試行はLedger鮮度問題[OPEN-166]でSTOP) | 既存player URL: `https://raw.githack.com/shimomura055/eigo-radio/main/er014_output/four_type_observation_01/voices/audio/b1_2v_v2/player.html`(無変更) |
| Tiny Bags(`USER-TEST-NEWS-LIGHT-TOPIC-01-CLOSEOUT-03`) | B1 | 完成(325.734秒、peak=0.73052、clippingなし。`point_one`は"only"5/6attempt脱落によりcanonical微修正[only→just]後1attempt目でNORMALIZED_MATCH。再生成なし・現行完成版のまま維持) | Audio Validation Gate PASS | **PASS**(2026-09-17、ユーザー本人が完成playerで通し試聴しOK/承認。`USER_TEST_READY`) | `https://rawcdn.githack.com/shimomura055/eigo-radio/e858649a/user_test/unified.html?src=er014_output/user_test_news_light_01/tiny_bags/b1b/player.html&level=B1&en=Are%20Tiny%20Bags%20Really%20Back%3F%20Fashion%27s%20Answer%20Comes%20With%20a%20Catch&ja=...`(evidence: `er014_output/user_test_news_light_01/tiny_bags/b1b/e2e/e2e_evidence.json`) |
| Tiny Bags(`USER-TEST-NEWS-LIGHT-TOPIC-01-CLOSEOUT-03`) | A2 | **完成**(`full_story_part2`のToteme/Kallmeyer、Human Review確認ページでの実読確認後ユーザー承認[TTS再生成なし]、`record_human_approval()`で正式記録。A2必須6% time-stretch post-processを承認済み音声へ追加適用[TTS新規生成なし、既存precedent[Trial-12/OPEN-112サブタスクG]と同型]。Assembly実行: 403.803秒、peak=0.95981、clippingなし) | Audio Validation Gate PASS(Human Approval記録によるHUMAN_APPROVED判定+slowdown post-process後にBLOCKED解消) | Human Review承認済み(ユーザーが確認ページでToteme/Kallmeyerの実読を確認し最終版として採用、2026-09-17。A2 playerは最終確認用) | `https://rawcdn.githack.com/shimomura055/eigo-radio/6ae84b80/user_test/unified.html?src=er014_output/user_test_news_light_01/tiny_bags/a2/player.html&level=A2&en=Are%20Tiny%20Bags%20Back%3F%20Fashion%27s%20Answer%20Is%20More%20Complicated&ja=...`(E2E evidence: `docs/pm/closeout_136_e2e/tiny_bags_a2_closeout03.json`+`.png`) |
| Convenience AI(`USER-TEST-NEWS-CONVENIENCE-AI-01-FIX-01`) | A2 | **未完成**(FIX-01で記事の時制・事実誤りは是正済み[LEDGER_COMPLIANT維持・Fact Checker時制指摘解消]。`point_two`はPronunciation Ledger登録[Ledger正式経路]後の再TTSで商品名"Oimo no Canele"自体はSecondary ASR[Phrase List]が完全一致で書き起こすようになったが、別箇所"AI while"→"a I Well"の新規ASR不一致[`TRUE_CONTENT_MISMATCH`]によりcascade全体は`ASR_VALIDATION_UNCERTAIN`のまま、Human Review Lock継続) | Audio Validation Gateブロック中(未解消のため未実行) | Human Review待ち(確認ページ、内容更新済み) | 確認ページ(FIX-01更新版): `https://rawcdn.githack.com/shimomura055/eigo-radio/86cbd93d/user_test/human_review.html?src=er014_output/user_test_news_convenience_ai_01/convenience_ai/a2/human_review/point_two_review.json`(evidence: `.../a2/human_review/e2e_evidence_fix01.json`) |
| Convenience AI(`USER-TEST-NEWS-CONVENIENCE-AI-01-FIX-01`) | B1 | **完成**(287.774秒、peak=0.95、clippingなし。FIX-01で記事の時制・事実誤り是正+`## Main Story`見出しのcanonical混入除去[OPEN-165]+`full_story_part1`/`point_one`/`point_two`を影響segmentとして再TTS。`full_story_part1`の"Lawson then"/"Then Lawson"語順差分は今回`HIGH_SIMILARITY_SAFE`でPASS、RESOLVED) | Audio Validation Gate PASS | 未試聴(技術的完成のみ、Playwright E2E確認済み。旧Human Review確認ページは`superseded_note`で解消済みと明記、削除せず保持) | `https://rawcdn.githack.com/shimomura055/eigo-radio/86cbd93d/user_test/unified.html?src=er014_output/user_test_news_convenience_ai_01/convenience_ai/b1b/player.html&level=B1&en=A%20Lemon%20Tart%2C%20Pickles%2C%20and%20an%20AI%20Suggestion&ja=...`(evidence: `.../b1b/e2e_evidence.json`) |
