# ============================================================
# er018_fiction_core_provocation_prompt_bias_retrial_01.py
# 管理ID: FICTION-CORE-PROVOCATION-PROMPT-BIAS-RETRIAL-01
# ============================================================
# 目的: 前回Trial(FICTION-CORE-PROVOCATION-RANDOM-DNA-TRIAL-01、
# er018_fiction_core_provocation_random_dna_trial_01.py)のStory DNA設計
# (5軸・2-3軸・seed・Run5 Future Coverage・4共通原則・語数280-420)は一切
# 変えず、共通developer message/Writer promptに残っていた旧Future設計由来の
# 「世界設定の選択肢を列挙する文言」(real/future/unreal/imagined-speculative
# 等の列挙)だけを除去し、同一seed/同一DNAで5 Run再生成して比較する
# (詳細差分は本ディレクトリのprompt_bias_diff.md、Trialのみ。Production変更なし)。
#
# 前回スクリプトer018_fiction_core_provocation_random_dna_trial_01.pyと出力
# er018_output/fiction_core_provocation_random_dna_trial_01/は一切変更しない
# (read-onlyでの参照のみ、analyze stepでの類似度比較用途)。
# Status: Trial専用、未承認draft実装(`APPROVED_FOR_PRODUCTION`ではない)。
# 到達上限: VALIDATED(Production採用は本タスクの対象外)。
#
# 実行方法(root直下、4 step):
#   .venv/Scripts/python.exe er018_fiction_core_provocation_prompt_bias_retrial_01.py \
#       --out-dir er018_output/fiction_core_provocation_prompt_bias_retrial_01 \
...
# ------------------------------------------------------------
# Core Provocation Prompt(前回Trial er018_fiction_core_provocation_random_
# dna_trial_01.pyの逐語ベースに、旧Future設計由来の世界設定列挙のみ除去。
# 詳細差分はprompt_bias_diff.mdへ記録)
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# Story Prompt(前回Trial er018_fiction_core_provocation_random_dna_trial_01.py
# の逐語ベースに、旧Future設計由来の世界設定列挙のみ除去。詳細差分は
# prompt_bias_diff.mdへ記録。Characters/Length/Listening-friendliness各ブロックは
# 前回どおり逐語のまま維持(世界設定列挙を含まないため)。
# ------------------------------------------------------------
...
def process_one_run(client, out_dir: str, run_number: int, dna_entry: dict,
    ...
    theme_id_for_log = f"fiction_core_provocation_prompt_bias_retrial_01_run{run_number}"

    provocation_model = routing.require_model_or_override(
        "A2_WRITER", routing.WRITER_MODEL,
        override_reason="FICTION-CORE-PROVOCATION-PROMPT-BIAS-RETRIAL-01: 自由生成Core "
                         "Provocation(Fiction、1-3案+選定、Bias除去版)にA2_WRITER "
    ...
    story_model = routing.require_model_or_override(
        "A2_WRITER", routing.WRITER_MODEL,
        override_reason="FICTION-CORE-PROVOCATION-PROMPT-BIAS-RETRIAL-01: 自由生成Story"
                         "(Fiction、Story DNAナッジ付き、Bias除去版)にA2_WRITER Approved "
...
# 前回Trial(FICTION-CORE-PROVOCATION-RANDOM-DNA-TRIAL-01)の5本(Bias除去前)。
# 今回の5本との相互類似度を見るための比較専用参照(read-onlyで読み込むのみ、
# 前回スクリプト・前回出力ディレクトリ自体は変更しない)。
PREVIOUS_TRIAL_OUT_DIR = "er018_output/fiction_core_provocation_random_dna_trial_01"
...
def step_analyze(out_dir: str) -> None:
    ...
    # 前回Trial(Bias除去前)の5本を比較専用ドキュメントとして追加(read-only)。
    previous_texts = {}
    previous_meta = []
    ...
    save_json(f"{out_dir}/similarity_matrix.json", {
        ...
        "note": "labels prefixed old_run* are the 5 stories from the previous "
                "trial (FICTION-CORE-PROVOCATION-RANDOM-DNA-TRIAL-01, before "
                "prompt bias removal), included for cross-trial similarity only "
...
def step_assemble(out_dir: str, seed_base: int) -> None:
    ...
    all_parts = [
        "# FICTION-CORE-PROVOCATION-PROMPT-BIAS-RETRIAL-01 -- stories_all.md\n",
    ...
    blind_parts = [
        "# FICTION-CORE-PROVOCATION-PROMPT-BIAS-RETRIAL-01 -- blind.md\n",
