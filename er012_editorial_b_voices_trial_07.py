# ============================================================
# er012_editorial_b_voices_trial_07.py
# EDITORIAL-B-FAMILY-VOICES-TRIAL-07-CONTRACT-REFINEMENT-01
# ============================================================
# Lane: Lane B / Voices-Perspective。Lane A(OPEN-112/OPEN-117/ER-011系、
# docs/pm/*、er011_*、er006_*、er003_*Production本体等)とは完全に独立。
# 本ファイルはer012_editorial_b_voices_trial_06.pyを土台にコピーし、
# Trial-06本体は一切変更していない。本タスクではGit操作(add/commit/push/
# reset等)を一切行わない(Fableが後で統合commitする)。
#
# 背景(ユーザー判断、EDITORIAL-B-FAMILY-VOICES-TRIAL-07-CONTRACT-
# REFINEMENT-01タスク文書より): Trial-06の記事を個別に手直しするのでは
# なく、Trial-06で観察された改善点を「Trial用Voices Editorial Contract」
# へ生成仕様候補として織り込み、その新Contractから同テーマの記事を
# 再生成して、今後の記事でも意図どおり再現できるかを検証する。Contract
# 更新は既存Contract("Research is backstage. People are on stage."/
# 人から始まる/Evidenceは裏付け/survey・data・researchを主人公にしない/
# 主要意見優先/Tensionで理由差を掘る/Closingで一段深い理解/Light・
# conversational・human-centered/Discovery・analytical briefingへ戻ら
# ない、いずれもTrial-04/05/06からの継続方針)と矛盾しない形で、以下の
# 3点を統合する(単独の孤立したルールとしてではなく、既存原則の具体化・
# 深掘りとして記述する):
# A. Tensionの役割の明確化: Evidenceを主役にしない、survey/research/
#    data/percentageをTensionの中心に置かない。Tensionの役割は「なぜ
#    合理的な人たちが同じテーマで違う答えを持つのか」を、価値観・経験・
#    制約・守りたいもの・問題の定義・何を「良い状態」と考えているかの
#    違いから掘ること。Evidenceは、その違いが現実に存在することを裏で
#    支えるためだけに保持する(Fact Safety/provenanceは維持しつつ、
#    本文で前面化しない)。
# B. Closingの役割の明確化: 単なる要約ではなく、2つのVoiceを見たことで
#    最初の問い(Hook)の見え方がどう変わったかを示す。「両方に良い点が
#    ある/人による」で終わらせず、「この問題は実は何についての問題
#    なのか」を一段深く見せる。可能なら表面的な対立軸そのものを、
#    Voice/Tensionの内容から自然に導かれる形で組み替える(具体的な
#    抽象軸を先に決め打ちしない)。Writer自身の解決策・コンサル提案には
#    しない。
# C. Compactness: 約350語を目標とするsoft target(hard capにしない)。
#    短くすること自体が目的ではなく、Voiceの人間中心性・Tensionの深さ・
#    Closingの意味を保ちながら、重複説明・Narrator分析・余分な
#    Research説明・同義の言い換え・不要な導入文を削る。450語超を標準と
#    しない。
#
# Perspective(固定席派/自由席派の社員2名、主要意見優先)・Research・
# 5区切りadapter(Hook/One Voice/Another Voice/Tension/Closing)・
# Analytical Leakage Check・Fact Safety(Fact Checker/Ledger Deviation
# Checker+Local Rewrite/Directional Fact Precheck)・Evidence Compression
# Editor(Production、無変更)は、Trial-06から関数名も含め無変更のまま
# 再利用する(Trial-06のPhase B/C[Editorあり/なし/改善案比較]は本Trialの
# スコープ外、run_writer_stage()のみを使用する)。一人称/三人称は固定
# しない(必須にしない、Writerが最も自然にVoicesとして成立する書き方を
# 選び、選択結果を観察・記録する)。書き換えるのはFocus Module Block
# (B_FAMILY_VOICES_FOCUS_MODULE_BLOCK、Tension/Closing/長さの節)のみ。
#
...
# Voices、Master記事正式採用、一人称の正式採用、既存Fact Safety/
# Validatorの大きな変更、Git操作(成果物は作業ツリーに残し、Fableが後で
# 統合commit)。cost > ¥500でSTOP。
#
...
THEME_ID = "editorial_b_voices_trial_07_assigned_desks"
OUT_DIR = f"er012_output/editorial_b_voices_trial_07"
RESEARCH_DIR = f"{OUT_DIR}/research"
os.makedirs(RESEARCH_DIR, exist_ok=True)

# EDITORIAL-B-FAMILY-VOICES-TRIAL-07: テーマ・TOPIC_JA自体はTrial-04/05
...
LABEL = "B1B"
# EDITORIAL-B-FAMILY-VOICES-TRIAL-07: run_trial07_pipeline()(Trial-06の
# run_trial06_pipeline()を関数名のみ変えて無変更のまま再利用)がAnalytical
# Leakage Check flaggedの場合に内部でWriterを最大2回再実行する(合計最大3
# attempts)。各attemptの出力はb1b_run01_attempt1/・b1b_run01_attempt2/...
# のようにattempt番号付きの別ディレクトリへ保存し、上書きしない(RUN_ID
# 自体は固定、attempt番号はpipeline内部で管理する)。
# EDITORIAL-B-FAMILY-VOICES-TRIAL-07-CONTRACT-REFINEMENT-01(Fable往復1回目):
# run02としてContract是正メモ改修(build_leakage_corrective_note)・Contract
# Compactness節強化後のWriterを再生成する。run01のtracked出力(b1b_run01_*/、
# OUT_DIR直下のaudit/・audit_candidate_prompt_base.txt・raw_usage_log_
# trial07_writer.jsonl・trial07_summary.json)は一切上書きしない。run02専用の
# 出力は全てb1b_run02_*/またはLEVEL_OUT_DIR配下へ閉じ込める(Trial-04 run02の
# 方式を踏襲)。
RUN_ID = "run02"
...
def run_research_stage() -> None:
    print("[TRIAL-07][Research] EDITORIAL-B-FAMILY-VOICES-TRIAL-05は新規Perplexity"
...
B_FAMILY_VOICES_FOCUS_MODULE_BLOCK = """【B Family Voices/Perspective Focus Module(EDITORIAL-B-FAMILY-VOICES-TRIAL-07-CONTRACT-REFINEMENT-01、2026-09-07。Production未採用。この記事タイプ専用の骨格再定義。Trial-06からの変更点: Tensionの役割・Closingの役割・記事全体の長さについての3節を、Trial-06での観察に基づき明確化した)】
...
【Tensionの役割("## Why They See It Differently"相当)(EDITORIAL-B-FAMILY-VOICES-TRIAL-07で
明確化。中心原則【Research is backstage. People are on stage.】をTensionにも一貫して
適用する)】
「どちらの考え方が正しいか」を決めようとしないでください。そうではなく、なぜ両方の
Voiceが、それぞれの立場からは合理的に見えるのかを掘り下げてください。**Tensionの中心は、
あくまでVoice Cardに描かれている2人の人物であり、Evidence(survey/research/data/
percentage)ではありません。** Tensionの段落を、"A survey found...", "The data show...",
"Research shows..."のような、調査・データそのものを主語にした文で始めたり、その説明へ
立ち戻ったりしないでください。具体的には、2人が同じ状況を生きていながらなぜ違う答えに
至るのかを、次のような違いから掘り下げてください: それぞれがどんな経験からその考えに
至ったのか / それぞれが何を優先し、何を守ろうとしているのか / それぞれがこの問題を
そもそもどう捉えているか(何についての問題だと定義しているか) / それぞれにとって
「良い状態」「自分の居場所がある」とはどういう状態を指すのか(候補となる切り口は上記
【この2つのVoiceがなぜ「単なる好みの違い」で終わらせてはいけないか】参照、どれか一つに
決め打ちしないでください)。EvidenceはVoice Cardの人物像・立場が実在することを裏で支える
ためだけに使ってよく、Fact Checker・Ledger Deviation Checkのための正式な事実源であり
続けますが、Tensionの文章の中心・主語にしないでください。この段落では2つのVoiceそれぞれ
が使った事実を横断的に参照してもかまいませんが、Verified Fact Ledgerに無い新しい因果
関係・新しい事実を作り出さないでください。単に「両方とも一理ある」とまとめるだけの記述に
しないでください。解決策の提案はここでも基本的に行わないでください。

【Closingの役割("## What This Tells Us"相当)(EDITORIAL-B-FAMILY-VOICES-TRIAL-07で明確化)】
これは要約でも、In One Lineの言い換えでもありません。2つのVoiceを両方見たことによって、
この問題そのものの見え方が、Hook(冒頭の問い)の時点からどう変わったかを書いてください。
「固定席派にも自由席派にも良い点がある」「人によって違う」というだけの結び方で終わらせ
ないでください。**目指すのは、この問題が実は何についての問題なのかを一段深く見せること
です。** 可能であれば、Hookで提示された表面的な対立軸(固定席か自由席か)そのものを、
Voice CardとTensionの内容から自然に導かれる、もう一段抽象的な軸(例: 何によって安心を
得るか、何が「自分の居場所」を決めるか)へ組み替えてください。**どの抽象軸に組み替える
べきかをこちらで先に決め打ちすることはしません。** 2つのVoiceとTensionの内容から、
あなた自身がその記事にとって最も自然な着地点を見つけてください。「どちらが正しいか」を
決めず、Writer自身の解決策・妥協案・コンサルティング的な提案(例: 半固定席の導入・
ゾーン制の提案)は、Closingでも書かないでください。内容はVerified Fact Ledgerが示す
...
【記事全体の長さについて(この記事専用、EDITORIAL-B-FAMILY-VOICES-TRIAL-07で明確化。
hard capではない)】
記事全体の総語数は、**約350語をsoft targetとしてください**(hard capではありません。
350語ちょうどに合わせる必要はなく、多少前後してもかまいません)。**短くすること自体を
目的にしないでください。** Voiceの人間中心性(その人の状況・必要・心配・守りたいものの
具体的な描写)、Tensionの深さ(なぜ違う答えになるかの掘り下げ)、Closingの意味(問いの
見え方がどう変わったか)を保ったうえで、次のような重複・余分な要素を削ってください:
同じ内容の重複説明、Narrator(語り手)による外側からの分析、Verified Fact Ledgerの内容
そのものを長々と紹介するような余分なResearch説明、既に書いた内容の同義の言い換え、記事の
本題に入るまでの不要な導入文。これらを削った結果として、文章が自然に350語前後に収まる
ことを目指してください。450語を超えることを標準としないでください。

**各Voiceは同じ意味を2回言わないでください(EDITORIAL-B-FAMILY-VOICES-TRIAL-07-
CONTRACT-REFINEMENT-01で追加)。** 1つの経験・1つの感覚は、そのVoiceの中で1回だけ
描写してください。同じ内容を、言葉を変えて2回目に繰り返す必要はありません(例:
「決まった場所があることで仕事を始めやすい」という内容を、前半で1回描写したら、
後半で似た言い回しに変えて再度書かないでください)。

**Closingは、前段(Voice A・Voice B・Tension)の内容の要約から書き始めないでください。**
Closingの最初の役割は要約ではなく再定義です。すでに書いた内容(それぞれの人物が何を
守りたいか・何を優先するか等)を並べ直してから結論に入るのではなく、Hookの問いの
見え方がどう変わったかという再定義そのものから書き始めてください。

**配分の目安(soft guidance、厳密な文数・語数の固定ルールではありません)**: Hook
60〜70語程度 / One Voice(Voice A)90〜100語程度 / Another Voice(Voice B)90〜100語
程度 / Tension 60〜70語程度 / Closing 40〜50語程度、合計で約350語程度。この配分は
あくまで目安であり、自然な文章の流れ・Tension/Closingの深さを犠牲にしてまで厳密に
一致させる必要はありません。
...
- Voice Cardの内容を経由せず、Verified Fact Ledgerの記述(出典名・数字の列挙)から
  直接Voiceの文章を組み立てること
- Closingを「両方に良い点がある」「人によって違う」という結び方だけで終わらせること
- 記事全体の総語数を350語ちょうどへ合わせるために、Voiceの具体的な描写・Tensionの
  掘り下げ・Closingの意味を不自然に削ること(soft targetであり、質を犠牲にした短縮は
  禁止)
- 重複説明・同義の言い換え・不要な導入文を削った直後に、意味の変わらない別の言い回しで
  同じ内容を書き足すこと(削るときは、基本的に何も足さない="replace-with-nothing"。
  削った分だけ記事全体が短くなることを想定してください)"""
...
def run_phase_a(audit_dir: str) -> dict:
    ...
    print(f"[TRIAL-07][Phase A] clean_single_insert_confirmed={clean_single_insert}")
...
def run_five_section_point_qa_monitoring(client, sections: dict, writer_model: str, out_dir: str) -> dict:
    ...
    print(f"[TRIAL-07][run03] Point Overlap/Value QA monitoring(5区切り専用) "
...
def run_voices_pattern_run03(client, theme_id: str, label: str, prompt: str, verified_ledger_text: str,
    ...
    print(f"[TRIAL-07][{theme_id}] {label}: Point Overlap/Value QA(monitoring専用、5区切り版)開始...")
    ...
    if five_sections is None:
        ...
        with open(f"{out_dir}/point_overlap_value_qa_monitoring.json", "w", encoding="utf-8") as f:
            json.dump(monitoring_summary, f, ensure_ascii=False, indent=2, default=str)
        print(f"[TRIAL-07][{theme_id}] {label}: 5区切り構造が検出できずQA monitoring不能として記録しました。")
    else:
    ...
    print(f"[TRIAL-07][{theme_id}] {label}: metrics={metrics} five_section_report={five_section_report}")

    print(f"[TRIAL-07][{theme_id}] {label}: fact checker呼び出し開始...")
    ...
    print(f"[TRIAL-07][{theme_id}] {label}: fact_check status={fc_status} verdict={verdict}")
    fact_qa_record = {
    ...
    if verdict == "FAIL":
        print(f"[TRIAL-07][{theme_id}] {label}: fact checkerがFAILと判定しました。自動続行せず"
    ...
    print(f"[TRIAL-07][{theme_id}] {label}: ledger逸脱チェック開始(Hook-aware)...")
    ...
    print(f"[TRIAL-07][{theme_id}] {label}: deviation overall_status="
    ...
    while major_items and cycle < local_rewrite.MAX_REWRITE_CYCLES:
        ...
        print(f"[TRIAL-07][{theme_id}] {label}: Local Rewrite cycle {cycle}/"
        ...
        for idx, deviation in enumerate(major_items, start=1):
            ...
            print(f"[TRIAL-07][{theme_id}] {label}: cycle {cycle} NG item {idx}: resolved={r['resolved']} "
        ...
        print(f"[TRIAL-07][{theme_id}] {label}: cycle {cycle} Local Rewrite後、Ledger全体を再判定...")
        deviation_result = vfl01.run_deviation_check(
            client, verified_ledger_text, article_text, model=ledger_model, hook_aware=True)
        recheck_major = [d for d in deviation_result["parsed"]["deviations"] if d["severity"] == "MAJOR"]
        print(f"[TRIAL-07][{theme_id}] {label}: cycle {cycle} 再判定 overall_status="
    ...
    if remaining_major or any_human_review:
        print(f"[TRIAL-07][{theme_id}] {label}: Local Rewrite cycleを尽くしてもLedger MAJORが残存、"
    ...
    if apply_directional_fact_precheck:
        print(f"[TRIAL-07][{theme_id}] {label}: 比較方向Fact事前チェック開始...")
        ...
        print(f"[TRIAL-07][{theme_id}] {label}: 比較方向Fact事前チェック完了。"
...
# Writerを再実行する(run_trial07_pipeline())。
#
# EDITORIAL-B-FAMILY-VOICES-TRIAL-07-CONTRACT-REFINEMENT-01での拡張:
# タスク文書の検出対象リストに明示されている「TensionでResearch説明に
# 戻る構成」「Closingが単純要約」は、Trial-06までのVoice A/Voice B限定の
# 6基準では検出できない(Trial-06 attempt1の実測でも、Tension段落が
# survey/percentage主語の文へ戻る劣化が実際に観測された)。そのため、
# 本Trialではvoice_a/voice_bに加えてtension/closing本文も判定対象へ拡張
# する。voice_a/voice_bは既存6基準のまま無変更。tension/closingは、
# 既存6基準のうちsection横断で意味が通るもの(evidence_subject/
# numbers_foreground/discovery_syntax/evidence_memorable)を流用しつつ、
# tension専用に「Tensionの中心がEvidenceの説明に戻っていないか」、
# closing専用に「単なる要約/「両方に良い点がある」止まりになっていない
# か、または解決策提案になっていないか」を追加する。
# ============================================================
...
# 後方互換のためTrial-06までの名前も残す(voice_a/voice_bで使用)。
LEAKAGE_CHECK_FIELDS = VOICE_LEAKAGE_FIELDS

TENSION_LEAKAGE_FIELDS = (
    "leak_evidence_subject",         # survey/research/data/percentageが主語になっていないか
    "leak_numbers_foreground",       # 複数の数字が前面で比較されていないか
    "leak_discovery_syntax",         # Discovery/Trend型の説明構文へ戻っていないか
    "leak_evidence_memorable",       # Evidenceの方が2人の違いより記憶に残る書き方になっていないか
    "leak_tension_reverts_to_research",  # Tensionの中心が「2人がなぜ違う答えに至るか」ではなく
                                          # Evidence/Research自体の説明に戻っていないか(TRIAL-07新規)
)
CLOSING_LEAKAGE_FIELDS = (
    "leak_closing_simple_summary",  # Closingが単なる要約/「両方に良い点がある」止まり、または
                                     # Writer自身の解決策・妥協案の提案になっていないか(TRIAL-07新規)
)
...
def run_analytical_leakage_check(client, sections: dict, model: str, reasoning_effort: str,
    ...
    print(f"[TRIAL-07][Leakage Check] attempt{attempt}: any_flagged={result['any_flagged']} "
...
def build_leakage_corrective_note(leakage_result: dict) -> str:
    ...
    EDITORIAL-B-FAMILY-VOICES-TRIAL-07-CONTRACT-REFINEMENT-01(run02)での変更:
    run01では、この是正メモがflagged項目の指摘のみで、Contract全体の優先事項
    (特にCompactness約350語soft target・Tension/Closingの役割)を再言及しな
    かったため、是正の書き直しでVoiceの外部報告文を削った代わりに新しい説明文
    が足され、語数がattempt1(400語)→attempt2(453語)へ増加した(run01
    Reportで開示済み)。run02では、flagged項目の指摘に加えて、Contract全体の
    優先事項(Compactness/Tension/Closingの役割/replace-with-nothing原則)を
    毎回のattemptで必ず再掲する。"""
    lines = [
...
def run_trial07_pipeline(client, theme_id: str, label: str, base_prompt: str, verified_ledger_text: str,
    ...
    for attempt in range(1, MAX_WRITER_ATTEMPTS + 1):
        ...
        print(f"[TRIAL-07] Writer attempt {attempt}/{MAX_WRITER_ATTEMPTS} 開始(out_dir={attempt_dir})...")
        t0 = time.time()
        ...
        if result.get("status") != "OK" or not result.get("article_text"):
            ...
            print(f"[TRIAL-07] attempt {attempt}: status={result.get('status')}のためLeakage Check"
                  f"をスキップします。")
            break
        ...
        if sections is None:
            ...
            print(f"[TRIAL-07] attempt {attempt}: 5区切り構造が検出できずLeakage Checkをスキップ"
                  f"しました。")
            break
        ...
        if not leakage["any_flagged"]:
            print(f"[TRIAL-07] attempt {attempt}: Analytical Leakage Check flagged項目なし。確定。")
            break
        if attempt == MAX_WRITER_ATTEMPTS:
            print(f"[TRIAL-07] attempt {attempt}: 最大attempt数に到達。flagged項目が残った状態の"
...
def run_writer_stage() -> dict:
    ...
    # run02: run01のOUT_DIR直下tracked出力(audit/・audit_candidate_prompt_base.txt・
    # raw_usage_log_trial07_writer.jsonl・trial07_summary.json)を一切上書きしないため、
    # 全てLEVEL_OUT_DIR(b1b_run02)配下またはrun02専用ファイル名へ出力する
    # (Trial-04 run02の方式を踏襲)。
    phase_a = run_phase_a(f"{LEVEL_OUT_DIR}/audit")
    if not phase_a["phase_a_pass"]:
        print("[TRIAL-07] Phase Aで意図しない差分を検出したため、Writerへ進まずSTOPします。")
        return {"phase_a": phase_a, "pipeline": None, "status": "STOP_PHASE_A_UNEXPECTED_DIFF"}

    client = vfl01.get_client()
    cl.install(f"{LEVEL_OUT_DIR}/raw_usage_log_trial07_writer_run02.jsonl")
    ...
    print(f"[TRIAL-07] Writer + Analytical Leakage Checkパイプライン開始"
          f"(最大{MAX_WRITER_ATTEMPTS} attempts)...")
    pipeline_result = run_trial07_pipeline(
        client, THEME_ID, LABEL, candidate_prompt, verified_ledger_text, TOPIC_JA, LEVEL_OUT_DIR)

    with open(f"{LEVEL_OUT_DIR}/trial07_summary_run02.json", "w", encoding="utf-8") as f:
        json.dump({
    ...
    print(f"[TRIAL-07] 完了。total_attempts={pipeline_result['total_attempts']} "
...
# ============================================================
# EDITORIAL-B-FAMILY-VOICES-TRIAL-07 Phase B/C: Editorあり/なし/改善案の
...
def run_fact_safety_and_qa_from_text(client, theme_id: str, label: str, article_text: str,
    ...
    print(f"[TRIAL-07][PhaseB/C][{theme_id}] {label}: Point Overlap/Value QA(monitoring専用、5区切り版)開始...")
    five_sections = split_five_voice_sections(article_text)
    if five_sections is None:
        monitoring_summary = {
            "qa_status": "SKIPPED",
    ...
    if five_section_report is not None:
        with open(f"{out_dir}/five_section_length_report.json", "w", encoding="utf-8") as f:
            json.dump(five_section_report, f, ensure_ascii=False, indent=2)
    print(f"[TRIAL-07][PhaseB/C][{theme_id}] {label}: metrics={metrics}")

    print(f"[TRIAL-07][PhaseB/C][{theme_id}] {label}: fact checker呼び出し開始...")
    fc_prompt = r3.build_fact_check_prompt(topic, article_text, [])
    ...
    fc_result, fc_status, fc_attempts, fc_model, fc_response_id, fc_search_usage, fc_sources = \
        r3.run_fact_checker_with_gates(make_fc_fn, sleep_fn=time.sleep)
    verdict = fc_result.get("verdict") if fc_result else None
    print(f"[TRIAL-07][PhaseB/C][{theme_id}] {label}: fact_check status={fc_status} verdict={verdict}")
    fact_qa_record = {
        "label": label, "final_status": fc_status, "model": fc_model, "response_id": fc_response_id,
        "web_search_call_count": fc_search_usage["web_search_call_count"] if fc_search_usage else None,
        "attempts": len(fc_attempts), "result": fc_result,
    ...
    if verdict == "FAIL":
        print(f"[TRIAL-07][PhaseB/C][{theme_id}] {label}: fact checkerがFAILと判定。"
    ...
    print(f"[TRIAL-07][PhaseB/C][{theme_id}] {label}: ledger逸脱チェック開始(Hook-aware)...")
    ledger_model = routing.require_model(gen._writer_process(label), routing.WRITER_MODEL)
    deviation_result = vfl01.run_deviation_check(
        client, verified_ledger_text, article_text, model=ledger_model, hook_aware=True)
    print(f"[TRIAL-07][PhaseB/C][{theme_id}] {label}: deviation overall_status="
          f"{deviation_result['parsed']['overall_status']} deviations={len(deviation_result['parsed']['deviations'])}")
    ...
    while major_items and cycle < local_rewrite.MAX_REWRITE_CYCLES:
        ...
        newly_discovered_claims = [d["claim_in_article"] for d in major_items
                                    if d["claim_in_article"] not in previously_seen_claims]
        print(f"[TRIAL-07][PhaseB/C][{theme_id}] {label}: Local Rewrite cycle {cycle}/"
              f"{local_rewrite.MAX_REWRITE_CYCLES} - Ledger MAJOR {len(major_items)}件を検出"
              f"({len(newly_discovered_claims)}件は新規)。局所Rewrite開始...")
        ...
        for idx, deviation in enumerate(major_items, start=1):
            ...
            r["point_context"] = point_context
            cycle_results.append(r)
            print(f"[TRIAL-07][PhaseB/C][{theme_id}] {label}: cycle {cycle} NG item {idx}: resolved={r['resolved']} "
                  f"human_review={r['human_review_required']} attempts={len(r['attempts'])}")
        ...
        print(f"[TRIAL-07][PhaseB/C][{theme_id}] {label}: cycle {cycle} Local Rewrite後、Ledger全体を再判定...")
        deviation_result = vfl01.run_deviation_check(
            client, verified_ledger_text, article_text, model=ledger_model, hook_aware=True)
        recheck_major = [d for d in deviation_result["parsed"]["deviations"] if d["severity"] == "MAJOR"]
        print(f"[TRIAL-07][PhaseB/C][{theme_id}] {label}: cycle {cycle} 再判定 overall_status="
              f"{deviation_result['parsed']['overall_status']} MAJOR={len(recheck_major)}件")
    ...
    any_human_review = any(r.get("human_review_required") for r in local_rewrite_results)
    if remaining_major or any_human_review:
        print(f"[TRIAL-07][PhaseB/C][{theme_id}] {label}: Local Rewrite cycleを尽くしてもLedger MAJORが"
    ...
    directional_precheck_status = None
    if apply_directional_fact_precheck:
        print(f"[TRIAL-07][PhaseB/C][{theme_id}] {label}: 比較方向Fact事前チェック開始...")
        vfl_path = f"{os.path.dirname(out_dir)}/research/stage_b3_vfl.json"
        directional_result = dfp.audit_article_directional_facts(
            article_text, verified_ledger_text, vfl_path=vfl_path)
        directional_precheck_status = directional_result["overall_status"]
        with open(f"{out_dir}/audit/directional_fact_precheck.json", "w", encoding="utf-8") as f:
            json.dump(directional_result, f, ensure_ascii=False, indent=2, default=str)
        print(f"[TRIAL-07][PhaseB/C][{theme_id}] {label}: 比較方向Fact事前チェック完了。"
              f"overall_status={directional_precheck_status}")
...
def _load_phase_a_final() -> dict:
    ...
    (run_writer_stage()、または`python er012_editorial_b_voices_trial_07.py write`)。"""
    summary_path = f"{OUT_DIR}/trial07_summary.json"
...
def run_phase_b() -> dict:
    ...
    client = vfl01.get_client()
    cl.install(f"{OUT_DIR}/raw_usage_log_trial07_phase_b.jsonl")

    with open(f"{RESEARCH_DIR}/verified_fact_ledger.txt", encoding="utf-8") as f:
        verified_ledger_text = f.read()

    version2_dir = f"{OUT_DIR}/phase_b_no_editor"
    print(f"[TRIAL-07][PhaseB] Editorなし版(pre_editor_article.mdそのもの)へFact Safety/QAを適用...")
    ...
    print(f"[TRIAL-07][PhaseB] 完了。version2_status={version2_result.get('status')} "
...
# ============================================================
# EDITORIAL-B-FAMILY-VOICES-TRIAL-07 Phase C: Trial側の派生Editor(Production
...
TRIAL07_IMPROVED_EDITOR_EXTRA_BLOCK = """【追加ルール(EDITORIAL-B-FAMILY-VOICES-TRIAL-07、Trial限定の派生ルール。
...
EVIDENCE_COMPRESSION_EDITOR_PROMPT_TEMPLATE_TRIAL07 = ec_editor.EVIDENCE_COMPRESSION_EDITOR_PROMPT_TEMPLATE.replace(
    ec_editor._EC_FORBIDDEN_EDITS_MARKER,
    TRIAL07_IMPROVED_EDITOR_EXTRA_BLOCK + "\n\n" + ec_editor._EC_FORBIDDEN_EDITS_MARKER)


def run_improved_editor(client, article_text: str, model: str) -> dict:
    assert TRIAL07_IMPROVED_EDITOR_EXTRA_BLOCK in EVIDENCE_COMPRESSION_EDITOR_PROMPT_TEMPLATE_TRIAL07, (
        "追加ブロックの挿入に失敗しています(marker不一致の可能性)。")
    prompt = EVIDENCE_COMPRESSION_EDITOR_PROMPT_TEMPLATE_TRIAL07.format(article_text=article_text)
...
def run_phase_c() -> dict:
    ...
    print(f"[TRIAL-07][PhaseC] 改善Editor案適用後のテキストへFact Safety/QAを適用...")
    ...
    with open(f"{OUT_DIR}/phase_c_summary.json", "w", encoding="utf-8") as f:
        json.dump(summary, f, ensure_ascii=False, indent=2, default=str)
    print(f"[TRIAL-07][PhaseC] 完了。version3_status={version3_result.get('status')}")
