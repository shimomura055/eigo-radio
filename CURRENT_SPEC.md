...
**管理ID: ER-PM-001**
**最終更新: 2026-08-24(ER-006-GATE-EVIDENCE-REVIEW-CASCADE-ON-MATH-ADOPT-01、数式Validator正式採用・ASR Secondary Cascade Production ON)**
...
| 項目 | 現在値 | 状態 | 根拠Decision | 最終更新日 |
|---|---|---|---|---|
| ASR診断 | **2026-08-22更新(ER-006-TTS-BATCH-WIRING-SOT-CLEANUP-01)**: 「Azure STTを全内容確認・境界検証に使用」という旧記述は、Audio Production Pipeline節の言語別Routing仕様と矛盾していたため整合させた。現行仕様: 英語Primary ASR=OpenAI `gpt-4o-mini-transcribe`、日本語ASR=Azure Speech STT(維持)、AzureはSecondary ASR Cascade(Phrase List付き)として英語のentity-like mismatch検出時にも使用する。SSOTは[er006_asr_provider_routing_01.py](er006_asr_provider_routing_01.py)。決定はASR単独では下さない(Validator・Secondary ASR Cascade・Human Reviewを含む複数段階で判断する) | `DECIDED` | ER-006-AUDIO-COST-PILOT-02(Routing切替)、ER-006-TTS-BATCH-WIRING-SOT-CLEANUP-01(本節記述の整合化) | 2026-08-22 |
