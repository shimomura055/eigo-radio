...
**管理ID: ER-PM-001**
**最終更新: 2026-08-29(ER-008-N8-PRODUCTION-WIRING-AND-FOLLOWUP-19、Disfluency QA/Point-only regeneration/Comment内部ラベル二重防御/A2 slowdown invariant gate/A2 Preview短縮/B1 Preview話し方調整をPRODUCTION_WIREDへ昇格)**
...
| A2 Preview長さ | No.8実測(157文字・4文・28.6秒)がユーザーから「長い」と指摘され、theme/problem/value/questionの4要素を律儀に別文で書き並べない、2文程度・80〜110字程度を目安とする指示へPrompt(`PREVIEW_ROLE`、`er003_v1_iran01_a2_generate.py`)を更新した。絶対文字数を固定するhard limitではなく目安(多少の増減は許容、150字・4文を超える長いPreviewは避ける)。No.8実データ(実際のarticle.md・comment_1/2テキスト)を使い、本番のPrompt/model/context経由で新規生成した結果157字→74字(2文)を確認(runtime evidence、No.8本編の音声自体は今回未差し替え、今後の新規生成から適用) | `DECIDED`(`PRODUCTION_WIRED`、Prompt配線のみ。No.8既存音声は未反映) | ER-008-N8-QA-CONTENT-SPEED-HARDENING-18(トライアル提示・ユーザー試聴)→ER-008-N8-PRODUCTION-WIRING-AND-FOLLOWUP-19(ユーザー正式採用・Prompt配線) | 2026-08-29 |
...
| B1 Comment 1-4話し方(現状調査のみ、未変更) | 現状調査の結果、Comment 1-4もPreview同様、専用の話し方instructionを持たず`ENGLISH_STYLE_PREFIX`のみで生成されている(実測WPM: comment_1=173.5/comment_2=152.3/comment_3=162.9/comment_4=148.1、いずれもPreviewの184.5より遅いが互いにもばらつきがある)。Previewと同系統の落ち着いたinstructionを追加する方向性はあり得るが、ユーザー試聴による正式確定はまだ行っておらず、今回はComment側への変更は未実施 | `TBD`(現状調査のみ) | ER-008-N8-PRODUCTION-WIRING-AND-FOLLOWUP-19 | 2026-08-29 |
