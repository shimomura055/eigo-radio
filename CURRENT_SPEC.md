...
**管理ID: ER-PM-001**
**最終更新: 2026-08-25(ER-007-JA-ASR-VALIDATOR-REDESIGN-AND-CASCADE-01、日本語ASR全文Validator・Secondary Cascade・Primary ASR=OpenAI化をProduction配線)**
...
| Validator(日本語) | **2026-08-25新設**: [er007_ja_asr_validator_01.py](er007_ja_asr_validator_01.py)の`classify_ja_asr_match()`を、日本語Production Audio QAの標準Validatorとして採用(旧「文頭prefix一致+文字数チェック」方式を置き換え)。文字単位の全文sequence diff(`difflib.SequenceMatcher`)を取り、opcode単位で数字/否定マーカー/固有名詞らしさ(カタカナ列・英字acronym)/読み(pykakasi、diff span前後4文字を文脈として付加してから比較)を個別判定する。5分類: EXACT_MATCH/NORMALIZED_MATCH/PHONETIC_MATCH(漢字/ひらがな等表記ゆれだが読み一致)/ASR_VALIDATION_UNCERTAIN(固有名詞・略語らしき語のみの差)/TRUE_CONTENT_MISMATCH。**既知の残存限界**: 形態素解析器なし(kakasiは規則ベース)のため、孤立漢字の異読み分岐(例: 「頃」のgoro/koro、「後」のあと/のち)を文脈4文字パディングでも完全には解消できない場合がある(safe方向のfalse positiveのみ、実測96 segment中約3件[約3.1%]、false negativeは未確認)。Regression fixture: [er007_ja_asr_validator_01_test.py](er007_ja_asr_validator_01_test.py)(20件、6種類のblind-spot category+entity-like+実データ homophoneケース、全PASS) | `DECIDED` | ER-007-JA-ASR-VALIDATOR-REDESIGN-AND-CASCADE-01 | 2026-08-25 |
