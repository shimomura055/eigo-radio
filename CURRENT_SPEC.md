...
**管理ID: ER-PM-001**
**最終更新: 2026-08-25(ER-007-JA-ASR-TTS-RETRY-PATH-FIX-01、日本語ASR Cascadeのstop_retrying無視bug修正・濁点/半濁点読みゆれのCascade対象化)**
...
| A2英語ナレーション速度(post-processing time-stretch案) | 自然言語Prompt(上記行)がsame-text条件下でも安定して機能しない([OPEN_ITEMS.md](OPEN_ITEMS.md)のOPEN-71〜74参照、ER-008-A2-SPEED-SAME-TEXT-ABC-09でB/CともAより速くなることを確認)ことを受け、既存の完成音声そのものをFFmpeg `atempo`フィルタ(pitch-preserving time-stretch、単純なsample-rate変更[pitchが下がる]ではない)で事後的に減速する代替案を検証した。No.7 A2 Full Story Part 1の実音声で3%/6%/9%減速を試した結果、目標値と実測減速率がほぼ一致する(例: 目標6%→実測5.96%)予測可能な制御性、pitch(F0)は実質不変(0%比で-0.9%程度)、4条件ともPrimary ASRで内容破損なし(`NORMALIZED_MATCH`)、クリッピングなしを確認した。比較Artifact: https://claude.ai/code/artifact/6efa0f4c-79a8-4d1e-a3ae-8c81d853a53d | `VALIDATION_ONLY`(Production Audio Pipeline未配線、採用可否はユーザー試聴後に判断) | ER-008-A2-TIMESTRETCH-ABC-10 | 2026-08-26 |
