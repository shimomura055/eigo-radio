...
はER-003-A2-STRUCT-02〜04で**A01・A02・ADD03の3記事とも**プロトタイプを
完成済み(`PROTOTYPE_BUILT / UNDER_EVALUATION`、11パート構成: Preview→
Key Phrases→Comment1→Full Story Part1→Comment2→Full Story Part2→
...
統合台本: [A02](ER-003-A2-STRUCT-03_A02_INTEGRATED_SCRIPT.md)
(QAは[ER-003-A2-STRUCT-03_REPORT.md](ER-003-A2-STRUCT-03_REPORT.md))、
[A01](ER-003-A2-STRUCT-04_A01_INTEGRATED_SCRIPT.md)、
[ADD03](ER-003-A2-STRUCT-04_ADD03_INTEGRATED_SCRIPT.md)
(A01/ADD03のQAは[ER-003-A2-STRUCT-04_REPORT.md](ER-003-A2-STRUCT-04_REPORT.md))。
Candidate B(Listening Questions)は未着手のまま。

**一般化の実例として確認できたこと**: A01は「意味上の転換点」、ADD03は
「発表→反応」と、記事によって異なる種類の分割根拠(7節の優先順位表)を
選んでおり、Comment文言もA02のものを一切流用していない(3記事で
重複する文はゼロ)。役割・判断基準のみが一般化され、具体的な文言は
記事ごとに新規作成されるという設計が、3記事分の実例で裏付けられた。
