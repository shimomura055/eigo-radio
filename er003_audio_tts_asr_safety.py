...
# 助数詞の直前に来る単独漢数字(一〜九)だけを対象にした限定的な
# 同値正規化(ER-008-B1-POINT2-FACT-FIX-AND-JA-NUMERAL-NORMALIZATION-07で
# 「つ」限定として導入、ER-011-KP-VALIDATOR-NUMERIC-HOMOPHONE-AND-GLOSS-
# RULES-PRODUCTION-WIRING-02で閉じたリストへ拡張、2026-09-06ユーザー承認
# 済みTrack A・縮小リスト)。「一つ/二つ/…/九つ」等は常に「かず+助数詞」の
# 形で数量そのものを表し、_extract_numbers_ja()が単独漢数字を対象外に
# した理由(固有名詞的な語内での偶然の一致、例:"京三")とは性質が異なる
# ——助数詞の直前という文脈があるため、意味・読み・数量が完全に同じ
# 表記ゆれとして安全に判別できる。
#
# 拡張の背景(Opus診断-15、Trial-17 Track A):
# `er007_ja_asr_validator_01.py`の数字保護ゲートが、「つ」限定の正規化
# だけでは「泊」等の他助数詞をカバーせず、canonical「中央値は約2泊」/
# ASR「中央値は約二泊。」を数字消失と誤判定していた(Trial-13 kp3_ja_
# charon、OPEN-116(a))。
#
# リスト範囲(縮小版、ユーザー承認済み):
# 泊/回/件/年/時間/か月/週/歳/つ。「日」「人」は意図的に対象外とする。
# 理由(Trial-17 Track A回帰で実証済み):
#   - 「日」: 慣用句「三日坊主」(正しい読みは不規則な"mikkabouzu")の
#     「三」が「日」の直前に来るため、フルリスト採用時に"3日坊主"へ
#     変換されてしまい、kakasiが数字文字"3"を音声化しない
#     (_kakasi_reading("3")は"3"のまま)ため、全文ひらがな書き起こしとの
#     読み一致フォールバックが壊れる実regressionを確認した
#     (正しくPASSすべきものがFAILへ悪化する安全側の機能低下)。
#   - 「人」: 「一人/二人」がそれぞれ不規則読み"hitori"/"futari"であり、
#     「三日」型と同型の構造的リスクを持つため、直接のregression再現は
#     未実施だが予防的に対象外とする。
# 一般の漢数字変換(「つ」「泊」等の助数詞を伴わない任意の漢数字、いわゆる
# 案A')は採らない(過剰な一般化の禁止、ユーザー明示不採用)。
_CLOSED_COUNTERS_JA = ("つ", "泊", "回", "件", "年", "時間", "か月", "週", "歳")
_KANJI_COUNTER_RE = re.compile(
    r"[一二三四五六七八九](?=(?:" + "|".join(_CLOSED_COUNTERS_JA) + r"))"
)


def normalize_kanji_counter_numerals_ja(text: str) -> str:
    """閉じた助数詞リスト(_CLOSED_COUNTERS_JA)の直前に来る単独漢数字だけを
    算用数字へ変換する(例:"二つ"->"2つ"、"二泊"->"2泊"、"二十"や"二回説明"
    の"二"のように直後が助数詞そのものでない場合や、リスト外の助数詞
    [日/人]は対象外のまま変更しない)。"""
    if not text:
        return text
