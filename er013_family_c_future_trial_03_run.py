# ============================================================
# er013_family_c_future_trial_03_run.py
# 管理ID: EDITORIAL-FUTURE-FAMILY-C-FINAL-TUNING-TRIAL-03
# ============================================================
# 目的: Family C(Future)最終調整版(hedging削減+A2短縮)の記事生成Trial
# driver。Trial-02の同一Ledger・同一World Scaffold・同一Layer2/3・同一
# World Packageを無変更のままread-onlyで再利用し(Research/Scaffold/
# Layer2/3の再生成費用¥0)、Writer(v3)→編集Gate v3(v2の5項目+枠内hedge
# 密度+語数)→Fact Checker A'(Layer1のみ、既存関数無改変)→Ledger
...
#
# 再利用(import・無変更):
#   - er013_family_c_future_trial_02_run.py が定義した費用計算ロジック
#     (compute_cost_jpy_so_far/_call_cost_usd/_price)と同一のロジックを
#     本ファイル内に複製する(Trial-01→02と同じ再利用パターン、01/02
#     いずれも同一ロジックを複製している。02自体はimportしない
#     [02はmain()実行時にscaffold/layer23の副作用を持つモジュールのため、
#     importだけでも意図しない結合を避ける安全側の判断])。
#   - er002_ja_web_research_r3(r3)/er003_v1_en_direct_vfl_01_generate
#     (vfl01)/er005_cost_logger(cl)/er006_model_routing_contract_01
#     (routing)/er010_ledger_local_rewrite_09(rewrite09): Trial-01/02と
#     同一の再利用方針(無変更)。
#   - er013_family_c_future_writer_03(fcw3)/_qa_03(fcq3): 本Trialのために
#     新設した独立モジュール。
#
# 費用上限: ¥30(ユーザー指示書の見積¥20〜30を上限としてそのまま採用)。
# 実行中にcost_stage()で都度実測し、上限到達時はRuntimeErrorでSTOPする
# (Trial-01/02と同一の安全装置パターン)。
#
# 実行方法(root直下から):
#   .venv/Scripts/python.exe er013_family_c_future_trial_03_run.py [stage ...]
...
THEME_ID = "family_c_future_trial_03"
...
# Trial-02で既に確定済みのResearch/World Scaffold/Layer2-3/World Package
# 成果物を再利用する(ユーザー指示書「同一Ledger・World Scaffold再利用」)。
...
REUSED_RESEARCH_FILES = (
    "world_scaffold_result.json", "world_scaffold_text.txt",
    "layer23_v2_result.json", "world_package_text.txt",
)
...
# ============================================================
# 費用実測(Trial-01/02と同一ロジック、read-onlyで転記)。
# ============================================================
USD_JPY = 160.0
...
# ============================================================
# Step 0: Trial-02のResearch/World Scaffold/Layer2-3/World Packageを
# read-onlyで再利用する(コピーのみ、LLM呼び出し無し、費用¥0)。
# ============================================================
def reuse_research_stage() -> dict:
    ...
    return {"scaffold_parsed": scaffold_parsed}


# ============================================================
# Step 1: 記事生成(Writer v3)+編集Gate v3(決定的スキャン、regen最大1回)+
...
LOCAL_REWRITE_MAX_CYCLES_TRIAL = 1  # Trial-01/02と同一の保守的な上限
...
def _generate_writer_output_with_marker_retry(client, out_dir: str, level: str, prompt: str) -> tuple:
    ...
    retryを行う(内容不満による再生成ではない、Trial-02と同一パターン)。"""
    ...
    for attempt in range(1, MAX_WRITER_MARKER_RETRY + 1):
        with cl.logging_context(THEME_ID, f"writer_{level}_attempt{attempt}"):
            writer_result = fcw3.generate_family_c_article_v3(
...
def generate_and_qa_family_c_article_v3(level: str) -> dict:
    ...
    word_count_range = fcw3.WORD_COUNT_TARGET_RANGE[level]
    ...
    for gate_attempt in range(1, MAX_EDITORIAL_GATE_ATTEMPTS + 1):
        prompt = fcw3.build_family_c_writer_v3_prompt(FAMILY_C_TOPIC_JA_PLACEHOLDER, level, world_package_text,
        ...
        print(f"[{THEME_ID}][{level}] 編集Gate(v3) attempt {gate_attempt}: FAIL "
    ...
    if rewrite_log:
        ...
        print(f"[{THEME_ID}][{level}] rewrite後の編集Gate(v3)再確認: overall_status={gate_result['overall_status']}")
...
def main() -> dict:
    ...
    if stages == ["all"]:
        ...
        for level in ("a2", "b1"):
            results[level] = generate_and_qa_family_c_article_v3(level)
            cost_stage()
    ...
    for s in stages:
        if s == "reuse_research":
            ...
            result[s] = generate_and_qa_family_c_article_v3(s)
        elif s == "evaluate":
