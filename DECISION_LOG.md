...
- **Cost**: Diagnostic section 生成 = ¥0（機械的処理、LLM不要）。Writer 全文 Retry = ¥0.82/retry1回（Luna API call、input 3430トークン + output 1422トークン実測）。Point overlap checker = ¥0（local 処理）。Ledger Deviation check = 実行時のみ費用発生。Diagnostic Retry 1 回の追加費用合計 = 約 ¥0.82（診断生成分除く Writer Retry のみ）
