...
## USER-TEST-NEWS-CONVENIENCE-AI-01-FINALIZE-A2: A2 point_two "Oimo no Canele"/"AI while"ユーザー正式承認(USER APPROVED)→Human Approval記録+Assembly/Gate PASS+player/E2E完成、A2/B1ともUSER_TEST_READY、OPEN-168新規登録

- 日付: 2026-09-17
- 種別: ユーザー実試聴後の正式判断(2026-09-17)に基づくfinalize。FIX-02確認ページで提示したA2 `point_two`新候補(attempt9由来、6% slowdown適用済み)を試聴し、「Oimo no」= OK/承認。よって「Oimo no Canele」全体・「AI while」ともUSER APPROVED。追加TTS再生成は行っていない。B1は前回(FIX-02)で既にユーザー品質承認済みのため追加試聴なし。
- **sha256照合**: FIX-02確認ページで提示した音声(`a2/human_review/point_two.mp3`の元、`audit_fix_02/candidate_attempt9_poststretch.wav`、sha256=`ed816a60...`)と、現在の`a2/narration/point_two.wav`が完全一致することを確認した。`point_two_original.wav`(sha256=`e233dec4...`)がattempt9の生wavと完全一致することも確認し、既に既存Production関数`apply_a2_slowdown_postprocess()`で6% time-stretch(実測比率1.0591、pre=20.461s→post=21.671s)が適用済みであることを確認した。**本タスクでは再適用していない(二重適用回避)**。
- **内蔵Primary ASR再検証(fresh、単発)**: `apply_a2_slowdown_postprocess()`内部と同一ロジック(`routing.transcribe`+`en_validator.classify_asr_match`)を現在の音声へ1回実行し、classification=`ASR_VALIDATION_UNCERTAIN`(asr_text中"Kanere"表記ゆれ、Oimo自体は問題なし、既存分析と整合)を確認・記録した。
- **追加Secondary ASR cascade(Ledger Phrase List使用、fresh)**: 既存Production cascade関数`secondary_asr.evaluate_attempt_with_cascade()`(fallback経路と同一関数)を1回実行し、classification=`NORMALIZED_MATCH`/verified_content=Trueを確認("Oimo no Canele"/"AI while"ともslowdown後も転写上維持されていることを証跡化)。詳細: `a2/audit_finalize_a2/finalize_a2_point_two_summary.json`。
- **Human Approval記録**: 既存Production関数`er003_v1_n3_01_assemble.record_human_approval()`(Tiny Bags A2 CLOSEOUT-03等と同一手続き)で、`a2/parts.json`の`point_two_body`原文(sha256=`fa2f9464...`)を対象にapproved_by=`user_via_fable_sandwich_pm`として記録した。`audit/tts_generation_results.json`のpoint_twoエントリを、FIX-02確認ページ提示時に残っていた非標準値`HUMAN_REVIEW_REQUIRED_CANDIDATE`(Gateの3値のいずれにも該当せずUNVALIDATEDへ落ちる不整合)から、実態(標準cascadeはPASSしたがpost-slowdown簡易再検証がUNCERTAIN)に即した`STOPPED`へ是正し、`slowdown_applied=True`・sha256・duration等を現物ファイルへ同期した(事実の変更ではなく記録形式の是正)。`review_lock_state.json`へ承認参照のnoteを追記(state/final_status自体は変更せず、既存Gate設計どおりHuman Approval記録経由でHUMAN_APPROVED判定させる)。
- **Assembly/Audio Validation Gate**: `run_pipeline.assembly_stage("a2")`(既存関数、無変更)を実行し、`status=OK`・duration=323.279秒・peak=0.95344・clipping=False、Gate opt-in(`derive_a_family_required_structure`込み)も`PASS`(MISSING_MANDATORY_A2_SLOWDOWN等のブロックなし)を確認した。
- **player/web**: `run_pipeline.player_stage("a2", title)`(既存関数、無変更)でplayer.html+web export(episode.mp3+segments)を生成。player.html内に新canonical文言"is scheduled to go on sale"・"Oimo no Canele"が表示されることを確認済み。
- **Browser E2E**: Playwright headless Chromium実ブラウザで、rawcdn.githackの「External Content Notice」中継確認(`button.url-action-button`クリック)を経由し、`https://rawcdn.githack.com/shimomura055/eigo-radio/437d6b71/user_test/unified.html?src=.../a2/player.html&level=A2&en=...&ja=...`を検証。再生4秒後`currentTime=2.89秒`・`duration=323.279秒`(Assembly実測と一致)・`error=null`・`readyState=4`、60秒seek後`currentTime=61.45秒`、本文中に"is scheduled to go on sale"/"Oimo no Canele"を含むことを確認、Key Phrase/Comment関連マーカーも表示確認。consoleエラーは既知の無害な広告ブロック1件のみ。evidence: `docs/pm/closeout_136_e2e/convenience_ai_a2_finalize.json`+`.png`。
- **B1整合確認(再試聴なし)**: `b1b/article.md`/`b1b/parts.json`/`b1b/player.html`がいずれも"Then Lawson planned a sale of the finished product."で一致することを確認。B1 player URL(SHA`187d51b4`)がHTTP 200で到達可能であることを確認(既存E2E evidence`b1b/human_review/e2e_evidence_fix02.json`は無変更のまま参照)。B1側の再build・再試聴は実施していない。
- **Status**: コンビニAI A2/B1 = **`USER_TEST_READY`**として完結(記事品質のユーザー判断待ちなし)。
- **A2 final player URL**: `https://rawcdn.githack.com/shimomura055/eigo-radio/437d6b71/user_test/unified.html?src=er014_output/user_test_news_convenience_ai_01/convenience_ai/a2/player.html&level=A2&en=Pickles%20in%20a%20Lemon%20Tart%3F%20When%20AI%20Joins%20the%20Convenience-Store%20Kitchen&ja=%E6%97%A5%E6%9C%AC%E3%81%AE%E3%82%B3%E3%83%B3%E3%83%93%E3%83%8B%E3%80%81AI%E3%81%A7%E6%96%B0%E3%81%97%E3%81%84%E5%91%B3%E3%82%92%E9%96%8B%E7%99%BA`。
- **Sheet投入用行(確定)**: 記事タイトル(English)="Pickles in a Lemon Tart? When AI Joins the Convenience-Store Kitchen"、記事タイトル(日本語)="日本のコンビニ、AIで新しい味を開発"、記事の概要(日本語)="ローソンがAIに提案させた意外な組み合わせ(レモンタルト+ピクルス)を人間が試作・調整して商品化予定。ファミリーマートは販売データを使う別のAI活用法。"、Family=News、ノーマル(A2)URL=上記、Advanced(B1)URL=FIX-02のG5参照(SHA`187d51b4`)、備考="最新ニュース"。
- **新規Open Item**: `OPEN-168`(A2必須6% slowdown post-processの内蔵再検証がLedger Phrase List付きSecondary ASR cascadeを使わず固有名詞segmentを誤ってブロックし続ける設計ギャップ、`CURRENT_SPEC.md` L1287の既知ギャップを実例[Tiny Bags A2 full_story_part2・本件]と共に独立Open Item化。Blocking対象なし、量産開始前にユーザー判断)。OPEN-159へも参照追記。
- **CURRENT_SPEC.md**: 変更不要と確認(既存L1287の記述で本件のgapは既に正確に記録済み)。
- **cost実測**: 本タスク差分は内蔵Primary ASR再検証1回+Secondary ASR cascade(Primary 2回+Azure 1回)のみ(TTS新規生成なし)、¥1未満(openai_asr 2回分)。累計¥201.73+微増。上限¥30に対し十分な余裕。
- **無変更証跡/lock記録**: `docs/pm/locks/audio_stage.lock`を原子的作成(タスク開始時に既存lock無しを確認)、タスク完了時に削除。`git status --porcelain -- '*.py'`は新規`finalize_a2_pipeline.py`のみ(既存`er0*.py` Production moduleは無変更)。`git status --porcelain -- user_test/unified.html`は空(無変更、既存ページをそのまま再利用)。事前指定外Read: なし(事前指定Read一覧[前回RESULT_PACKET・`fix_02_pipeline.py`・`audit_fix_02/*.json`・Tiny Bags先例スクリプト・`er003_v1_n3_01_assemble.py`該当関数・`docs/pm/closeout_136_e2e/`・OPEN_ITEMS該当行]に加え、`er003_v1_n3_01_tts_generate.py`の`apply_a2_slowdown_postprocess`/`generate_a2_segment_with_slowdown`本体[内蔵再検証ロジックを正確に再現するため]・`er003_v1_crosslevel_audio_02_common.py`の`generate_english_segment_with_fallback`[Secondary ASR cascade呼び出しパターン確認のため]・`er006_secondary_asr_01.py`の`evaluate_attempt_with_cascade`/`is_homophone_candidate_mismatch`シグネチャ確認、いずれも委任文の「既存関数を無変更で再利用」を安全に遵守するために必要な確認)。
- Git SHA: `437d6b71`(Human Approval記録・Assembly/Gate/player/web export・finalize script、push済み)。SSOT(DECISION_LOG/OPEN_ITEMS/ARTIFACT_REGISTRY)反映commitは別途。
- 参照: `docs/pm/RESULT_PACKET_NEWS_CONVENIENCE_AI_01.md`「## FINALIZE-A2」節、`docs/pm/delegation_log/USER-TEST-NEWS-CONVENIENCE-AI-01-FINALIZE-A2.md`。
...
- 日付: 2026-09-17
- 種別: ユーザー正式承認(2026-09-17、`APPROVED_FOR_PRODUCTION`)に基づくProduction wiring。背景は`docs/pm/RESULT_PACKET_VOICES_SPEC_REVIEW_01.md`(Personalized News A2ユーザー試聴NG=Voiceが調査・統計を語る[問題A]、各Voiceの立場が曖昧[問題B]、の原因分析・候補A〜F整理)。採用: 候補A(Voiceは自分の経験・価値観・立場に徹する)、候補C(Voice内数字は本人経験由来の数値のみ許可、survey/report/study等の外部統計をVoice自身の論証材料にしない)、候補F(反対側の中心的懸念は別VoiceまたはTensionに置く、既存Tension「共通前提→分岐点→非対称性」3段構造の趣旨を明文化)、候補E(nuance[自分の立場の中の迷い]は許容、相手側の中心論点は抱え込ませない)。不採用: 候補B(Narrator/Evidenceレイヤー新設)・候補D(1 Voice=1 Primary Positionの恒久ルール化)。
- **Prompt変更**(`er012_b_family_voices_writer_generic_01.py`、2V/3V共通template両方): 「Evidenceは脇役であること・Voice内の数字は最大1つ」段落を「本人の経験に属する数字のみ」へ更新、新規段落「Voiceは自分の経験・立場に徹すること、相手側の中心的懸念はTensionへ」を追加、Tension役割文言へ「両Voiceの中心的懸念を公平に受け止める場所」という一文を追加、禁止事項まとめへ2行追加(外部統計の論証材料化禁止/相手側中心論点の先取り禁止)、`_voice_card_block_text()`のsupporting_evidence数字許可条件を「本人経験由来の数値が含まれる場合に限る」へ更新。A2側(`main_a2_2v()`が`run_writer_stage_generic(..., instruction=gen_articles.A2_KAI1_INSTRUCTION)`経由で2V共通templateをそのまま使う経路)にも自動的に適用される(A2専用の分岐は無い)。
- **Validator変更**: `VOICE_LEAKAGE_FIELDS`(2V/3VのVoiceセクションで共有)へ`leak_position_blur`(相手側Voiceが中心的に抱えている懸念・反論を、このVoiceが自分の中心的な主張として代弁していない場合PASS)を追加。`leak_numbers_foreground`の基準文言を「数字は本人の経験に属する数値であること」へ更新。Tensionセクションのfield集合(`TENSION_LEAKAGE_FIELDS_BASE`/`TENSION_LEAKAGE_FIELDS_2V`)は無変更(position blur判定はVoiceセクション限定)。是正メモ(`build_leakage_corrective_note_2v/_3v`)へ「立場境界」優先事項1行を追加。
- **Acceptance Gate**: `run_pipeline_2v()`/`run_pipeline_3v()`が、MAX_WRITER_ATTEMPTS(3)到達後もAnalytical Leakage Check flagged項目が残存する場合、`final_result["leakage_residual"]=True`+`final_result["status"]="LEAKAGE_RESIDUAL_STOP"`を記録するよう変更。`main_a2_2v()`/`main_b1_2v()`/`main_b1_3v()`のwrite_new_theme stageは既存の`final_result.get("status")=="OK"`判定をそのまま使っているため、この値変更だけで自動的に後続stage(comment/key_phrases/japanese_title/voice_check/tts/assemble/player)へ進まずSTOPする(MAX_ATTEMPTS自体・既存Gate基準は変更なし、手動override無し)。「前例踏襲でPARTIAL扱いとしユーザー試聴へ進める」という従来運用は本Decision以降廃止。
- **fact_attribution_mode(A')との関係**: 既定OFF据え置き。`FACT_ATTRIBUTION_MODE_DEFAULT`定義箇所(`er012_b_family_editorial_type_registry_01.py`)へ、将来ONにする場合は候補Cとの両立条件(統計を扱うVoiceにのみ限定適用する等)を別途設計する必要がある旨のコメントを追加(現状未設計)。
- **regression**: `run_project_regression.py`で2897件収集・2894 PASS・既知FAIL3件(`er003_test_p2j_investigate`のテスト件数照合arithmetic、本タスクと無関係)・新規FAIL 0件。`ThreeVoiceByteInvarianceAgainstHeadTests`の`build_focus_module_block_3v`/`build_leakage_schema_3v`/`build_leakage_check_prompt_3v`/`build_leakage_corrective_note_3v`/`run_pipeline_3v`は意図的な変更のためbyte不変性チェック対象から除外し、それぞれ新しい契約テスト(新原則文言の存在確認、`leak_position_blur`がVoiceセクションのみに追加されfield集合が変わらないことの確認、Acceptance Gateのsource確認)へ置換。新規テスト5件追加。
- **runtime evidence(Personalized News A2、`main_a2_2v()`全stage実行、既存Ledger再利用・Research再実行なし)**: write_new_theme stageでWriter attempt1にてAnalytical Leakage Check**0件PASS**(voice_a/voice_bとも7項目全PASS、`leak_position_blur`含む)、Fact Checker A' verdict=`REVIEW_REQUIRED`(推薦システムの「切り替えられない」という一般化claimの地域・サービス限定不足、既存の複合Voice帰属パターンの範囲内でnon-blocking)、Ledger Deviation Checker=`LEDGER_COMPLIANT`(Local Rewrite cycle 1で1件のLedger外claim["switch the system off"]を検出・自動修正)。新タイトル"The Same Feed, Two Different Mornings"(日本語タイトルは自然な直訳「同じフィード、二つの違う朝」へconfig更新)。comment/key_phrases(`REDUNDANCY_PASS`)/japanese_title/voice_check/tts/assemble/player全stage完走。TTS/ASR: 14 segment全て`OK`(Human Review Lock発生なし)。Assembly: `status=OK`、duration=331.192秒、peak=0.95492、clipping=False。player.html+web export(episode.mp3+segments 35件)完成。Playwright E2E(headless Chromium、rawcdn.githack commit`a54d0201`)で実再生確認: `after_play_4s`(currentTime=3.03秒、paused=false、readyState=4、error=null、duration=331.1915[Assembly実測と一致])、`after_seek`(60秒seek+1.5秒待機でcurrentTime=61.44秒)、Key Phrase表示・英日タイトル表示確認、consoleエラーは既知の無害な広告ブロック1件のみ。cost実測: 本タスク差分¥73.94(累計、writer/comment/key_phrases/japanese_title/voice_check/tts、model_id: TTS=`gemini-2.5-pro-preview-tts`(Aoede/Algieba/Erinome)、Writer/QA=`gpt-5.6-luna`)。
- **runtime evidence(Personalized News B1、`main_b1_2v()`のwrite_new_theme stage、既存theme module`er014_output/four_type_observation_01/voices/voices_theme_personalized_news_01.py`のTHEME_CONFIG・Ledger再利用)**: 事前offline再評価(新Validatorで既存音声化済み記事[`voices/audio/b1_2v_v2/b1b/article.md`]を判定のみ実行)でvoice_b 6項目FAIL+tension 2項目FAIL(`leak_position_blur`自体はPASS、既存の統計前景化・Discovery構文由来の残存)を確認したため再生成を実施。attempt1でLeakage是正retryが発火(voice_b numbers_foreground/discovery_syntax、tension evidence_subject/discovery_syntax)、是正後のattempt2でFact Checker A'が`FAIL`と判定(Ledger内の記述「短期テストでは政治的態度の測定可能な変化は確認されていない」が、2026-02-18公開の新しいNature論文の知見と矛盾するとWeb Searchで検出。これはVoice立場境界の仕様変更とは無関係な、既存Ledgerの内容鮮度の問題)。既存retry機構どおりFact Checker FAILは即座に`NG_REVIEW_REQUIRED`として記事を確定させず、リトライは追加されない(MAX_ATTEMPTS自体の変更・Gate緩和はしていない)。本タスクの制約「Research再実行なし」によりLedger自体の更新は範囲外のため、ここでSTOP(記事未確定、音声化未実施、既存音声[known flag残存のまま]は無変更のまま維持)。cost実測: ¥12.32。
- **AI hiring 3V B1/A2 offline再評価**(判定のみ、regenerateなし): B1(`editorial_b_voices_3v_audio_trial_01/b1b/article.md`)は新Validatorで**0件PASS**(`leak_position_blur`含む全項目、仕様レビューが「中リスク」と指摘したOwner Voiceの言及も、自分の立場の中のnuanceとして判定されFAILしなかった)。A2(`user_test_voices_a2_minimal_01/ai_hiring_3v_a2/a2/article.md`)は voice_2(narrator_analysis/discovery_syntax)・voice_3(numbers_foreground/discovery_syntax)・tension(discovery_syntax/evidence_memorable)でFAIL(`leak_position_blur`自体はPASS)。両記事とも再生成は実施していない(判定のみ)。
- **free_address B1/A2 regression**(新Validatorでの誤検知確認): B1(`editorial_b_family_production_phase1_02/b1b/article.md`)・A2(`editorial_b_voices_a2_free_address_04/a2/article.md`)ともに新Validator(7項目)で**0件FAIL**(誤検知なし、`leak_position_blur`含む)。
- **並行タスク衝突**: `docs/pm/locks/audio_stage.lock`は音声段階(japanese_title以降)の直前に作成、Personalized News A2完成後に削除。ただしKey Phrase選定stage(text stageと想定していたが実際には内部でMaster Audio Store/Pronunciation Ledgerへの参照・登録を行う実装だった)をlock取得前に実行したため、同時並行していた別タスク(`USER-TEST-NEWS-CONVENIENCE-AI-01-FIX-01`)のcommit(`86cbd93d`)に本タスクの追記分が混在した(共有チェックアウトのため)。差分は追記のみ(`git diff --stat`で確認、削除なし)でデータ破損なし。今後の同種タスクでは「Key Phrase選定stageも音声共有ストアを書きうる」ことをlock対象の事前チェックに追加する必要がある(教訓として記録、恒久ルール化はFable判断)。加えて、本タスクと`USER-TEST-NEWS-CONVENIENCE-AI-01-FIX-01`は同一working tree(`C:\Users\tensh\eigo-radio`)を共有しており、`DECISION_LOG.md`編集時に相手の未commit編集と同時に存在する状態を確認した(相手側も同一事象を自ら報告済み)。Editツールの完全一致置換の性質上、上書き事故は発生しなかったが、SSOT編集の並行衝突リスクは`docs/pm/locks/audio_stage.lock`が対象とする音声共有ストアだけでなくSSOTファイル自体にも及ぶことが判明した(教訓として記録)。
- Status: `PARTIAL`(Personalized News A2は基盤配線+新原則Prompt+Validator+Acceptance Gate+完全E2Eまで到達、Personalized News B1はFact Checker FAIL[Voice立場境界とは無関係のLedger鮮度問題]でSTOP、AI hiring 3V/free_addressはoffline判定のみ)。必須項目1〜13の充足状況は`docs/pm/RESULT_PACKET_VOICES_SPEC_01.md`10節参照。
- 参照: `docs/pm/RESULT_PACKET_VOICES_SPEC_01.md`、`docs/pm/RESULT_PACKET_VOICES_SPEC_REVIEW_01.md`、`docs/pm/delegation_log/B-FAMILY-VOICES-POSITION-AND-EVIDENCE-SPEC-01.md`。

## USER-TEST-NEWS-LIGHT-TOPIC-01-CLOSEOUT-03: Tiny Bags B1ユーザー試聴PASS記録+A2 Toteme/Kallmeyer Human Review承認→Assembly/Gate/player/E2E完成

- 日付: 2026-09-17
- 種別: ユーザー正式判断(2026-09-17)2件の反映+A2完成作業。**記事品質承認**(本ユーザー判断)と**Production spec Status**(`PRODUCTION_WIRED`等、B-Family Voices等の別仕様のStatus)は別軸であり、本エントリは前者のみを扱う。News通常経路の基盤Statusは既存記載のまま変更しない。
- **B1ユーザー承認**: 完成player(`https://rawcdn.githack.com/shimomura055/eigo-radio/e858649a/user_test/unified.html?src=er014_output/user_test_news_light_01/tiny_bags/b1b/player.html&level=B1&en=Are%20Tiny%20Bags%20Really%20Back%3F%20Fashion%27s%20Answer%20Comes%20With%20a%20Catch&ja=...`)で全体試聴、ユーザーが**OK/承認**(成果物の正式なユーザー品質承認、Trial評価ではない)。article/audio/playerは現行完成版を維持、再生成は行っていない。Status=ユーザー試聴PASS。
- **A2 `full_story_part2`(Toteme/Kallmeyer)Human Review承認**: RESUME-02のHuman Review確認ページ(`https://rawcdn.githack.com/shimomura055/eigo-radio/e858649a/user_test/human_review.html?src=er014_output/user_test_news_light_01/tiny_bags/a2/human_review/full_story_part2_review.json`)でToteme/Kallmeyerの実読を確認したユーザーが、「現在の音声でOK、最終版として採用」と正式判断(2026-09-17)、追加TTS再生成なし。既存Production関数`er003_v1_n3_01_assemble.record_human_approval()`(Space Weapons B1 `preview`/`full_story_part1`のRESUME-03承認と同一の既存正式手順、無変更)で、canonical_text(Gate参照優先フィールド)のsha256=`9d83044b5a55e731b0d40a2ff5f33c534e0bafb82a28acde58fc9da258750b05`を`audit/human_approved_segments.json`へ記録(approved_by=user、reason欄にRESUME-02確認ページURL・採用attempt・sha256照合結果を明記)。承認対象の音声は、確認ページで提示したattempt3[minimalfallback]のwav(sha256=`f55c90250480a9cffcc892bb81582606d336634ce2f58f5b0e12ee9854358b87`)と、その時点のnarration/full_story_part2.wav(同sha256)が一致することを確認済み(STOP条件のsha256不一致には該当しない)。`review_lock_state.json`のfull_story_part2エントリは、Space Weapons RESUME-03の先例(state自体は`HUMAN_REVIEW_REQUIRED`のまま変更せず、`human_approval_reference`フィールドを追記するのみ)にそのまま倣い、既存フィールドは変更せず`human_approval_reference`を追記した(Gateの実際の許可判定はaudit/human_approved_segments.jsonのHuman Approval記録によるHUMAN_APPROVED分岐で行われる設計のため、`state`をRESOLVEDへ書き換える必要はない、`_segment_gate_status()`の既存設計どおり)。
- **A2必須post-process(6% time-stretch)の追加適用**: Human Approval記録後にAssemblyを実行したところ、Audio Validation Gateが`full_story_part2=HUMAN_APPROVED(MISSING_MANDATORY_A2_SLOWDOWN)`でブロックした。原因は、A2本文segmentに必須の6% time-stretch post-process(`apply_a2_slowdown_postprocess()`)がstatus=="OK"の場合にのみ標準フローで自動適用される既存仕様であり、本segmentは一度もASR検証に合格していない(最終status=ASR_VALIDATION_UNCERTAIN)ため未適用のまま残っていたため。これは`er011_open121_trial12_a2_full_story_part1_slowdown_apply_01.py`・`er011_open112_theme2_audio_review_fix_02_subtaskg_apply_slowdown_01.py`と同型の既知パターン(Human Review Lock経由で承認された音声が必須post-processを一度も受けないままAssembleへ到達しようとするケース)であり、両先例と全く同じ既存Production関数`apply_a2_slowdown_postprocess()`(無変更)を、ユーザー承認済みのnarration/full_story_part2.wavへ適用した(TTS新規生成なし、確定的なFFmpeg time-stretch+内蔵Primary ASR再検証のみ、`er014_output/user_test_news_light_01/tiny_bags/audio_fix/a2_full_story_part2_slowdown_apply_01.py`)。結果: duration比1.0597(目標6%=1.0600)、post-slowdown内蔵ASR再検証は`ASR_VALIDATION_UNCERTAIN`のまま(Toteme→"Totème"、Kallmeyer→"Kilmeyer"、いずれも既存Human Approval記録の対象であるcanonical_textは変更していないため、Gateの`HUMAN_APPROVED`判定は引き続き有効)。これによりGate上の`MISSING_MANDATORY_A2_SLOWDOWN`ブロックが解消された。追加cost: openai_asr 1回(約¥0.3、`raw_usage_log_audio_fix.jsonl`実測)。
- **Assembly/Gate結果**: `stage_assemble_a2()`実行→`status=OK`、duration=403.803秒、peak=0.95981、clipping無し(headroom safety valve適用なし、判定閾値0.98に対し実測0.95981のため)。Audio Validation Gate(Gate OFF経路・opt-in ON経路とも)PASS(override無し)。
- **player/URL**: `build_web_player_common`でplayer.html+web export(episode.mp3+segments 35件)生成。最終URL: `https://rawcdn.githack.com/shimomura055/eigo-radio/6ae84b80/user_test/unified.html?src=er014_output/user_test_news_light_01/tiny_bags/a2/player.html&level=A2&en=Are%20Tiny%20Bags%20Back%3F%20Fashion%27s%20Answer%20Is%20More%20Complicated&ja=%E5%B0%8F%E3%81%95%E3%81%84%E3%83%90%E3%83%83%E3%82%B0%E3%81%AF%E6%9C%AC%E5%BD%93%E3%81%AB%E6%B5%81%E8%A1%8C%E3%81%97%E3%81%A6%E3%81%84%E3%82%8B%E3%81%AE%E3%81%8B%E3%80%81%E3%83%95%E3%82%A1%E3%83%83%E3%82%B7%E3%83%A7%E3%83%B3%E3%81%AE%E7%AD%94%E3%81%88%E3%81%AF%E4%B8%80%E7%AD%8B%E7%B8%84%E3%81%A7%E3%81%AF%E3%81%84%E3%81%8B%E3%81%AA%E3%81%84`(A2 playerは「最終確認用」であり固有名詞の再判断は求めない)。実ブラウザE2E(Playwright headless Chromium、rawcdn.githack実URL、commit`6ae84b80`、「Open the page」中継確認を経由): Play開始→4秒後currentTime=3.875秒(`error=null`/`duration=403.803s`[Assembly実測と一致]/`readyState=4`)→60秒seek成功(currentTime=61.43秒)→Key Phrase5件・Comment box4件・canonical script内にToteme/Kallmeyerの表記を含む本文表示を確認(console上に無関係な`ERR_BLOCKED_BY_RESPONSE.NotSameOrigin`警告1件のみ、既存記事と同型・機能影響なし)。evidence: `docs/pm/closeout_136_e2e/tiny_bags_a2_closeout03.json`+`tiny_bags_a2_closeout03.png`。
- **OPEN-159の扱い**: 本件(Tiny Bags Toteme/Kallmeyer)はユーザー承認によりclose(2026-09-17)。proper-name一般課題としてのOPEN-159本体(固有名詞・人名発音処理パイプライン全体の設計ギャップ)は引き続き`DEFERRED_UNTIL_USER_TEST_COMPLETE`のまま継続。
- **Sheet投入用情報(確定)**: 記事タイトル(English)="Are Tiny Bags Back? Fashion's Answer Is More Complicated"(A2代表)、記事タイトル(日本語)="小さいバッグは本当に流行しているのか、ファッションの答えは一筋縄ではいかない"、記事の概要(日本語)="小型バッグの人気と大型バッグの復権が同時に語られるファッション記事。2025年から続く二極化(小さいバッグの話題性 vs 実用的な大きいバッグ需要)を、複数のブランド事例[Prada/Loewe/Toteme/Kallmeyer等]と業界データ[Trendalytics 152%等]で描く。"、Family="News(ライト系)"、ノーマル(A2)=上記player URL、Advanced(B1)=`https://rawcdn.githack.com/shimomura055/eigo-radio/e858649a/user_test/unified.html?src=er014_output/user_test_news_light_01/tiny_bags/b1b/player.html&level=B1&en=Are%20Tiny%20Bags%20Really%20Back%3F%20Fashion%27s%20Answer%20Comes%20With%20a%20Catch&ja=...`、備考="最新ニュース(ライト系)"。
- **PM_GOVERNANCE 8節追記**: 2026-09-17の並列Agent実測教訓(共有working tree由来の混入・lockのcheck-then-writeレース・Key Phrase stageの音声書き込み・原則の明文化)を追記(変更履歴節に1行追記)。
- **既存ユーザーテスト記事一覧/Sheet管理表**: `user_test/`配下・`docs/`配下・`ARTIFACT_REGISTRY.md`を確認したが、`ARTIFACT_REGISTRY.md`のNews-family表以外に専用の記事一覧・管理表ファイルは存在しない(Sheet自体はユーザー側の外部管理、本リポジトリには複製しない)。ARTIFACT_REGISTRY.mdのTiny Bags行(A2/B1)のみ更新した。
...
## USER-TEST-REVIEW-PAGE-FORMAT-RULE-01: Key Phrasesラベル残存+Level生表示を恒久ルール化・E2Eチェッカー新設・対象9本を新SHAで再発行

- 日付: 2026-09-17
- 種別: ユーザー正式指示(2026-09-17、恒久ルール)の実装反映。並行Agentなし、API呼び出し0、TTS/ASR/音声変更0(表示・生成テンプレート・SSOTのみ)。
- **指摘内容**: 「既に一度指摘済みの表示崩れが新規記事[Tiny Bags等]で再発した」。ユーザー試聴ページのKey Phrases欄が`English: push off the stage日本語: 大きなバッグが主役の座を奪う`のようにラベル文字列付き・2列に分離されない1本の太字表示になっていた(`docs/pm/closeout_136_e2e/tiny_bags_a2_closeout03.png`で実際に再現を確認)。また、ヘッダー・Human Review確認ページに内部レベル文字列`A2`/`B1`がそのまま表示されていた。
- **原因**: `user_test/unified.html`の`kpParts()`関数が、Key Phrase行の生テキスト(タイムライン表`table.timeline`の`Key Phrase N`行、`build_web_player_common.py`等が`f"English: {used_form}<br>日本語: {japanese_gloss}"`の形で生成)からラベルを取り除いて英日2列(`.kpitem`)へ分割する処理を担っているが、旧regexは「英語:」+「日本語gloss:」/「EN:」+「JA:」/「英語:」+「日本語:」の3パターンのみに対応しており、実際に多用されている「English:」+「日本語:」の組み合わせに未対応だった(フォールバックにより元の生文字列がそのまま`<b>`へ入り、ラベル付き・1列表示になっていた)。この不備自体は、ユーザーが指摘直後に本人手動で`user_test/unified.html`へ直接commit(`5f9b9add`「fix: parse English/Japanese key phrase labels in unified user test」)し、「English:」+「日本語(gloss)?:」regexを追加して先行修正済み(以後Sonnetが実装したものではない)。続けてlevel表示についても同ユーザーが`38f2cf97`「fix: use Standard/Advanced labels on user-test pages」でunified.htmlのeyebrow/titleをA2→Standard、B1→Advancedへマッピングする`levelLabel`変数を追加済みだった。一方、`user_test/human_review.html`のeyebrow(`English Your Way · ${data.level||''} · Human Review`)は同種の`levelLabel`マッピングが未反映のまま`data.level`(例: `"A2"`)を生表示しており、9-12節のHuman Review確認ページにもレベル表示不整合が残っていた。個別の`player.html`(`build_web_player_common.py`が生成する`<h2>Key Phrases</h2>`専用の`table.kp`)自体は元々英日2列・ラベル無し形式であり、今回問題ではなかった(問題はunified.html側の抽出ロジックのみ)。
- **修正内容(Sonnet実装分)**: `user_test/human_review.html`のeyebrow描画へ、unified.htmlと同一の`levelLabel`マッピング(A2→Standard、B1→Advanced、それ以外はそのまま)を追加(`docs/pm/RESULT_PACKET_REVIEW_PAGE_FORMAT_01.md`参照、diffはcommit`55c8a324`)。加えて恒久チェッカー`docs/pm/tools/user_test_page_e2e_check.py`(新規、Playwright headless Chromium)を追加し、(i)Key Phrase領域のラベル正規表現(`^(English|英語|EN|日本語|日本語gloss|JA)\s*[:：]`)不在、(ii)英語列・日本語列の2列要素数一致、(iii)ヘッダーがStandard/Advanced表示でA2/B1単独表示が無いこと、(iv)Playが実際に進行すること(`currentTime`増加・`paused=false`・`error=null`)、(v)Key Phrases/Full Script/Comment要素の存在、の5項目をDOM assertionとして機械判定できるようにした。開発中に判明した実装上の注意点2点を記録: (a) `raw.githack.com`/`rawcdn.githack.com`が第三者サービス側の仕様として「External Content Notice」警告ページ(POSTフォーム送信で`__Http-phish`cookieをセットして通過)を新規commit初回アクセス時に挟むようになっており、チェッカーは同一`BrowserContext`を使い回してcookieを共有し自動で通過する処理を追加した(当リポジトリ側では制御不可能な外部挙動)。(b) `<audio>.play()`が返すPromiseをPlaywrightの`eval_on_selector`でawaitすると、headless環境のautoplay policyにより無期限に未解決のままハングする実測不具合があったため、Promiseをawaitしない(`.catch()`のみ付けてfire-and-forgetする)実装へ修正した。
- **9本再発行**: 対象9本(Space Weapons A2/B1、AI Control A2/B1、Tiny Bags A2/B1、Convenience AI A2/B1、Personalized News A2新版)のplayer.html/mp3/segmentsは無変更(commit`b9be84da`→`55c8a324`間で`er014_output`/`er012_output`配下の差分無しを`git diff --stat`で確認、代表4本のepisode.mp3 sha256も記録・`docs/pm/closeout_136_e2e/format_rule_01/mp3_sha256_evidence.txt`)。新SHA`55c8a324`(unified.html/human_review.html修正反映後)でURLを再発行し、E2Eチェッカーで9本×5項目=45項目全PASSを確認(`docs/pm/closeout_136_e2e/format_rule_01/e2e_result.json`、スクリーンショット`docs/pm/closeout_136_e2e/format_rule_01/screenshots/`)。
- **Legacy 4記事(6ページ、`er003_output/b1_p9a/{A02,ADD03}/web/`・`er003_output/n3_01/{hanshin,health}/web/`)の確認結果**: Key Phrases表示(`1. term`/日本語訳の2列、ラベル文字列無し)は新ルールに適合済み。一方、eyebrow表示は全6ページとも`English Your Way · B1 · User Test`/`· A2 ·`のように内部レベル文字列を生表示しており、Standard/Advanced表記ルールを満たしていない。これらは静的な`user_test_simple*.html`(unified.html/human_review.htmlとは別実装)であり、本タスクは「既存承認済みページとして同ルールを満たしているかの確認のみ・修正はスコープ外」と指定されていたため、是正は実施していない(是正が必要であれば別タスクとして扱う)。
...
## NEWS-ITERATIVE-R2-PRODUCTION-WIRING-01: News記事Entertainment生成方式(Original→Entertainment revision→Further entertainment revision、2回目revisionを最終記事とする)のユーザー正式採用(APPROVED_FOR_PRODUCTION、配線未完了)

- 日付: 2026-09-24
- 区分: ユーザー正式採用判断(Trial評価ではなくProduction正式採用)。Phase 0はEvidence保存+SSOT反映+Production配線先read-only recon(Production code変更なし、¥0)。
- 採用対象(方式定義、Production上の意味): (1) Original generation(素材[ある場合]とテーマからP7 Promptで初回記事を生成)。(2) Entertainment revision(Trial呼称R1、修正指示「この記事を、事実関係は変えずに、もっとエンターテインメント性の高い記事に修正してください。」を`previous_response_id`で連鎖)。(3) Further entertainment revision(Trial呼称R2、修正指示「この記事を、事実関係は変えずに、さらにもっとエンターテインメント性の高い記事に修正してください。」を同様に連鎖)。(4) **2回目revision(上記3、Trial呼称R2)の結果を最終記事として使用する**。3回目以降のrevision(Trial呼称R3)はProduction標準に含めない(Trial実施の事実としてのみEvidence保存)。逐語Prompt全文・model(`gpt-5.6-luna`、`er006_model_routing_contract_01.WRITER_MODEL`)・reasoning effort(`high`)・`previous_response_id`連鎖の実際のresponse_id列は`docs/evidence/news_iterative_r2_adoption_2026-09-24/prompts.md`参照。
- 3テーマTrial結果(横断表、Fable評価転記): 下水道(素材なし)=Original弱→R1改善→**R2最良**→R3冒頭改善だが比喩4系統で過剰。Article A AI電話代行(強いHook・素材あり)=Original既に良→R1良→**R2最良**(タイトル・統一)→R3はタイトル誇張・演出句増で過剰。Article B旅行荷物(地味・素材あり)=Original良(復唱あり)→R1良→**R2最良域**(場面冒頭・統一)→R3は冒頭◎だが見立て5系統で冗長。R2が勝ったテーマ3/3。R3が勝ったテーマ0(冒頭改善のみなら3/3でR3)。詳細: `docs/evidence/news_iterative_r2_adoption_2026-09-24/comparison_and_decision.md`。
- R2採用理由: 3テーマとも最良域に到達し、進行役文・比喩の増殖が許容範囲に収まる最後の段階(Fable判定Q1)。Fact維持確認は3テーマ・全12段階でPASS(素材/Original外の固有名詞・数字・出来事の追加は0件、STOP条件非該当)。Latency・Costは量産上許容可能(Original→R2累計: 下水道50.2秒/AI電話45.3秒/旅行荷物32.2秒、cost実測: 下水道4call¥1.02、Trial-02[A+B]8call¥1.97、1記事あたり概ね0.75〜1.0円[R2止め])。
- R3不採用理由: 3テーマとも「冒頭は改善するが、本文は比喩系統+1〜3・演出句・タイトル誇張などの装飾増」という一貫したパターンで、強いテーマ(Article A)では特に過剰(「正体を追え！」「マジックのタネ」「幕を閉じます」等)。R3が勝つテーマが1件も無いため、テーマ別使い分けの根拠も無い(Fable判定Q2〜Q5)。
- User正式承認(原文引用): 「News記事のEntertainment改善方式として、Original → Revision 1 → Revision 2 の逐次Revision方式を採用し、Production標準をR2とする。」Statusを`APPROVED_FOR_PRODUCTION`へ変更する。これはTrial評価ではなく、Production正式採用判断。ただし、Gate 3をすべて満たすまでは`PRODUCTION_WIRED`としないこと。」(2026-09-24、`NEWS-ITERATIVE-R2-PRODUCTION-WIRING-01`委任文)
- Evidence path: `docs/evidence/news_iterative_r2_adoption_2026-09-24/`(README.md/prompts.md/articles/12記事/comparison_and_decision.md/prompt_echo_and_ad_observation.md)。元Trial artifact: `er015_output/news_iterative_entertainment_trial_01/`、`er015_output/news_iterative_entertainment_trial_02/`、`NEWS-ITERATIVE-ENTERTAINMENT-TRIAL-01_REPORT.md`、`NEWS-ITERATIVE-ENTERTAINMENT-TRIAL-02_REPORT.md`。
- 関連Trial管理ID: `NEWS-ITERATIVE-ENTERTAINMENT-TRIAL-01`、`NEWS-ITERATIVE-ENTERTAINMENT-TRIAL-02`、`NEWS-R25-PRODUCTION-METHOD-TRIAL-01`(参考、R2.5量産方式比較)。
...
## NEWS-STANDARD-A2-VOCAB-6000-CUTOFF-PRODUCTION-WIRING-01: User formally approved Standard A2 v5(6,000-cutoff)、Production module実装・Gate 3判定(ユーザー正式決定、2026-09-25)

- 日付: 2026-09-25
- 区分: ユーザー正式決定(Standard A2 v5「6,000語ライン+自然さ優先」を正式採用[`APPROVED_FOR_PRODUCTION`])+Production配線作業(Gate 3判定)。
- ユーザー指示原文(逐語、正式採用仕様): 「Standard A2では、頻出上位約6,000語以内=原則そのまま使用可、必要に応じてKey Words/Phrases側で学習補助/6,000語を超える語=より簡単で自然な表現がある場合は置換を強く優先、ただし置換によって英文の自然さや意味を損なわない/固有名詞は別扱い/不可欠な専門語は、簡単な代替で意味が失われる場合は残してよい/難語の説明を本文へ追加してStoryを膨らませない。v5 Promptを正式Standard A2として採用。**「6000語超を必ず置換」ではない**(septic tank/wastewater/artery等は必要語として残り得る。municipalitiesのように自然な簡単語へ置換できる難語は優先的に平易化)。」
- 分離事項: Metaのscope曖昧性(some parts of the calls ↔ parts of some calls)は本仕様と分離。個別Prompt対応を追加せず、OPEN-177の既存サブ項目(7)として保持する。
- status: `APPROVED_FOR_PRODUCTION`(Standard A2 v5、Advanced Natural English Adaptationは既存`NEWS-STANDARD-A2-PROMPT-V2-TRIAL-01`エントリで既に`APPROVED_FOR_PRODUCTION`)。配線Status: `APPROVED_FOR_PRODUCTION / WIRING INCOMPLETE`(Gate 3全14項目中、未充足項目が複数あるため`PRODUCTION_WIRED`にはしない。詳細は本エントリ末尾のGate 3チェックリスト、および`NEWS-STANDARD-A2-VOCAB-6000-CUTOFF-PRODUCTION-WIRING-01_REPORT.md` §13)。
- approval date: 2026-09-25
- supporting Trial: `NEWS-STANDARD-A2-VOCAB-6000-CUTOFF-TRIAL-01`(v5到達`VALIDATED`、Fable参考評価=必要語保持・不自然置換なし・2記事再現)。関連v1〜v4: `NEWS-STANDARD-A2-PROMPT-V2-TRIAL-01`(v1 `REJECTED`、v2到達`VALIDATED`・superseded)、`NEWS-STANDARD-A2-VOCAB-EFFECTIVENESS-TRIAL-01`(v3到達`VALIDATED`、下水道単体)、`NEWS-STANDARD-A2-VOCAB-BANDING-TRIAL-01`(v4 `REJECTED`、難語→難語置換の再現により頻度帯設計を撤回)。
- 偵察結果(Production経路の現状、2026-09-25時点の事実確認): (a) 日本語Entertainment R2(Original→R1→R2)のProduction正式初回入口は存在しない(`CURRENT_SPEC.md` L828「配線先Production経路: 未確定」)。(b) Advanced(Natural English Adaptation)のProduction実装も存在しない(Trialスクリプト`er015_news_ja_to_en_adaptation_trial_01.py`等のみ、Production module化されていない、OPEN-177(1))。(c) 既存Production記事契約(`# Title`+`### `×2+`## In one line`)は英語A2/B1 News Writer経路(`er003_v1_n3_01_articles_generate.py`)固有の構造であり、本Family(Entertainment英語適応)はTitle行のみの単純contract(v5 Prompt末尾"Output only the English title and the English body.")。(d) retry/fallback/regenerationはWriter系全体で`er003_v1_en_direct_vfl_01_generate.py`(vfl01)の`run_writer_no_search`/`run_writer_with_technical_retry`に一元化されている(fallbackモデル定義は`er006_model_routing_contract_01.PROCESS_MODEL_MAP`に無し)。(e) Key Words/Phrasesは記事本文からの抽出(`er003_v1_n3_01_scaffold_generate.py`の`run_key_phrases`)であり、本Standard A2生成とは独立工程(役割分担に矛盾なし、本タスクでは未接続のため実接続テストはしていない)。
- 実装(Production module、Trial importなし): `er003_v1_n3_01_standard_a2_generate.py`。定数`STANDARD_A2_DEVELOPER`/`STANDARD_A2_PROMPT_V5`(v5逐語)、`STANDARD_A2_PROMPT_SHA256`(LF正規化sha256、import時にfail-closedでassert)。関数`generate_standard_a2(advanced_text, *, client=None, model=None, effort="high", max_retries=1) -> StandardA2Result`。API呼び出しは既存Production依存`vfl01.run_writer_no_search()`を再利用(本タスクで`developer`引数[既定値=既存`WRITER_DEVELOPER_MESSAGE`、完全後方互換]と戻り値`usage`キー[追加のみ、既存キー無変更]を追加、既存呼び出し元21箇所は無変更のまま動作)。retry方針は空出力→1回再試行(max_retries既定1)、fallbackモデルなし。model routingは`routing.PROCESS_MODEL_MAP`へ`"STANDARD_A2_ADAPTATION": WRITER_MODEL`を新規追加(値はWRITER_MODELと同一、新規モデル追加ではない)。cost計算は`er012_b_family_production_runner_01._load_pricing()`と同一ロジックを独立実装(`er005_output/cost_baseline_01/pricing_snapshot.json`、USD_JPY=160.0)。機械チェック(Ledger不要の簡易版、独立実装): タイトル行の存在、Advanced→Standardの数字トークン差分(`NUMBERS_MISSING`/`NUMBERS_ADDED`)、固有名詞候補diff(観測のみ、失敗判定なし)。単体テスト`er003_v1_n3_01_standard_a2_generate_test_01.py`(14件、unittest、API mock、全PASS)。
- runtime evidence(実API、Sewer/Meta各1 call、Production module経由): Sewer(`er015_output/news_natural_advanced_standard_a2_trial_01/a1_advanced_sewer.md`入力)response_id=`resp_0b7ae0a1ec33f9e9006ab5f3a3aa0c87d0b3df742d870d73e6`、model_id_actual=`gpt-5.6-luna`、cost_jpy=0.5045、attempts=1、retried=false、fallback_detected=false、checks_failed=[]。Meta(`er015_output/news_ja_to_en_adaptation_trial_01/arms/arm3/output.md`入力)response_id=`resp_00296e5e7aa628bc006ab5f3c89afc87d0b4f05123e9f7ac1e`、model_id_actual=`gpt-5.6-luna`、cost_jpy=0.2979、attempts=1、retried=false、fallback_detected=false、checks_failed=[]。合計cost_jpy=0.8024(予算¥20以内)。保存先: `er003_output/standard_a2_wiring_01/sewer/`、`er003_output/standard_a2_wiring_01/meta/`(各`standard_a2.md`+`runtime_evidence.json`)。6,000語超残存語(wordfreq直接測定、Trial importなし): Sewer 18語(septic/sewers/invisible/surprisingly/repairs/treats/cleaned/pipes/handles/connects/flush/baths/artery/sewer/inspections/collects/kitchens/wastewater)、Meta 9語(understudy/concierges/leak/belonged/curtain/performer/trusting/backstage/paused)。いずれも必要語・許容比喩語として残存しており、不自然置換・難語→難語置換の再発は観測されない(v4で問題視された現象は再現していない)。
- regression: `er006_model_routing_contract_01_test.py`(全PASS)、`er003_discovery_focus_staged_production_01_test_01.py`(24 tests PASS)、`er011_discovery_focus_s2_full_trial_01_test_01.py`(22 tests PASS、いずれもvfl01.run_writer_no_search変更の影響範囲)、`er003_v1_en_direct_vfl_01_generate_deviation_v2_test.py`(6/6 PASS)。この repo にpytest未導入・統一test runner無しのため、影響範囲モジュールを個別実行して確認した(全ファイル一括実行はしていない事実として記録)。
- Gate 3チェックリスト(14項目、○×): 1 Production正式初回経路への接続=**×**(runner不在のため未接続)。2 retry・fallback・regenerationとの整合=**△**(関数自体は既存retry方針[空出力1回再試行]・既存fallback未定義方針と一致するが、統合先オーケストレーター自体が存在しないため「整合」の実地確認はできない)。3 Trial専用scriptだけに残していない=**○**(Production module実装済み)。4 Advanced Natural→Standard A2正式経路との整合(Advanced経路の存在含む)=**×**(Advanced自体が未実装)。5 Key Words/Phrasesとの役割分担に矛盾なし=**△**(設計上矛盾はないが未接続のため実地確認なし)。6 Production正式pathでruntime発火=**×**(CLI直接実行、runner経由ではない)。7 actual model_id・routing確認=**○**(gpt-5.6-luna確認、routing.require_model使用)。8 regression・validator・integration test=**△**(単体テスト・影響範囲regressionはPASSだが、Production pipelineとのintegration testは経路不在のため実施不能)。9 Sewer・MetaでProduction経路から期待挙動=**×**(Production経路自体が無いため、経路からの実行ではない)。10 CURRENT_SPEC=**○**(本コミットで反映)。11 DECISION_LOG=**○**(本エントリ)。12 OPEN_ITEMS=**○**(本コミットでOPEN-177更新)。13 Git=**○**(本コミットで反映)。14 Dangling Reference Check=**○**(`er015_`のimportはer003_*.py/er012_*.py/er006_*.py中に無し[コメント・テスト内のパス文字列のみ]、`STANDARD_A2_PROMPT_V5`は定義1箇所+参照のみ)。
- 最終Status: `APPROVED_FOR_PRODUCTION / WIRING INCOMPLETE`(Gate 3の1〜9のうち×3件・△3件が残るため`PRODUCTION_WIRED`にはしない。STOP条件[Advanced Natural→Standard A2の正式初回経路が存在せず、経路の設計に仕様判断が必要]に該当するため、経路設計(日本語Entertainment R2の入口確定・Advanced Production化・Standard A2との接続順序)はFable/ユーザー判断待ちとしてここでは行わない)。
- 未完了事項(事実列挙、Fable/ユーザー判断待ち): (1) 日本語Entertainment R2のProduction正式初回経路の確定(`NEWS-ITERATIVE-R2-PRODUCTION-WIRING-01`の既存宿題、本タスクでは未着手)。(2) Advanced(Natural English Adaptation)のProduction module化(本タスクでは未着手、Standard A2はAdvancedの出力を入力として要求するため、Advancedが無いと実運用のend-to-end経路が成立しない)。(3) (1)(2)確定後の、Standard A2 runner接続位置・呼び出し順序・Audio path・Key Phrase抽出元(Advanced/Standardどちらの本文を使うか)の設計判断。(4) Meta「通話の一部」scope曖昧性の一次情報確認(OPEN-177(7)、本タスクの対象外)。
- 関連: `NEWS-STANDARD-A2-PROMPT-V2-TRIAL-01`(Advanced正式承認・OPEN-177新設)、`NEWS-ITERATIVE-R2-PRODUCTION-WIRING-01`(Entertainment日本語生成方式、配線先未確定)、OPEN-177(更新)。
- commit: (本コミットで反映)

## NEWS-ADVANCED-A2-PRODUCTION-E2E-WIRING-01: Advanced(Natural English Adaptation)/Standard(A2 v5)をProduction正式経路へ配線、Sewer/Meta E2E runtime evidence、Gate 3再判定(2026-09-25)

- 日付: 2026-09-25
- 区分: Production配線作業(Gate 3再判定)。新しいWriter仕様・新しいFactルール・新しいKey Phrase仕様・新しいAudio仕様はいずれも追加していない。
- 目的: 既存2記事(Sewer/下水道、Meta/AI Phone Call)を使い、承認済みAdvanced(Natural English Adaptation、B1)・Standard(A2 v5、6,000語ライン)をProduction正式経路へ配線し、記事生成→scaffold/Key Phrase/TTS/Assembly/Audio Validation→completed playerまでE2E完走させる。
- 配線設計(Fable指示、D1〜D8):
  - D1 入口: `er012_e_family_entertainment_two_level_runner_01.py`(新規Production runner)。入力=日本語完成Entertainment R2記事ファイル(path+sha256+由来管理IDをentry_point.jsonへ記録)。日本語R2自動生成自体は範囲外(`CURRENT_SPEC.md` L828は`WIRING INCOMPLETE`のまま)。
  - D2 Advanced段: `er003_v1_n3_01_advanced_adaptation_generate.py`(新規)。`NEWS-JA-TO-EN-ADAPTATION-TRIAL-01` arm3のDEVELOPER/ARM3_BLOCKを逐語使用(`ADVANCED_UNCHANGED_PORTION_SHA256`でimport時fail-closed assert、Trial fileとの一致はテスト`er003_v1_n3_01_advanced_adaptation_generate_test_01.py`で確認、Production自体はTrialをimportしない)。COMMON_BLOCKの記事固有「Preserve…」6bullet(AI電話/人間コンシェルジュ等Meta固有語)を、記事非依存の一般形5項目(opening expectation/reversal/central metaphor or storytelling device/order of information/ending)へ置換。既存Production contract(`# `Title+Main Story+ちょうど2つの`### `節+`## In one line`、出典`er003_v1_n3_01_articles_generate.py`【記事構成】節)を、`NEWS-ENTERTAINMENT-PRODUCTION-LINE-TRIAL-01`のCONTRACT_LINESと同一文言で接尾ブロック付与。
  - D3 Standard段: 既存`er003_v1_n3_01_standard_a2_generate.py`のv5 user prompt末尾「Output only…」の直前に構造保持行`Keep the same Markdown structure (the "# " title, the two "### " sections, and the final "## In one line" section); do not add or remove sections.`を追加(v5の語彙・簡略化指示は無変更、`STANDARD_A2_PROMPT_SHA256`を`cbb72357...`→`ff860ab6...`へ更新)。
  - D4 retry/fallback/regeneration: 両段とも`vfl01.run_writer_with_technical_retry()`(既存primitive)を使用するため、同primitiveへ後方互換`developer`引数(既定値=既存`WRITER_DEVELOPER_MESSAGE`)を追加(`er003_v1_en_direct_vfl_01_generate.py`)。Standardは従来`run_writer_no_search()`への独自「空応答→1回retry」ループだったが、本タスクで構造Gate付きretryへ統一(v5へ構造保持行を追加したこととの整合)。fallbackモデルは既存どおり無し。`routing.PROCESS_MODEL_MAP`へ`"NATURAL_ENGLISH_ADAPTATION": WRITER_MODEL`を新規追加(既存`"STANDARD_A2_ADAPTATION"`はそのまま)。runnerの`--regenerate-stage advanced|standard`で同じ関数を再呼び出しできることをテストで確認。
  - D5 Fact/Ledger: Meta既存Ledger(`er017_output/news_entertainment_production_line_trial_01/ledger/verified_fact_ledger.txt`、MUSE-001〜018)を再利用。Sewerは`vfl01.build_researcher_prompt`/`build_verification_prompt`/`build_verified_ledger_text`(既存関数)をrunner内で直接呼び出し新規構築(`NEWS-ENTERTAINMENT-PRODUCTION-LINE-TRIAL-01` Phase Bと同一の呼び出し構造、Trial scriptはimportせず独立実装)。実API: Researcher+Verification(web_search)、20 facts VERIFIED、cost_jpy=9.93。Advanced/Standard両出力に`vfl01.run_deviation_check(hook_aware=False)`を適用。失敗(`overall_status == "LEDGER_DEVIATION"`)時は既存retry1回→なお失敗ならSTOP(本文を手で直さない)、という実装。
  - D6 Meta事実確定: Reuters一次報道(`https://www.marketscreener.com/news/meta-testing-a-human-concierge-for-its-new-personal-ai-agent-muse-ce785ad8de8cf025`、reuters.com/marketscreener.com直接curlはbot対策でブロックされたため`r.jina.ai`経由で全文取得)を確認。原文引用「Meta has been testing a "human concierge" ... which entails having human contractors quietly handle **some of the phone calls** placed via the digital agent」。判定: 正しいscopeは「一部の通話(call単位)を人間が処理した」であり、「1通話の中の一部分」ではない。既存Ledger(MUSE-006)は既にこのscopeと整合しており、修正不要と確認(OPEN-177(7)解消)。証跡: `er012_output/e_family_two_level_wiring_01/meta/fact/meta_fact_scope.md`。
  - D7 downstream: `er003_v1_n3_01_scaffold_generate.py`(`run_theme_scaffold`、Preview/Comment 1-4、`run_key_phrases`/`run_key_phrase_selection`)・`er003_v1_n3_01_tts_generate.py`(`generate_b1_segments`/`generate_a2_segments`)・`er003_v1_n3_01_assemble.py`(`stage_assemble_b1`/`stage_assemble_a2`、`verify_episode_audio_validation_gate`)を関数importで無改変のまま再利用。判断根拠(委任文言「Family Bのscaffold」の解釈): 委任が明示的に引用した関数名`run_key_phrases`/`run_key_phrase_selection`は文字通り`er003_v1_n3_01_scaffold_generate.py`(A-Family/n3)にのみ存在し、かつそのsplit_article_text()が要求する構造(Title+Main Story+###×2+In one line)がAdvanced/Standardのcontract(D2/D3)と完全一致する一方、literal `er012_b_family_production_runner_01.py`のbuild_parts()は別の5区切り[Hook/VoiceA/VoiceB/Tension/Closing]構造を要求し構造的に非互換なため、A-Family(n3)のPreview/Comment 1-4/scaffold/TTS/Assemblyを指すと判断した。player.htmlは新規に本runner専用の関数として実装(既存`audio_review_player`共有ヘルパーのみ再利用)。
  - D8 Key Phrase: 新仕様追加なし。既存`run_key_phrases`をAdvanced/Standard本文それぞれから独立に実行(記事ごとに個別選定、既存挙動のまま)。
- 実装ファイル: `er003_v1_n3_01_advanced_adaptation_generate.py`(新規)、`er003_v1_n3_01_advanced_adaptation_generate_test_01.py`(新規、13 tests)、`er003_v1_n3_01_standard_a2_generate.py`(修正)、`er003_v1_n3_01_standard_a2_generate_test_01.py`(修正、15 tests)、`er003_v1_en_direct_vfl_01_generate.py`(`run_writer_with_technical_retry`へ後方互換`developer`引数追加)、`er006_model_routing_contract_01.py`(`NATURAL_ENGLISH_ADAPTATION`追加)、`er012_e_family_entertainment_two_level_runner_01.py`(新規Production runner)、`er012_e_family_entertainment_two_level_runner_test_01.py`(新規、12 tests)。
- runtime evidence(実API、Sewer/Meta):
  - Sewer: Ledger新規構築(20 facts VERIFIED、cost_jpy=9.93)。Advanced(1回目)生成→deviation check `LEDGER_DEVIATION`(MAJOR 2件: 記事の中核claim「既存の老朽化した公共下水道を浄化槽へ切り替える」がLedgerの実際scope[未整備区域の計画変更等]を超える一般化、および下水道の技術的説明[損傷発見の困難さ等]がLedger未記載)→既存retry(1回)実施→再度`LEDGER_DEVIATION`(同種issue再現)→**STOP**(本文を手で直さず、Ledger topicの調整もせず)。downstream(scaffold/TTS/Assembly)は未実行。cost_jpy合計=17.01。
  - Meta: Ledger既存再利用(reuse_source.json記録)。Advanced生成(1回目)→deviation check `LEDGER_COMPLIANT`(retry不要)、model_id_actual=`gpt-5.6-luna`、response_id=`resp_0048be94dfa89428006ab5fe4cef5487d08db8f82556080b71`、cost_jpy=0.3049。Standard生成(1回目)→deviation check `LEDGER_DEVIATION`→retry(1回)→`LEDGER_COMPLIANT`、response_id=`resp_00ccd1b80781bf93006ab5fee177a887d0903e61eb0cecf86b`、cost_jpy=0.9619。scaffold(Preview/Comment 1-4、Key Phrase選定)完走(b1b/a2ともCANONICALIZATION_PASS、a2はredundancy QA retry 1回で解消)。TTS: b1b全segment OK(13 segment+5 Key Phrase)。a2はcomment_1/2/3が既存`ER-009-JA-FOREIGN-TOKEN-GATE-01`により未知の外来語「Meta」検出でSTOPPED(TTS呼び出し自体を行わずHuman Review待ち、既存Gate、無改変)。Assembly: b1b=OK(duration 275.554s、peak 0.73347、clipping無し)。a2=`EPISODE_BLOCKED_BY_AUDIO_VALIDATION`(comment_1/2/3=STOPPED、既存`verify_episode_audio_validation_gate`、無改変、override無し)。player.html生成(b1b完全収録、a2はGATE_BLOCKED表示)。cost_jpy合計=17.96。
  - 合計cost_jpy=34.97(予算¥300の約11.7%)。
- Gate 3チェックリスト(20項目、詳細は`er012_output/e_family_two_level_wiring_01/gate3_checklist.md`): ×1件(項目18、E2E 4製品完走。Sewer 0/2、Meta B1完走、Meta A2はAssembly Gate blocked)。他19項目は○(配線・retry整合・Trial非依存・Advanced/Standard経路整合・Key Phrase役割分担・Production runtime発火・regression[新規40 tests+既存1033 tests、全PASS]・runtime evidence・model_id/routing確認・コスト評価・SSOT反映・Advanced逐語性・contract機能・Meta事実確定・Audio Gate尊重・deviation check実施・Dangling Reference Check)。
- 最終Status: `APPROVED_FOR_PRODUCTION / WIRING INCOMPLETE`(Advanced/Standardとも。配線自体[入口・Prompt・retry・downstream接続]は完了・実証済みだが、Gate 3の全項目○[4製品E2E完走]には未到達のため`PRODUCTION_WIRED`にはしない)。
- 未解決事項: (1) Sewer記事の中核claim(既存の老朽化した公共下水道を浄化槽へ切り替える動き)を裏付ける一次情報が、本タスクで構築したLedgerの範囲(未整備区域の計画変更等)では確認できなかった。これは日本語Entertainment R2記事自体の事実的な妥当性に関わる可能性があり、本タスクの権限(Prompt個別修正禁止・本文を手で直さない)では解消できない。Fable/ユーザー判断が必要。(2) Meta a2のcomment_1/2/3は、「Meta」という社名の日本語TTS読み(カタカナ表記等)が既存Human Review Lockの対象になっており、人間による読み確定が必要(既存の運用フロー、新規仕様ではない)。(3) 日本語Entertainment R2自体の自動生成経路(Original→R1→R2)は本タスクでも未着手のまま(`CURRENT_SPEC.md` L828、`NEWS-ITERATIVE-R2-PRODUCTION-WIRING-01`の既存宿題)。
- 関連: `NEWS-STANDARD-A2-VOCAB-6000-CUTOFF-PRODUCTION-WIRING-01`(前回Gate 3判定)、`NEWS-ENTERTAINMENT-PRODUCTION-LINE-TRIAL-01`(Ledger構築方式・Production contract接尾ブロック方式の出典)、OPEN-177(更新)。
- commit: (本コミットで反映)

## PM-GOVERNANCE-DEV-TTS-STANDARD-SYNC-REMINDER-01: 正式リリース前TTSはStandard同期(PM_GOVERNANCE.md 7-1/7-2)の運用是正(2026-09-25)

- 日付: 2026-09-25
- 区分: 既存ガバナンス(`PM_GOVERNANCE.md` 7-1「正式リリース前は原則Standard同期・正式リリース後の実量産はBatch API」・7-2「Batchを使ってよい例外4条件」)の再確認・運用是正。**新仕様ではない**(7-1/7-2自体は変更しない)。
- 契機: `NEWS-ADVANCED-A2-PRODUCTION-E2E-WIRING-01`(直近のE2E配線タスク)で、入口runner`er012_e_family_entertainment_two_level_runner_01.py`に`TTS_EXECUTION_MODE`の明示指定が実装されておらず、低レベル実装`er006_batch_tts_wiring_01.py`の`DEFAULT_TTS_EXECUTION_MODE`(既定BATCH、量産Production既定・変更しない)へ暗黙に依存する状態だった。これによりBatch待ちが発生し得る構成のまま実行されていたことが確認された(既定BATCH依存によるBatch長時間化はTrial-13[2026-09-05]・タオルTrial-11[2026-09-12]でも既出の再発パターン)。
- 実施内容:
  1. `er012_e_family_entertainment_two_level_runner_01.py`に`--tts-mode {STANDARD,BATCH}`(既定`STANDARD`)を追加し、実行時に`os.environ["TTS_EXECUTION_MODE"]`へ明示設定する。`--tts-mode BATCH`指定時は`--batch-reason`(PM_GOVERNANCE.md 7-2の例外条件に該当する理由)を必須化し、未指定なら`parser.error`で即停止する。`entry_point.json`へ`tts_execution_mode`/`tts_batch_reason`を記録する。既存テスト`er012_e_family_entertainment_two_level_runner_test_01.py`へ`TtsModeCliTests`(既定STANDARD確認・BATCH+理由の正常parse確認・不正値拒否確認・理由未指定時のsubprocessエラー確認、計4件)を追加。
  2. 横展開点検: `er0*.py`のうちTTS関連文字列(`tts_generate|make_batch_tts_call_fn|run_tts|TTS_EXECUTION_MODE`)を含む133ファイルを機械分類(詳細`docs/pm/tts_mode_audit_2026-09-25.md`)。既に`TTS_EXECUTION_MODE=STANDARD`を明示済み32件(是正実施の本runner含む)、TTS呼び出し自体が無い8件(player.html生成のみ/QA検出ロジックのコメント参照のみ)、TTS_EXECUTION_MODE切替機構自体のtest/evidence2件(両方式の意図的検証、7-2例外1に該当)、既存実行済みHistoricalの一回限りscript65件・mock使用テスト21件(以上86件は今回是正対象に含めない、既に完了・再実行なし)、TTS_EXECUTION_MODE導入前に別方式[関数monkeypatch]でStandard相当を実現済み2件、Production共有ライブラリ本体1件(`er006_batch_tts_wiring_01.py`、既定値[BATCH]は量産Production既定のため変更しない)、Production独自CLIを持たないライブラリモジュールで呼び出し元の設定状況を全件確認できていないもの2件(`er012_b_family_voices_a2_production_01.py`/`er012_b_family_voices_production_01.py`、要判断)。グレップパターン外で発見した`er013_family_c_production_runner_01.py`(TTS生成モジュールをimportするが自身はTTS_EXECUTION_MODEを含まない独立Production runner)も要判断として報告する。
  3. 委任テンプレート`docs/pm/templates/DELEGATION_READ_EFFICIENCY_BLOCK.md`へ固定ラベル`T-2`(全委任で常時貼付)を新設(「TTSを伴う委任は正式リリース前である限り`TTS_EXECUTION_MODE=STANDARD`を実行コマンドに明示する。Batchは7-2の例外条件の理由を委任文に明示した場合のみ」)。既存の`T-1`[施策1 Read Efficiency Trial用ラベル]とは別ラベル(ラベル衝突回避のためT-1を流用せずT-2を新設)。`docs/pm/templates/DELEGATION_STANDARD_TEMPLATE.md`の固定ブロック見出しへT-2を追記。
  4. `docs/pm/tools/check_delegation_prompt.py`へ、委任文が`TTS|tts|音声化|narration`のいずれかを含み、かつ`TTS_EXECUTION_MODE=STANDARD`の明示も`batch-reason`の記載も見つからない場合の**警告**(`warnings`フィールドへ記録、`reasons`には含めない、status/PASS判定には影響させない=ブロッキングではない)を追加。単体テスト3件追加(`check_delegation_prompt_test_01.py`の`CheckTtsStandardModeReminderTest`)。
- `PM_GOVERNANCE.md`本文(7-1/7-2/7-3/7-4のルール自体)は無変更。7-4末尾へ「運用是正履歴: 2026-09-25 本管理ID」の1行のみ追記した。
- regression: `er012_e_family_entertainment_two_level_runner_test_01.py`(16件、新規4件含む)・`docs/pm/tools/check_delegation_prompt_test_01.py`(7件、新規3件含む)・`python -m unittest discover -s . -p "*_test_01.py"`(全1037件、新規+8件、PASS)。
...
## NEWS-E2E-PRE-KEYPHRASE-CLOSEOUT-02-READING-VALIDATION-WIRING-03: 辞書expected reading→ASR照合のProduction配線(Phase 3b、PRODUCTION_WIRED)

- 日付: 2026-09-25
- 区分: ユーザー正式決定に基づくProduction実装(位置づけ: 新商品仕様ではなく「確定した読みをTTSが守ったか確認する品質保証の欠落修正」。期待動作: `Meta`→辞書`メタ`→TTS→ASR→expected=`メタ`→ASRが`メタ`相当ならPASS、`メタン`等ならFAIL→既存retry/fallback経路へ)。設計はPhase 3a read-only調査(`docs/pm/recon_reading_validation_wiring_01.md`、commit 8a1b3a7b)の推奨案どおり。
- 原因: 辞書(`DEFAULT_JA_READING_DICTIONARY`)の確定読みが、TTS入力にもASR照合(`er007_ja_asr_validator_01.classify_ja_asr_match`)にも一切使われておらず、Latin表記("Meta")vsカタカナ("メタン")の差がentity_likeヒューリスティックに該当し、ASR Cascade(最大4回の追加ASR、TTS再生成なし)のいずれか1回が偶然読み完全一致を返せば無条件PASSする構造だった(Meta a2 comment_3で実際に発生、`OPEN_ITEMS.md` OPEN-177参照)。
- 実装: `classify_foreign_tokens_in_japanese_text()`(`er003_audio_tts_asr_safety.py`)のREADING_DICTIONARY findingへ構造化フィールド`"reading"`を追加。`expected_readings: dict | None = None`(既定None、完全後方互換)という新規オプション引数を、`generate_charon_japanese_with_reading_safety()`/`generate_a2_japanese_with_reading_safety()`/`generate_a2_japanese_with_fallback()`(`er003_v1_n3_01_tts_generate.py`)、`generate_charon_japanese()`(`er003_v1_sing01_voice01_generate.py`、B1)、`generate_narration_snippet_verified_strict()`のja分岐のみ(`er003_v1_repro01_main_generate.py`、英語分岐は無変更)、`evaluate_attempt_ja_with_cascade()`/`_detail()`(`er007_ja_secondary_asr_01.py`、Primary#1・#2・Secondary#1・#2の全ステップへ転送)、`classify_ja_asr_match()`→`protected_check_ja()`(`er007_ja_asr_validator_01.py`)の9関数へ素通し配線した。`protected_check_ja()`内、辞書登録トークン単体のopcode差分に限り、既存の`entity_like`/`phonetic_uncertain`ヒューリスティックより先に登録読みと直接照合し(濁点許容比較を再利用)、一致すれば許容差(既存`reading_equal`と同じ扱い)、不一致なら`cascade_eligible=False`を強制して既存の`TRUE_CONTENT_MISMATCH`→retry(standard 2回→fallback 1回、合計上限3回)→Human Review Lockへそのまま合流させる(新Gate・新上限・新ログ経路は作らない)。英語TTS経路・ER-009 Gateの4分類ロジック・辞書内容自体は無変更。
- regression母集団調査(¥0、事前): `er011_output`/`er012_output`配下のREADING_DICTIONARY finding付きsegment 18件(AI 16件・Meta 3件、うち2件は同一文にAI+Meta重複)を全数抽出し、新旧の`classify_ja_asr_match()`分類を比較。18件中17件は分類が完全一致(regressionなし、大半はLatin表記のままASRされトップレベル一致でopcodeループへ到達しない)、変化した1件はまさにMeta comment_3「メタン」再現ケース(旧`ASR_VALIDATION_UNCERTAIN`→新`TRUE_CONTENT_MISMATCH`、意図した挙動変化)。採用した照合ルール: 「Latin表記そのまま(正規化後の文字列一致)、または辞書登録読み(濁点許容)のいずれかに一致すればPASS」。
- test: unit/integration 63件新規追加、全PASS(`er007_ja_asr_validator_01_test.py`: メタ一致PASS/メタン不一致FAIL/未登録語regression、`er009_ja_foreign_token_gate_01_test_01.py`: findings→expected_readings抽出+B1/A2両経路への転送、`er007_ja_secondary_asr_01_test.py`: Cascade全4ステップへの転送、`er007_reading_validation_wiring_test_01.py`[新規ファイル]: voice01/repro01経路の深い配線確認)。既存`unittest discover -p "*_test_01.py"`1052件中1件が新規kwarg`expected_readings`未対応の固定シグネチャmock(`er011_open121_repetition_qa_production_wiring_01_test_01.py`の`fake_ja_cascade`)でTypeErrorとなったため、mock側のシグネチャを`expected_readings=None`受け取り可能に修正(Production側の挙動は無変更)。修正後、1052件全PASS。
- runtime evidence(実API、¥5上限、`TTS_EXECUTION_MODE=STANDARD`明示、standard_attempts=1でTTS呼び出しを厳密に1回へ上限): Meta a2 comment_3の実テキストを、Production関数の下位`generate_a2_japanese_with_fallback()`(内部処理は`generate_a2_japanese_with_reading_safety()`と同一)経由で専用out-dir`er012_output/reading_validation_evidence_01/`へ1回生成。今回の実TTSは「メタ」(登録読みと完全一致)を発話し、ASRも「メタ」と書き起こして`PHONETIC_MATCH`即PASS(expected_readings適用下での正しい一致判定を実証)。費用¥2.40(gemini TTS 1回+openai_asr 1回、上限¥5以内)。詳細: `er012_output/reading_validation_evidence_01/runtime_evidence.json`。FAIL側(不一致→`TRUE_CONTENT_MISMATCH`→既存retry経路)は、追加の実API課金を避けるため実データを使ったunit/integration test(mock ASR)で確認済み(上記test節)。
- 既存artifactへの影響: 現行Meta a2 artifact(`er012_output/e_family_two_level_wiring_01/meta/`)は再TTS・再assembleしていない(Point構造を今後変更予定のため、ユーザー指示どおり据え置き)。comment_3のASR「メタン」誤認識は当時のartifact(status=OK)にそのまま残り、次回そのsegmentが実際に再生成された場合にのみ本チェックが適用される。
- 未解決事項(スコープ外、別Open Item起票): Cascadeが1回一致で無条件PASSする構造自体、および中間ASR結果(Primary#2/Secondary#1/#2)が成果物JSONに残らない監査トレーサビリティの欠落は、本実装のスコープ外(`recon_reading_validation_wiring_01.md` 1.2節)。`OPEN_ITEMS.md` OPEN-181で新規起票。
- Status: `DECIDED`(`PRODUCTION_WIRED`)。全条件充足: Production初回path実装/retry・fallback・regeneration整合/regression・integration test/Production pathでのruntime evidence/SSOT反映/Git commit・push。
- Dangling Reference Check: 依存する3仕様はいずれも`CURRENT_SPEC.md`で`PRODUCTION_WIRED`/`WIRED(ON)`済み(ER-009 Gate L1244、日本語ASR Validator+Cascade L1283/L1287、Human Review Lock L1248)。
- 関連: `docs/pm/recon_reading_validation_wiring_01.md`(Phase 3a設計)、`OPEN_ITEMS.md` OPEN-177(comment_3「メタン」項目を本実装で更新)・OPEN-181(新規、Cascade監査トレーサビリティ欠落)、`CURRENT_SPEC.md` L1244(ER-009行、本実装の追記反映)。
- commit: (本コミットで反映)

## ADVANCED-VOCAB-V2-PRODUCTION-RESTORE-01: Advanced語彙ルールv2をProductionへ組み込み(v3はREJECTED確定)

- 日付: 2026-09-26
- 区分: ユーザー正式決定(Trial-01 fix01のv2仕様を`APPROVED_FOR_PRODUCTION`としてProduction wiringまで実施。v3[Trial-02、Topic Core Word例外+Metaphor専用ルール]は`REJECTED`)。
- v3 REJECTED理由(ユーザー確定、4点): (1) Topic Core Word例外(exception E)は「記事テーマの根幹に関わる語」という判断基準が曖昧で、実質的にCore Word例外の範囲が際限なく拡大しうる。(2) Topic Core Word/Metaphor例外いずれも、語ごとにAIが「これは記事の核心か」「これは比喩表現か」を都度判定する必要があり、境界判断のコストが単純なA〜D 4例外より高い。(3) Metaphor専用ルール(比喩表現に使われている語は原則SIMPLIFY寄りとする)は、比喩語のみを狙い撃ちして置換する結果、その語を含む一文だけが不自然になりうる(比喩表現は文全体の言い回しと結びついているため、単語だけの置換では自然さを保てない)。(4) QCD(Quality/Cost/Delivery)の観点で、A〜D 4例外(v2)に対しTopic Core Word/Metaphor拡張(v3)が明確な品質改善を示さなかった(Trial-02自体がPASS/FAILの定量比較ではなくschema拡張の実装確認に留まり、v2に対する優位性を示すevidenceが無い)。
- v2仕様(ユーザー正式決定、逐語): 一般英語頻度順位約12,000位超を原則平易化候補(Standard v5同様「必ず置換ではなく、より簡単で自然な表現がある場合に平易化を強く優先し、意味・自然さを損なう場合は強制しない」)。surface/lemmaの安全な正規化(順位判定を機械で行う経路がある場合)。A: 易しい既知語から意味推測可能な形態・複合語 / B: 日本語に定着した外来語 / C: 固有名詞・正式な呼称・designation(引用符内の実際の呼称を含む) / D: 置換すると意味精度・自然さを明確に損なう不可欠語。Topic Core Word例外なし。Metaphor専用ルールなし。定型表現例外なし。出典: `er015_output/advanced_vocab_rule_trial_01/prompt_advanced_vocab_rule_v1.txt`(v1)と fix01のv2 Prompt(`er015_output/advanced_vocab_rule_trial_01/v2/prompt_advanced_vocab_rule_v2.txt`)。v2の文言を正とする。
- W-1 Recon(¥0): `er003_v1_n3_01_advanced_adaptation_generate.py::generate_advanced_adaptation()`が単一の`build_prompt()`出力を`vfl01.run_writer_with_technical_retry()`(構造Gate付きretry)へ渡す構造を確認。構造retryは同一user_message(build_prompt出力)を再利用するのみでプロンプト再構築なし。呼び出し元`er012_e_family_entertainment_two_level_runner_01.py::run_writer_stage()`は、Ledger deviation MAJOR時も同じ`adv_gen.generate_advanced_adaptation(ja_text, ...)`を再呼び出しするため、`build_prompt()`にv2ルールを埋め込めば初回・構造retry・deviation再生成のすべてで自動的に同一ルールが適用されることを確認した。組み込み方式は(i)Adaptation prompt本体へルール文言を追加(Standard v5と同型、候補語リストなし、モデル自身の語感に依存)と(ii)Adaptation後に順位リスト付き改稿passを追加(Trialと同型、追加API呼び出し1回)の2案を比較し、(i)を暫定採用した(コスト・配線の複雑さで最小差分、(ii)の要否は次回以降の通常Production Runでの観察後に判断)。詳細: `docs/pm/recon_advanced_vocab_v2_wiring_01.md`。
- W-2実装: `er003_v1_n3_01_advanced_adaptation_generate.py`へ`ADVANCED_VOCAB_RULE_V2_BLOCK`(v2 Promptの語彙難易度ルール段落・A〜D例外・引用符内designation扱い・idiom単独例外禁止の4段落、逐語ベース。候補語リスト/decisions出力schema等、改稿pass[Trial]固有の記述のみ除去)を新規定数として追加し、`build_prompt()`内でARM3_BLOCKの後・Production contract接尾ブロックの前に挿入した。`ADVANCED_VOCAB_RULE_V2_SHA256`でimport時にfail-closedでassert(既存`ADVANCED_UNCHANGED_PORTION_SHA256`と同型の防御)。Standard v5モジュール(`er003_v1_n3_01_standard_a2_generate.py`)は無変更(git diffで確認)。v3(Topic Core Word/Metaphor専用ルール/`er015_advanced_vocab_rule_trial_02.py`)はProduction側へ一切import・混入していない(Grep確認: "Topic Core"/"topic_core"/"is_metaphor"/"exception_used"は本タスクで追加したコメント・テストのみに出現し、実行時プロンプト文字列には含まれない)。
- test: `er003_v1_n3_01_advanced_adaptation_generate_test_01.py`へ`VocabRuleV2Tests`(v2ルールがprompt内に存在/12,000ラインとA〜D例外文言の存在/v3固有マーカー[Topic Core/topic_core/is_metaphor/exception_used/Metaphor restriction]がbuild_prompt()出力に不在/sha256 assert正常系・異常系/Standard A2モジュールのSTANDARD_A2_PROMPT_SHA256 assertが無エラーで通る[無変更確認]、計6件)を追加し、既存`test_build_prompt_contains_all_blocks_in_order`をvocab rule blockの位置を含む順序確認へ更新。単体25件PASS、既存`unittest discover -p "*_test_01.py"`(全1068件、他並列タスクによる追加分含む)もPASS(exit code 0)。
- runtime evidence(実API、¥10上限、実測¥0.4134): Production正式経路の生成関数を直接呼び出し(`er003_v1_n3_01_advanced_adaptation_generate.py --ja-file docs/evidence/news_iterative_r2_adoption_2026-09-24/articles/ai_phone_revision2.md --out-dir er003_output/advanced_vocab_v2_wiring_01/`)、Meta記事のAdvanced版を1本生成した(既存`er012_output/.../meta/`は上書きしていない、TTS/Standard生成は実施せず)。`model_id_actual=gpt-5.6-luna`(requestedと一致、fallback_detected=False)、`structure_status=STRUCTURE_PASS`(attempts=1、retryなし)、`response_id=resp_03c0189413b73dc7006ab70de91cac87d08384337e33c35c9f`、`prompt_unchanged_portion_sha256`一致。生成本文中の頻度順位12,000超語(wordfreq `top_n_list('en', 20000)`、分析専用スクリプトで算出、Production非依存)は4語: `concierges`(rank>20,000[zipf=1.70]、引用符内designation→exception C相当で保持)/`backstage`(rank 12,912、back+stageの複合語→exception A相当)/`muse`(rank 13,199、Meta AIエージェントの固有名詞→exception C)/`onstage`(rank 15,670、on+stageの複合語→exception A)。詳細: `er003_output/advanced_vocab_v2_wiring_01/runtime_evidence.json`、`er003_output/advanced_vocab_v2_wiring_01/frequency_rank_over_12000.json`。retry/fallback/regenerationが同一promptを使うことは実際に発火させず、コード追跡(`run_writer_with_technical_retry()`のループ構造、`run_writer_stage()`の再呼び出し箇所)で確認した(委任文の指示どおり)。
- 未解決事項・暫定判断(Fable/ユーザー判断待ち): (1) 組み込み方式(i)は暫定判断であり、方式(ii)(順位リスト付き改稿pass追加)の要否は次回以降の通常Production Runでの観察後に判断する(`OPEN_ITEMS.md` OPEN-182新設)。(2) 今回のruntime evidenceはMeta記事1本(N=1)のみであり、モデルが個別語の順位を提示されなくても一般的にv2ルールへ従えるかどうかの結論ではない。
- Status: `APPROVED_FOR_PRODUCTION`(v2語彙ルール自体)→`PRODUCTION_WIRED`(語彙ルールのbuild_prompt()組み込みのみ。Advanced自体の全体Status`APPROVED_FOR_PRODUCTION / WIRING INCOMPLETE`[Gate 3未充足、OPEN-177]は本タスクのスコープ外のため変更しない)。
...
## NEWS-FAMILY-X-B3-FACT-SELECTION-PRODUCTION-WIRING-01: Storyline決定+B3 Fact選定(LLM 1 call)をProduction配線、日本語Entertainment Writer(Original→R1→R2)自動生成も同時配線、Meta「Muse human concierge」でRuntime evidence取得(2026-09-26)

- 日付: 2026-09-26
- 区分: ユーザー正式決定に基づくProduction配線実装(Phase 1)。B3(Full Ledger→中心Storyline決定→4テストによるFact選定→Selected Fact Brief)は`APPROVED_FOR_PRODUCTION`(2026-09-26ユーザー確定)。
- ユーザー確定事項(逐語ベース要旨、変更禁止): (1) B3はTrial限定ではなく量産可能なProduction経路へ接続する。(2) Storylineを人間が毎回指定する運用にはしない(Full Ledger確認→AIが中心Storylineを決定→Fact選別→B3 4テスト→Selected Fact Brief→Writerの手順を量産Production処理へ移す)。(3) 正式フロー: Research→Full Fact Ledger→AIが中心Storylineを1つ決定→B3 4テストを全Factへ適用→Selected Fact Brief→Writer→Original→R1→R2→Advanced→Standard。(4) Storyline決定+B3 Fact選定はLLM 1 call。(5) B3の4テスト定義(Test1:削除しても中心Storyline理解可能か→YESなら原則除外/Test2:他Factとの因果に実際に使われるか→NOなら原則除外/Test3:Storyline上の重要な「なぜ?」に必要か→NOなら原則除外/Test4:「面白い/具体的/数字がある/珍しい/補足として便利」だけで残していないか→YESなら原則除外)。(6) Fact数目安3〜5、Hard ruleではない、6件以上はrecheck_note必須。(7) LLM出力はJSON Evidence必須(selected_storyline/Full Ledger Fact IDs/各Factの4テスト結果/selected・excluded/理由/最終Selected Fact IDs/Selected Fact Brief)。(8) 日本語Writer(Original→R1→R2、P7、既承認`APPROVED_FOR_PRODUCTION / WIRING INCOMPLETE`)のProduction配線とB3配線をまとめて実施してよい(Trial-only script限定状態は禁止)。既承認のP7 Prompt本文・Revision指示は変更しない。(9) Family Aは変更しない(対象はFamily X/Entertainment系統のみ)。(10) Mandatory STOP: 完成記事(R2/Advanced/Standard)提示時点でSTOP、ユーザー確認前に後工程へ進まない。(11) 正式Production経路(新runner)経由で実記事1 runのruntime evidenceを取得する。(12) コストをResearch/Ledger/Storyline+B3/Original/R1/R2/Advanced/Standardで分離計測する。(13) STOP条件(Storyline仕様判断・B3テスト変更・既承認O→R1→R2仕様との矛盾・Production path接続不可・Fact精度/Storytelling重大劣化・cost想定外増加・新Human Review工程要求)。
- 実装: 新規Production module 3点。(a) `er019_family_x_storyline_b3_fact_selection_01.py`(Storyline決定+B3 4テストのLLM 1 call。Developer/User Prompt定数・JSON schema[selected_storyline/fact_tests/selected_fact_ids/selected_fact_brief/recheck_note]・Fact ID整合検証[`validate_selection_output()`]・技術的失敗[JSON不正/未知Fact ID]時1回retry→STOP[`run_storyline_b3_selection()`]。Trial[`er015_family_x_writer_fact_selection_trial_02.py`]のFACT_TESTSハードコード辞書は一切参照しない、新規LLM自動判定として実装)。(b) `er019_family_x_ja_writer_o_r1_r2_01.py`(`er015_news_original_baseline_repro_01.R0_PROMPT`/`DEVELOPER_MESSAGE`および`er015_news_iterative_entertainment_trial_01.REVISION_INSTRUCTIONS["r1"/"r2"]`をProduction moduleへ逐語移設、sha256一致をtestで検証。Writer入力素材を旧来の「Sonnet手書き2〜3文」からB3のSelected Fact Briefへ置換、「テーマ：」行はB3が決定したStorylineへ置換。previous_response_id連鎖[r1/r2]、失敗時fallback_full_text。r3は生成しない[正式フローはOriginal→R1→R2まで])。(c) `er019_family_x_entertainment_production_runner_01.py`(正式入口。`--theme/--slug/--out-dir/--stage research|ledger|storyline_b3|writer|advanced|standard|all/--regenerate-stage/--stop-after`[既定standard]。Research/Ledgerは既存`er012_e_family_entertainment_two_level_runner_01.run_researcher_for_topic`/`run_verification_for_topic`をそのまま呼び出し[topic引数を明示可能な既存実装]、Advanced/Standardは同runnerの`run_writer_stage()`[既存deviation check retry・fallback方針]をそのまま呼び出し。scaffold/tts/assemble/player関連の関数を一切import・実装しておらず、Standard生成で構造的にMandatory STOPする)。
- Runtime evidence(実API、Meta「Muse human concierge」、実測合計¥40.00): `er019_output/family_x_b3_production_wiring_01/run_01/`。Research: web_search 7回、Full Ledger 15 Fact(VERIFIED 15/AMBIGUOUS 0/REJECTED 0)。Storyline+B3(1 call、retry不要): Selected Storyline「MetaはAIエージェント「Muse」の電話機能で一部の通話を訓練済みの人間契約スタッフに担わせる実験を行ったが、ユーザー情報がスタッフに共有され得る懸念や適切な開示の不足を受けてミスと認め、機能を当面ロールバックした。」、15 Fact中3件採用(MUSE-HC-006/010/012)、recheck_note=null(6件未満のため不要)。JA Writer(Original→R1→R2、previous_response_id連鎖、fallback未使用): R2完成(973字)。Advanced: 1回目MAJOR(Ledger deviation、契約スタッフの認識対象の混同+ロールバック時制の誤り)→1回retry→LEDGER_COMPLIANT(構造Gate STRUCTURE_PASS、fallback_detected=false)。Standard: 1回目でLEDGER_COMPLIANT(v5 6,000語ライン、STRUCTURE_PASS)。全stage`model_id_actual=gpt-5.6-luna`。コスト内訳(stage別、cost.json): research=14.47/ledger=14.27/storyline_b3=0.67/ja_original=0.24/ja_r1=0.24/ja_r2=0.26/advanced=4.99(1 retry込み)/standard=4.87、合計¥40.00(予算Cap¥150に対し余裕あり)。
- 既知の観察事項(仕様変更・修正は行わない、事実記録のみ): (1) `run_writer_stage(only=...)`をコスト分離目的で"advanced"→"standard"の2回に分けて呼んだため、既存Production実装の`writer_run_summary.json`保存ロジック(呼び出し末尾で毎回上書き)により、最終的に"standard"キーのみが残り"advanced"のキーが失われる(既存primitiveの設計上の制約、Advanced単体呼び出し[`--regenerate-stage advanced`単体]では従来から問題なし)。本タスクではraw_usage_log.jsonl+成果物から手動再構成し`b1b/audit/final_advanced_summary_reconstructed.json`へ記録した。(2) Advanced段のLedger Deviation Check結果は同一種類の文言(「ユーザーからAIだと見えた」旨の表現)に対して実行のたびに判定が変動した(1回目実行時はLEDGER_COMPLIANT判定、2回目実行[`--regenerate-stage advanced`]では同種の表現がMAJOR判定→再生成後PASS)。これは`vfl01.run_deviation_check`(LLM判定)の既知の非決定性であり、本タスクで変更・調整は行っていない。(3) Standard段は1回目実行時にMAJOR(2回目retry後も解消せず)でSTOPしたため、`er019_output/family_x_b3_production_wiring_01/run_01/`に対し2回目の呼び出し(`--regenerate-stage advanced`、Full Ledger/Selected Brief/JA記事は再利用し重複Research・重複Storyline+B3 callは発生させていない)を実施し、Advanced再生成→Standard生成が両方PASSする形でRuntime evidenceを完成させた(既存の1-retry-then-STOP capを回避・無効化してはいない。単に既存`--regenerate-stage`機能を通常の運用同様に再度呼び出しただけであり、各呼び出し内部のGate・retry上限は無変更)。
- SSOT反映: `CURRENT_SPEC.md` L828(Entertainment Writer方式)へ追記(配線先Production経路確定・Storyline+B3ステージ新設・Mandatory STOPを明記、Status`APPROVED_FOR_PRODUCTION / WIRING INCOMPLETE`は維持、`PRODUCTION_WIRED`への変更なし)。`OPEN_ITEMS.md` OPEN-177(1)「日本語R2自動生成未配線」を解消として追記、OPEN-183新設(記事のユーザー確認待ち、Mandatory STOP)。
- テスト: 新規`er019_family_x_b3_production_wiring_01_test_01.py`(24件、オフラインmock。JSON schema検証・Fact ID整合・6件以上時のrecheck_note分岐・Selected Brief生成・P7 Prompt/Revision指示のsha256逐語一致・previous_response_id連鎖とfallback切替・runner stage順序・Full Ledger/Selected Brief分離・後工程[scaffold/tts/assemble/player]関数が本runnerに一切存在しないこと、全24件PASS)。既存`unittest discover -s . -p "*_test_01.py"`全件PASS(exit code 0、他並行タスクのTrial出力を含めても正常終了)。
- 未解決事項: (1) 本行は記事(Storyline・Fact選定・R2/Advanced/Standard本文)のユーザー確認待ち(`OPEN_ITEMS.md` OPEN-183、Mandatory STOP)。(2) Gate 3全24項目の`PRODUCTION_WIRED`判定はFable/ユーザーが行う(本エントリはSonnet仮判定の技術的証跡整理のみ)。(3) 上記「観察事項」(1)(2)の恒久対応(writer_run_summary.jsonの上書き解消、deviation check非決定性への対応要否)は本タスクのスコープ外、必要ならFable/ユーザーが別途判断する。
- **修正1回目(2026-09-26、Fable差し戻し対応、commit`b9fe61e5`)**: Fable指摘「Gate 3 #13(runtime evidenceの整合性)は手作業補完(`final_advanced_summary_reconstructed.json`)では未充足」「Gate 3 #2(Selected Brief整形)にStoryline文重複バグ」を受け修正した。**#13**: (1) `er012_e_family_entertainment_two_level_runner_01.run_writer_stage()`の`writer_run_summary.json`保存を、既存ファイルとマージする恒久修正(バグ修正、新仕様ではない。呼び出し末尾で毎回上書きしていたため`only="advanced"`→`only="standard"`のように分けて呼ぶと先の"advanced"キーが消えていた)。(2) run_01自体の`writer_run_summary.json`は、手動再構成ではなく新規スクリプト`er019_writer_run_summary_reconstruction_01.py`で、raw_usage_log.jsonlのAPI呼び出し順序(コード上の不変条件)から機械的に再構成した(推測に基づく手作業補完を排除)。旧(バグで壊れた)ファイル・旧手動再構成ファイルはいずれも削除せず`_superseded`系ファイル名で保持。**#2**: `storyline_b3/selected_brief.md`で「## Selected Facts」冒頭にStoryline文が重複出力されるバグ(Prompt指示上、LLM出力`selected_fact_brief`自体がStoryline文で始まるため)を`build_selected_brief_markdown()`で修正(先頭が完全一致する場合のみ重複除去、非一致時は憶測で改変しない)。run_01の`selected_brief.md`原本(Writerへ実際に渡した実物)は再生成せず保持し、修正後ロジックのオフライン適用結果を参照用に別ファイル保存(Writer未使用、API呼び出し0)。**観察記録(修正・再生成は行っていない、ユーザー判断待ち)**: (a) Fact drift(Ledger「ロールバックした」完了形 vs JA/Advanced/Standard「ロールバックされます」未来形、Advanced 1回目deviation checkがMAJOR指摘したが却下判定は保存されず2回目で同一表現がLEDGER_COMPLIANTになった非決定性)。(b) Storyline選定がTrial-02の`CORE_STORYLINE`が含んでいた「AIだと気付かれると人間へ引き渡す」因果連鎖を含まない(MUSE-HC-009がTest2=NOで除外)。いずれも`run_01/audit/fable_editorial_findings.md`に逐語記録、`OPEN_ITEMS.md` OPEN-183備考へ1行追記。テスト: `er012_e_family_entertainment_two_level_runner_test_01.py`(43/43 PASS、新規1件)・`er019_writer_run_summary_reconstruction_01_test_01.py`(8/8 PASS、新規)・Selected Brief整形テスト2件追加。
- **Fable Gate最終判定(2026-09-26)**: Fable評価: (1) Gate 3 24項目を照合。#13(runtime evidence)は修正1回目で`writer_run_summary.json`のマージ保存を共有module側で恒久修正し、run_01のsummaryを生ログ(raw_usage_log.jsonl)からプログラム再構成(由来記録付き)したことで充足。#2/#5(Brief整形)はStoryline重複を修正(run_01原本は監査目的で不変)して充足。#18(regression)はTTS配線commit後の全体regression全PASSで充足。#20(DECISION_LOG)は修正1回目エントリ(commit`bac7c296`)で充足。#24(ユーザー承認仕様との一致): Storyline AI決定・LLM 1 call・B3 4テスト・Selected Brief/Full Ledger分離・O→R1→R2同時配線・Family A不変・記事完成でMandatory STOP、いずれも確定指示13項目と一致。(2) 配線とは別に、記事本体には時制ドリフト(Ledger「ロールバックした」→本文"will be rolled back")とAI Storylineが「なぜ人間を使ったか(009)」を含まない点があり、`run_01/audit/fable_editorial_findings.md`に記録。記事は再生成せずユーザー判断(X-1/X-2)待ち。(3) 費用¥40.00/記事(B3増分¥0.67)。分類: **配線: PRODUCTION_WIRED**(Fable Gate 3判定、2026-09-26)。**記事: ユーザー記事確認待ち(OPEN-183)、音声化へ進まない。**詳細: `NEWS-FAMILY-X-B3-FACT-SELECTION-PRODUCTION-WIRING-01_REPORT.md` §10-11、`er019_output/family_x_b3_production_wiring_01/gate3_checklist.md`。
- 関連: `NEWS-FAMILY-X-B3-FACT-SELECTION-PRODUCTION-WIRING-01_REPORT.md`、`docs/pm/RESULT_PACKET_FXB3.md`(Phase 0調査結果)、`NEWS-FAMILY-X-WRITER-FACT-SELECTION-TRIAL-01`/`-02`(B3手法の初出Trial)、`NEWS-ITERATIVE-R2-PRODUCTION-WIRING-01`(P7方式のユーザー正式採用、本行で自動生成配線を実施)、`NEWS-ADVANCED-A2-PRODUCTION-E2E-WIRING-01`(Advanced/Standard runner本体、無変更で再利用)、`OPEN_ITEMS.md` OPEN-177・OPEN-183(新設)。
- commit: `b9fe61e5`(修正1回目)、本コミットでFable Gate最終判定(`PRODUCTION_WIRED`)のDECISION_LOG/CURRENT_SPEC.md/OPEN_ITEMS.md/gate3_checklist.md反映を実施

## TTS-LOCAL-REWRITE-CONNECTED-SPEECH-PRODUCTION-WIRING-01: 10分cool-down+Local Rewrite+Natural English QA(Human Review Lock手前の回復経路)とConnected Speech 5-role適用範囲をProduction配線、Family X comment_4「point/points」で実end-to-end evidence取得(2026-09-26)

- 日付: 2026-09-26
- 区分: ユーザー正式決定に基づくProduction配線実装(`APPROVED_FOR_PRODUCTION`、2026-09-26)。
- ユーザー確定事項(逐語ベース要旨、変更禁止): (A) Local Rewrite + Natural English QA。TTS retryで同一問題が解消しない場合、問題span周辺だけLocal Rewriteし、①意味保持②segment role保持③Fact非矛盾④前後文脈⑤Natural English⑥必要最小限の変更を確認して再TTSする。Natural English QAはLuna 1 callでよい。(B) 10分cool-down(既決定): attempt1 NG→即時attempt2→NG→10分cool-down→attempt3→NGならLocal Rewrite。(C) Connected Speech適用範囲(既決定): 適用する=①Full Story②Comment③Preview④Topic intro⑤In One Line。適用しない=Heading readout、Key Phrase。(D) Human Review前にLocal Rewrite recoveryが入る(Local Rewrite→QA→再TTSで解消しない場合のみHuman Review Lock)。
- Recon(¥0): `docs/pm/recon_tts_local_rewrite_wiring_01.md`。既存Production TTS retry経路をrole別に棚卸しした結果、Comment/Preview/Topic intro(`voice01.generate_charon_english`経由、B1)は`enable_connected_speech_equivalence_layer`引数自体が関数シグネチャに存在せず、OPEN-122 Equivalence Layerが構造的に発火し得なかった(docs/pm/recon_connected_speech_scope_01.md「事実1」の実装反映漏れ)。In One Lineは旧OPEN-122配線で「本文ではないため対象外」と明示的に除外されていたが、本管理IDのユーザー承認済み5 role適用範囲により対象化する。
- 実装: 新規Production module `er020_tts_retry_local_rewrite_01.py`。(1) `connected_speech_enabled_for(segment_id)`という単一のSSOT判定関数へrole→適用判定を集約(経路ごとの個別ハードコード条件式を廃止)。(2) `maybe_cooldown_before_attempt()`: `PRODUCTION_MAX_TTS_ATTEMPTS`(=3)ループの最終attempt直前のみ固定600秒cool-down(実sleep、`sleep_fn`注入可)。(3) `run_local_rewrite_recovery()`: NG span特定(canonical/ASR語差分)→Luna(`gpt-5.6-luna`)候補生成(json_schema strict、5候補)→Luna 7 Gate QA(意味保持/role保持/Fact非矛盾/前後文脈/Natural English[独立Gate]/局所性/非全文、うち5つはLLM・2つはharness機械判定)→最小変更候補を選定→再TTS(1回、cool-downなし)。全Gate PASS候補がない、または再TTSも不合格の場合はNoneを返し、呼び出し元は従来通りHuman Review Lockへ進む。回復成功時は呼び出し元がstatus="OK"のままreturnし、`review_lock.record_outcome()`はRESOLVEDへ遷移する(Human Review Lockを経由しない)。Trial系譜(`er020_tts_cooldown_local_rewrite_trial_01.py`/`er020_tts_local_rewrite_natural_english_qa_trial_02.py`、いずれもVALIDATED)のロジックを、特定segment(comment_4の"point/points")に限定しない汎用形へ一般化して移設した(Trialファイル自体は無変更のまま併存)。
- 配線先: `er003_v1_sing01_voice01_generate.generate_charon_english`(Comment/Preview/Topic intro、新規`enable_connected_speech_equivalence_layer`引数を追加)・`er003_v1_sing01_news_tail_fix.generate_news_narration_wide_margin`(Full Story/In One Line、既存引数を流用)の2関数へ、cool-down呼び出し・Connected Speech引数転送・Local Rewrite回復ヘルパー(`_local_rewrite_recovery_for_charon_english`/`_local_rewrite_recovery_for_news_narration`、`review_lock.derive_segment_key()`でsegment_idを自動導出、標準layout外のout_pathでは安全にNoneを返し無効化)を実装した。呼び出し元`er003_v1_n3_01_tts_generate.py`(B1: topic_intro/preview/comment_1-4/full_story_part1-3/point_one/point_two/in_one_line、A2: 同様のrole判定一元化のみ)を`connected_speech_enabled_for()`参照へ更新。既存point_one/point_two(Point本文)は、承認済み5 roleの文言には明示的に登場しないが、既存Production wiringで既にFull Story本文と同一関数・スコープでTrueが適用済みだったため、回帰させずFULL_STORY roleへ含めた(Sonnet裁量、recon 3節に明記)。
- 既知Gap(隠蔽せず記録): A2経路(`er003_v1_crosslevel_audio_02_common.generate_english_segment_with_fallback`経由の`generate_a2_segment_with_slowdown`)は、role判定の一元化(in_one_line新規True化含む)のみ実施し、cool-down/Local Rewrite回復コードは本タスクでは未配線(第3の関数への複製実装は本タスクの時間内で同水準のtest/runtime evidenceを取れる保証がなく拡大を避けた)。拡張要否はFable/ユーザー判断待ち。OPEN-121(TTS Repetition/False Start QA)は別仕様のため適用範囲を変更していない。
- Runtime evidence(実API、`TTS_EXECUTION_MODE=STANDARD`、専用out-dir`er020_output/tts_local_rewrite_production_wiring_01/`、既存記事artifactは無変更、実測合計¥7.15[gemini 5.86+openai_asr 0.27+openai(Luna)1.02]、予算上限¥100に対し余裕あり): (a) 5 role適用確認: full_story_part1(FULL_STORY)・preview(PREVIEW)・topic_intro(TOPIC_INTRO)・in_one_line(IN_ONE_LINE)の4 roleは各1 segment・attempt1でOK(`enable_connected_speech_equivalence_layer=True`を実際に受理・転送)。Comment roleの適用確認は(b)のcomment_4 runで兼ねた。非適用2 role: point_one_heading(HEADING_READOUT、`enable_connected_speech_equivalence_layer`引数が関数シグネチャに存在しないことを`inspect.signature()`で確認、実TTSはSTOPPED→review_lock状態HUMAN_REVIEW_REQUIRED、Local Rewrite回復が発火しないことも実証)・kp1_en(KEY_PHRASE、同様に引数なし、実TTSはOK)。(b) cool-down→Local Rewrite→QA→再TTS→ASR PASSの実end-to-end経路(Family X comment_4「bring the main point together」、実際の2026-09-26 Family X本番runで3回ともASR_VALIDATION_UNCERTAINだった既知segment): 実際に`voice01.generate_charon_english(max_attempts=3, enable_connected_speech_equivalence_layer=True)`をProduction経路(Review Lockデコレータ含む)で呼び出し、attempt1/2/3すべてASRが"point"を"points"と聞き取り不一致(3回目のASR文言をrecoveryが`last_asr_text`として受理)、attempt2→3間で実600.009秒cool-down(`cooldown_started_at`16:48:31→`cooldown_ended_at`16:58:31、JST)、Local Rewrite候補生成(Luna`gpt-5.6-luna`、5候補)→7 Gate QA(Luna`gpt-5.6-luna`)で候補`candidate_3`("bring the main point together"→"boil it down to the main idea")を選定、再TTS(cool-downなし)でASR完全一致→`status="OK"`確定。`review_lock_state.json`でcomment_4の最終`state`は`RESOLVED`(`HUMAN_REVIEW_REQUIRED`には到達せず)、対比としてpoint_one_headingは`HUMAN_REVIEW_REQUIRED`のまま(Local Rewrite回復機構を持たないため、意図通り)。総wall clock 702.4秒。詳細: `er020_output/tts_local_rewrite_production_wiring_01/comment_4_cooldown_local_rewrite_evidence.json`・`roles_evidence.json`・`evidence_theme_01/b1b/audit/review_lock_state.json`。
- 実行時に発見した修正: retts_result単独のattempts_log(1件)だけを上位へ返すと、`review_lock.record_outcome()`の累積TTS/ASR call数guard(Part F)が実消費attempt数を数え落とすバグを、上記runtime evidence確認中に発見し修正した(メインループ+re-TTSのattempts_logを連結)。
- テスト: 新規`er020_tts_retry_local_rewrite_01_test_01.py`(22件、オフラインmock。role taxonomy全5+2+未知/None・cool-down発火条件[attempt1/2では発火せず、max_attempts>=3の最終attemptかつgate=Trueのみ]・NG span特定・7 Gate QA選定[全PASS/一部FAIL]・回復パイプライン4分岐[RESOLVED/NO_CANDIDATE_PASSED_QA/RETTS_FAILED/NO_NG_SPAN]・呼び出し元ヘルパーの非標準layoutでの安全なNone返却、全22件PASS)。既存`unittest discover -s . -p "*_test_01.py"`(全1156件+新規22件)実行、3件failure(いずれも「Production module/Family Aファイルにuncommitted git diffが無いこと」を検証する既存自己チェックテスト[`er019_family_x_pointless_01_test_01.py`・Trial-01/02自身の`ProductionModuleUnchangedTest`]で、本タスクが正規に`er003_v1_sing01_voice01_generate.py`/`er003_v1_sing01_news_tail_fix.py`等を変更したことによる一時的な差分検知。commit後は`git status`/`git diff`が空になり自然に解消する既知の性質、DECISION_LOG既往エントリでも同種の「既知failure」として扱われている)。
- **Fable Gate最終判定(2026-09-26)**: Fable評価: (1) Checklist16項目はB1経路・A2経路とも証跡が揃っている(#7/#8はA2側を修正1回目で充足。A2のPreview/Commentは英語ナレーションが存在しない構造的事実であり非充足ではない)。role→Connected Speech適用判定が単一関数`connected_speech_enabled_for()`に集約され、経路別フラグが廃止された点、cool-down(固定600秒)→Local Rewrite→7 Gate QA(Natural English含む、Luna 1 call)→再TTS→未解消時のみHuman Review Lock、の順序がB1・A2で同一moduleにより保証されている点を確認。(2) runtime evidence: B1(comment_4)で実600.009秒cool-down→Local Rewrite→再TTS ASR一致→`RESOLVED`の完全経路、A2で実600.004秒cool-down→Local Rewrite→再TTS→`HUMAN_REVIEW_LOCKED_RETTS_FAILED`(正常終端)の経路を確認。A2側で`RESOLVED_BY_LOCAL_REWRITE`の実例は未取得だが、同一module関数の発火・終端到達が実証されており配線証跡としては充足。(3) 留保2点(配線Statusには影響しない記録事項): ①A2 evidenceの失敗モードが「数字表記(digit)とspelled-outのASR不一致」で、Local Rewriteでは解決しにくい種類。ASR照合の数値正規化(`classify_asr_match`のbenign扱い)は別Open項目として記録する(`OPEN_ITEMS.md` OPEN-184新設)。②A2 evidenceの費用がcost logger未使用の概算(¥15〜20)。以後のTTS runtime evidenceは必ずcost loggerを通す。(4) Natural English Gateの"main idea"判定(Trial-02 FAIL vs 本番PASS)は候補文全体が異なり一貫した判定と確認済み。(5) 費用: 初回¥7.15+修正1回目概算¥15〜20。分類: **PRODUCTION_WIRED**(Fable Gate判定、2026-09-26。ユーザー承認仕様[Local Rewrite+Natural English QA、10分cool-down、Connected Speech 5役割適用/Heading・Key Phrase非適用]との一致を確認。B1・A2両経路)。詳細: `TTS-LOCAL-REWRITE-CONNECTED-SPEECH-PRODUCTION-WIRING-01_REPORT.md` §10-11。
- SSOT反映: `CURRENT_SPEC.md` L1262(新規行、TTS retry cool-down/Local Rewrite/Natural English QA仕様+Connected Speech 5 role適用範囲)。`OPEN_ITEMS.md` OPEN-122(適用範囲をA2/B1英語本文4segment限定から5 role[In One Line新規含む]へ更新、構造的欠落だった引数追加を明記、A2側Gapを記録)。Fable Gate最終判定(2026-09-26)によりCURRENT_SPEC.md本行Statusを`PRODUCTION_WIRED`(B1・A2両経路)へ更新、OPEN-122を「Connected Speech 5役割適用/2役割非適用 PRODUCTION_WIRED」へ更新、新規OPEN-184(ASR数値表記正規化の失敗モード)を追加した。
- Dangling Reference Check: Trial artifact(`er020_output/tts_cooldown_local_rewrite_trial_01/`・`er020_output/tts_local_rewrite_natural_english_qa_trial_02/`)・旧OPEN-122「A2/B1英語本文4segment限定」表記への参照が本実装のコード・コメントに残っていないことをGrepで確認(新規モジュールは汎用関数として独立実装、旧範囲表記はOPEN_ITEMS.md本行が更新済み)。
- 未解決事項: (1) A2経路のcool-down/Local Rewrite回復拡張要否(上記Gap、Fable/ユーザー判断待ち)→**修正1回目で対応済み(下記)**。(2) point_one/point_two扱い(Sonnet裁量、recon 3節)の是非確認。(3) Checklist16項目の`PRODUCTION_WIRED`最終判定はFable/ユーザーが行う(本エントリはSonnet仮判定の技術的証跡整理のみ)。
- **修正1回目(2026-09-26、Fable差し戻し対応)**: Fable指摘「Production採用仕様は『Production正式TTS初回/retry path』全体と『role差による漏れがない形』を要求しており、A2(Standard)経路`generate_english_segment_with_fallback`に本回復経路が未配線ではChecklist #1/#7未充足」を受け、A2経路をB1経路と同水準へ配線した。**配線構成**: `er003_v1_crosslevel_audio_02_common.generate_english_segment_with_fallback`のfallback(minimal instruction)経路(標準2回+fallback1回=総予算3回の実質最終attempt)へ、B1と同一の`retry_primitive.maybe_cooldown_before_attempt()`/`run_local_rewrite_recovery()`(複製実装ではなく同一module関数、共通ロジックは新規ヘルパー`_run_a2_minimal_fallback_attempt`/`_local_rewrite_recovery_for_english_segment_with_fallback`へ抽出しB1側コードは無変更)を接続した。あわせて、A2 topic_intro呼び出し(`er003_v1_n3_01_tts_generate.py::generate_a2_segments`)が`enable_connected_speech_equivalence_layer`引数自体を渡していなかった漏れ(role差による非適用)を修正し、`connected_speech_enabled_for("topic_intro")`を参照するようにした。**構造的事実(Gapではなく記事構成の違い、隠蔽せず記録)**: A2のPreview/Comment(1-4)は英語ナレーションとして存在せず(`generate_a2_japanese_with_reading_safety`経由の日本語音声)、この2 roleはA2側にそもそも適用対象が無い。A2で実際に英語として存在し適用されるのはFull Story(Point本文含む)/Topic intro/In One Lineの3 role。**runtime evidence(実API、専用out-dir`er020_output/tts_local_rewrite_production_wiring_01/a2/`、既存記事artifactは無変更)**: (a) 3 role(full_story_part1相当/topic_intro/in_one_line)各1 segment・attempt1でOK、`enable_connected_speech_equivalence_layer=True`を受理。(b) cool-down→Local Rewrite→QA→再TTS→ASR再検証の実end-to-end経路: 架空theme上の新規segment(数値表記の言い換えが必要なテキスト)で、標準2回NG→実測cool-down600.004秒(`cooldown_started_at`17:32:19→`cooldown_ended_at`17:42:19、JST)→fallback(3回目)もNG→Local Rewrite候補生成(Luna`gpt-5.6-luna`、5候補)→7 Gate QA全PASS候補を選定→再TTS(cool-downなし)を実行したが、再TTS後もASR再検証が不一致となり最終状態`HUMAN_REVIEW_LOCKED_RETTS_FAILED`(`run_local_rewrite_recovery()`が定義する4分岐の1つ、Human Review Lockへ正しく進む正常な終端)。B1の"boil it down to the main idea"のような完全解決(`RESOLVED_BY_LOCAL_REWRITE`)には到達しなかったが、cool-down実測・Local Rewrite候補生成/QA(Luna実model_id)・再TTS・ASR再検証という回復パイプライン全工程が実際に発火し、正しい終端状態へ到達することを確認した(詳細REPORT §4)。**Natural English Gate "main idea"整合性確認**: Fable指摘(Trial-02で"main idea"を含む候補がFAILした経緯とrun_01でのPASSの整合性)について、`er020_output/tts_local_rewrite_production_wiring_01/natural_english_gate_main_idea_reconciliation.md`に、両方の実際のLuna判定ログを突き合わせた結果(矛盾ではなく、"bring...together"型の機械的単語置換は両runで一貫してFAIL、"boil it down to"等の慣用的言い換えは一貫してPASS)を記録した(再生成なし、記録のみ)。**テスト**: 新規`er007_ja_tts_retry_path_fix_test_01.py::A2CooldownLocalRewriteWiringTests`(5件: cooldown発火/非発火、Local Rewrite回復のRESOLVED/失敗分岐、Human Review到達条件)・`A2TopicIntroConnectedSpeechRoleWiringTests`(1件、topic_intro role配線のsource-based regression guard)・`er020_tts_retry_local_rewrite_01_test_01.py`に`_local_rewrite_recovery_for_english_segment_with_fallback`の非標準layout安全性テスト1件を追加。既存regression `python -m unittest discover -s . -p "*_test_01.py"` 実行、既知の1件failure(Family A files working-tree diff self-check、commit後に自然解消)のみで新規failureなし。
- 関連: `TTS-LOCAL-REWRITE-CONNECTED-SPEECH-PRODUCTION-WIRING-01_REPORT.md`、`docs/pm/recon_tts_local_rewrite_wiring_01.md`、`TTS-COOLDOWN-LOCAL-REWRITE-TRIAL-01`/`TTS-LOCAL-REWRITE-NATURAL-ENGLISH-QA-TRIAL-02`(Trial系譜)、`OPEN-122-CONNECTED-SPEECH-EQUIVALENCE-LAYER-PRODUCTION-WIRING-01`(旧範囲、本行で更新)、`OPEN_ITEMS.md` OPEN-122。
- commit: (本コミットでFable Gate最終判定`PRODUCTION_WIRED`のDECISION_LOG/CURRENT_SPEC.md/OPEN_ITEMS.md反映を実施)

## FICTION-FAMILY-Z-PRODUCTION-E2E-01: Family Z(Fiction)ユーザー確定ルールのSSOT記録+Phase 0事前調査(権利/構造)の反映(2026-09-26)

- 日付: 2026-09-26
- 区分: ユーザー正式決定のSSOT記録(¥0、コード・Prompt変更なし、判断内容の書き換えではなく逐語記録)。
- ユーザー確定事項(逐語ベース要旨、変更禁止): (1) Fiction系を既存Future Family(Family C=Future Story)から独立させ、**Family Z = Fiction**(Literature/Public Domain文学・Real Story・Historical True Crime、基本は外部Seed方式、AI完全自由創作を標準方式にしない)とする。Family Yは将来Voices系を想定し、Family Zとは分離管理する。(2) Story内容の基本方針: 元作品・元エピソードを無理に変更しない(人物名変更・舞台変更・結末変更・不要な設定変更をしない。知っている物語を英語で聞く体験にも価値がある)。許容する編集は尺に収めるための脇筋削除・登場人物整理・背景説明圧縮・心理描写圧縮・重複場面削除のみ。中心Story・因果・重要な人物関係・結末・原作の魅力を壊さない。(3) Seed必須条件(4項目): Original/primary textを直接確認できる/日本法+米国法双方で利用可能/短編化しても中心Storyを保持できる/Story品質基準4項目中原則3以上。検索要約だけをSeedにしない。一次資料確認不能はNG。(4) Fiction共通の外国人名ルール(正式): 外国人の人名は本文中ではFirst name/Last nameのどちらか一方だけを使用(例: Eugene Aram→Aram、Richard Houseman→Houseman、Daniel Clarke→Clarke。First name側へ統一も可)。例外(フルネーム可)=同姓人物が複数いて区別不能/片方だけでは事実関係が曖昧/固有名称としてフルネーム自体が重要、のみ。単なる慣習でフルネームを使わない。適用実績: 「The Second Skeleton」追従修正(commit `6c0b0e02`、意味・Fact不変)。(5) Story Type metadata: `story_type`。literature(Literature/Fiction)=UI表示なし・音声冒頭追加なし / real_story=UI「REAL STORY」・音声冒頭 "This is a true story." / true_crime=UI「TRUE CRIME」・音声冒頭 "This is a true crime story."。Writer本文へ説明文を書かせず、Production側がmetadataから自動的にUI/音声introを切り替える。(6) TTS: Family Zでも既承認TTS Production仕様(attempt1→即時2→10分cool-down→3→Local Rewrite+Natural English QA[Luna 1 call]、Connected Speech: Full Story/Comment/Preview/Topic intro/In One Line適用、Heading readout/Key Phrase非適用)をそのまま使用する。(7) PRODUCTION_WIRED条件: Production正式初回path/retry・fallback・regeneration/story_type/naming rule/TTS/runtime evidence/tests/CURRENT_SPEC/DECISION_LOG/OPEN_ITEMS/Git/Dangling Reference Check。Trial scriptだけで成立した場合は不可。
- Phase 0結果(事実、`docs/pm/recon_family_z_production_e2e_01.md`、commit `5284391b`): 走れメロス(1940年公表、太宰1948年没)はTrial-02の権利確認が日本法のみで判定されており(「Aozora公開・没年1948[著作権切れ]」)、米国ではURAA(1996-01-01発効)により、1996年1月1日時点で本国(日本)においてまだ著作権保護期間中だった(没後50年=1998年末まで保護)ため米国著作権が回復し、発行日(1940年)から95年=2035年末まで保護される可能性が高い(Cornell Public Domainチャート根拠、2036年PD)。日米双方でPD確定済みの代替候補(羅生門/The Bet/Livingstoneライオンの場面等)がTrial-02候補表に事実として存在する。Fiction専用Production entry pointは存在せず、最近傍機構はFamily C(`plan_story_segments()`/A2 Comment仕様B/Key Phrase共有module/Assembly〜player)だが"Future Story"専用設計であり、In One Line相当のsegmentがFamily Cに存在しない。`story_type`フィールド・外国人名ルールの自動チェックは既存コードに存在しない。`er020_tts_retry_local_rewrite_01.py`の`resolve_narrative_role()`はsegment_id完全一致判定のため、Family C既存の命名規約(`story_001`等)ではConnected Speechが構造的に不適用となる。
- 既存記述との整理: `DECISION_LOG.md` `PM-USER-VALIDATION-DIRECTION-RECORD-01`(9319行付近、9324行「Future→Fiction再定義は未確定」)は、本管理IDによるFamily Z新設(Family Cとは別の新Family)により整理される。Family C(Future Story)自体の再定義ではなく、Fiction専用の新Family Zを別途新設する方針である旨をCURRENT_SPEC.md新設節に相互参照として明記した。
- SSOT反映: `CURRENT_SPEC.md`にFamily C節(1080行付近)の直前へ「## Family Z(Fiction)」節を新設し、上記1〜7とPhase 0結果を記載(Status: 仕様はユーザー確定[2026-09-26]、Production配線は未実装・Phase 0完了・`USER_DECISION_REQUIRED`[権利・構造論点])。`OPEN_ITEMS.md`へ新規OPEN行(Family Z Production E2E: `USER_DECISION_REQUIRED`[走れメロス米国権利/Family Cとの関係/In One Line設計/Story構成]、Phase 0 recon参照)を追加。`docs/pm/REPORT_LEDGER.md`へ本管理ID行を追加(完了日=進行中、初回正式報告=済[2026-09-26]、Feedback=未、累積再掲対象=Y)。
- 併せて`er018_output/fiction_real_story_and_true_crime_trial_01/stories/true_crime_eugene_aram/story.md`(修正前と同一内容の複製)へ、`true_crime/story.md`と同じ人名修正(Daniel Clarke/Eugene Aram/Richard Houseman → Clarke/Aram/Houseman)を適用し本文を逐語同一化、ファイル冒頭に「canonical は true_crime/story.md、本ファイルは試行履歴の複製」の1行注記を追加した(意味・Fact不変、API呼び出し・再生成・音声化なし)。
- 未解決事項: 走れメロス米国著作権status(続行/差し替え/専門家確認、いずれもユーザー判断)、Family C・Family Zの関係(完全に別Family/拡張/統合)、story_type metadataの具体的スキーマ・UI文言・音声intro文言の最終承認、外国人名ルールのPrompt組込方法、Connected Speech 5-role適用のsegment_id規約統一方法、In One Line相当segmentの新規Prompt設計。いずれも`OPEN_ITEMS.md`新規OPEN行で追跡する。
- 関連: `docs/pm/recon_family_z_production_e2e_01.md`(commit `5284391b`)、`FICTION-REAL-STORY-AND-TRUE-CRIME-TRIAL-01`、`er018_output/fiction_real_story_and_true_crime_trial_01/true_crime/naming_rule_diff.md`(commit `6c0b0e02`)。
- commit: (本コミットでSSOT反映[CURRENT_SPEC.md/OPEN_ITEMS.md/REPORT_LEDGER.md]+story.md複製の人名修正+注記追加を実施)

## PM-USER-DECISIONS-SSOT-CONSOLIDATION-02: ユーザーが2026-09-26に正式決定した7件のSSOT反映(Family Z構造論点/TTS確定/Family X B3 X-1・X-2/Vocabulary Standard方針/Fiction True Crime F-1〜F-4/EN-ASR意味等価性レビューREPORT化/REPORT_LEDGER整理)

- 日付: 2026-09-26
- 区分: ユーザー正式決定のSSOT記録・重複整理(¥0、コード・Prompt変更なし、判断内容の書き換えではなく逐語記録+参照整理)。
- **1. Family Z(`FICTION-FAMILY-Z-PRODUCTION-E2E-01`、OPEN-185)**: OPEN-185の未決論点(2)(3)(4)をユーザーが決定。Z-2: Family Z専用runner+Family C既存関数再利用、`APPROVED_FOR_PRODUCTION`(Family C本体は不要に変更しない)。Z-3: In One Line PromptをFiction向けに最小変更して流用、`APPROVED_FOR_PRODUCTION`(新規独立機構は作らない)。Z-4: Story本文は単一canonical text、Dialogue部分は既に承認済みのspeaker/male-female Voice assignmentルールに従う、`APPROVED_FOR_PRODUCTION`。**当該Voiceルールの正式所在はGrep調査(B-Family Voices A/B/Narrator固定[Algieba/Erinome/Aoede]・Family C article_config単位のVoice指定・pitch推定による性別的印象判断[本ログ7826行付近]を確認)で特定できなかった(該当ルール未特定、CURRENT_SPECには「参照先はFable確認中」と記載)**。Z-1(走れメロス権利)は未決のまま、別管理ID`FICTION-FAMILY-Z-RIGHTS-RECHECK-01`で確認中。差し替えは行っていない。Production配線(`PRODUCTION_WIRED`)は未着手のまま。SSOT反映: `CURRENT_SPEC.md`「Family Z(Fiction)」節へ追記、`OPEN_ITEMS.md` OPEN-185を上記のとおり更新(残論点をZ-1のみへ縮小)。
- **2. TTS**: `TTS-LOCAL-REWRITE-CONNECTED-SPEECH-PRODUCTION-WIRING-01`のFable判定`PRODUCTION_WIRED`(2026-09-26)にユーザー異議なし、**確定**。`CURRENT_SPEC.md`該当行へユーザー確定注記を追加。OPEN-184(数値ASR不一致、digit vs spelled-out)は、「数値だけの小Trial」を実施せず`EN-ASR-SEMANTIC-EQUIVALENCE-REVIEW-01`(包括対策レビュー)へ吸収する方針をユーザーが決定。OPEN-184は「吸収済み(→EN-ASR-SEMANTIC-EQUIVALENCE-REVIEW-01)」として`CLOSED`とし、Failure mode記述・経緯は履歴としてそのまま保持した。
- **3. Family X B3(`NEWS-FAMILY-X-B3-FACT-SELECTION-PRODUCTION-WIRING-01`、OPEN-183)**: X-2(Meta記事のAI Storylineをそのまま許容し、「なぜ人間を使ったか」Fact[MUSE-HC-009]を落とした点)は今回は仕様変更しないことで決着。X-1(時制ドリフト)は`NEWS-FAMILY-X-B3-DIVERSITY-TRIAL-01`内でFact fidelity修正として対応中、原因調査は`NEWS-FAMILY-X-B3-ADVANCED-RETRY-ROOTCAUSE-01`(進行中)。追加2記事(小さいバッグ/ホルムズ)のDiversity Trialは継続。OPEN-183の備考へ1行追記。
- **4. Vocabulary/Standard(`NEWS-VOCAB-LEVEL-PRODUCTION-WIRING-01`、`APPROVED_FOR_PRODUCTION`、配線未実施)**: Standardはv5構文簡易化を維持しつつ、firmな6,000語Bandを追加する。Band除外条件は(1)固有名詞、(2)推測容易な派生・複合語、(3)日本語定着語、の3種のみ。Topic Core Word(sewer/septic等、6,000超でも置換すると意味・Fact・自然さが崩れる主題語)は理想形ではないがTrial結果上の現実的運用として残してよい。不採用: 10,000語Band/Advanced 14,000語Band/後段A/B/C/D単語単位事後置換パス/Promptへの個別語ハードコード。Advancedは一律Bandを新設せず、既存の`ADVANCED_VOCAB_RULE_V2_BLOCK`(`ADVANCED-VOCAB-V2-PRODUCTION-RESTORE-01`で`PRODUCTION_WIRED`済み)をそのまま使う。これにより`STANDARD-A2-6000-GENERATION-FIRST-TRIAL-01`のS-1(v5維持を採用)/S-2(A/B/C/D事後置換パスは外す)/S-3(個別語例示は一般表現へ)、および`NEWS-VOCAB-BAND-6000-10000-14000-TRIAL-01`のVB-1(Band単独では不十分、v5構文簡易化との組合せを採用)/VB-2(v5文再構築+firm 6,000 Band+3除外条件の組合せをユーザーが直接承認)/VB-3(主題語はBand例外として保持する既定挙動を採用)はユーザー決定済み(上書き)として整理する。SSOT反映: `CURRENT_SPEC.md`「Entertainment英語版生成方式(Standard/A2)」行・「(Advanced)」行へ追記、`docs/pm/REPORT_LEDGER.md`の該当2行を「ユーザーFeedback 済(2026-09-26: `NEWS-VOCAB-LEVEL-PRODUCTION-WIRING-01`方針で上書き)」「累積再掲対象 N」へ更新。`OPEN_ITEMS.md`にはこの2Trial固有の未決行は存在しなかった(Grep確認0件)ため、close対象のOPEN行は無し。
- **5. Fiction/True Crime(`FICTION-REAL-STORY-AND-TRUE-CRIME-TRIAL-01`)**: 本管理ID自体はこれまで`DECISION_LOG.md`本体に専用エントリが無く(`FICTION-FAMILY-Z-PRODUCTION-E2E-01`エントリからの相互参照のみ)、`FICTION-REAL-STORY-AND-TRUE-CRIME-TRIAL-01_REPORT.md`にのみ判断待ち事項(F-1〜F-4)が記録されていたため、本エントリで初めてSSOT本体へ反映する。F-1: Real Story「Ten Days Inside」(Nellie Bly)はユーザー評価済み、`VALIDATED`。F-2: True Crime「The Second Skeleton」(Eugene Aram)はユーザー評価済み、`VALIDATED`。F-3: 「True Crimeは事実改変なしを優先し実名を全保持する」という同Reportの代替案(Fiction家族の主人公のみ固有名ルールからの意図的逸脱、Fable推奨=記録する、だったが本SSOTには未反映のまま残っていた)は**不採用**。正式採用されたFamily Z共通人名ルール(外国人名は原則First/Last片方、必要な場合[3条件]のみフルネーム例外)をTrue Crimeにも含め全Story Typeへ適用する(superseded by Family Z人名ルール、2026-09-26)。F-4: PD犯罪記録集(Newgate Calendar系等)の利用は**認める**。ただし正式Seedとして使う資料は(a)直接本文確認できる、(b)出所が明確、(c)rights確認できる、(d)当該資料にないFactを他の要約から継ぎ足さない、の4条件を満たすことを条件とする。SSOT反映: `CURRENT_SPEC.md`「Family Z(Fiction)」節へF-3/F-4の整理注記を追加(項目3(a)・項目4の運用整理として)。`docs/pm/REPORT_LEDGER.md`の該当行を「ユーザーFeedback 済(2026-09-26)」「累積再掲対象 N」へ更新。
- **6. EN-ASR-SEMANTIC-EQUIVALENCE-REVIEW-01**: Fableのレビュー報告(read-only Recon`docs/pm/recon_en_asr_semantic_equivalence_01.md`+批判レビュー22項目、実装・Trial・Production変更なし)を、会話ログ(`f1538907-8efe-486d-9790-ef5c6cd789fa.jsonl`)から最後に出現する「★★★★報告ここから」ブロック内の「## A. EN-ASR-SEMANTIC-EQUIVALENCE-REVIEW-01」節(「### 現在の状態」〜「### 補足・技術詳細」まで)を逐語抽出(要約なし)し、`EN-ASR-SEMANTIC-EQUIVALENCE-REVIEW-01_REPORT.md`として新規作成した。Status: `USER_DECISION_REQUIRED`(A-1範囲/A-2 Tier2除外維持/A-3 Trial実施可否の3点判断待ち)。`OPEN_ITEMS.md`にOPEN-186(新規)を追加し、OPEN-184をこの管理IDへ吸収した旨を明記した。`docs/pm/REPORT_LEDGER.md`へ新規行(初回正式報告 済2026-09-26、Feedback 未、累積再掲対象 Y)を追加した。
- **7. REPORT_LEDGER全体整理**: `docs/pm/REPORT_LEDGER.md`の既存行を次のとおり更新した。TTS行: Feedback 済(2026-09-26 `PRODUCTION_WIRED`確定)、累積再掲対象 N。Family X B3行: Feedback 済(2026-09-26 X-2決着、X-1はDiversity Trial/原因調査へ移管)、累積再掲対象 N(移管先で報告)。Family Z行: Feedback 一部済(Z-2/Z-3/Z-4 `APPROVED_FOR_PRODUCTION`、Z-1のみ`FICTION-FAMILY-Z-RIGHTS-RECHECK-01`で継続)、累積再掲対象 Y(Z-1のみ)。新規行: `NEWS-FAMILY-X-B3-ADVANCED-RETRY-ROOTCAUSE-01`/`FICTION-FAMILY-Z-RIGHTS-RECHECK-01`/`NEWS-FAMILY-X-B3-DIVERSITY-TRIAL-01`(いずれも進行中、初回報告 未)を追加した。
- **Dangling Reference Check**: 上記反映テキストは「A/B/C/D」「Advanced v2以外の未承認Trial名」「14,000語Band」等をいずれも**不採用として明記した文脈でのみ**参照しており、正式仕様として参照していないことを確認した。
- 関連: `EN-ASR-SEMANTIC-EQUIVALENCE-REVIEW-01_REPORT.md`、`docs/pm/recon_en_asr_semantic_equivalence_01.md`、`docs/pm/recon_vocab_level_production_wiring_01.md`、`STANDARD-A2-6000-GENERATION-FIRST-TRIAL-01_REPORT.md`、`NEWS-VOCAB-BAND-6000-10000-14000-TRIAL-01_REPORT.md`、`FICTION-REAL-STORY-AND-TRUE-CRIME-TRIAL-01_REPORT.md`、`TTS-LOCAL-REWRITE-CONNECTED-SPEECH-PRODUCTION-WIRING-01_REPORT.md`。
- commit: (本コミットでSSOT反映[CURRENT_SPEC.md/DECISION_LOG.md/OPEN_ITEMS.md/docs/pm/REPORT_LEDGER.md]+新規`EN-ASR-SEMANTIC-EQUIVALENCE-REVIEW-01_REPORT.md`作成を実施)

## PM-USER-DECISIONS-SSOT-CONSOLIDATION-03: Family Z権利Gate正式採用(Z-1決着)+Z-4 Voice参照先確定+ASR包括対策Status訂正+Family X B3 Diversity Trial/Advanced Retry Rootcauseレビュー記入(2026-09-26)

- 日付: 2026-09-26
- 区分: ユーザー正式決定+Fable判定のSSOT記録(¥0、コード・Prompt変更なし、判断内容の書き換えではなく逐語記録)。
- **1. Family Z権利Gate(Z-1決着、ユーザー正式決定、逐語)**: 「日本人向け・日本国内向けサービスであるため、Family Zの権利条件は以下で正式採用: (1)日本国内でPublic Domain、または日本国内で適法に利用可能であることを必須条件とする (2)米国PDは必須条件にしない (3)海外向け配信・提供を将来行う場合のみ、その対象法域のrights確認を追加する (4)原作がPDでも、現代の第三者翻訳・英訳・挿絵等は別著作物として扱い、権利未確認のものは流用しない (5)Family Zのadaptationは、確認済み原文/正式Seedから自前生成する。走れメロスは日本でPD確認済みのため初回Family Z E2E対象として継続、差し替え不要。本rights方針は`APPROVED_FOR_PRODUCTION`。Family Z自体はZ-2/Z-3/Z-4と合わせ、Production wiringとE2E evidence完了まで`PRODUCTION_WIRED`にしない。」根拠: `FICTION-FAMILY-Z-RIGHTS-RECHECK-01`が確認した、Gate2運用定義がソース表示依拠で日本法のみで完結していること、Keller判例由来の教訓が未適用であったこと、米国PD必須の根拠記載がSSOT中に見当たらなかったこと。SSOT反映: `CURRENT_SPEC.md`「Family Z(Fiction)」Seed必須条件(b)を上記(1)〜(5)へ書き換え、旧文言「日本法+米国法双方で利用可能」は履歴注記として残した。rights記録ブロックの説明へ「米国等の他法域statusは条件ではなく記録事項(走れメロス: URAA回復で2035年末まで米国保護の可能性、法的最終判断ではない)」を追記。`OPEN_ITEMS.md` OPEN-185を「Z-1決着(残論点なし)、Status=`APPROVED_FOR_PRODUCTION`(配線は未着手)」へ更新。`FICTION-FAMILY-Z-RIGHTS-RECHECK-01_REPORT.md`末尾に「§6 ユーザー決定(2026-09-26)」として上記を逐語追記した。Trial-02 REPORT §11の旧記述(「日本法+米国法の両方でPD」)は履歴のため書き換えていない。
- **2. Z-4 Voice assignmentルールの参照先(Fable確認結果)**: `PM-USER-DECISIONS-SSOT-CONSOLIDATION-02`で「参照先はFable確認中」としていた箇所を確定した。参照先はFamily C(Future Story)で運用実績のあるDialogue Voice割当機構であり、narrator=Aoede、登場人物台詞=Erinome/Charon等を`classify_quote_voice_window()`(`er013_family_c_production_01.py` L66-95、OPEN-156修正を汎用化した関数)で割り当て、記事ごとに`voice_tts_names`で人物→Voiceを指定する(`DECISION_LOG.md` `USER-TEST-FINAL-AUDIO-BATCH-06` 委任B/C、Status=`VALIDATED`[Trial]、既知課題OPEN-156[引用符境界またぎ誤判定]、性別印象はSSOT未記載のためピッチ推定で代替した経緯あり[本ログ7826行付近])。Family ZはZ-2の方針どおりこの既存関数を再利用し、新しいVoice仕様を作らない。SSOT反映: `CURRENT_SPEC.md`Z-4節の該当箇所を上記参照先へ置換。
- **3. ASR包括対策のStatus訂正(ユーザー指示)**: OPEN-186(`EN-ASR-SEMANTIC-EQUIVALENCE-REVIEW-01`)のStatusを`USER_DECISION_REQUIRED`から`REVIEW_PENDING`(Trial deferred)へ訂正した。ユーザー指示(逐語): 「A-1: レビュー対象はTier 1〜3の3層全面案(Production採用決定ではない、Fableレビュー対象)。A-2: Tier 2も除外せずレビュー対象に含める(安全性・false acceptリスクをFableが確認、採否はレビュー後)。A-3: 今はTrialしない。Claude+Fableレビュー完了後、Trial実施要否をユーザー判断。」SSOT反映: `OPEN_ITEMS.md` OPEN-186のStatus・備考を更新。`EN-ASR-SEMANTIC-EQUIVALENCE-REVIEW-01_REPORT.md`冒頭Statusも同様に訂正し、「初回Fableレビュー(Tier 1中心)は暫定、Tier 2/3安全性補完レビュー(`docs/pm/recon_en_asr_semantic_equivalence_02_tier2_tier3.md`)後に最終化」と注記した。`docs/pm/REPORT_LEDGER.md`該当行のFeedback列を「済(2026-09-26: Status=REVIEW_PENDING、3層全面+Tier 2含めてレビュー継続、Trial deferred)」、再掲対象を「Y(レビュー完了時に再報告)」へ更新した。
- **4. NEWS-FAMILY-X-B3-DIVERSITY-TRIAL-01(Fable評価・分類)**: `er019_output/family_x_b3_diversity_trial_01/NEWS-FAMILY-X-B3-DIVERSITY-TRIAL-01_REPORT.md`の空欄だった§Fable評価・§分類へFableの評価(small_bag=`VALIDATED`、meta=`VALIDATED`[Fact fidelity修正後]、hormuz=`NOT_COMPLETED`[Advanced段Ledger Deviation Gate STOP、Gateの正常動作]、効果音=`USER_DECISION_REQUIRED`[提案のみ])を記入した。最重要所見: hormuzのMAJOR 3件・meta記事の時制ドリフトはいずれもJA Writer段(R2時点)で既に発生していたことを確認し、英訳段retryでは構造的に解消できないことを特定した(これはB3仕様の問題ではなくWriter経路のGate配置の問題、詳細は下記5)。`docs/pm/REPORT_LEDGER.md`該当行を「初回正式報告 済(2026-09-26)、Feedback 未、再掲対象 Y(効果音・hormuz扱い)」へ更新。`OPEN_ITEMS.md` OPEN-183備考へ「small_bag/meta記事はユーザー確認待ち・音声化未実施、hormuzはGate STOPで本文なし」を追記した。
- **5. NEWS-FAMILY-X-B3-ADVANCED-RETRY-ROOTCAUSE-01(Fableレビュー)**: `NEWS-FAMILY-X-B3-ADVANCED-RETRY-ROOTCAUSE-01_REPORT.md`末尾へ「§13 Fableレビュー(2026-09-26)」を追記した。要旨: 根本原因分析を支持(①英訳段retryが1回目MAJORの内容を一切受け取らず同一JA入力で再生成するだけ、②逸脱発生源がJA Original段でありJA WriterにLedger照合Gateが無いため英訳段retryでは自己修復不能)。1回目Checkerの生JSON未保存という観測性の欠落を確認(前回報告のparaphraseは未検証だった)。最小対策として[a]観測性修正(全deviation check結果保存、挙動変更なし、Production直接修正可、¥0)と[b]挙動修正(JA段Ledger照合Gate追加+英訳段retryへのmust-fix/issue persistence追加、Trial必須)を提示、[c]deterministic時制チェックは不採用とした。`docs/pm/REPORT_LEDGER.md`該当行を「初回正式報告 済(2026-09-26)、Feedback 未、再掲対象 Y」へ更新。`OPEN_ITEMS.md`に新規OPEN-187(Family X Writer経路: JA段Ledger照合Gate不在+英訳retryのmust-fix/issue persistence不在、Status=`USER_DECISION_REQUIRED`)を追加した。
- **6. REPORT_LEDGER**: `FICTION-FAMILY-Z-RIGHTS-RECHECK-01`行を「Feedback 済(2026-09-26ユーザー決定: 日本国内PD必須・米国PD非必須・走れメロス継続)、再掲対象 N」へ更新。`FICTION-FAMILY-Z-PRODUCTION-E2E-01`行を「Feedback 済(Z-1〜Z-4すべて決着)、再掲対象 N(Phase 1実装は別途)」へ更新。
