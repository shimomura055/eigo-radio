...
class WorkflowStaticTests(unittest.TestCase):
    ...
    def test_repo_scope_guards_command_substitutions_with_if(self):
        # AUTO-001-05-03-02-R1: repo_scopeステップのcurl呼び出し・python呼び出し
        # の両方が、config_checkと同型のif文保護パターンを使っていること。
        self.assertIn("if http_code=$(curl -sS -o \"$resp_file\"", self.text)
        self.assertIn(
            "if line=$(python -m scripts.controller_token_check repo-scope",
            self.text,
        )

    def test_issue_read_guards_command_substitutions_with_if(self):
        self.assertIn(
            "if line=$(python -m scripts.controller_token_check issue-read",
            self.text,
        )

    def test_curl_command_substitution_guarded_exactly_twice(self):
        # repo_scope・issue_readの各1箇所ずつ、合計2箇所。
        self.assertEqual(self.text.count("if http_code=$(curl -sS -o \"$resp_file\""), 2)

    def test_reason_code_internal_error_fallback_present_in_all_three_steps(self):
        # config_check・repo_scope・issue_readのそれぞれで、grep不一致時に
        # INTERNAL_ERRORへ確実にフォールバックすること。
        self.assertEqual(self.text.count('reason_code="INTERNAL_ERROR"'), 3)

    def test_summary_distinguishes_downstream_steps_skipped_due_to_upstream_failure(self):
        # AUTO-001-05-03-02-R1(継続修正): repo_scope・issue_readがskippedに
        # なった場合も、reason_code=NONEではなく前段原因に対応した固定
        # reason codeを表示する。
        for fixed_code in (
            "REPOSITORY_SCOPE_SKIPPED_CONFIG_FAILURE",
            "REPOSITORY_SCOPE_SKIPPED_TOKEN_FAILURE",
            "ISSUE_READ_SKIPPED_CONFIG_FAILURE",
            "ISSUE_READ_SKIPPED_TOKEN_FAILURE",
            "ISSUE_READ_SKIPPED_SCOPE_FAILURE",
        ):
            self.assertIn(fixed_code, self.text)
        self.assertIn("SCOPE_OUTCOME: ${{ steps.repo_scope.outcome }}", self.text)
        self.assertIn("ISSUE_OUTCOME: ${{ steps.issue_read.outcome }}", self.text)

    def test_summary_never_hardcodes_reason_code_none_as_final_value_for_scope_or_issue(self):
        # scope_reason・issue_reasonの初期値はSCOPE_REASON/ISSUE_REASON(または
        # NONE)であり、skipped分岐で必ず上書きされる。Job Summary出力行は
        # 変数参照(${scope_reason} / ${issue_reason})経由であり、
        # 「reason_code=NONE」という固定文字列を直接埋め込んでいない。
        self.assertNotIn('echo "- repositoryスコープ検証: reason_code=${SCOPE_REASON:-NONE}', self.text)
        self.assertNotIn('echo "- Issue read検証: reason_code=${ISSUE_REASON:-NONE}', self.text)
        self.assertIn("reason_code=${scope_reason}", self.text)
        self.assertIn("reason_code=${issue_reason}", self.text)


# ---------------------------------------------------------------------------
# AUTO-001-05-03-02-R1: GitHub Actions既定shellのfail-fast挙動を模した
# subprocessテスト。workflow YAMLから各stepの実スクリプトを抽出し、
# `bash --noprofile --norc -eo pipefail`(GitHub Actionsの既定)で実行する
# ことで、実際のrun 30510639443で発生した「非ゼロ終了時にreason_codeが
# GITHUB_OUTPUTへ保存されない」問題が、config_checkだけでなくrepo_scope・
# issue_readでも再発しないことを検証する(AC-11関連)。
# `curl`はbash関数として差し替え、実ネットワーク接続は一切行わない。
# credential(実際のGitHub App秘密鍵・token等)は一切使用せず、ダミー文字列
# だけを使う。
# ---------------------------------------------------------------------------

def _extract_step_run_lines(
    text: str, step_id: str, substitutions: dict[str, str] | None = None
) -> str:
    """`id: <step_id>`直後の`run: |`ブロックを、実ファイルからそのまま抽出する。
    抽出したテキストをテストで直接実行することで、workflow本体とテストの
    実装が乖離することを防ぐ。

    `substitutions`は、GitHub Actionsのランナー自身が(bashを起動する前に)
    `${{ ... }}`式をリテラル値へ置換する処理を模したもの。ここで置換しない限り、
    抽出したテキストには`${{ github.repository }}`のような文字列がそのまま
    残り、bashの変数展開として解釈できず構文エラーになる。"""
...
def _bash_single_quote(value: str) -> str:
    """任意の文字列(JSON本文などダブルクォートを含み得るもの)を、bashの
    単一引用符リテラルとして安全に埋め込めるようエスケープする。"""
    return "'" + value.replace("'", "'\\''") + "'"


def _fake_curl_function(*, exit_code: int, http_code: str, body: str) -> str:
    """`curl`をbash関数として上書き定義するスニペットを返す。実ネットワーク
    接続は一切行わず、指定した終了コード・HTTPステータス・レスポンス本文
    だけを返す。関数定義はPATH経由の実行ファイル探索より優先されるため、
    OS依存の実行権限やPATH区切り文字の差異(Windows/Linux)を気にせず、
    抽出した実stepスクリプトが呼び出す素の`curl`をそのまま差し替えられる。"""
    return (
        "curl() {\n"
        '  local out_file=""\n'
        '  local prev=""\n'
        ...
        "  done\n"
        '  if [ -n "$out_file" ]; then\n'
        f"    printf '%s' {_bash_single_quote(body)} > \"$out_file\"\n"
        "  fi\n"
...
class RepoScopeFailFastSubprocessTests(unittest.TestCase):
    """AUTO-001-05-03-02-R1(継続修正): repo_scopeステップの実スクリプトを
    抽出し、GitHub Actions既定の厳格shellで実行する。`curl`はbash関数として
    差し替え、実ネットワーク接続を一切行わない。"""
    ...
    def setUpClass(cls):
        cls.script = _extract_step_run_lines(
            ...
            "repo_scope",
            substitutions={"${{ github.repository }}": EXPECTED_FULL_NAME},
        )
    ...
    def _run(self, *, curl_exit_code: int, curl_http_code: str, curl_body: str):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            output_path.write_text("", encoding="utf-8")
            env = dict(os.environ)
            env["APP_TOKEN"] = "dummy-app-token-value"
            env["GITHUB_OUTPUT"] = str(output_path)
            result = _run_bash_script_strict(combined, env)
            outputs = _parse_github_output(output_path)
            return result, outputs

    def test_api_failure_via_curl_exit_code_saves_reason_code(self):
        result, outputs = self._run(curl_exit_code=7, curl_http_code="000", curl_body="")
        self.assertNotEqual(result.returncode, 0, msg=result.stderr)
        self.assertEqual(outputs.get("reason_code"), "REPOSITORY_SCOPE_QUERY_FAILED")

    def test_api_failure_via_non_200_http_status_saves_reason_code(self):
        result, outputs = self._run(curl_exit_code=0, curl_http_code="500", curl_body="")
        self.assertNotEqual(result.returncode, 0, msg=result.stderr)
        self.assertEqual(outputs.get("reason_code"), "REPOSITORY_SCOPE_QUERY_FAILED")

    def test_malformed_json_saves_internal_error_reason_code(self):
        result, outputs = self._run(
    ...
    def test_valid_json_with_wrong_structure_saves_response_validation_failed(self):
        result, outputs = self._run(
            curl_exit_code=0, curl_http_code="200", curl_body='{"total_count": 1}'
        )
        self.assertNotEqual(result.returncode, 0, msg=result.stderr)
        self.assertEqual(outputs.get("reason_code"), "RESPONSE_VALIDATION_FAILED")

    def test_zero_repositories_saves_mismatch_reason_code(self):
        body = json.dumps({"total_count": 0, "repositories": []})
        result, outputs = self._run(curl_exit_code=0, curl_http_code="200", curl_body=body)
        self.assertNotEqual(result.returncode, 0, msg=result.stderr)
        self.assertEqual(outputs.get("reason_code"), "REPOSITORY_SCOPE_MISMATCH")
        self.assertEqual(outputs.get("repository_count"), "0")

    def test_multiple_repositories_saves_mismatch_reason_code(self):
        body = json.dumps({
            "total_count": 2,
            "repositories": [
                {"full_name": EXPECTED_FULL_NAME},
        ...
        self.assertNotEqual(result.returncode, 0, msg=result.stderr)
        self.assertEqual(outputs.get("reason_code"), "REPOSITORY_SCOPE_MISMATCH")
        self.assertEqual(outputs.get("repository_count"), "2")
        # 他repository名をJob Summary相当の出力へ含めない。
        self.assertNotIn("other-repo", result.stdout)

    def test_repository_mismatch_saves_mismatch_reason_code(self):
        body = json.dumps({
            "total_count": 1,
            "repositories": [{"full_name": "shimomura055/other-repo"}],
        })
        result, outputs = self._run(curl_exit_code=0, curl_http_code="200", curl_body=body)
        self.assertNotEqual(result.returncode, 0, msg=result.stderr)
        self.assertEqual(outputs.get("reason_code"), "REPOSITORY_SCOPE_MISMATCH")
        self.assertEqual(outputs.get("matched"), "false")

    def test_matching_repository_passes(self):
        body = json.dumps({"total_count": 1, "repositories": [{"full_name": EXPECTED_FULL_NAME}]})
        result, outputs = self._run(curl_exit_code=0, curl_http_code="200", curl_body=body)
        self.assertEqual(result.returncode, 0, msg=result.stderr)
        self.assertEqual(outputs.get("reason_code"), "NONE")
        self.assertEqual(outputs.get("matched"), "true")

    def test_app_token_value_never_appears_in_stdout_or_stderr(self):
        result, _outputs = self._run(curl_exit_code=0, curl_http_code="500", curl_body="")
        self.assertNotIn("dummy-app-token-value", result.stdout)
        self.assertNotIn("dummy-app-token-value", result.stderr)

    def test_response_body_never_appears_in_stdout_or_stderr(self):
        body = json.dumps({"total_count": 1, "repositories": [{"full_name": "MARKER-BODY-XYZ"}]})
        result, _outputs = self._run(curl_exit_code=0, curl_http_code="200", curl_body=body)
        self.assertNotIn("MARKER-BODY-XYZ", result.stdout)
        self.assertNotIn("MARKER-BODY-XYZ", result.stderr)


class IssueReadFailFastSubprocessTests(unittest.TestCase):
    """AUTO-001-05-03-02-R1(継続修正): issue_readステップの実スクリプトを
    抽出し、GitHub Actions既定の厳格shellで実行する。`curl`はbash関数として
    差し替え、実ネットワーク接続を一切行わない。"""
    ...
    def setUpClass(cls):
        cls.script = _extract_step_run_lines(
            ...
            substitutions={
                ...
                "${{ inputs.issue_number }}": cls.EXPECTED_ISSUE_NUMBER,
            },
        )
    ...
    def test_api_failure_via_curl_exit_code_saves_reason_code(self):
        result, outputs = self._run(curl_exit_code=7, curl_http_code="000", curl_body="")
        self.assertNotEqual(result.returncode, 0, msg=result.stderr)
        self.assertEqual(outputs.get("reason_code"), "ISSUE_READ_FAILED")

    def test_api_failure_via_non_200_http_status_saves_reason_code(self):
        result, outputs = self._run(curl_exit_code=0, curl_http_code="404", curl_body="")
        self.assertNotEqual(result.returncode, 0, msg=result.stderr)
        self.assertEqual(outputs.get("reason_code"), "ISSUE_READ_FAILED")
    ...
    def test_issue_number_mismatch_saves_reason_code(self):
        body = json.dumps({"number": 7, "title": "MARKER-TITLE-XYZ", "body": "MARKER-BODY-XYZ"})
        result, outputs = self._run(curl_exit_code=0, curl_http_code="200", curl_body=body)
        self.assertNotEqual(result.returncode, 0, msg=result.stderr)
        self.assertEqual(outputs.get("reason_code"), "ISSUE_READ_FAILED")
        self.assertEqual(outputs.get("issue_number_matched"), "false")
        self.assertNotIn("MARKER-TITLE-XYZ", result.stdout)
        self.assertNotIn("MARKER-BODY-XYZ", result.stdout)

    def test_matching_issue_number_passes(self):
        body = json.dumps({"number": int(self.EXPECTED_ISSUE_NUMBER), "title": "x", "body": "y"})
        result, outputs = self._run(curl_exit_code=0, curl_http_code="200", curl_body=body)
        self.assertEqual(result.returncode, 0, msg=result.stderr)
        self.assertEqual(outputs.get("reason_code"), "NONE")
        self.assertEqual(outputs.get("issue_number_matched"), "true")

    def test_app_token_value_never_appears_in_stdout_or_stderr(self):
        result, _outputs = self._run(curl_exit_code=0, curl_http_code="404", curl_body="")
        self.assertNotIn("dummy-app-token-value", result.stdout)
        self.assertNotIn("dummy-app-token-value", result.stderr)

    def test_response_body_never_appears_in_stdout_or_stderr(self):
        body = json.dumps({"number": 999, "title": "MARKER-TITLE-ABC", "body": "MARKER-BODY-ABC"})
        result, _outputs = self._run(curl_exit_code=0, curl_http_code="200", curl_body=body)
        self.assertNotIn("MARKER-TITLE-ABC", result.stdout)
        self.assertNotIn("MARKER-BODY-ABC", result.stdout)
        self.assertNotIn("MARKER-TITLE-ABC", result.stderr)
        self.assertNotIn("MARKER-BODY-ABC", result.stderr)
