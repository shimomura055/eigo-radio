...
data = json.load(open("er006_output/pool_pilot_01/adoption_audit_01/review_data.json", encoding="utf-8"))
...
def level_block(topic_key, level, ld):
    with open(ld["audio_mp3"], "rb") as f:
...
def build_page(topic_key, td):
    b1 = level_block(topic_key, "b1", td["levels"]["b1"])
    a2 = level_block(topic_key, "a2", td["levels"]["a2"])
    return f'''<title>{["Public Benches","Subscriptions","Startups Growth"][td["no"]-1]}</title>
<style>
:root {{
...
<div class="wrap">
  <p class="eyebrow">Pool Topic No.{td["no"]} · FINAL_ADOPTION_PENDING_USER_REVIEW</p>
  <h1>{esc(td["title_en"])}</h1>
  <p class="sub-ja">{esc(td["title_ja"])}</p>
  {b1}
  {a2}
  <footer>ER-006-POOL-ADOPTION-AUDIT-01 · Production差分・Provenanceは別記録(ER-006-POOL-ADOPTION-AUDIT-01_diff.md)を参照。⚠マークの区間は機械ASR検証未合格のまま採用されています。</footer>
</div>
'''

import os
os.makedirs("er006_output/pool_pilot_01/adoption_audit_01/review_html", exist_ok=True)
for topic_key, td in data.items():
    page = build_page(topic_key, td)
    path = f"er006_output/pool_pilot_01/adoption_audit_01/review_html/{topic_key}.html"
