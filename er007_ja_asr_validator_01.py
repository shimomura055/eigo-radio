...
def normalize_ja(text: str) -> str:
    ...
    同じ表記ゆれとして安全に判別できる限定的な正規化であり、「二十」の
    ような助数詞を伴わない一般の漢数字、および意図的に対象外とした
    「日」「人」(不規則読みによる副作用のリスクが実証されている、
    ER-011-KP-VALIDATOR-NUMERIC-HOMOPHONE-AND-GLOSS-RULES-PRODUCTION-
    WIRING-02参照)は変更しない(ER-008-B1-POINT2-FACT-FIX-AND-JA-
    NUMERAL-NORMALIZATION-07で「つ」限定として導入、上記管理IDで拡張。
