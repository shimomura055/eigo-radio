#!/usr/bin/env python3
"""
FAMILY-A-DISCOVERY-STAGE2-INTERPRETATION-RULE-TRIAL-08
D-2目視artifact生成(Role文字列のヒューリスティック分類は行わず、
Point本文をそのまま並置する。既存の比較artifact形式
[er011_output/news_discovery_comparison_artifact_l0_01/discovery_comparison.html]に
倣ったテーブル形式)。
...
TRIAL08_DIR = BASE_DIR / "er011_output" / "discovery_stage2_interpretation_rule_trial_08"
TRIAL07_DIR = BASE_DIR / "er011_output" / "discovery_layer3_focus_trial_07"
OUT_DIR = TRIAL08_DIR
...
def find_runs(condition: str, level_dir: str):
    cond_dir = TRIAL08_DIR / level_dir / condition
...
def find_trial07_baseline(level_dir: str):
    run_dir = TRIAL07_DIR / level_dir / "baseline" / "run1"
    analysis = read_json(run_dir / "analysis.json")
    return analysis
...
def fact_cell(analysis):
    ...
    return (f"<td><span class='{cls}'>{esc(verdict)}</span><br>"
            f"unsupported_claims={n}<br>word_count={wc}</td>")
...
def build_level_section(level: str):
    ...
    if orig_text:
        ...
        rows_html.append(
            ...
            f"<td>Production採用済み(参考、本Trial対象外)</td></tr>")

    # Trial-07 baseline(block=""、focus module未適用、参考)
    t07 = find_trial07_baseline(level_dir)
    if t07:
        rows_html.append(
            f"<tr><td><strong>Trial-07 baseline(参考、Ledger v3・block=\"\")</strong></td>"
            f"{point_cell(t07.get('point_one_text'), 'point_one_vs_point_two', t07)}"
            f"{point_cell(t07.get('point_two_text'), 'point_two_vs_point_one', t07)}"
            f"{fact_cell(t07)}</tr>")
...
<head>
<meta charset="UTF-8">
<title>FAMILY-A-DISCOVERY-STAGE2-INTERPRETATION-RULE-TRIAL-08 Comparison</title>
<style>
...
<body>
<h1>FAMILY-A-DISCOVERY-STAGE2-INTERPRETATION-RULE-TRIAL-08 — Point本文比較(D-2目視artifact)</h1>
<p>Output: <code>{OUT_DIR}</code></p>
<div class="note">
Role文字列のヒューリスティック分類は行っていない(D-2=(a)、廃止)。多様性はPoint本文そのもの
と cross_point_overlap(既存Point Overlap Countermeasure機構の出力、新規指標ではない)で
Fable/ユーザーが目視判定する。current_focus=現行Focus Module(見出しのみ改稿)、
adjusted_focus=Part A案1(既存断定回避段落末尾へscope-generalization禁止の1文のみ追加、
未承認候補)。全runでHousehold Verified Fact Ledger v4(FACT-03修正版)を使用。
</div>
{sections}
</body>
</html>
