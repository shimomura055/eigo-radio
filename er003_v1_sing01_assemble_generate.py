# ============================================================
# er003_v1_sing01_assemble_generate.py
# ER-003-B1-NOVEL-AUDIO-01: SING01 Full Audio組み立て(AUDIO-03 Shell baseline)
# ============================================================
# ER-003-B1-SCAFFOLD-AUDIO-03で確立したAudio Shell(Intro/Outro/
# Notification/pause/Key Phrase presentation/post-processing、Comment=
# Charon・News/Preview=Aoede)を無変更で継承し、SING01の新規コンテンツを
# 組み込んでIntro〜OutroのFull Audioを組み立てる。新しいpause秒数・
# gain方式は設計しない。Point explanation("Here's the point.")は
# AUDIO-03で生成済みの音声をそのまま再利用する(テキスト無変更のため)。
#
# 実行方法:
#   .venv/Scripts/python.exe er003_v1_sing01_assemble_generate.py
...
import er003_v1_sing01_audio_generate as sing01
...
POINT_EXPLANATION_PAUSE_SECONDS = audio03.POINT_EXPLANATION_PAUSE_SECONDS
...
POINT1_TO_POINT2_PAUSE_SECONDS = audio03.POINT1_TO_POINT2_PAUSE_SECONDS
IN_ONE_LINE_TO_OUTRO_PAUSE_SECONDS = audio03.IN_ONE_LINE_TO_OUTRO_PAUSE_SECONDS
COMMENT_EN_TO_JA_PAUSE_SECONDS = audio03.COMMENT_EN_TO_JA_PAUSE_SECONDS
COMMENT_JA_TO_EN_PAUSE_SECONDS = audio03.COMMENT_JA_TO_EN_PAUSE_SECONDS

# AUDIO-03で確定済みのPoint explanation音声をそのまま再利用(テキスト
# "Here's the point."は無変更のため新規生成しない)
POINT_EXPLANATION_AUDIO_PATH = "er003_output/b1_scaffold_audio_03/A02/narration/point_explanation_en.wav"
...
def load_all_sources() -> dict:
    ...
    for item in kp_items:
        ...
        mono, sr, _, _ = common.read_wav_float(f"{NARRATION_DIR}/kp{rank}_ja.wav")
...
def apply_gain_and_convert(sources: dict) -> dict:
    """AUDIO-03(=A2最終承認script)と同一方式。target_rms = Preview と
    Full Story Part1のRMS平均。Previewは無加工のアンカー。Outroはpost-gain
    IntroへマッチのうえA2と同じ二段減衰。"""
...
def build_pieces_with_timeline(parts: dict) -> list:
    """AUDIO-03のShell構成(Japanese title除去済み、Point explanation英語版、
    Comment=Charon)をそのまま踏襲する。新しい並び・pause値は作らない。"""
...
def main():
    ...
    print("[SING01-ASSEMBLE] 音源読み込み開始...")
    sources = load_all_sources()
    print("[SING01-ASSEMBLE] gain調整開始...")
    parts = apply_gain_and_convert(sources)
    print("[SING01-ASSEMBLE] assemble開始...")
    result = assemble_with_timeline(parts)
    assembled = result["assembled"]

    out_path = f"{OUT_DIR}/assembled/English_Your_Way_B1_SING01.wav"
    ...
    print("[SING01-ASSEMBLE] 完了。summary:", json.dumps(summary, ensure_ascii=False, default=str))
