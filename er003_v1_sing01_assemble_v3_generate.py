# ============================================================
# er003_v1_sing01_assemble_v3_generate.py
# ER-003-B1-NOVEL-AUDIO-01-VOICE-02: Point explanation削除+Point One/Two=Aoede
# ============================================================
# ユーザー指示2件:
#   1. "First point."/"Second point."とそれに続くPoint One/Two本文は
#      記事本体(Full Story/Preview等と同種のListeningコンテンツ)なので
#      Aoedeへ変更する。
#   2. "Point explanation"("Here's the point.")は、元のA2仕様では
#      Preview(英語)に対する日本語訳・解説として機能していたセグメント
#      であり、これをそのまま英語化すると"Here's a quick preview."と
#      内容が重複する。B1の仕様としては不要なため、削除する。
#
# 上記2件以外(Welcome/Topic intro/Preview intro/Key phrases intro/
# Full story intro/番号ラベル/Key Phrase日本語meaning = Charon、
# Preview/Key Phrase英語Component/Full Story Part1・2 = Aoede、
# Comment1-4/In One Line = Charon)はVOICE-01から無変更。
#
# 実行方法:
#   .venv/Scripts/python.exe er003_v1_sing01_assemble_v3_generate.py
...
def load_all_sources() -> dict:
    ...
    # In One LineはNavigator役としてCharonのまま(前回VOICE-01から変更なし)
...
def main():
    ...
    print("[SING01-VOICE02-ASSEMBLE] 音源読み込み開始...")
    sources = load_all_sources()
    print("[SING01-VOICE02-ASSEMBLE] gain調整開始...")
    parts = apply_gain_and_convert(sources)
    print("[SING01-VOICE02-ASSEMBLE] assemble開始...")
    result = assemble_with_timeline(parts)
    assembled = result["assembled"]

    out_path = f"{OUT_DIR}/assembled/English_Your_Way_B1_SING01_v3.wav"
    common.write_wav_float(out_path, assembled, SR, 2)
    metrics = common.measure_metrics(assembled[:, 0], SR)

    with open(f"{OUT_DIR}/audit/gain_report_v3.json", "w", encoding="utf-8") as f:
        json.dump(parts["gain_report"], f, ensure_ascii=False, indent=2)
    with open(f"{OUT_DIR}/audit/timeline_v3.json", "w", encoding="utf-8") as f:
    ...
    with open(f"{OUT_DIR}/run_summary_assemble_v3.json", "w", encoding="utf-8") as f:
        json.dump(summary, f, ensure_ascii=False, indent=2, default=str)
    print("[SING01-VOICE02-ASSEMBLE] 完了。summary:", json.dumps(summary, ensure_ascii=False, default=str))
