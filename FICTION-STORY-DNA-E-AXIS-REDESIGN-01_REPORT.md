...
## §10 Fable評価(ブラインド: blind.mdを先に読み、key開封後も判断不変)

「(1)P=Run5(未来の駅、亡き娘の記憶を見知らぬ人に借りられる)/Q=Run4(2068年、
記憶で支払う会社、事故の記憶を差し出すと『今夜の日付』)/R=Run3(退職する
同僚が友情の夜の記憶を託し、その中に不正の証拠)/S=Run2(学校のテスト、
Room 17の少女、Minaが皆に偽の記憶を植えた)/T=Run1(認知症の男の『娘役』を
演じる派遣、ロッカー47の真鍮の鍵)。発想タイプは5種で重複なし(旧Bias除去
Trialは4種)。(2)目的A(E軸分散): E値はRun1=現実的だが社会的に珍しい、
Run3=ルール1つ違う現実、Run4/5=future。Storyの世界観はDNAと一致し、E軸
なしRun2は技術/未来を補完せず超自然寄りの学校怪談になった。現実系の人間
ドラマ(Run1)が自然に出た。(3)目的B(殺人/死): 殺人0/5(旧3/5)、死は背景
として1〜2本(Run5の亡き娘、Run1の失われた娘の示唆)、暴力はRun4の事故の
み。E軸値の変更だけで殺人が消えたことは『旧E軸(near future/技術・制度)
×テーマ記憶』が暴力へ引っ張っていたことと整合するが、1サンプルであり
因果は断定しない(Run4候補1にkilling案が1件生成され選ばれなかった事実も
併記)。(4)残る傾向: ①E=futureのRun4は再び『今夜の日付の記憶=未来の記憶』
構造(暴力なしだが同型の仕掛け)。②時刻表記が5本全部に出現(6:03/9:59/
5:47/4:05・3:50/3:18・3:55)—Prompt由来ではない強い生成癖。③人物名反復
(Mara×2、Mina×2)。④『友情の記憶を手放す』(Run3)は前回Run3(幽霊市場)と
同モチーフ。(5)Run5はrandom.choice母集団長の変更でC/D値も変わった(旧
C=逆転/D=病院→新C=喪失/D=公共空間)ため、Run5だけは同一DNA比較になって
いない(実装上の副作用、要記録)。(6)Coverage Run暫定判断(a: future 1値
固定)は多様性を失う副作用あり。費用¥4.69。」

## §11 分類

「**VALIDATED**(新E軸は偏り改善に有効。Fiction familyの採用・仕様化では
ない。Production/SSOT変更なし)。ユーザー判断事項: ①Run5 Coverage Runの
母集団: (a) `future` 1値固定を維持 / (b) 非現実系3値(future / one
changed rule / supernatural)から抽選 / (c) Coverage Run自体を廃止し5 Run
とも通常抽選(Fable推奨: **(b)**。理由: Coverageの目的『非現実側を最低1本
確保』を保ちつつfuture固定の多様性喪失を避ける)。②殺人/死の切り分けを
追加観測するか(同一設定でseedを変えた5 Run、¥5程度。Fable推奨: 実施は
①決定後、1回のみ)。③時刻表記・人物名反復への対処(前回提案(d)反復禁止
リスト+題名重複チェック)を次Trialにするか(Fable推奨: ②と同時に観測の
み、制約追加は保留)。」
