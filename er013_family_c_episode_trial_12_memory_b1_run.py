# ============================================================
# er013_family_c_episode_trial_12_memory_b1_run.py
# 管理ID: FAMILY-C-SEGMENT-COMMENT-TRIAL-12(委任1: Memory B1)
# ============================================================
# 目的: Memory A2 Trial-11でユーザーが試聴し「問題なし」と評価したStory
# segmentation方式(できるだけ自然な連続発話としてまとめ、Voice変化/
# Comment挿入位置/scene・semantic boundary/TTS安定性上の理由がある場所
# だけ分割する)を、Memory B1へ展開する。
#
# 本ファイルは`er013_family_c_episode_trial_10_memory_b1_run.py`の複製・
# 改変であり(旧ファイルは無編集のまま保持)、下記の点のみ変更する。
#   1. Story segmentation: Trial-10 B1(段落ごとの機械的split、"No,"の
#      ような短い同一Voice台詞も独立segment化)を廃し、Voice変化点
#      (device2箇所/brother1箇所)とscene boundary(自然な時間・場面の
#      切れ目)だけで区切る新設計(build_story_segments_trial12)を使う。
#   2. Brother Voice: Erinome→Algieba(Memory A2 Trial-11と統一)。
#      Trial-11の`tts_brother`と同一関数経路(bvoices.
#      generate_voice_body_wide_margin、既存Production関数)を使うため、
#      a2m参照先をTrial-10版からTrial-11版(BROTHER_VOICE_NAME=Algieba)
#      へ切り替える。
#   3. 話者判定(classify_quote_voice): `er013_family_c_episode_trial_10_
#      twins_b1_run.py`のOPEN-156修正(直近の閉じ引用符より後ろだけを
#      before windowにする)を本記事へも移植する(個別修正、汎用機構化
#      しない)。本記事では該当する複数引用符段落[p8, p20]を確認した
#      結果、この修正による割当変化は無い(RESULT_PACKETに記録)。
#   4. B1 Comment/Preview/Key Phrase: 内容・Promptとも変更しない
#      (ユーザーから品質問題の指摘なし)。Trial-10の既存音声assetを
#      そのままコピーしてreuseし、再TTS・再ASRは行わない(予算¥40は
#      新Story segmentのTTS+ASRのみに使う)。
#   5. 本文: Trial-10で独立生成済みのreader_facing_article_b1.txtを
#      正本候補として固定・コピーする。Writer再実行なし。
#
# 実行方法:
...
import er013_family_c_episode_trial_11_memory_run as a2m  # 再利用のみ、編集禁止
                                                            # (Algieba/tts_narrator等)
...
# パス定数
# ============================================================
PREV_OUT_DIR = "er013_output/family_c_episode_trial_10/memory_b1"  # 旧版(参照・reuse元、無編集)
OUT_DIR = "er013_output/family_c_episode_trial_12/memory_b1"
...
ARTICLE_ID = "family_c_memory_trial_12_b1"
LEVEL = "FAMILY_C_TRIAL_12_MEMORY_B1"  # 新規level文字列、既存Gateの辞書は無変更。

ARTICLE_B1_PATH = f"{PREV_OUT_DIR}/reader_facing_article_b1.txt"  # 正本候補(Trial-10固定)
EXPECTED_ARTICLE_SHA256 = "89c4259a798b05f7b1db697f2b5894ce424f6971e84bbf6b4a93f77095b87f20"

TOPIC_TITLE = a2m.TOPIC_TITLE  # "The Future of Memory"(A2/B1共通文言)
TOPIC_INTRO_EN_TEXT = a2m.TOPIC_INTRO_EN_TEXT

BROTHER_VOICE_NAME = a2m.BROTHER_VOICE_NAME  # "Algieba"(Trial-11と統一)
...
#   26段落(0-indexed)。device(記憶保管装置)の直接発話2箇所[p3,p7]を
#   Charon、brother(兄)の直接発話1箇所[p16]をAlgiebaへ分離する。
#   Lena本人の台詞(p5, p8x2, p20x2)はnarrator(Aoede)のまま分離しない
#   (Trial-10 B1のambiguous_quotes.jsonと同一5件、既存方針を踏襲)。
#
#   ユーザー指示(2026-09-16)の安全ガイド(概ね100語以内・120語を大きく
#   超えない)を満たすため、Voice変化点に加えて下記3箇所にscene/semantic
#   boundaryとしてforce splitを入れる(理由は各SEGMENT_SPLIT_REASON参照):
#     - p10/p11境界: 「安堵→自由に感じ始める」という直後の反応から、
#       「その後の五年間の生活」の時間圧縮描写への切り替わり。
#     - p12/p13境界: "Then, five years later, on a cold morning" という
#       明確な時間跳躍(scene transition)。
#     - p20/p21境界: 兄・若い頃の自分の言葉の場面から、装置が新しい
#       日付を提示する次の決断の場面への切り替わり。
#   これらを入れないと、p7trailing〜p20とp16trailing〜p25がそれぞれ
#   183語/159語となり、Trial安全ガイド(120語を大きく超えない)を大きく
#   超えてしまう。
# ============================================================
QUOTE_RE = re.compile("“[^”]*”")

SCENE_BOUNDARY_FORCE_SPLIT_BEFORE_PARAGRAPH = {11, 13, 21}
...
def classify_quote_voice(paragraph_text: str, start: int, end: int) -> str:
    ...
    # 使う。本記事の複数引用符段落(p8, p20)を確認した結果、この修正による
    # 割当変化は無い(いずれも前後80文字以内にdevice/brotherキーワードが
    # 存在しないため、修正前後でnarrator判定のまま)。
    before_raw = paragraph_text[max(0, start - 80):start]
    last_close = before_raw.rfind("”")
    if last_close != -1:
...
def split_paragraph_by_quotes(paragraph_text: str) -> list:
    """段落内の"..."を境に(voice, raw_text, is_quote)へ分割する。
    raw_textを順に連結するとparagraph_textへ完全一致で復元できる。"""
    chunks = []
    pos = 0
    for m in QUOTE_RE.finditer(paragraph_text):
        if m.start() > pos:
            chunks.append(("narrator", paragraph_text[pos:m.start()], False))
        voice = classify_quote_voice(paragraph_text, m.start(), m.end())
        chunks.append((voice, m.group(0), True))
        pos = m.end()
    if pos < len(paragraph_text):
        chunks.append(("narrator", paragraph_text[pos:], False))
    return chunks


SEGMENT_SPLIT_REASON = {
    "story_001": "段落0-2+段落3リード文(引用符直前)を統合。直後device変化点(story_002)で分割。",
    "story_002": "Voice変化点(device、'Return date?')、分割不可避。",
    "story_003": "段落3トレイル文+段落4-6を統合(同一narrator連続)。直後device変化点で分割。",
    "story_004": "Voice変化点(device、'Are you sure?')、分割不可避。",
    "story_005": "段落7トレイル文+段落8(Lena自身の台詞2箇所含む、同一narrator)+段落9-10を統合。"
                 "「安堵→自由」という反応の完結点で分割(scene boundary)。",
    "story_006": "段落11-12(五年間の生活の時間圧縮描写)を統合。'Then, five years later'の明確な"
                 "時間跳躍(scene transition)直前で分割。",
    "story_007": "段落13-15(時間跳躍後の場面転換+brother引用直前のリード文)を統合。"
                 "直後brother変化点で分割。",
    "story_008": "Voice変化点(brother、'Do not make my last day your whole life,')、分割不可避。",
    "story_009": "段落16トレイル文+段落17-20(Lena自身の台詞2箇所含む、同一narrator)を統合。"
                 "兄・若い自分の言葉の場面が終わり、装置が新しい日付を提示する次の決断の場面へ"
                 "切り替わる直前で分割(scene boundary)。",
    "story_010": "段落21-25(結末部、決断から結末まで)を統合。Trial-10と同一の結末区切り。",
}


def build_story_segments_trial12(paragraphs: list) -> tuple:
    flat = []  # (p_idx, voice, raw_text, is_quote)
    for p_idx, para in enumerate(paragraphs):
        if para.strip() == "":
            continue
        for voice, raw, is_quote in split_paragraph_by_quotes(para):
            flat.append((p_idx, voice, raw, is_quote))
    ...
    def flush() -> None:
        if not current_chunks:
            return
        # 段落境界(元の"\n\n")を跨ぐchunk同士は単純結合すると空白が失われ
        # run-on文になる(例: "robot.Lena")。TTS用raw_textでは段落境界に
        # 半角スペース1個を補う(normalize_for_ttsが余分な空白は圧縮する)。
        # paragraph_contributions(reconstruct_article_from_story_segments用、
        # 段落ごとに再結合してから"\n\n"で連結する)には影響しない。
        parts = []
        prev_p = None
        ...
        segments.append({
            ...
            "kind": "split" if current_voice in ("device", "brother") else "merge",
...
# ============================================================
# Comment placement(累積語数35%/65%、直近merge境界へスナップ、Trial-10と
# 同一アルゴリズム。新segment境界に対して再計算する)。
# ============================================================
...
# ============================================================
# 既存asset reuse(Trial-10 memory_b1から、再TTS・再ASRなし)
# ============================================================
def reuse_copy_wav(rel_audio_name: str) -> str:
    """Trial-10のaudio/配下のwav(+.ok)を新OUT_DIRへコピーする(バイト同一、
    再TTSしない)。戻り値は新パス。"""
...
def reused_audit_entry(prev_entry: dict, new_path: str) -> dict:
    """Trial-10のtts_generation_results.json内のsegmentエントリをそのまま
    引き継ぐ(内容・sha256は同一バイトのため不変、pathのみ新OUT_DIRへ更新)。
    sha256が記録されている場合はそのままでよい(バイト同一コピーのため
    実ファイルhashと一致する)。"""
...
def main() -> None:
    ...
    src_sha = a2m.sha256_text_file(ARTICLE_B1_PATH)
    ...
    assert len(paragraphs) == 26, f"unexpected paragraph count: {len(paragraphs)}"

    story_segments, ambiguous_quotes = build_story_segments_trial12(paragraphs)
    ...
    budget = a2m.BudgetTracker(args.budget_jpy, 30.0, f"{OUT_DIR}/raw_usage_log.jsonl")

    audit_segments: dict = {}
    gain_report: dict = {}

    # --- Story segment TTS(新規10件のみ、Support/KP/nav等は再TTSしない) ---
    ...
    for seg in story_segments:
        if seg["kind"] == "split" and seg["voice"] in ("device", "brother"):
            speaker_entries.append({
                ...
                "attribution_basis": "引用符前後80文字以内の'screen'/'device'/'robot'/'storage unit'/"
                                      "'brother'キーワード決定的検出(OPEN-156修正[直近の閉じ引用符より"
                                      "後ろだけをbefore windowにする]を移植、本記事では割当変化なし)。",
            })
    save_json(f"{OUT_DIR}/speaker_map.json", {
        "device_voice": "Charon", "brother_voice": BROTHER_VOICE_NAME,
        "brother_voice_change_note": "Trial-10ではErinome。Memory A2 Trial-11で採用したAlgiebaへ統一"
                                      "(新Voice比較Trialなし、既存承認Voice候補内)。",
        "lena_split_decision": "Lena本人の台詞はnarrator(Aoede)のまま分離しない(Trial-10と同一方針)。",
    ...
    comment_placement = [
        {"comment": 1, "position_description": "導入部(Preview後・Story前、Full story introの直後、"
                                                 "固定配置)",
         "selection_reason": "Comment 1は物語の理解補助として常にStory先頭固定(Trial-10と同一方針)。",
        ...
        {"comment": 2,
         ...
         "selection_reason": "累積語数が記事全体の約35%へ到達した直後の、直近の地の文segment"
                              ...
                              f"旧位置cumulative_word_fraction={prev_c2.get('cumulative_word_fraction_at_boundary')}"
                              "から移動している場合は新segment境界の統合による)。",
        ...
        {"comment": 3,
         ...
         "selection_reason": "累積語数が記事全体の約65%へ到達した直後の、直近の地の文segment"
                              "(merge種別)へスナップして自動選定(新segmentation後の再計算、"
                              f"旧位置cumulative_word_fraction={prev_c3.get('cumulative_word_fraction_at_boundary')}"
                              "と比較)。",
    ...
    final_path = f"{ASSEMBLED_DIR}/family_c_memory_trial_12_b1.wav"
    ...
    # tts_generation_results.jsonへASR結果を反映(Gate再実行時にも整合させる)。
    save_json(f"{AUDIT_DIR}/tts_generation_results.json",
              {"segments": audit_segments, "key_phrases": audit_key_phrases})
    ...
    cost_summary = {
        "budget_cap_jpy": args.budget_jpy, "warn_threshold_jpy": 30.0,
        "total_estimate_jpy": round(total_jpy, 2),
        "note": "raw_usage_log.jsonlからの累計。新Story segment(10件)のTTS+ASRのみが新規費用。"
                "Comment/Preview/Key Phrase/nav/topic_introはTrial-10からのbyte-identicalコピー"
                "reuseのため再TTS・再ASRなし(既存comment_consistency.json/"
                "key_phrase_consistency.json/player_display_audio_consistency.json[非story行]を"
                "そのまま引き継いだ)。",
...
def convert_all_to_mp3_b1() -> dict:
    ...
    results = []
    ep_wav = f"{ASSEMBLED_DIR}/family_c_memory_trial_12_b1.wav"
    ep_mp3 = f"{WEB_DIR}/family_c_memory_trial_12_b1.mp3"
...
def build_player_html_b1(seq_labels, segments, kp_items, comment_texts, run_summary, preview_text) -> None:
    ...
    episode_url = "./web/family_c_memory_trial_12_b1.mp3"
    html = f"""<!doctype html><html><head><meta charset="utf-8">
<title>Family C Trial-12: The Future of Memory (B1)</title>
<style>{player_mod.PLAYER_STANDARD_CSS}</style>
<script>{player_mod.SEEK_SCRIPT}</script>
</head><body>
<h1>Family C Future — The Future of Memory (Trial-12 B1 / VALIDATED候補・ユーザー試聴待ち、level=B1)</h1>
