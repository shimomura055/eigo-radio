# ============================================================
# er003_v1_b1_p4b_generate.py
# ER-003-B1-P4B: 短文分割＋単一「合図」マーカーでListening Preview完成版
# ============================================================
# P4Aで、Preview全文を1回のTTS callで生成する方式では、マーカー以外の
# 日本語(「最後の数分」)が崩れて聞こえる問題が生じた。本ステージでは
# Preview全文の一括生成をやめ、6つの短いchunk(原則1文)へ分割し、
# chunkごとに個別TTS→内容確認→(marker chunkのみ)MFA→英語Key Phrase
# 置換を行う。B1本文・通し音声の生成はスコープ外。
#
# 実行方法:
#   .venv/Scripts/python.exe er003_v1_b1_p4b_generate.py
...
OUT_DIR = "er003_output/b1_p4b/A01"
...
# ============================================================
# Step 1: source保存 + chunk分割 + 静的検証(TTS呼び出し前ゲート)
# ============================================================
def _step_build_and_verify_plan() -> dict:
    ...
    chunk_plan = p4b.build_chunk_plan(pattern_a_text, used_forms)
    static_check = p4b.verify_chunk_plan_static(chunk_plan, pattern_a_text)
...
# ============================================================
# Step 2: chunkごとの内容確認(診断用ASR、境界決定には未使用)
# ============================================================
def check_chunk_content(recognized_text_ja: str, chunk: dict) -> dict:
    """chunkの種類に応じて、「合図」の出現数(marker chunkは1、normalは0)
    ・日本語比率・(該当chunkのみ)「最後の数分」フレーズの残存を確認する。
    p4.check_ja_contentは5種の異なるkatakanaマーカーを前提にしており
    そのままでは使えないため、単一の汎用マーカー「合図」向けに同じ
    設計思想(句読点除去後に照合、無音長は使わない)で新規実装する。"""
    ...
    marker_count_in_recognized = stripped.count(p4b.MARKER_TOKEN)
    if chunk["chunk_type"] == "marker":
        marker_ok = marker_count_in_recognized == 1
    else:
        marker_ok = marker_count_in_recognized == 0

    ascii_letter_count = len(p4b._ASCII_LETTER_PATTERN.findall(recognized_text_ja))
    ...
    if chunk["contains_last_few_minutes"]:
        last_few_minutes_check = {
            "expected_phrase": p4b._LAST_FEW_MINUTES_PHRASE,
            "present_in_recognized": p4b._LAST_FEW_MINUTES_PHRASE in stripped,
        }

    passed = (
        is_japanese and marker_ok and no_unintended_english
    ...
    return {
        ...
        "marker_count_in_recognized": marker_count_in_recognized,
        "marker_ok": marker_ok,
...
def _generate_one_chunk_attempt(chunk: dict, tts_call_fn, sleep_function) -> dict:
    prompt = p4b.build_tts_prompt(chunk["tts_text"], p4b.JAPANESE_STYLE_PREFIX)
    pcm, retries, ok, err = common._call_tts_with_retry(
        tts_call_fn, prompt, max_retry=p4b.MAX_TTS_TECHNICAL_RETRY, sleep_fn=sleep_function)
    if not ok:
    ...
    content_check = check_chunk_content(recognized, chunk)
...
def generate_chunk_with_content_gate(chunk: dict, tts_call_fn, sleep_function) -> dict:
    ...
    技術的には成功したが内容確認(content_check)に失敗した場合は、同一
    payloadでこの関数のレベルでもう1回だけ生成をやり直す。2回連続で
    内容確認に失敗、または技術的失敗した場合は停止する。"""
    attempts = []
...
def _step_generate_all_chunks(chunk_plan: list, tts_call_fn, sleep_function) -> dict:
    ...
    for chunk in chunk_plan:
        ...
        if r["status"] != "OK":
            return {"status": "STOPPED", "phase": "chunk_generation", "chunk_id": chunk["chunk_id"], "results": results}
    return {"status": "OK", "results": results}


# ============================================================
# Step 3: marker chunkごとのMFA alignment(「合図」の区間特定)
# ============================================================
def _step_mfa_align_chunk(chunk: dict, ja_wav_path: str) -> dict:
    ...
    marker_specs = [{"marker_id": f"chunk{chunk['chunk_id']}_合図", "token_sequence": p4b.MARKER_TOKEN_SEQUENCE}]
    spans, errors = p4.find_all_marker_spans(words, marker_specs)

    if errors or spans[0] is None:
        return {"status": "STOPPED", "reason": f"chunk{chunk['chunk_id']}: 「合図」区間を特定できませんでした: {errors}", "textgrid_path": dest_textgrid}

    return {"status": "OK", "textgrid_path": dest_textgrid, "span": spans[0], "align_result": align_result}
...
def _step_mfa_align_all_marker_chunks(chunk_plan: list, chunk_results: dict) -> dict:
    ...
    for chunk in chunk_plan:
        ...
        results[chunk["chunk_id"]] = r
        if r["status"] != "OK":
            return {"status": "STOPPED", "phase": "mfa_alignment", "chunk_id": chunk["chunk_id"], "results": results}
    return {"status": "OK", "results": results}
...
def _step_generate_key_phrases(chunk_plan: list, tts_call_fn, sleep_function) -> dict:
    ...
    for chunk in chunk_plan:
        ...
        if used_form == "shot on target":
            if not os.path.exists(p4b.EXISTING_SHOT_ON_TARGET_PATH):
                return {"status": "STOPPED", "reason": f"既存音声が見つかりません: {p4b.EXISTING_SHOT_ON_TARGET_PATH}"}
            shutil.copyfile(p4b.EXISTING_SHOT_ON_TARGET_PATH, out_path)
            samples, sr, ch, _ = common.read_wav_float(out_path)
            results[chunk["chunk_id"]] = {
                "used_form": used_form, "path": out_path, "reused": True,
                "source_path": p4b.EXISTING_SHOT_ON_TARGET_PATH, "sha256": _sha256_file(out_path),
                "call_count": 0, "retry_count": 0, "samples": samples, "sample_rate": sr,
                "duration_seconds": round(len(samples) / sr, 4),
            }
            continue

        prompt = p4b.build_tts_prompt(used_form, p4b.ENGLISH_STYLE_PREFIX)
        pcm, retries, ok, err = common._call_tts_with_retry(
            tts_call_fn, prompt, max_retry=p4b.MAX_TTS_TECHNICAL_RETRY, sleep_fn=sleep_function)
        if not ok:
...
# ============================================================
# Step 5: marker chunkごとの置換(合図区間→英語Key Phrase)
# ============================================================
def _replace_marker_in_chunk(chunk: dict, ja_samples: "np.ndarray", sr: int, span: dict, en_samples: "np.ndarray") -> dict:
    ...
    needed_trailing = round(p4b.GAP_BEFORE_TARGET_SECONDS - p4b.EN_TRIM_SAFETY_MARGIN_SECONDS, 4)
    needed_leading = round(p4b.GAP_AFTER_TARGET_SECONDS - p4b.EN_TRIM_SAFETY_MARGIN_SECONDS, 4)
    ...
    return {
        ...
        "within_tolerance_before": abs(gap_before - p4b.GAP_BEFORE_TARGET_SECONDS) <= p4b.GAP_TOLERANCE_SECONDS,
        "within_tolerance_after": abs(gap_after - p4b.GAP_AFTER_TARGET_SECONDS) <= p4b.GAP_TOLERANCE_SECONDS,
    }
...
def run(tts_call_fn=None, sleep_function=sleep_fn) -> dict:
    _mkdirs()
    call_fn = tts_call_fn or gclient.make_tts_call_fn(p4b.VOICE_NAME)
