...
# ============================================================
# 目的(ユーザー決定 2026-09-12): Discovery Focus Module(Discovery/Why型
# 記事のためのPrompt拡張モジュール。Part A本体のみ。保険文抑制Prompt制約案
# [Part B案1、`cautionary_constrained`]は使わない=Part A単独条件)の一般化
# 可能性を、Trial-11(タオル臭)とは異なる新テーマ「なぜ目覚ましが鳴る直前に
# 目が覚めることがあるのか?(Why do we sometimes wake up just before the
# alarm?)」でN+1(2件目)確認する。**テーマはユーザー確定、変更禁止**。
# A2/B1B各1本、記事生成→既存QA一式まで。Support/Audioは別script
# (audio_run)で実施する。**Trial(Production実装ではない)。Production採用
# はしない**(Gate 1=VALIDATED相当までが上限)。
#
# ------------------------------------------------------------
# Reconciliation Check(PM_GOVERNANCE 2-1、実施記録):
# ------------------------------------------------------------
# - er011_discovery_generalization_towels_trial_11_run.py(Trial-11、直近の
#   同一設計N=1検証)を確認し、同一経路(prod_gen.build_common_block/
#   build_prompt/run_one_pattern、既存Production関数を無変更で直接呼ぶ)を
#   踏襲した。Trial-11固有のVerified Fact Ledgerはタオル臭専用であり本テーマ
#   には使えないため、新規Ledgerを作成する(News Trial-09/Trial-11と同一
#   手順、下記)。
# - Discovery Focus Module Part A本体(`t9.CURRENT_FOCUS_BLOCK`)はTrial-11と
#   同一のものをread-onlyでそのまま再利用する(一字一句不変)。Part B文言
#   (cautionary_clause)は一切追加しない。
# - Verified Fact Ledger作成手順はTrial-11のNews Trial-09踏襲手順
#   (vfl01.build_researcher_prompt(topic=)/build_verification_prompt
#   (topic, ledger_parsed))をそのまま踏襲する。
# - 重複・競合・実装漏れは確認されなかった(新規テーマのTrialとして着手、
#   OPEN-121/OPEN-145/OPEN-146は記事生成段階では不発火想定。発火可能性が
#   あるのは音声段階[audio_run側]のASR検証・Repetition QAのみ)。
#
# 再利用(import・無変更、read-onlyでの参照のみ): Trial-11と同一
#   - er011_discovery_stage3_rule_adjustment_trial_09(t9): CURRENT_FOCUS_
#     BLOCK / analyze_run / load_text。
...
#
# 費用上限: 本script(ledger+article)単独 ¥170(見込み¥100〜130、Trial-11
# text-gen実測¥117.72を参考)。**ただし本管理ID全体[記事+Support+Audio]の
# 合計上限は¥300](ユーザー決定)であり、audio_run側の費用と合算して
# ¥300を超過する見込みの場合は実行前にSTOPする(combined_cost_check())。
# 同期実行のみ。バックグラウンド待機・二重起動禁止。
#
# 実行方法(root直下から):
#   .venv/Scripts/python.exe er011_discovery_generalization_wake_before_alarm_trial_12_run.py [stage ...]
...
BUDGET_JPY_CAP = 170.0
COMBINED_BUDGET_JPY_CAP = 300.0
AUDIO_COST_LOG_PATH = f"{OUT_DIR}/audit/raw_usage_log_audio_01.jsonl"  # audio_run側と同一パス(参照のみ)

# Part A本体のみ(t9.CURRENT_FOCUS_BLOCKを無変更でそのまま使う。Part B文言
# [cautionary_clause]は一切追加しない=Part A単独条件)。Trial-11と同一。
...
WAKE_LEDGER_PATH = f"{RESEARCH_DIR}/verified_fact_ledger.txt"
WAKE_STAGE_B3_VFL_PATH = f"{RESEARCH_DIR}/stage_b3_vfl.json"
...
RESEARCH_QUESTION_JA = (
    "\"Why do we sometimes wake up just before the alarm?\"(なぜ目覚ましが鳴る直前に目が覚める"
    "ことがあるのか?)というテーマについて調査してください。予期された起床時刻の直前に自然に目が"
    "覚める現象(サーカディアンリズム[概日リズム]による体内時計の学習・予期的なコルチゾール/ACTHの"
    "上昇[anticipatory cortisol awakening response]、睡眠段階[レム睡眠と浅い睡眠]のタイミング、"
    "自己覚醒[self-awakening]に関する実験研究、体内時計の光・習慣による同調[entrainment]、"
    "個人差や信頼性の限界等)について、睡眠医学・生理学・時間生物学(chronobiology)の査読論文等、"
    "信頼できる情報源に基づいて調査してください。"
)

# ------------------------------------------------------------
# 保険文検出regex(Trial-11と同一定義、read-onlyで転記して再利用。既存
# 検出器を独自判断で変更しない)。
# ------------------------------------------------------------
...
# ============================================================
# 費用実測(全provider対応、Trial-11 runと同一ロジック、read-onlyで転記して
# 再利用)。
# ============================================================
USD_JPY = 160.0
...
def compute_cost_jpy_so_far() -> dict:
    return _sum_log_jpy(f"{OUT_DIR}/raw_usage_log.jsonl")


def cost_stage() -> dict:
    result = compute_cost_jpy_so_far()
    save_json(f"{OUT_DIR}/cost_summary.json", result)
    print(f"[{THEME_ID}][cost] 実測合計(記事生成側)={result['total_jpy']} JPY (単独上限{BUDGET_JPY_CAP}) "
          f"by_provider={result['by_provider_jpy']} unpriced_records={result['unpriced_records']}")
    if result["total_jpy"] > BUDGET_JPY_CAP:
        raise RuntimeError(f"費用上限超過(実測{result['total_jpy']}円 > 単独上限{BUDGET_JPY_CAP}円)。STOP。")
    combined_cost_check()
    return result


def combined_cost_check() -> dict:
    """管理ID全体(記事生成+audio_run)の合算費用を¥300上限でチェックする
    (ユーザー決定の総額上限)。audio_run側logがまだ存在しない場合は0円として
    扱う。"""
    article_cost = _sum_log_jpy(f"{OUT_DIR}/raw_usage_log.jsonl")["total_jpy"]
    audio_cost = _sum_log_jpy(AUDIO_COST_LOG_PATH)["total_jpy"]
    total = round(article_cost + audio_cost, 2)
    print(f"[{THEME_ID}][combined_cost_check] 記事生成={article_cost}円 + audio={audio_cost}円 "
          f"= 合計{total}円 (管理ID全体上限{COMBINED_BUDGET_JPY_CAP}円)")
    if total > COMBINED_BUDGET_JPY_CAP:
        raise RuntimeError(f"管理ID全体の費用上限超過(実測合計{total}円 > 上限{COMBINED_BUDGET_JPY_CAP}円)。STOP。")
    return {"article_cost_jpy": article_cost, "audio_cost_jpy": audio_cost, "total_jpy": total}


# ============================================================
# Gate 4相当の静的確認(Production関数の再定義・monkeypatchをしていない
# ことの機械確認。Trial-11と同一ロジック)。
# ============================================================
def gate4_check_stage() -> dict:
    ...
    result = {
        ...
        "focus_module_source": ("er011_discovery_stage3_rule_adjustment_trial_09.CURRENT_FOCUS_BLOCK"
                                 "(read-only reuse, unmodified, Part A only, Trial-11と同一)"),
...
# ============================================================
# Step 0: 新テーマVerified Fact Ledger作成(Trial-11/News Trial-09の手順を
# 踏襲。vfl01.build_researcher_prompt(topic=)/build_verification_prompt
# (topic,...)というtopic引数を明示的に渡せる既存関数を使い、
# vfl01.run_researcher/run_verificationと同一のclient呼び出し構造をtopicだけ
# 差し替えて再現)。VERIFIED[CONFIRMED]のみへフィルタし、AMBIGUOUS/REJECTEDは
# 本Ledgerから除外する(全件CONFIRMED、未検証は載せない方針)。
# ============================================================
...
def ledger_stage() -> dict:
    ...
    with open(WAKE_LEDGER_PATH, "w", encoding="utf-8") as f:
        f.write(verified_ledger_text)
    save_json(f"{RESEARCH_DIR}/verified_fact_ledger_structured.json", {
        "topic_id": "wake_before_alarm", "research_question_ja": RESEARCH_QUESTION_JA,
        "kept_facts": kept_facts, "counts": counts,
        "note": "CONFIRMED(VERIFIED)のみ採用(News Trial-09/Trial-11と同一方針: 全件CONFIRMED、未検証は載せない)。"
    ...
    save_json(WAKE_STAGE_B3_VFL_PATH, {"parsed": research_result["parsed"]})
...
# ------------------------------------------------------------
# Writer用topic(WAKE_TOPIC_JA): このプレースホルダはledger_stage()実行後、
# 実際にCONFIRMED(VERIFIED)factを確認した上で、write_topic_from_ledgerで
# 上書きする(Trial-11のTOWELS_TOPIC_JAと同じ運用: Ledgerに存在しない新しい
# 主張・数字は追加しない)。
# ------------------------------------------------------------
WAKE_TOPIC_JA = None  # ledger確定後にset_wake_topic()で設定する


def set_wake_topic(topic_ja: str) -> None:
    global WAKE_TOPIC_JA
    WAKE_TOPIC_JA = topic_ja
    save_json(f"{RESEARCH_DIR}/writer_topic.json", {"topic_ja": topic_ja})


def load_wake_topic() -> str:
    global WAKE_TOPIC_JA
    if WAKE_TOPIC_JA:
        return WAKE_TOPIC_JA
    path = f"{RESEARCH_DIR}/writer_topic.json"
    if os.path.exists(path):
        WAKE_TOPIC_JA = load_json(path)["topic_ja"]
        return WAKE_TOPIC_JA
    raise RuntimeError("WAKE_TOPIC_JA未設定。ledger_stage()実行後にset_wake_topic()を呼ぶこと。")
...
def generate_article_stage(level: str) -> dict:
    ...
    verified_ledger_text = open(WAKE_LEDGER_PATH, encoding="utf-8").read()
    topic_ja = load_wake_topic()
    ...
    with cl.logging_context(THEME_ID, meta["stage_tag"]):
        result = prod_gen.run_one_pattern(
            client, THEME_ID, meta["label"], prompt, verified_ledger_text, topic_ja, out_dir)
...
# ============================================================
# Step 2: 評価(t9.analyze_runを無変更で流用。wake(本Trial)/towels
# (Trial-11)双方の既存artifactを同一関数で再分析し、比較可能な指標を得る。
# Trial-11側artifactは読み取りのみで一切変更しない)。
# ============================================================
TOWELS_TRIAL11_DIR = "er011_output/discovery_generalization_towels_trial_11"
...
def evaluate_stage() -> dict:
    ...
    for level in ("a2", "b1b"):
        ...
        wake_analysis[level] = analysis

    towels_analysis = {}
    for level in ("a2", "b1b"):
        out_dir = f"{TOWELS_TRIAL11_DIR}/{level}"
        run_summary_path = f"{out_dir}/run_summary.json"
        if not os.path.exists(run_summary_path):
            print(f"[{THEME_ID}][evaluate] towels {level}: run_summary.jsonが見つかりません(スキップ)。")
            continue
        result = load_json(run_summary_path)
        analysis = t9.analyze_run(out_dir, result)
        towels_analysis[level] = analysis

    comparison = {"wake_before_alarm_trial_12": wake_analysis, "towels_trial_11": towels_analysis}
    save_json(f"{OUT_DIR}/comparison_vs_towels_trial_11.json", comparison)
    print(f"[{THEME_ID}] evaluate_stage完了。wake={list(wake_analysis.keys())} "
          f"towels={list(towels_analysis.keys())}")
    return comparison
...
def main() -> dict:
    ...
    if stages == ["all"]:
        raise RuntimeError("本scriptは'all'一括実行をサポートしない(ledger後にtopic確定の人手介在ステップが"
                            "必要なため)。ledger -> [set_wake_topicを別途実行] -> gate4 -> a2 -> b1b -> evaluate "
                            "の順で個別に実行すること。")
