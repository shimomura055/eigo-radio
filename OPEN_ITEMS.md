...
| ID | 内容 | 状態 | 種類 | Blocking | 次Action |
|---|---|---|---|---|---|
| OPEN-31 | A2英文のnaturalness。3記事監査(ER-003-CROSSLEVEL-AUDIO-03、2026-08-10)でSHOULD_REVISE**5件**・OPTIONAL6件を検出、いずれも**未判断・台本へ未反映**。内訳(記事別): **A01**(3件)— (1)"Rogers sent the ball across the front of goal."→"Rogers crossed the ball into the box."案 (2)"Messi sent the ball across goal from the right."→"Messi crossed the ball from the right."案 (3)"The referee then added more time."→"The game went into added time."(この1件のみ[DECISION_LOG.md](DECISION_LOG.md)で**修正方針が確定**、ただし台本反映は未実施)。**A02**(1件)— "During that time, apps under the plan would not open at first."→"...would be switched off by default."案(意味誤読リスク: 「最初は開かない」と誤読されるおそれ、default概念は直後で説明済みのため難易度影響なし)。**ADD03**(1件)— Brent原油価格段落の時系列flashback構造(7/13→回想→7/14の順を、時系列順への並べ替え、または日付参照の明示化を提案)。Naturalness QAフロー自体は`DECIDED`(方針)だが、大規模自動実装はまだ | `UNDER_REVIEW` | 品質候補(A2固有) | Non-blocking(次回各記事の完成音声assemble前に確認要) | 詳細根拠は[ER-003-CROSSLEVEL-AUDIO-03_REPORT.md](ER-003-CROSSLEVEL-AUDIO-03_REPORT.md)11-12節。次回A01/A02/ADD03のassemble前に、上記5件(特にA01のadded time修正方針)の反映要否をユーザーが判断する |
