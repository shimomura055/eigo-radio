...
| OPEN-13 | ER-003-A2-STRUCT-01 Candidate A: Full Story分割+日本語コメント(Comment1〜4)による構造支援。11パート構成(Preview→Key Phrases→Comment1→Full Story Part1→Comment2→Full Story Part2→Comment3→Point One→Point Two→Comment4→In One Line)、A02で精緻化済み | `PROTOTYPE_BUILT / UNDER_EVALUATION`(ユーザー指定の正確な日本語文言・分割位置で確定版を作成) | 構造支援候補 | Non-blocking | ユーザーがA02プロトタイプ([ER-003-A2-STRUCT-02_A02_PROTOTYPE.md](ER-003-A2-STRUCT-02_A02_PROTOTYPE.md))を確認し、ADOPTED/REJECTEDを判断する。ADOPTEDの場合、音声プロトタイプ→他記事への一般化(役割定義のみ流用、文言は記事ごとに新規作成)の順で進める |
