...
**管理ID: ER-PM-001**
**最終更新: 2026-08-12(ER-003-A2-AUDIO-AB-01反映)**
...
| OPEN-32 | A2英語音声の読み上げ速度: 明示的なspeed parameter・post-processing time-stretchはB1/B2/A2いずれも未実装(既存監査ER-003-A2-00で確認済み)。**2026-08-12更新**: A02でA版(速度指定なし)/B版(約135 WPM目安)を実際に生成しA/B比較を用意した。校正の結果、**同一のprompt instructionでもsegmentごとに反応が大きく異なり、指示を強めても必ずしも遅くならない(非単調)ことが判明**。最終的に5segment中3つ(Full Story Part1/Part2/In One Line)は目標付近(132〜138)に収まったが、Point One(121)・Point Two(130)はやや遅めに振れた。5segment単純平均は134.41 WPMとほぼ目標一致 | `A/B READY FOR LISTENING` | 速度候補(A2固有、B1/B2は現状維持で確定) | Non-blocking | `ER-003-A2-SPEED`として管理。[ER-003-A2-AUDIO-AB-01_REPORT.md](ER-003-A2-AUDIO-AB-01_REPORT.md)・[比較Artifact](https://claude.ai/code/artifact/f105ed9c-98cd-4058-b58b-716ec9fa1600)でユーザーがA/B試聴し、135 WPMという目標値自体・B版の自然さを今後の方針として採用してよいか判断する。prompt指示の非単調性を踏まえ、正式採用する場合はsegmentごとの許容誤差を大きめに取るか、pitchを保持するtime-stretchとの併用を検討。`levels.py`(別の無関係なレガシー番組の設定、wpm105-115)を誤って参照しないよう引き続き注意 |
| OPEN-33 | Key Phrase統合発音方式(意味grounding+語末音素保持+phrase一体感)をA02の5件へ適用(opt out/covered apps/urge to watch/digital switch-off period は新規生成、personalized feedはCROSSLEVEL-AUDIO-01/02のユーザー承認版を維持し後退させていない)。urge to watchは初回ASRで"Urged to watch"と誤検出されたためtense保持instructionを追加し再生成、2回目で正確な内容を確認 | `CANDIDATE / TO_BE_LISTENED` | 音声品質候補(A2/B1/B2共通) | Non-blocking | A02のA/B比較音声内で試聴可能。ユーザーが自然さを確認し、A01/ADD03の対応するKey Phraseへの展開可否を判断 |
