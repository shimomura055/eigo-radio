...
| OPEN-13 | ER-003-A2-STRUCT-01 Candidate A: Full Story分割+日本語コメント(Comment1〜4)による構造支援。11パート構成(Preview→Key Phrases→Comment1→Full Story Part1→Comment2→Full Story Part2→Comment3→Point One→Point Two→Comment4→In One Line)。**A01・A02・ADD03の3記事とも統合台本まで完成**(A01/ADD03は記事固有の分割位置・日本語Commentを新規作成、A02の文言はコピーしていない) | `PROTOTYPE_BUILT / UNDER_EVALUATION`(3記事ともA2-03英語本文との無差分をプログラムで検証済み) | 構造支援候補 | Non-blocking | ユーザーが3記事の統合台本([ER-003-A2-STRUCT-03_A02_INTEGRATED_SCRIPT.md](ER-003-A2-STRUCT-03_A02_INTEGRATED_SCRIPT.md)、[ER-003-A2-STRUCT-04_A01_INTEGRATED_SCRIPT.md](ER-003-A2-STRUCT-04_A01_INTEGRATED_SCRIPT.md)、[ER-003-A2-STRUCT-04_ADD03_INTEGRATED_SCRIPT.md](ER-003-A2-STRUCT-04_ADD03_INTEGRATED_SCRIPT.md))を確認し、ADOPTED/REJECTEDを判断する。ADOPTEDの場合、音声プロトタイプ→自動一般化コードの実装(分割位置判定・Comment生成)の順で進める |
