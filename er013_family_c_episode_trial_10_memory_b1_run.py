# ============================================================
# er013_family_c_episode_trial_10_memory_b1_run.py
# 管理ID: USER-TEST-FINAL-AUDIO-BATCH-06(委任B: Family C「The future of
# memory」B1 episode)
# ============================================================
# 目的: Family C Trial-08で採用済みの記事本文(memory/reader_facing_
# article.txt、A2版のStory core=正本候補として固定)を参照しながら、
# 既存B1独立生成Writer契約(er013_family_c_future_writer_08_b1.py)で
# B1向け本文を新規生成し、Home RobotsでユーザーがFIX-04/05で確定させた
# B1構成(日本語タイトルなし、Preview/Comment 1〜3はeasy English、
# Support voice=Charon、Comment 4なし)を**既定動作**として実装する
# (Home Robots B1スクリプトのような--drop-japanese-title等のフラグは
# 持たない。今回新規に作る記事のためHome Robots固有の後追い修正フラグは
# 不要)。
#
# 本ファイルは`er013_family_c_episode_trial_09b_b1_run.py`を複製したもの
# (Home Robots用スクリプトは無編集のまま維持し、本ファイルのみを実行
# する)。v2run相当の再利用元は`er013_family_c_episode_trial_10_memory_run`
# (本タスクA2、以下a2m)。
#
# 話者: A2と同じ3-voice設計(narrator=Aoede、装置=Charon、兄=Erinome)。
# B1本文はWriterによる独立生成のため引用符の文言はA2と一致しない。
# 話者判定はA2のような段落indexの決め打ちではなく、引用符前後の文脈
# キーワード("screen"/"device"→device、"brother"→brother)による決定的
# 判定を用いる(Home Robots B1のbuild_all_story_segments_b1と同型、
# キーワードのみ本記事向けに調整)。
#
# 実行方法(A2完成・commit後):
#   .venv/Scripts/python.exe er013_family_c_episode_trial_10_memory_b1_run.py \
#       --budget-jpy 120
...
import er003_v1_b1_scaffold_01_generate as b1sup
...
import er013_family_c_episode_trial_10_memory_run as a2m  # 再利用のみ、編集禁止
...
# パス定数
# ============================================================
OUT_DIR = "er013_output/family_c_episode_trial_10/memory_b1"
...
ARTICLE_ID = "family_c_memory_trial_10_b1"
LEVEL = "FAMILY_C_TRIAL_10_MEMORY_B1"  # 新規level文字列、既存Gateの辞書は無変更。

TOPIC_TITLE = a2m.TOPIC_TITLE  # "The Future of Memory"(A2と同一文言、音声再利用のため)
TOPIC_INTRO_EN_TEXT = a2m.TOPIC_INTRO_EN_TEXT
# B1正式仕様(CURRENT_SPEC「B1 Support」節)どおり日本語タイトルは使用しない。

BROTHER_VOICE_NAME = a2m.BROTHER_VOICE_NAME  # "Erinome"(A2と同一Voice構成)

TTS_CALL_EST_JPY = a2m.TTS_CALL_EST_JPY
LLM_CALL_EST_JPY = a2m.LLM_CALL_EST_JPY
ASR_DIAG_CALL_EST_JPY = a2m.ASR_DIAG_CALL_EST_JPY

# ============================================================
# Core Provocation / Theme(er013_output/family_c_future_trial_08/memory/
# core_idea.json selection.selectedより転記、事前指定Read範囲内。
# A2版と完全同一のCore Provocationを使う)。
# ============================================================
CORE_PROVOCATION = (
    "In the future, people can place painful memories in secure storage and "
    "schedule their return for a later date. But if forgetting only postpones "
    "the pain, can a person still heal--and what might their earlier self have "
    "known that their present self has forgotten?"
)
THEME_LABEL = "the future of memory"

MAX_WRITER_ATTEMPTS = 2
WRITER_REASONING_EFFORT = vfl01.REASONING_EFFORT
JUDGE_REASONING_EFFORT = "medium"
MAX_KP_ATTEMPTS = 4

# Story core bullets(A2版reader_facing_article.txtから人手抽出、Trial限定の
# 固定コンテキスト。Home RobotsのHOME_ROBOTS_STORY_CORE_BULLETSと同型)。
MEMORY_STORY_CORE_BULLETS = """- Protagonist: Lena (only named character). She places a single painful \
memory (her brother's death) into a memory-storage device that can lock a memory \
away and let its owner schedule the date it returns.
- Inciting incident: The device asks "Return date?" and Lena chooses five years \
from today; the device asks "Are you sure?" and Lena admits "No... but I cannot \
carry it now," and the memory is locked away.
- For five years Lena lives free of the memory but cannot fully reach her brother \
in dreams; she wonders what the box knows about her that she does not know about \
herself.
- Turning point: Five years later the box opens on its own. The memory returns, \
including her brother's actual last words ("Do not make my last day your whole \
life") and her own younger voice promising "I will come back to this... I will \
remember all of it" -- words her present self had forgotten.
- Climax action: The device offers a new date to delay the memory again; Lena's \
hand shakes above it, and she must decide whether delaying the pain again is \
healing or only abandoning a frightened part of herself.
- Ending: Lena does not press the button. She stays on the floor and listens -- \
to the rain, to her brother's last words, to the woman she had been. The story \
ends on the idea that her earlier self had not been healed but had known the way \
back, and Lena finally lets both the pain and the love remain.
- Core Provocation: In the future, people can place painful memories in secure \
storage and schedule their return for a later date. But if forgetting only \
postpones the pain, can a person still heal -- and what might their earlier self \
have known that their present self has forgotten?"""

STORY_CORE_CHECKPOINTS = [
    {"id": "protagonist_name", "description": "主人公の名前(Lena)", "keywords": ["Lena"]},
    {"id": "memory_storage_device", "description": "辛い記憶を預けて後日戻せる記憶保管装置の設定",
     "keywords": ["memory", "box"]},
    {"id": "five_year_return", "description": "五年後に記憶が戻る設定",
     "keywords": ["five year", "five-year"]},
    {"id": "brother_last_words", "description": "兄の最後の言葉が記憶と共に戻る",
     "keywords": ["brother"]},
    {"id": "decision_not_to_delay", "description": "記憶を再び箱へ戻すか迷い、結局戻さない決定的行動",
     "keywords": ["press", "button"]},
    {"id": "ending_lets_both_remain", "description": "痛みと愛の両方を受け入れる結末",
     "keywords": ["remain", "both"]},
]


# ============================================================
# Comment 1〜3 / Preview(easy English、既定動作。既存B1 Support easy
# English経路[er003_v1_b1_scaffold_01_generate、developer message=
# "英語のListening Support原稿を作成してください。"]をそのまま呼ぶ。
# role文はHome Robots B1と同一の theme非依存文言[固有名詞を含まない
# 汎用役割記述]を使う)。
# ============================================================
...
def generate_writer_with_marker_retry(client, reference_article: str) -> tuple:
    model = routing.require_model_or_override(
        ...
        override_reason="USER-TEST-FINAL-AUDIO-BATCH-06(委任B): Family C memory B1 Trial "
                         "WriterにA2_WRITER Approved Model(Luna)を転用(新規process未定義、"
                         "コスト影響なし、DEV/Trial限定)")
    prompt = writer_b1.build_family_c_writer_v8_b1_prompt(
        core_provocation=CORE_PROVOCATION, theme_label=THEME_LABEL,
        reference_article=reference_article,
        story_core_bullets=MEMORY_STORY_CORE_BULLETS)
...
def stage_writer_and_safety(client) -> dict:
    ...
    reference_article = a2m.load_article_text()  # A2版Story core参照元(read-onlyで再利用)
    ...
    judge_model = routing.require_model_or_override(
        "A2_WRITER", routing.WRITER_MODEL,
        override_reason="USER-TEST-FINAL-AUDIO-BATCH-06(委任B): Plausibility Bridge/Imagined "
                         "Future軽判定にA2_WRITER Approved Model(Luna)を転用(新規process未定義、"
                         "コスト影響なし、DEV/Trial限定)")
    fact_blocks = layers["fact_blocks"]
    if not fact_blocks:
        current_fact_result = {"skipped": True,
                                "reason": "CURRENT FACT 0件(B1 Trial限定skip、A2/trial-08と同一方針)。",
                                "layer_pass": True}
    else:
    ...
    eval_model = routing.require_model_or_override(
        "A2_WRITER", routing.WRITER_MODEL,
        override_reason="USER-TEST-FINAL-AUDIO-BATCH-06(委任B): 補助Story Spark評価(6軸)にA2_WRITER "
                         "Approved Model(Luna)を転用(新規process未定義、コスト影響なし、DEV/Trial限定)")
    ...
    save_json(f"{OUT_DIR}/story_core_check.json", {
        ...
        "reference_article_path": a2m.ARTICLE_PATH,
        "checkpoints": story_core_check,
        "all_found": all(c["found"] for c in story_core_check),
    })

    normalized = a2m.normalize_for_tts(reader_text)
...
# 話者判定キーワード: "screen"/"device"→device、"brother"→brother
# (本記事専用のキーワード指定、Home Robots["robot"/"mother"]と同型の
# 個別記事対応であり、新しい汎用機構ではない)。
# ============================================================
QUOTE_RE = re.compile("“[^”]*”|\"[^\"]*\"")
...
def classify_quote_voice(paragraph_text: str, start: int, end: int) -> str:
    ...
    window = before + " " + after
    # USER-TEST-FINAL-AUDIO-BATCH-06(委任B、個別記事対応): B1 Writerは
    # 独立生成のため装置の呼称が一定しない(実測: "the storage robot"を
    # 採用)。"screen"/"device"に加え"robot"/"storage unit"も装置voiceの
    # 判定キーワードに含める(本記事専用の個別対応、新しい汎用機構ではない)。
    if any(kw in window for kw in ("screen", "device", "robot", "storage unit")):
        return "device"
...
def build_all_story_segments_b1(paragraphs: list) -> tuple:
    ...
    def add_seg(voice: str, kind: str, p_idx: int, raw_text: str) -> None:
        segments.append({
            ...
            "tts_text": a2m.normalize_for_tts(raw_text),
...
# ============================================================
# Stage 2: Comment 1〜3 / Preview(easy English、既定動作)
# ============================================================
def run_comment_text_en(client, comment_num: int, article_text: str, budget) -> str:
...
def run_preview_text_en(client, article_text: str, comment_1: str, comment_2: str, budget) -> str:
    label = "preview_llm"
    budget.check_before(LLM_CALL_EST_JPY, label)
...
def main() -> None:
    ...
    budget = a2m.BudgetTracker(args.budget_jpy, 90.0, f"{OUT_DIR}/raw_usage_log.jsonl")
    ...
    paragraphs = a2m.split_into_paragraphs(reader_text)
    ...
    # --- Story segment TTS(全件新規、A2音声のreuseなし) ---
    for seg in segments:
        out_path = f"{AUDIO_DIR}/{seg['id']}.wav"
        if seg["voice"] == "narrator":
            r = a2m.tts_narrator(seg["tts_text"], out_path, "en", seg["id"], budget)
        elif seg["voice"] == "device":
            r = a2m.tts_device(seg["tts_text"], out_path, seg["id"], budget)
        elif seg["voice"] == "brother":
            r = a2m.tts_brother(seg["tts_text"], out_path, seg["id"], budget)
        else:
            raise ValueError(f"unknown voice: {seg['voice']}")
        audit_segments[seg["id"]] = a2m._to_audit_entry(r, seg["tts_text"])
    ...
    for seg in segments:
        if seg["kind"] == "split" and seg["voice"] in ("device", "brother"):
            speaker_entries.append({
                ...
                "attribution_basis": "引用符前後160文字以内の'screen'/'device'/'brother'キーワード"
                                      "決定的検出(本記事専用キーワード、build_all_story_segments_b1参照)",
            })
    save_json(f"{OUT_DIR}/speaker_map.json", {
        "device_voice": "Charon", "brother_voice": BROTHER_VOICE_NAME,
        "lena_split_decision": "Lena本人の台詞はnarrator(Aoede)のまま分離しない(A2/Home Robots B1と"
                                "同一方針、話者を増やしすぎない既存2/3-voice設計思想を踏襲)。",
        "ambiguous_quote_count": len(ambiguous_quotes),
        "entries": speaker_entries,
    })

    # --- Comment 1〜3(easy English、既定動作) ---
    ...
    if os.path.exists(comments_en_path):
        ...
        for n in (1, 2, 3):
            comment_texts[n] = run_comment_text_en(client, n, article_text, budget)
        with open(comments_en_path, "w", encoding="utf-8") as f:
            for n in (1, 2, 3):
                f.write(f"## Comment {n}\n\n{comment_texts[n]}\n\n")

    # Support voice=Charon(既定動作、B1正式仕様どおり)。canonical textは
    # comments_en.mdのまま、TTSはa2m.tts_device(Charon経路)で行う。
    comment_wavs = {}
    for n in (1, 2, 3):
        txt = comment_texts[n]
        path = f"{AUDIO_DIR}/comment_{n}_en.wav"
        r = a2m.tts_device(txt, path, f"comment_{n}_en", budget)
        audit_segments[f"comment_{n}_en"] = a2m._to_audit_entry(r, txt)
        comment_wavs[n] = (path, r)

    # --- Preview(easy English、既定動作、Support voice=Charon) ---
    ...
    if os.path.exists(preview_en_path):
        ...
        preview_text = run_preview_text_en(client, article_text, comment_texts[1], comment_texts[2], budget)
        save_text(preview_en_path, preview_text)
    preview_wav_path = f"{AUDIO_DIR}/preview_en.wav"
    r_preview = a2m.tts_device(preview_text, preview_wav_path, "preview_en", budget)
    audit_segments["preview_en"] = a2m._to_audit_entry(r_preview, preview_text)

    # --- Key Phrase(resumable、既存正式経路+入口再呼び出し最大4回) ---
    ...
    for item in kp_items:
        ...
        r_num = a2m.reuse_shared_number_word(rank, num_path)
        en_path = f"{AUDIO_DIR}/kp{rank}_english.wav"
        r_en_kp = a2m.tts_key_phrase_english(used_form, en_path, f"kp{rank}_english", budget)
        ja_path = f"{AUDIO_DIR}/kp{rank}_japanese.wav"
        r_ja_kp = a2m.tts_narrator(ja_gloss_tts, ja_path, "ja", f"kp{rank}_japanese", budget)

        audit_key_phrases[str(rank)] = {
            "number": a2m._to_audit_entry(r_num, a2m.NUMBER_WORDS[rank]),
            "english": a2m._to_audit_entry(r_en_kp, used_form),
            "japanese": a2m._to_audit_entry(r_ja_kp, ja_gloss_tts),
        }
        kp_blocks_stereo.append((rank, num_path, en_path, ja_path))

        en_asr = r_en_kp.get("asr_text") or a2m.asr_diag(en_path, "en", budget, f"kp{rank}_english_asr_diag")
        ja_asr = r_ja_kp.get("asr_text") or a2m.asr_diag(ja_path, "ja", budget, f"kp{rank}_japanese_asr_diag")
        kp_consistency[str(rank)] = {
            "player_display_en": used_form, "canonical_en": used_form, "tts_input_en": used_form,
            "asr_en": en_asr, "match_en": a2m._normalize_loose(en_asr) == a2m._normalize_loose(used_form),
            "player_display_ja": ja_gloss_display, "canonical_ja": ja_gloss_display,
            "tts_input_ja": ja_gloss_tts, "asr_ja": ja_asr,
            "match_ja": a2m._normalize_loose(ja_asr) == a2m._normalize_loose(ja_gloss_tts),
        }
    save_json(f"{OUT_DIR}/key_phrase_consistency.json", kp_consistency)

    # --- SFX/Nav資産(article非依存共有資産+A2同一文言のtopic_intro_en流用) ---
    ...
    a2m.copy_shared_charon_nav(AUDIO_DIR)
    # A2のtopic_intro_en.wav(文言完全一致)をそのままコピーして再TTSしない
    # (Home RobotsのB1がv2run.reuse_v1_wavでA2音声を流用したのと同一方針)。
    a2_topic_wav = f"{a2m.AUDIO_DIR}/topic_intro_en.wav"
    b1_topic_wav = f"{AUDIO_DIR}/topic_intro_en.wav"
    if not os.path.exists(b1_topic_wav) and os.path.exists(a2_topic_wav):
        shutil.copyfile(a2_topic_wav, b1_topic_wav)
        with open(b1_topic_wav + ".ok", "w", encoding="utf-8") as f:
            f.write("ok")
    r_topic_en = a2m._resumable_reuse(b1_topic_wav) or a2m.tts_narrator(
        TOPIC_INTRO_EN_TEXT, b1_topic_wav, "en", "topic_intro_en", budget)
    audit_segments["topic_intro_en"] = a2m._to_audit_entry(r_topic_en, TOPIC_INTRO_EN_TEXT)
    ...
    comment_placement = [
        {"comment": 1, "position_description": "導入部(Preview後・Story前、Full story introの直後、"
                                                 "固定配置)",
         "selection_reason": "Comment 1は物語の理解補助として常にStory先頭固定(A2/Home Robots B1と同一方針)。",
    ...
    # --- gain + timeline構築(Family A構成をそのまま踏襲、A2と同一pause値) ---
    ...
    sil(0.5)  # Story終了直後→Outro(Comment4なし、A2既存のOutro直前pause値[0.5秒]を適用)
    ...
    final_path = f"{ASSEMBLED_DIR}/family_c_memory_trial_10_b1.wav"
    ...
    normalized = (open(normalized_path, encoding="utf-8").read() if os.path.exists(normalized_path)
                  else a2m.normalize_for_tts(reader_text))
    ...
    # --- player/display/audio consistency ---
    consistency_rows = []
    for seg in segments:
        asr_text = audit_segments[seg["id"]].get("asr_text")
        if asr_text is None:
            asr_text = a2m.asr_diag(seg["audio_path"], "en", budget, f"{seg['id']}_asr_diag")
        consistency_rows.append({
            "segment_id": seg["id"], "voice": seg["voice"],
            "player_display_text": seg["tts_text"], "canonical_text": seg["tts_text"],
            "tts_input_text": seg["tts_text"], "asr_text": asr_text,
            "match": a2m._normalize_loose(asr_text) == a2m._normalize_loose(seg["tts_text"]),
        })
    fixed_checks = [
        ("topic_intro_en", TOPIC_INTRO_EN_TEXT, "en", None),
        ("preview_en", preview_text, "en", audit_segments["preview_en"].get("asr_text")),
    ]
    for n in (1, 2, 3):
        fixed_checks.append((f"comment_{n}_en", comment_texts[n], "en",
                              audit_segments[f"comment_{n}_en"].get("asr_text")))
    for key, (_fname, text) in a2m.SHARED_CHARON_NAV.items():
    ...
    for name, text, lang, precomputed in fixed_checks:
        ...
        if asr_text is None:
            ...
            asr_text = a2m.asr_diag(path, lang, budget, f"{name}_asr_diag")
        consistency_rows.append({
            "segment_id": name, "voice": "nav/fixed",
            "player_display_text": text, "canonical_text": text, "tts_input_text": text,
            "asr_text": asr_text, "match": a2m._normalize_loose(asr_text) == a2m._normalize_loose(text),
        })
    ...
    for n in (1, 2, 3):
        ...
        asr_text = r.get("asr_text") or a2m.asr_diag(path, "en", budget, f"comment_{n}_reconfirm_asr")
        comment_consistency.append({
            ...
            "match": a2m._normalize_loose(asr_text) == a2m._normalize_loose(comment_texts[n]),
        })
    ...
    cost_summary = {
        "budget_cap_jpy": args.budget_jpy, "warn_threshold_jpy": 90.0,
        "total_estimate_jpy": round(total_jpy, 2),
        "note": "raw_usage_log.jsonl全体(複数回のresume実行を含む)からの累計。A2と同じ方式論"
                "(precedentベースの安全側単価推定)。全segment/preview/comment/key phraseが"
                "新規生成のため、A2のような音声reuseによる費用削減はない(共有Charon nav資産・"
                "topic_intro_enの流用のみ)。",
...
def convert_all_to_mp3_b1() -> dict:
    ...
    results = []
    ep_wav = f"{ASSEMBLED_DIR}/family_c_memory_trial_10_b1.wav"
    ep_mp3 = f"{WEB_DIR}/family_c_memory_trial_10_b1.mp3"
...
def build_player_html_b1(seq_labels, segments, kp_items, comment_texts, run_summary, preview_text) -> None:
    ...
    fixed_label_to_text = {
        "Welcome (Charon)": (a2m.SHARED_CHARON_NAV["welcome"][1], "Charon(nav)"),
        "Topic intro": (TOPIC_INTRO_EN_TEXT, "Aoede(narrator)"),
        "Preview intro (Charon)": (a2m.SHARED_CHARON_NAV["preview_intro"][1], "Charon(nav)"),
        "Key phrases intro (Charon)": (a2m.SHARED_CHARON_NAV["key_phrases_intro"][1], "Charon(nav)"),
        "Full story intro (Charon)": (a2m.SHARED_CHARON_NAV["full_story_intro"][1], "Charon(nav)"),
    ...
    episode_url = "./web/family_c_memory_trial_10_b1.mp3"
    html = f"""<!doctype html><html><head><meta charset="utf-8">
<title>Family C Trial-10: The Future of Memory (B1)</title>
<style>{player_mod.PLAYER_STANDARD_CSS}</style>
<script>{player_mod.SEEK_SCRIPT}</script>
</head><body>
<h1>Family C Future — The Future of Memory (Trial-10 B1 / VALIDATED候補・ユーザー試聴待ち、level=B1)</h1>
